...
def recalibrate(d, title ):
    ...
    res = d
    newtitle = title
    logging.info(f"D...RECAL  {d} ... {type(d)}   /{float(d)}/    /{title}/ ")
    ...
    if newtitle[-1] == "_" and len(newtitle) > 2:
        newtitle = newtitle[:-1]
    logging.info(f"D...RECALfin  {res} ... {newtitle}")
    return res, newtitle
...
def process_data(data, index):
 ...
 "mminput5_cfg": "sub xxx;22;28;5;title5"
    ...
    print()
    logging.info(f"D...  PDa receivd: /{d}/  on port {mynetport} ")
    ...
    if is_float(d) or is_int(d):
        ...
        logging.info(f"D...  PDa  4 /{d}/  on port {mynetport} ")
        mmwrite( mmtemplate, mmfile, PORT_override=PRIMARY_PORT)
        print(f"DEBUG5 {d}", flush=True)
        logging.info(f"D...  PDa  5 /{d}/  on port {mynetport} ")
        ...
        if influx.check_port():
            ...
            try:
                ...
                logging.info(f"D...  PDa  InOK /{commavalues}/  on port {mynetport} ")
            except:
                logging.info(f"D...  PDa  InXX /{commavalues}/  on port {mynetport} ")
        ...
        logging.info(f"D...  PDa  txt1 /{mmtemplate}/  on port {mynetport} ")
        mmwrite( mmtemplate, mmfile , PORT_override=PRIMARY_PORT)
        print("i... SUCCESS  MMWRITE -----", bg.white, fg.black, mmtemplate, fg.default, bg.default)
        logging.info(f"D...  PDa  txt2 /{mmtemplate}/  on port {mynetport} ")
