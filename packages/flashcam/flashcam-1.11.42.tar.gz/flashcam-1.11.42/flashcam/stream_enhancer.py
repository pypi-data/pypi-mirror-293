...
# ------------------------------------------------------- HISTO ---------------------------
    ...
    #                    '-movflags': '',
    #                    '+faststart':'',
                        '-preset':'slower'   #the slower the better compression,
                ...
                for i in range(3): # doesnt help much to create a file
                    ...
                    if self.skwriter_detm is not None:
                        ...
                        self.skwriter_detm.writeFrame(self.frame)
            ...
            self.assure_DATA()
            filename = os.path.expanduser("~/DATA/"+ f"{me}_laps{fseconds:04d}_{filename}_{str(config.CONFIG['netport'])}_{mycodec}.mkv" )
    ...
    #                    '+faststart':'',
                    ...
                    #
                self.skwriter_laps.writeFrame(self.frame)#[:,:,::-1])  #write the frame as RGB not BGR
