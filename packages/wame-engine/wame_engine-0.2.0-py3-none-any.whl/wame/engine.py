...
class Engine:
    ...
    def _mainloop(self) -> None:
        ...
        if self._renderer is None:
            error:str = "A renderer (like PyGame native or OpenGL) must be set as the graphics API before the engine can start. Set the renderer type with engine.set_renderer() with any wame.Renderer type"
            raise RendererNotSet(error)
        ...
        while self._running:
            if self.settings.antialiasing:
                if self._renderer == Renderer.OPENGL:
    ...
    def register_scene(self, name:str, scene:Scene, overwrite:bool=False) -> None:
        ...
        if not overwrite:
            if name in self._scenes:
                ...
                raise SceneAlreadyExists(error)
    ...
    def set_renderer(self, pipeline:Renderer) -> None:
        '''
        Set the rendering pipeline
        
        Parameters
        ----------
        pipeline : `wame.Renderer`
            The rendering pipeline to use
        ...
        self._renderer = pipeline
        
        if self._renderer == Renderer.PYGAME:
            self.screen = pygame.display.set_mode(self._size.to_tuple(), pygame.HWSURFACE | pygame.DOUBLEBUF)
        elif self._renderer == Renderer.OPENGL:
            ...
            error:str = f"Renderer option {self._renderer} provided is not of type `wame.Renderer`"
    ...
    def start(self) -> None:
        ...
        ------
        `wame.RendererNotSet`
            If the engine is started without a renderer selected
        `wame.SceneNotSet`
