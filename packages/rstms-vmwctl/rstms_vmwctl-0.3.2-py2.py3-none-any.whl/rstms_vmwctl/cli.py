...
def cli(
    ...
    if no_iso:
        iso = None
    client = Client(host, url, username, password, path_list, arp_command, iso, mount_path, ca_file, client_cert, client_key)
...
@click.option("--time-sync/-no-time-sync", is_flag=True, default=True, help="sync time with host")
