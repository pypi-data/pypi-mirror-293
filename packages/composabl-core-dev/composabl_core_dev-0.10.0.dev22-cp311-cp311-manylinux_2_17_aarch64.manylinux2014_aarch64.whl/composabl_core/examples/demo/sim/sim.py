...
class Sim(gym.Env):
    ...
    def __init__(self, env_init: dict = {}):
        ...
        if self.space_type == "discrete":
            ...
            self.action_space = gym.spaces.Dict({
                ...
                "increment_box": gym.spaces.Box(low=np.array([-2]), high=np.array([-1]), dtype=np.float32),  # Increment by 1 or reset
                "nested_action": gym.spaces.Dict({
                    "increment_nested": gym.spaces.Discrete(2),
                    "increment_nested_box": gym.spaces.Box(low=-2, high=5, shape=(1,), dtype=np.float32),
                })
            })
    ...
    def get_action_mask(self):
        if self.space_type == "discrete":
            ...
            mask = {
                ...
                "nested_action": {
                    "increment_nested": random.choice([np.array([0, 1]), np.array([1, 0])]),
    ...
    def _process_action(self, action):
        ...
        if self.space_type == "discrete" or self.space_type == "multibinary" or self.space_type == "text":
