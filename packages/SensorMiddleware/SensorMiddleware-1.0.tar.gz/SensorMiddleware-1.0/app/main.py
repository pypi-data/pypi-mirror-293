#!/usr/bin/python3
# -*- coding: utf-8 -*-
from aws import upload_report_to_s3_bucket
from core import SensorControler, CannotReadScanData
...
def getscontrol():
    global sensor_controler
    if sensor_controler is None:
        sensor_controler = SensorControler('envvar')

    return sensor_controler


@app.command(short_help="Run cloud scan, specify where the scan data is stored (envvar/...)")
def runscan(get_data_from: str) -> NoReturn:
    print('[bold green] Starting scan ... [/bold green]')

    try:
        scontrol = SensorControler(get_data_from)
    except (CannotReadScanData, RuntimeError) as e:
...
def stopscan(scontrol: SensorControler = typer.Option(None, callback=getscontrol)) -> NoReturn:
    pass
