...
import sqlalchemy_altibase as altibase


class CompileTestImportedFromInternalDialect(fixtures.TestBase, AssertsCompiledSQL):
    __dialect__ = altibase.dialect()
    ...
    def test_limit_offset(self):
        ...
        self.assert_compile(stmt, "SELECT 1 ROWS LIMIT :param_1 OFFSET :param_2")
    ...
    def test_delete_extra_froms(self):
        ...
        self.assert_compile(q, "DELETE FROM t1 FROM t1, t2 WHERE t1.c1 = t2.c1")

    def test_delete_extra_froms_alias(self):
        a1 = sql.table("t1", sql.column("c1")).alias("a1")
        t2 = sql.table("t2", sql.column("c1"))
        q = sql.delete(a1).where(a1.c.c1 == t2.c.c1)
        self.assert_compile(q, "DELETE FROM a1 FROM t1 AS a1, t2 WHERE a1.c1 = t2.c1")
