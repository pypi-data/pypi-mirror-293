...
def copy_vault(
    ...
    print(f'Copied {vault_path} to {target_path}')
...
def obsidian2quarto(
    ...
    target_path = Path(vault_path) / config["quarto-dir"]
    copy_vault(vault_path=vault_path, target_path=target_path, ignore_names=config["ignore"])
    convert_vault(target_path, partial(
        convert_md, filters=[
