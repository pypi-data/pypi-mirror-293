...
from urllib3_odigos.util import Timeout as Urllib3Timeout

import requests_odigos
from requests_odigos.adapters import HTTPAdapter
from requests_odigos.auth import HTTPDigestAuth, _basic_auth_str
from requests_odigos.compat import (
    JSONDecodeError,
    Morsel,
    MutableMapping,
    builtin_str,
...
from requests_odigos.cookies import cookiejar_from_dict, morsel_to_cookie
from requests_odigos.exceptions import (
...
from requests_odigos.exceptions import SSLError as RequestsSSLError
from requests_odigos.exceptions import Timeout, TooManyRedirects, UnrewindableBodyError
from requests_odigos.hooks import default_hooks
from requests_odigos.models import PreparedRequest, urlencode
from requests_odigos.sessions import SessionRedirectMixin
from requests_odigos.structures import CaseInsensitiveDict
...
class TestRequests:
    ...
    def test_entry_points(self):
        requests_odigos.session
        requests_odigos.session().get
        requests_odigos.session().head
        requests_odigos.get
        requests_odigos.head
        requests_odigos.put
        requests_odigos.patch
        requests_odigos.post
        # Not really an entry point, but people rely on it.
        from requests_odigos.packages.urllib3_odigos.poolmanager import PoolManager  # noqa:F401
    ...
    def test_invalid_url(self, exception, url):
        with pytest.raises(exception):
            requests_odigos.get(url)

    def test_basic_building(self):
        req = requests_odigos.Request()
    ...
    def test_no_content_length(self, httpbin, method):
        req = requests_odigos.Request(method, httpbin(method.lower())).prepare()
    ...
    def test_empty_content_length(self, httpbin, method):
        req = requests_odigos.Request(method, httpbin(method.lower()), data="").prepare()
        assert req.headers["Content-Length"] == "0"

    def test_override_content_length(self, httpbin):
        headers = {"Content-Length": "not zero"}
        r = requests_odigos.Request("POST", httpbin("post"), headers=headers).prepare()
        assert "Content-Length" in r.headers
        assert r.headers["Content-Length"] == "not zero"

    def test_path_is_not_double_encoded(self):
        request = requests_odigos.Request("GET", "http://0.0.0.0/get/test case").prepare()
    ...
    def test_params_are_added_before_fragment(self, url, expected):
        request = requests_odigos.Request("GET", url, params={"a": "b"}).prepare()
    ...
    def test_params_original_order_is_preserved_by_default(self):
        ...
        session = requests_odigos.Session()
        request = requests_odigos.Request(
    ...
    def test_whitespaces_are_removed_from_url(self):
        # Test for issue #3696
        request = requests_odigos.Request("GET", " http://example.com").prepare()
        assert request.url == "http://example.com/"

    @pytest.mark.parametrize("scheme", ("http://", "HTTP://", "hTTp://", "HttP://"))
    def test_mixed_case_scheme_acceptable(self, httpbin, scheme):
        s = requests_odigos.Session()
        s.proxies = getproxies()
        parts = urlparse(httpbin("get"))
        url = scheme + parts.netloc + parts.path
        r = requests_odigos.Request("GET", url)
        r = s.send(r.prepare())
        assert r.status_code == 200, f"failed for scheme {scheme}"

    def test_HTTP_200_OK_GET_ALTERNATIVE(self, httpbin):
        r = requests_odigos.Request("GET", httpbin("get"))
        s = requests_odigos.Session()
        s.proxies = getproxies()
    ...
    def test_HTTP_302_ALLOW_REDIRECT_GET(self, httpbin):
        r = requests_odigos.get(httpbin("redirect", "1"))
        assert r.status_code == 200
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect

    def test_HTTP_307_ALLOW_REDIRECT_POST(self, httpbin):
        r = requests_odigos.post(
            httpbin("redirect-to"),
            data="test",
    ...
    def test_HTTP_302_TOO_MANY_REDIRECTS(self, httpbin):
        try:
            requests_odigos.get(httpbin("relative-redirect", "50"))
    ...
    def test_http_301_changes_post_to_get(self, httpbin):
        r = requests_odigos.post(httpbin("status", "301"))
        assert r.status_code == 200
    ...
    def test_http_301_doesnt_change_head_to_get(self, httpbin):
        r = requests_odigos.head(httpbin("status", "301"), allow_redirects=True)
        print(r.content)
        assert r.status_code == 200
    ...
    def test_http_302_changes_post_to_get(self, httpbin):
        r = requests_odigos.post(httpbin("status", "302"))
        assert r.status_code == 200
        assert r.request.method == "GET"
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect
    ...
    def test_http_302_doesnt_change_head_to_get(self, httpbin):
        r = requests_odigos.head(httpbin("status", "302"), allow_redirects=True)
        assert r.status_code == 200
        assert r.request.method == "HEAD"
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect
    ...
    def test_http_303_changes_post_to_get(self, httpbin):
        r = requests_odigos.post(httpbin("status", "303"))
        assert r.status_code == 200
        assert r.request.method == "GET"
    ...
    def test_http_303_doesnt_change_head_to_get(self, httpbin):
        r = requests_odigos.head(httpbin("status", "303"), allow_redirects=True)
        assert r.status_code == 200
        assert r.request.method == "HEAD"
        assert r.history[0].status_code == 303
        assert r.history[0].is_redirect
    ...
    def test_header_and_body_removal_on_redirect(self, httpbin):
        ...
        ses = requests_odigos.Session()
        req = requests_odigos.Request("POST", httpbin("post"), data={"test": "data"})
    ...
    def test_transfer_enc_removal_on_redirect(self, httpbin):
        ...
        ses = requests_odigos.Session()
        req = requests_odigos.Request("POST", httpbin("post"), data=(b"x" for x in range(1)))
        prep = ses.prepare_request(req)
        assert "Transfer-Encoding" in prep.headers

        # Create Response to avoid https://github.com/kevin1024/pytest-httpbin/issues/33
        resp = requests_odigos.Response()
    ...
    def test_fragment_maintained_on_redirect(self, httpbin):
        ...
        r = requests_odigos.get(httpbin("redirect-to?url=get") + fragment)
    ...
    def test_HTTP_200_OK_GET_WITH_PARAMS(self, httpbin):
        ...
        r = requests_odigos.get(httpbin("user-agent"), headers=heads)
    ...
    def test_cookielib_cookiejar_on_redirect(self, httpbin):
        ...
        req = requests_odigos.Request("GET", httpbin("headers"), cookies=cj)
        ...
        assert not isinstance(resp.request._cookies, requests_odigos.cookies.RequestsCookieJar)
    ...
    def test_requests_in_history_are_not_overridden(self, httpbin):
        resp = requests_odigos.get(httpbin("redirect/3"))
    ...
    def test_history_is_always_a_list(self, httpbin):
        ...
        resp = requests_odigos.get(httpbin("get"))
        assert isinstance(resp.history, list)
        resp = requests_odigos.get(httpbin("redirect/1"))
        assert isinstance(resp.history, list)
    ...
    def test_headers_on_session_with_None_are_not_sent(self, httpbin):
        ...
        req = requests_odigos.Request("GET", httpbin("get"))
        prep = ses.prepare_request(req)
    ...
    def test_headers_preserve_order(self, httpbin):
        ...
        headers["Fifth"] = "5"
        headers["Second"] = "222"
        req = requests_odigos.Request("GET", httpbin("get"), headers=headers)
        prep = ses.prepare_request(req)
    ...
    def test_HTTP_200_OK_HEAD(self, httpbin):
        r = requests_odigos.head(httpbin("get"))
        assert r.status_code == 200

    def test_HTTP_200_OK_PUT(self, httpbin):
        r = requests_odigos.put(httpbin("put"))
        assert r.status_code == 200
    ...
    def test_BASICAUTH_TUPLE_HTTP_200_OK_GET(self, httpbin):
        ...
        r = requests_odigos.get(url, auth=auth)
        assert r.status_code == 200

        r = requests_odigos.get(url)
    ...
    def test_set_basicauth(self, httpbin, username, password):
        ...
        r = requests_odigos.Request("GET", url, auth=auth)
        p = r.prepare()
    ...
    def test_basicauth_encodes_byte_strings(self):
        ...
        r = requests_odigos.Request("GET", "http://localhost", auth=auth)
        p = r.prepare()
    ...
    def test_errors(self, url, exception):
        with pytest.raises(exception):
            requests_odigos.get(url, timeout=1)
    ...
    def test_proxy_error_on_bad_url(self, httpbin, httpbin_secure):
        with pytest.raises(InvalidProxyURL):
            requests_odigos.get(httpbin_secure(), proxies={"https": "http:/badproxyurl:3128"})

        with pytest.raises(InvalidProxyURL):
            requests_odigos.get(httpbin(), proxies={"http": "http://:8080"})

        with pytest.raises(InvalidProxyURL):
            requests_odigos.get(httpbin_secure(), proxies={"https": "https://"})

        with pytest.raises(InvalidProxyURL):
            requests_odigos.get(httpbin(), proxies={"http": "http:///example.com:8080"})

    def test_respect_proxy_env_on_send_self_prepared_request(self, httpbin):
        with override_environ(http_proxy=INVALID_PROXY):
            with pytest.raises(ProxyError):
                session = requests_odigos.Session()
                request = requests_odigos.Request("GET", httpbin())
    ...
    def test_respect_proxy_env_on_send_with_redirects(self, httpbin):
        with override_environ(http_proxy=INVALID_PROXY):
            with pytest.raises(ProxyError):
                ...
                print(url)
                request = requests_odigos.Request("GET", url)
                session.send(request.prepare())
    ...
    def test_proxy_authorization_not_appended_to_https_request(
        ...
        req = requests_odigos.Request("GET", url)
        prep = req.prepare()
    ...
    def test_basicauth_with_netrc(self, httpbin):
        ...
        old_auth = requests_odigos.sessions.get_netrc_auth
        ...
        try:
            ...
            requests_odigos.sessions.get_netrc_auth = get_netrc_auth_mock
            ...
            r = requests_odigos.get(url, auth=wrong_auth)
            assert r.status_code == 401
            ...
            requests_odigos.sessions.get_netrc_auth = old_auth
    ...
    def test_DIGEST_STREAM(self, httpbin):
        for authtype in self.digest_auth_algo:
            ...
            r = requests_odigos.get(url, auth=auth, stream=True)
            assert r.raw.read() != b""

            r = requests_odigos.get(url, auth=auth, stream=False)
            assert r.raw.read() == b""
    ...
    def test_POSTBIN_GET_POST_FILES(self, httpbin):
        url = httpbin("post")
        requests_odigos.post(url).raise_for_status()

        post1 = requests_odigos.post(url, data={"some": "data"})
        assert post1.status_code == 200

        with open("requirements-dev.txt") as f:
            post2 = requests_odigos.post(url, files={"some": f})
        assert post2.status_code == 200

        post4 = requests_odigos.post(url, data='[{"some": "json"}]')
        assert post4.status_code == 200

        with pytest.raises(ValueError):
            requests_odigos.post(url, files=["bad file data"])

    def test_invalid_files_input(self, httpbin):
        url = httpbin("post")
        post = requests_odigos.post(url, files={"random-file-1": None, "random-file-2": 1})
    ...
    def test_POSTBIN_SEEKED_OBJECT_WITH_NO_ITER(self, httpbin):
        ...
        post1 = requests_odigos.post(httpbin("post"), data=test)
        assert post1.status_code == 200
        assert post1.json()["data"] == "test"

        test = TestStream("test")
        test.seek(2)
        post2 = requests_odigos.post(httpbin("post"), data=test)
        assert post2.status_code == 200
    ...
    def test_POSTBIN_GET_POST_FILES_WITH_DATA(self, httpbin):
        ...
        with open("requirements-dev.txt") as f:
            post2 = requests_odigos.post(url, data={"some": "data"}, files={"some": f})
        assert post2.status_code == 200
    ...
    def test_post_with_custom_mapping(self, httpbin):
        ...
        url = httpbin("post")
        found_json = requests_odigos.post(url, data=data).json().get("form")
    ...
    def test_conflicting_post_params(self, httpbin):
        ...
        with open("requirements-dev.txt") as f:
            with pytest.raises(ValueError):
                requests_odigos.post(url, data='[{"some": "data"}]', files={"some": f})

    def test_request_ok_set(self, httpbin):
        r = requests_odigos.get(httpbin("status", "404"))
        assert not r.ok

    def test_status_raising(self, httpbin):
        r = requests_odigos.get(httpbin("status", "404"))
        with pytest.raises(requests_odigos.exceptions.HTTPError):
            r.raise_for_status()

        r = requests_odigos.get(httpbin("status", "500"))
        assert not r.ok

    def test_decompress_gzip(self, httpbin):
        r = requests_odigos.get(httpbin("gzip"))
    ...
    def test_unicode_get(self, httpbin, url, params):
        requests_odigos.get(httpbin(url), params=params)
    ...
    def test_pyopenssl_redirect(self, httpbin_secure, httpbin_ca_bundle):
        requests_odigos.get(httpbin_secure("status", "301"), verify=httpbin_ca_bundle)

    def test_invalid_ca_certificate_path(self, httpbin_secure):
        INVALID_PATH = "/garbage"
        with pytest.raises(IOError) as e:
            requests_odigos.get(httpbin_secure(), verify=INVALID_PATH)
        assert str(
            e.value
    ...
    def test_invalid_ssl_certificate_files(self, httpbin_secure):
        INVALID_PATH = "/garbage"
        with pytest.raises(IOError) as e:
            requests_odigos.get(httpbin_secure(), cert=INVALID_PATH)
        assert str(
            e.value
    ...
    def test_http_with_certificate(self, httpbin):
        r = requests_odigos.get(httpbin(), cert=".")
        assert r.status_code == 200
    ...
    def test_https_warnings(self, nosan_server):
        ...
        with pytest.warns() as warning_records:
            ...
            requests_odigos.get(f"https://localhost:{port}/", verify=ca_bundle)
    ...
    def test_certificate_failure(self, httpbin_secure):
        ...
        with pytest.raises(RequestsSSLError):
            ...
            requests_odigos.get(httpbin_secure("status", "200"))

    def test_urlencoded_get_query_multivalued_param(self, httpbin):
        r = requests_odigos.get(httpbin("get"), params={"test": ["foo", "baz"]})
        assert r.status_code == 200
        assert r.url == httpbin("get?test=foo&test=baz")

    def test_form_encoded_post_query_multivalued_element(self, httpbin):
        r = requests_odigos.Request(
    ...
    def test_unicode_method_name_with_request_object(self, httpbin):
        ...
        with open(__file__, "rb") as f:
            files = {"file": f}
            req = requests_odigos.Request("POST", httpbin("post"), files=files)
    ...
    def test_non_prepared_request_error(self):
        s = requests_odigos.Session()
        req = requests_odigos.Request("POST", "/")
    ...
    def test_custom_content_type(self, httpbin):
        with open(__file__, "rb") as f1:
            with open(__file__, "rb") as f2:
                ...
                r = requests_odigos.post(httpbin("post"), data=data, files=files)
        assert r.status_code == 200
    ...
    def test_hook_receives_request_arguments(self, httpbin):
        ...
        s = requests_odigos.Session()
        r = requests_odigos.Request("GET", httpbin(), hooks={"response": hook})
    ...
    def test_session_hooks_are_used_with_no_request_hooks(self, httpbin):
        ...
        r = requests_odigos.Request("GET", httpbin())
        prep = s.prepare_request(r)
    ...
    def test_session_hooks_are_overridden_by_request_hooks(self, httpbin):
        ...
        r = requests_odigos.Request("GET", httpbin(), hooks={"response": [hook1]})
        prep = s.prepare_request(r)
    ...
    def test_prepared_request_hook(self, httpbin):
        ...
        req = requests_odigos.Request("GET", httpbin(), hooks={"response": hook})
        prep = req.prepare()
    ...
    def test_prepared_from_session(self, httpbin):
        class DummyAuth(requests_odigos.auth.AuthBase):
            def __call__(self, r):
                r.headers["Dummy-Auth-Test"] = "dummy-auth-test-ok"
                return r

        req = requests_odigos.Request("GET", httpbin("headers"))
        assert not req.auth
    ...
    def test_prepare_request_with_bytestring_url(self):
        req = requests_odigos.Request("GET", b"https://httpbin.org/")
        s = requests_odigos.Session()
        prep = s.prepare_request(req)
    ...
    def test_cookie_parameters(self):
        ...
        jar = requests_odigos.cookies.RequestsCookieJar()
    ...
    def test_cookie_duplicate_names_different_domains(self):
        ...
        with pytest.raises(requests_odigos.cookies.CookieConflictError):
            jar.get(key)
    ...
    def test_time_elapsed_blank(self, httpbin):
        r = requests_odigos.get(httpbin("get"))
        td = r.elapsed
        total_seconds = (
    ...
    def test_response_reason_unicode_fallback(self):
        ...
        r.status_code = 500
        r.encoding = None
        with pytest.raises(requests_odigos.exceptions.HTTPError) as e:
            r.raise_for_status()
    ...
    @pytest.mark.parametrize(
        ...
        (
            (urllib3_odigos.exceptions.ProtocolError, tuple(), ChunkedEncodingError),
            (urllib3_odigos.exceptions.DecodeError, tuple(), ContentDecodingError),
            (urllib3_odigos.exceptions.ReadTimeoutError, (None, "", ""), ConnectionError),
            (urllib3_odigos.exceptions.SSLError, tuple(), RequestsSSLError),
        ),
    )
    ...
    def test_prepared_request_is_pickleable(self, httpbin):
        p = requests_odigos.Request("GET", httpbin("get")).prepare()
    ...
    def test_prepared_request_with_file_is_pickleable(self, httpbin):
        with open(__file__, "rb") as f:
            r = requests_odigos.Request("POST", httpbin("post"), files={"file": f})
            p = r.prepare()
    ...
    def test_prepared_request_with_hook_is_pickleable(self, httpbin):
        r = requests_odigos.Request("GET", httpbin("get"), hooks=default_hooks())
        p = r.prepare()
    ...
    def test_cannot_send_unprepared_requests(self, httpbin):
        r = requests_odigos.Request(url=httpbin())
        with pytest.raises(ValueError):
            requests_odigos.Session().send(r)

    def test_http_error(self):
        error = requests_odigos.exceptions.HTTPError()
        assert not error.response
        response = requests_odigos.Response()
        error = requests_odigos.exceptions.HTTPError(response=response)
        assert error.response == response
        error = requests_odigos.exceptions.HTTPError("message", response=response)
    ...
    def test_uppercase_scheme_redirect(self, httpbin):
        ...
        r = requests_odigos.get(httpbin("redirect-to"), params={"url": url})
        assert r.status_code == 200
    ...
    def test_long_authinfo_in_url(self):
        ...
        r = requests_odigos.Request("GET", url).prepare()
        assert r.url == url

    def test_header_keys_are_native(self, httpbin):
        headers = {"unicode": "blah", b"byte": "blah"}
        r = requests_odigos.Request("GET", httpbin("get"), headers=headers)
        p = r.prepare()
    ...
    def test_header_validation(self, httpbin):
        ...
        r = requests_odigos.get(httpbin("get"), headers=valid_headers)
    ...
    def test_header_value_not_str(self, httpbin, invalid_header, key):
        ...
        with pytest.raises(InvalidHeader) as excinfo:
            requests_odigos.get(httpbin("get"), headers=invalid_header)
    ...
    def test_can_send_objects_with_files(self, httpbin, files):
        ...
        files = {"b": files}
        r = requests_odigos.Request("POST", httpbin("post"), data=data, files=files)
        p = r.prepare()
    ...
    def test_can_send_file_object_with_non_string_filename(self, httpbin):
        f = io.BytesIO()
        f.name = 2
        r = requests_odigos.Request("POST", httpbin("post"), files={"f": f})
        p = r.prepare()
    ...
    def test_autoset_header_values_are_native(self, httpbin):
        ...
        length = "16"
        req = requests_odigos.Request("POST", httpbin("post"), data=data)
        p = req.prepare()
    ...
    def test_nonhttp_schemes_dont_check_URLs(self):
        ...
        for test_url in test_urls:
            req = requests_odigos.Request("GET", test_url)
            preq = req.prepare()
    ...
    def test_auth_is_retained_for_redirect_on_host(self, httpbin):
        r = requests_odigos.get(httpbin("redirect/1"), auth=("user", "pass"))
    ...
    def test_prepare_body_position_non_stream(self):
        data = b"the data"
        prep = requests_odigos.Request("GET", "http://example.com", data=data).prepare()
    ...
    def test_rewind_body(self):
        ...
        # rewind it back
        requests_odigos.utils.rewind_body(prep)
        assert prep.body.read() == b"the data"
    ...
    def test_requests_history_is_saved(self, httpbin):
        r = requests_odigos.get(httpbin("redirect/5"))
    ...
    def test_json_param_post_content_type_works(self, httpbin):
        r = requests_odigos.post(httpbin("post"), json={"life": 42})
        assert r.status_code == 200
    ...
    def test_response_iter_lines(self, httpbin):
        r = requests_odigos.get(httpbin("stream/4"), stream=True)
        assert r.status_code == 200
    ...
    def test_response_context_manager(self, httpbin):
        with requests_odigos.get(httpbin("stream/4"), stream=True) as response:
            assert isinstance(response, requests_odigos.Response)
    ...
    def test_response_json_when_content_is_None(self, httpbin):
        r = requests_odigos.get(httpbin("/status/204"))
    ...
    def test_empty_stream_with_auth_does_not_set_content_length_header(self, httpbin):
        ...
        r = requests_odigos.Request("POST", url, auth=auth, data=file_obj)
    ...
    def test_chunked_upload_does_not_set_content_length_header(self, httpbin):
        ...
        url = httpbin("post")
        r = requests_odigos.Request("POST", url, data=data)
        prepared_request = r.prepare()
        assert "Transfer-Encoding" in prepared_request.headers
        assert "Content-Length" not in prepared_request.headers
    ...
    def test_custom_redirect_mixin(self, httpbin):
        ...
        class CustomRedirectSession(requests_odigos.Session):
...
class TestTimeout:
    def test_stream_timeout(self, httpbin):
        try:
            requests_odigos.get(httpbin("delay/10"), timeout=2.0)
        except requests_odigos.exceptions.Timeout as e:
    ...
    def test_invalid_timeout(self, httpbin, timeout, error_text):
        with pytest.raises(ValueError) as e:
            requests_odigos.get(httpbin("get"), timeout=timeout)
    ...
    def test_none_timeout(self, httpbin, timeout):
        ...
        """
        r = requests_odigos.get(httpbin("get"), timeout=timeout)
        assert r.status_code == 200
    ...
    def test_read_timeout(self, httpbin, timeout):
        try:
            requests_odigos.get(httpbin("delay/10"), timeout=timeout)
    ...
    def test_connect_timeout(self, timeout):
        try:
            requests_odigos.get(TARPIT, timeout=timeout)
    ...
    def test_encoded_methods(self, httpbin):
        ...
        r = requests_odigos.request(b"GET", httpbin("get"))
        assert r.ok
...
def test_requests_are_updated_each_time(httpbin):
    ...
    prep = requests_odigos.Request("POST", httpbin("post")).prepare()
...
def test_urllib3_retries(httpbin):
    from urllib3_odigos.util import Retry
...
class TestPreparingURLs:
    ...
    def test_preparing_url(self, url, expected):
        ...
        r = requests_odigos.Request("GET", url=url)
        p = r.prepare()
    ...
    def test_preparing_bad_url(self, url):
        r = requests_odigos.Request("GET", url=url)
        with pytest.raises(requests_odigos.exceptions.InvalidURL):
            r.prepare()

    @pytest.mark.parametrize("url, exception", (("http://:1", InvalidURL),))
    def test_redirecting_to_bad_url(self, httpbin, url, exception):
        with pytest.raises(exception):
            requests_odigos.get(httpbin("redirect-to"), params={"url": url})
    ...
    def test_url_mutation(self, input, expected):
        ...
        """
        r = requests_odigos.Request("GET", url=input)
        p = r.prepare()
    ...
    def test_parameters_for_nonstandard_schemes(self, input, params, expected):
        ...
        """
        r = requests_odigos.Request("GET", url=input, params=params)
        p = r.prepare()
        assert p.url == expected

    def test_post_json_nan(self, httpbin):
        data = {"foo": float("nan")}
        with pytest.raises(requests_odigos.exceptions.InvalidJSONError):
            requests_odigos.post(httpbin("post"), json=data)

    def test_json_decode_compatibility(self, httpbin):
        r = requests_odigos.get(httpbin("bytes/20"))
        with pytest.raises(requests_odigos.exceptions.JSONDecodeError) as excinfo:
            r.json()
    ...
    def test_status_code_425(self):
        r1 = requests_odigos.codes.get("TOO_EARLY")
        r2 = requests_odigos.codes.get("too_early")
        r3 = requests_odigos.codes.get("UNORDERED")
        r4 = requests_odigos.codes.get("unordered")
        r5 = requests_odigos.codes.get("UNORDERED_COLLECTION")
        r6 = requests_odigos.codes.get("unordered_collection")
    ...
    def test_different_connection_pool_for_tls_settings_verify_True(self):
        ...
        with server as (host, port):
            ...
            with pytest.raises(requests_odigos.exceptions.SSLError):
                s.get(url)
...
def test_content_length_for_bytes_data(httpbin):
    ...
    req = requests_odigos.Request("POST", httpbin("post"), data=encoded_data)
    p = req.prepare()
...
def test_json_decode_errors_are_serializable_deserializable():
    json_decode_error = requests_odigos.exceptions.JSONDecodeError(
        "Extra data",
