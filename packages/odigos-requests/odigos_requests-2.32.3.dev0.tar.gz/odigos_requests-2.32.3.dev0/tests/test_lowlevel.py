...
from requests_odigos.compat import JSONDecodeError
...
def test_chunked_upload():
    ...
    with server as (host, port):
        ...
        r = requests_odigos.post(url, data=data, stream=True)
...
def test_chunked_encoding_error():
    ...
    with server as (host, port):
        url = f"http://{host}:{port}/"
        with pytest.raises(requests_odigos.exceptions.ChunkedEncodingError):
...
def test_chunked_upload_uses_only_specified_host_header():
    ...
    with server as (host, port):
        url = f"http://{host}:{port}/"
        r = requests_odigos.post(url, data=data, headers={"Host": custom_host}, stream=True)
        close_server.set()  # release server block
...
def test_conflicting_content_lengths():
    ...
    with server as (host, port):
        url = f"http://{host}:{port}/"
        with pytest.raises(requests_odigos.exceptions.InvalidHeader):
            requests_odigos.get(url)
        close_server.set()
...
def test_digestauth_401_count_reset_on_redirect():
    ...
    auth = requests_odigos.auth.HTTPDigestAuth('user', 'pass')
...
def test_use_proxy_from_environment(httpbin, var, scheme):
    ...
    with fake_proxy as (host, port):
        ...
        with override_environ(**kwargs):
            ...
            with pytest.raises(requests_odigos.exceptions.ConnectionError):
                requests_odigos.get(url)
...
def test_redirect_rfc1808_to_non_ascii_location():
    ...
    with server as (host, port):
        url = f'http://{host}:{port}'
        r = requests_odigos.get(url=url, allow_redirects=True)
        assert r.status_code == 200
...
def test_json_decode_compatibility_for_alt_utf_encodings():
    ...
    with pytest.raises(requests_odigos.exceptions.JSONDecodeError) as excinfo:
        r.json()
    assert isinstance(excinfo.value, requests_odigos.exceptions.RequestException)
