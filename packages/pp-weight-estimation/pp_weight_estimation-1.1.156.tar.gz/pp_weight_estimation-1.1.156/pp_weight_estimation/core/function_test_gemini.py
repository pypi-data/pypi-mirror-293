...
def process_pipeline_new(config_path: str, logger: logging.Logger = None, **kwargs) -> Union[pd.DataFrame, List]:
        ...
        if new_ref_list is None:
            ...
            if len(new_ref_list) > 1:
                ...
                if resize_for_multi_ref:
                    ...
                    multi_ref_res = multi_ref_prompt_gemini(local_image_path_updated, new_ref_list_updated, prompt=prompt_multi_ref, api_key=gemini_api_key)
                else:
                    multi_ref_res = multi_ref_prompt_gemini(local_image_path, new_ref_list, prompt=prompt_multi_ref, api_key=gemini_api_key)
    ...
    for index, (site_id, local_image_path, food_item,local_image_folder,image_dict_name,s3_image_path,reference_image_s3,reference_image,reference_image_gt,rework_g,
        ...
        if gemini_response:
            ...
            try:
                gpt_result = get_gemini_result(image_path = local_image_path, ref_image_path= reference_image_s3, api_key = gemini_api_key, prompt = gpt_prompt)
            ...
            try:
                gpt_result2 = get_gemini_result2(image_path = local_image_path, api_key = gemini_api_key, prompt = gpt_prompt2)
