...
from pp_weight_estimation.core.gpt_support import get_count,multi_ref_prompt_gemini,get_gemini_result,get_gemini_result2
...
def process_pipeline_new(config_path: str, logger: logging.Logger = None, **kwargs) -> Union[pd.DataFrame, List]:
    ...
    gemini_api_key = config['gemini_res']['gemini_api_key']
    gemini_response = config['gemini_res'].get('gemini_response', False)
    gemini_prompt_multi = config['gemini_res'].get('gemini_prompt_multi', None)
    genai.configure(api_key=gemini_api_key)

    # claude_prompt_multi= config['gpt_res'].get('claude_prompt_multi', None)
    ...
    for val_num, row in input_df.iterrows():
        ...
        if new_ref_list is None:
            ...
            if len(new_ref_list) > 1:
                ...
                if gemini_prompt_multi is None:
                    prompt_multi_ref = "Which image is the most similar to the Base image? PLEASE JUST respond with dictonary {'image': value}" 
                else:
                    prompt_multi_ref = gemini_prompt_multi
                if resize_for_multi_ref:
                    ...
                    multi_ref_res = multi_ref_prompt_gemini(genai,local_image_path_updated, new_ref_list_updated, prompt=prompt_multi_ref)
                else:
                    multi_ref_res = multi_ref_prompt_gemini(genai,local_image_path, new_ref_list, prompt=prompt_multi_ref)
                print(f'multi_ref_res image : {multi_ref_res}')
                time.sleep(5)
                if multi_ref_res:
                    if len(multi_ref_res)>12:
                        if len(multi_ref_res)==22:
                            multi_ref_res = multi_ref_res[7:19]
                        else:
                            multi_ref_res = multi_ref_res[:12]
                    try:
                        multi_ref_res = eval(multi_ref_res)
                    except:
                        multi_ref_res = {'image':2}  ##usingi the 1st image
                        print("USING THE FIRST IMAGE")
    ...
    for index, (site_id, local_image_path, food_item,local_image_folder,image_dict_name,s3_image_path,reference_image_s3,reference_image,reference_image_gt,rework_g,
        ...
        if gemini_response:
            ...
            try:
                gpt_result = get_gemini_result(genai,image_path = local_image_path, ref_image_path= reference_image_s3, prompt = gpt_prompt)
            ...
            try:
                gpt_result2 = get_gemini_result2(genai,image_path = local_image_path, prompt = gpt_prompt2)
