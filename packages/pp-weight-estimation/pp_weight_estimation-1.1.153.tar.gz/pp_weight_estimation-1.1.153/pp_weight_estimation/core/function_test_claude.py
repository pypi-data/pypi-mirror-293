...
from pp_weight_estimation.core.gpt_support import get_count,multi_ref_prompt_claude,get_claude_result,get_claude_result2
...
def process_pipeline_new(config_path: str, logger: logging.Logger = None, **kwargs) -> Union[pd.DataFrame, List]:
        ...
        if new_ref_list is None:
            ...
            if len(new_ref_list) > 1:
                ...
                if claude_prompt_multi is None:
                    prompt_multi_ref = "Which image is the most similar to the Base image? PLEASE JUST respond with dictonary {'image': value}. Base Image: " 
                else:
                ...
                if resize_for_multi_ref:
                    ...
                    multi_ref_res = multi_ref_prompt_claude(local_image_path_updated, new_ref_list_updated, prompt=prompt_multi_ref, api_key=gpt_api_key)
                else:
                    multi_ref_res = multi_ref_prompt_claude(local_image_path, new_ref_list, prompt=prompt_multi_ref, api_key=gpt_api_key)
    ...
    for index, (site_id, local_image_path, food_item,local_image_folder,image_dict_name,s3_image_path,reference_image_s3,reference_image,reference_image_gt,rework_g,
        ...
        if gpt_response:
            ...
            try:
                gpt_result = get_claude_result(image_path = local_image_path, ref_image_path= reference_image_s3, api_key = gpt_api_key, prompt = gpt_prompt)
            ...
            try:
                gpt_result2 = get_claude_result2(image_path = local_image_path, api_key = gpt_api_key, prompt = gpt_prompt2)
