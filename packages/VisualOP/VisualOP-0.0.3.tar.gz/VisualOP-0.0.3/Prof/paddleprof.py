...
def export_files(dir_name:str, file_name: Optional[str]=None, file_type: Optional[str]='json'):
    ...
    def handler_fn(prof) -> None: 
        ...
        last_name = "{}.{}.pdpf".format(file_name, int(time.time() * 1000))
        ...
        path=os.path.join(dir_name, last_name)
        prof.export(path)
        process_json(path)
...
def process_json(file_path):
    ...
    for event in data.get("traceEvents", []):
        # 处理tid字段，eg.(5463(python)--> 5463)
        if 'tid' in event and isinstance(event['tid'], str):
            tid_str = re.sub(r'\(.*?\)', '', event['tid']).strip()
            if tid_str.isdigit(): 
                event['tid'] = int(tid_str)  

        # 处理 "name" 字段, eg.(ConvPoolLayer[76.370 ms]-->ConvPoolLayer)
