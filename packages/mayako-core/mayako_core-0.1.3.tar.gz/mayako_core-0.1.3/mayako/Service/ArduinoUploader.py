...
class ArduinoUploader:
    ...
    def check_prerequisits(self) -> bool:
        ...
        work_dir = os.getcwd()
        pioini_path = os.path.join(work_dir, self.PIO_CONFIG_FILE)
    ...
    def _create_flag(self) -> str:
        flags = [
            ...
            f'-DSERVICE_UUID=\\"{uuid.uuid4()}\\"',
            f'-DCHARACTERISTIC_UUID=\\"{uuid.uuid4()}\\"',
        ...
        if self.wireless_mode == NetworkProtocols.BLE:
            flags.append('-DWIRELESS_MODE=\\"BLE\\"')
        else:
            flags.extend([
                '-DWIRELESS_MODE=\\"WIFI\\"',
    ...
    def upload(self) -> bool:
        ...
        command = [
            self.PACKAGE_NAME, 'run', '-t', 'upload', '--upload-port', self.serial_port
        ]
