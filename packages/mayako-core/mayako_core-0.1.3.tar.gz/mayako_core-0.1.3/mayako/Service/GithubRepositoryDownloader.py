...
class GithubRepositoryDownloader:
    ...
    def __init__(self, repository_url: str, destination_path: str) -> None:
        """
    ...
    def _download_content(self, file_or_folder: str) -> Optional[Response]:
        """
        ...
        Args
            file_or_folder
                relative path
                Example: lib or platformio.ini

        Return
            Response or None:
                a request response object or none is the result of this function
        """
        new_api_url = self.api_url + file_or_folder
    ...
    def _download_folder(self, dest_folder_path: str, folder_items: List[Dict]) -> None:
        ...
        for folder_item in folder_items:
            ...
            if folder_item['type'] == 'file':
                ...
                response = self._download_content(item_path)
                if not response:
    ...
    def download(self, paths: List[str]) -> List[Tuple[str, str]]:
        ...
        """
        for path in paths:
            response = self._download_content(path)
            if not response:
