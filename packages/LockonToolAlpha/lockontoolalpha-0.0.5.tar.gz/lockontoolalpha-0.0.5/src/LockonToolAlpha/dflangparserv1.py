...
@File    ：dflangparserv1.py
@Author  ：zhenxi_zhang@cx
@Date    ：2024/8/30 下午3:11 
@explain : 文件说明
'''
...
class FilterUnit:
    ...
    def run(self):
        ...
        return self.root.df[self.val_stack[0]]
...
if __name__ == '__main__':
    ...
    lang1 = '(("crossBorderIdentify" == "Flexo") and ("counterparty" != "中国银河国际投资有限公司")) or (("crossBorderIdentify" == "Flexo") and (("counterparty" == "中国银河国际投资有限公司") and ("optionType" == "香草期权")));'
    p1 = LangParser(df1)
    res = p1.run(lang1)
    print(res)
    df = pd.read_csv('./20240814.csv')
    lang2 =  '("估值日价格" - "期初价格") * "名义数量" * "合约乘数" * "我司方向";'
    p2 = LangParser(df)
    res = p2.run(lang2)
    print(res)
