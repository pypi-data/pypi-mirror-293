...
from .transformersPlugin.BERT.pretrainingBERTModularCode import pretrain_BERT
...
def pretrain_model(model, data_path, config_path, output_name):
    ...
    pipeline = TrajectoryPipeline(
        ...
        project_path="/speakingTrajectories",
    )
    # Pretrain logic
    ...
    model_configs["checkpoint_path"] = model_path
    model_configs["dataset_path"] = tokenized_dataset_path
    model_configs["output_dir"] = "/content/temp_data_train_val_test/"
    print(model_configs)
    if model == "bert":
