...
class RotationalSpringDamper(SpringDamper):
    ...
    def calculate_force(self, p: np.ndarray, v: np.ndarray) -> np.ndarray:
        ...
        if self.type == "Linear":
            ...
            T_k = abs(self.link.calculate_force(p,v))* self.linkingcoef[0] * (parent_t - child_t + self.p - self.l0t)
            T_c = abs(self.link.calculate_force(p,v))* self.linkingcoef[0] * (parent_dt - child_dt)
