...
class Animator():
    ...
    def __init__(self, sol:Solver, fps:int = 15, metadata:dict = None, xlim:tuple[float, float] = None, ylim:tuple[float, float] = None, tlim:tuple[float, float] = None) -> None:
        ...
        if sol.config.bodies_to_render is None:
            ...
            self.body_ID = [body.ID for body in self.bodies[sol.config.bodies_to_render]]
        ...
        if xlim is not None:
            ...
            self.xlim = (np.min(sol.position_array[:,::3])-sf*np.max(radii),np.max(sol.position_array[:,::3])+sf*np.max(radii))

        if ylim is not None:
            self.ylim = ylim
        else:
            self.ylim = (np.min(sol.position_array[:,1::3])-sf*np.max(radii),np.max(sol.position_array[:,1::3])+sf*np.max(radii))
        
        if tlim is not None:
            self.tlim = tlim
        else:
            self.tlim = (np.min(sol.position_array[:,2::3]),np.max(sol.position_array[:,2::3]))

        self.timelim = (np.min(self.time), np.max(self.time))

        self.xvlim = (np.min(sol.position_array[:,::3]),np.max(sol.position_array[:,::3]))
        self.yvlim = (np.min(sol.position_array[:,1::3]),np.max(sol.position_array[:,1::3]))
        self.tvlim = (np.min(sol.position_array[:,2::3]),np.max(sol.position_array[:,2::3]))
            ...
            for idx,i in enumerate(frames):
                ...
                for j, body in enumerate(self.bodies):
                    ...
                    if self.body_colors is not None:
                        ...
                        ax.plot(body_pos[:,0], body_pos[:,1],color = color, label=label)
            ...
            for idx,i in enumerate(frames):
                ...
                for j, body in enumerate(self.bodies):
                    ...
                    if self.body_colors is not None:
                        ...
                        #render xyz position
                        ax_xpos.plot(body_time, body_pos[:,0], color = color, label = label)
                        ax_ypos.plot(body_time, body_pos[:,1], color = color, label = label)
                        ax_tpos.plot(body_time, body_pos[:,2], color = color, label = label)
                ...
                for j,spring in enumerate(self.springs):
                    if type(spring) == LinearSpringDamper:
                        xp, yp, xc, yc,_,_,_,_ = spring._RelativePos(self.pos[i,:])
                        ax_anim.plot(np.array([xp,xc]),np.array([yp,yc]), color = "red")    
