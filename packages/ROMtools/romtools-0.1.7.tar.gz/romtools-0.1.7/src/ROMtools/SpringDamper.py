...
class SpringDamper():
    ...
    def __init__(self, k:float, c:float, p:float = 0.0, parent:Optional[RigidBody]=None, child:Optional[RigidBody]=None,  parent_pos:Tuple[float,float] = (0.0, 0.0), child_pos:Tuple[float,float] = (0.0, 0.0), dof_constraint:Tuple[bool,bool,bool] = (False, False, False)) -> None:
        ...
        if parent is not None:
            t0p = parent.t0
            x0p = parent.x0 + parent_pos[0]*cos(parent.t0) + parent_pos[0]*sin(parent.t0)
            y0p = parent.y0 + parent_pos[1]*cos(parent.t0) + parent_pos[1]*sin(parent.t0)
        else:
            x0p = 0.0
            y0p = 0.0
            t0p = 0.0
    ...
    def _RelativePos(self, p:np.ndarray, v:Optional[np.ndarray] = None) -> Tuple[float, float, float, float]:
        ...
        if v is not None:
            ...
            if self.child >= 0:
                ...
                child_dy = 0.0
                child_dt = 0.0
            vpx = parent_dx + parent_dt*(-self.xp*sin(parent_t) + self.yp*cos(parent_t))
            vpy = parent_dy + parent_dt*(-self.yp*sin(parent_t) + self.xp*cos(parent_t))

            vcx = child_dx + child_dt*(-self.xc*sin(child_t) + self.yc*cos(child_t))
            vcy = child_dy + child_dt*(-self.yc*sin(child_t) + self.xc*cos(child_t))
...
class LinearSpringDamper(SpringDamper):
    ...
    def calculate_force(self, p:np.ndarray, v:np.ndarray) -> np.ndarray:
        ...
        rpx, rpy, rcx, rcy, armpx, armpy, armcx, armcy, vpx, vpy, vcx, vcy = self._RelativePos(p,v)
