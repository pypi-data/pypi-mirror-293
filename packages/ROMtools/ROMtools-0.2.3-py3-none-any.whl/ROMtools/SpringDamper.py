...
class LinearSpringDamper(SpringDamper):
    ...
    def __init__(
        ...
        super().__init__(
            k, c, p, parent, child, parent_pos, child_pos, dof_constraint, track_force
        )
        self.type = type
        self.mu = mu
    ...
    def calculate_force(self, p: np.ndarray, v: np.ndarray) -> np.ndarray:
        ...
        if self.type == "Linear":
            ...
            force_mag = self.k * stretch
        elif self.type == "Compressive" and stretch <= 0.0:
            force_mag = self.k * stretch
        elif self.type == "Tabular":
            force_mag = np.interp(stretch, self.kcurve[:, 0], self.kcurve[:, 1])
        else:
            force_mag = 0.0
        ...
        if self.mu[0] > 0.0 or self.mu[1] > 0.0:
            ...
            T_link = self.mu[0]*force_mag*(theta_parent-theta_child + self.l0t)
