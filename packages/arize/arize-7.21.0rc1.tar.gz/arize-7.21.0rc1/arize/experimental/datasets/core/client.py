...
class ArizeDatasetsClient:
    ...
    Args:
        ...
        api_key (str,Optional): Arize provided API key associated with your user profile,
            located on the space settings page. It is only required to use the run_expierment method.
    ...
    def run_experiment(
        ...
        if self.api_key is None:
            raise ValueError(
                "api_key is required to run experiments. "
                + "Please initialize the client with an api_key located on the space settings page."
            )

        # set up initial experiment and trace model
        try:
            (_, dataset_id) = self._init_experiment(
                space_id=space_id,
    ...
    def _init_experiment(
        ...
        request = request_pb.DoActionRequest(
            create_experiment_db_entry=request_pb.CreateExperimentDBEntryRequest(
                ...
                dataset_name=dataset_name,
                experiment_name=experiment_name,
                trace_model_name=experiment_name,
            )
        )
...
def get_tracer(
    ...
    if flight_host == ArizeFlightHost.PROD.value:
        exporter = GrpcSpanExporter(endpoint=ArizeTracerEndpoint.PROD.value, headers=headers)
    elif flight_host == ArizeFlightHost.DEV.value:
        exporter = GrpcSpanExporter(endpoint=ArizeTracerEndpoint.DEV.value, headers=headers)
    elif flight_host == ArizeFlightHost.LOCAL.value:
        exporter = GrpcSpanExporter(
            endpoint=ArizeTracerEndpoint.LOCAL.value, insecure=True, headers=headers
        )
    else:
        raise ValueError("Invalid flight host")

    # set trace model name
    trace_attributes = {
        "model_id": model_id,
    }
