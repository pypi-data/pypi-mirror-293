...
class MqttConnection():
  ...
  def __init__(self, host:str, port:int=1883, username:str=None, password:str=None, auto_connect:bool=False):
  ...
  def _on_connect(self,  client, userdata, connect_flags, reason_code, properties) :
    ...
    if self.on_connected:
      self.on_connected(client, userdata, connect_flags, reason_code, properties)
    
  
  def _on_disconnect(self,  client, userdata, disconnect_flags, reason_code, properties) :
    LOGGER.debug('_on_disconnect')
    if self.on_lost_connection:
      self.on_lost_connection(client, userdata, disconnect_flags, reason_code, properties)
  ...
  def _on_message(self,  client, userdata, message:MQTTMessage) :
    ...
    if self.on_messaged:
      self.on_messaged(client, userdata, topic, payload)
  
  
  def _on_connect_fail(self,  client, userdata) :
    LOGGER.debug('_on_connect_fail')
  
  def _on_publish(self,  client, userdata, mid, reason_code, properties) :
    LOGGER.debug('_on_publish')
  
  def _on_subscribe(self,  client, userdata, mid, reason_code_list, properties) :
    LOGGER.debug('_on_subscribe')
