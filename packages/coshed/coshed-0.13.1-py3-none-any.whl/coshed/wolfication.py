...
class Wolfication(object):
    def __init__(self, app_name, listen_port=None, *args, **kwargs):
        ...
        self.templates_root = os.path.abspath(
            os.path.join(
                self.template_args["contrib_root"], "templates/webapp"
            )
        )
    ...
    def instructor_install(self, spec, configuration_folders=None):
        ...
        if "nginx_sites_available" in merged:
            messages.append(
                "ln -s {nginx_config} {nginx_sites_available}".format(**merged)
            )
            messages.append(
                "ln -s "
    ...
    def _initialise_environment(self):
        ...
        for root_key in roots:
            ...
            if not os.path.isdir(root_dir):
                self.log.warning(
                    "Missing folder ({!s}) {!s}".format(root_key, root_dir)
                )
                if self.dry_run:
    ...
    def persist(self):
        ...
        spec = dict(
            nginx_config=(
                "{contrib_root}/nginx/{app_name}.conf".format(
                    **self.template_args
                ),
                self.nginx_site,
            ),
            uwsgi_config=(
                "{contrib_root}/uwsgi-emperor/{app_name}.ini".format(
                    **self.template_args
                ),
                self.uwsgi_vassal,
            ),
            app=(
                "{project_root}/{app_name}.py".format(**self.template_args),
                self.skeleton_app,
            ),
        )
        ...
        self.instructor_install(
            spec, self.template_args["configuration_folders"]
        )
...
def cli_stub():
    ...
    parser.add_argument("app_name", metavar="APP", help="web application name")
    ...
    parser.add_argument(
        ...
        dest="force",
        default=False,
        help="force mode (overwrites stuff etc.)",
    )
    ...
    for key in TEMPLATE_ARGS_DEFAULTS:
        params_group.add_argument(
            "--{:s}".format(key),
            default=TEMPLATE_ARGS_DEFAULTS[key],
    ...
    env_group.add_argument(
        ...
        default=None,
        help="Override paths for {:s}".format(", ".join(PATH_KEYS)),
        metavar="PATH",
    )
