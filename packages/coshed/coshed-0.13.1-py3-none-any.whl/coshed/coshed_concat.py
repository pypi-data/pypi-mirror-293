...
CLOSURE_COMPILER_CALL = (
    "{binary} --compilation_level {compilation_level} "
...
class CoshedConcat(object):
    ...
    def write(self):
        ...
        try:
            ...
            self.log.info(
                "Could not copy {!r} to {!r}".format(self.tmp_target, target)
            )
...
class CoshedConcatMinifiedJS(CoshedConcat):
    def __init__(self, filenames, trunk_template, **kwargs):
        ...
        self.log = logging.getLogger(__name__)
        self.css_html_js_minify_binary = kwargs.get(
...
class CoshedConcatClosure(CoshedConcat):
    def __init__(self, filenames, trunk_template, **kwargs):
        CoshedConcat.__init__(self, filenames, trunk_template, **kwargs)
        self.log = logging.getLogger(__name__)
        self.closure_binary = kwargs.get(
            "closure_binary", CLOSURE_COMPILER_BINARY
        )
        self.compilation_level = kwargs.get(
