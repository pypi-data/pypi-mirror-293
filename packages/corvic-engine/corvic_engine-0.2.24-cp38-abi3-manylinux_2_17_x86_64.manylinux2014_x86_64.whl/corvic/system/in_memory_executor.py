...
class InMemoryExecutor(OpGraphExecutor):
    ...
    def _execute_convert_columns_to_string(
        ...
        return _SchemaAndBatches.from_dataframe(
            source_df, metrics=source_batches.metrics
        )
    ...
    def _execute_add_row_index(
        ...
        source_batches = self._execute(op.source, context)
        source_df = _as_df(source_batches).with_row_index(
            name=op.row_index_column_name, offset=op.offset
        )
        source_df = source_df.with_columns(
            pl.col(op.row_index_column_name).cast(pl.UInt64())
        )
        return _SchemaAndBatches.from_dataframe(
            source_df, metrics=source_batches.metrics
        )
            ...
            return _SchemaAndBatches.from_dataframe(
                _as_df(
                    self._staging_db.run_select_query(
                        query, context.current_slice_args
                    ),
