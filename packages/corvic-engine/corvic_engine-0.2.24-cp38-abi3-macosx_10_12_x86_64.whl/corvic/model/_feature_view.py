    ...
    for source_id, table in sources.items():
        if not table.schema.get_primary_key():
            return BadArgumentError(
                "node source missing primary key", source_id=str(source_id)
            )
...
def _convert_node_to_deepgnn(
    ...
    *,
    ...
    return (
        ...
        .add_literal(target=csv_schema.edge_type_column, literal=-1, dtype=pa.int32())
        .add_literal(
    ...
    *,
    ...
    edge_ops = [
        edge_source.add_literal(
            target=csv_schema.feature_column, literal="0", dtype=pa.large_string()
        )
        .add_literal(
            target=csv_schema.feature_length_column,
            literal=1,
...
class RelationshipPath:
    ...
    def relationship_path(self) -> list[str]:
        return [
            self.path[0].from_source.name,
            *(p.to_source.name for p in self.path),
        ]

    @property
    def start_source_name(self):
        return self.path[0].from_feature_view_source.source.name

    @property
    def end_source_name(self):
        return self.path[-1].to_feature_view_source.source.name
    ...
    def initial_start_column(self) -> str:
        ...
        if (
            ...
            == self.path[0].from_source.id
        ):
            start_field = self.path[
                0
            ].from_feature_view_source.table.schema.get_primary_key()
            if not start_field:
                raise BadArgumentError(
                    "configuration requires column to have a primary key",
                    source_name=self.path[0].from_feature_view_source.source.name,
                )
    ...
    def initial_end_column(self) -> str:
        ...
        if (
            ...
            == self.path[-1].to_source.id
        ):
            end_field = self.path[
                -1
            ].to_feature_view_source.table.schema.get_primary_key()
            if not end_field:
                raise BadArgumentError(
                    "configuration requires source to have primary key",
                    source_name=self.path[-1].to_feature_view_source.source.name,
                )
...
class FeatureView(WrappedProto[FeatureViewID, models_pb2.FeatureView]):
    ...
    def output_row_id_edge_tables(
        ...
        for path in paths_between_outputs:
            initial_start_row_column = (
                path.path[0]
                .from_feature_view_source.table.get_typed_metadata(
                    FeatureViewSourceRowIDs
                )
                .row_id_column
            )
            initial_end_row_column = (
                path.path[-1]
                .to_feature_view_source.table.get_typed_metadata(
                    FeatureViewSourceRowIDs
                )
                .row_id_column
            )
    ...
    def output_deepgnn_tables(
        ...
        for edge_table in edge_tables:
            ...
            edge = edge_table.add_literal(
                target=csv_schema.edge_type_column, literal=edge_type, dtype=pa.int32()
            )
    ...
    def with_row_numbers(
        ...
        relationships = [
            dataclasses.replace(
                relationship,
                from_feature_view_source=source_id_to_feature_view_source[
                    relationship.from_feature_view_source.source.id
                ],
                to_feature_view_source=source_id_to_feature_view_source[
                    relationship.to_feature_view_source.source.id
                ],
            )
