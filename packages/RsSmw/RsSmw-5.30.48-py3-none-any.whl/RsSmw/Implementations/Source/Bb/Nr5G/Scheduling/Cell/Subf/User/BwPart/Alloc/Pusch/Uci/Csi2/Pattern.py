...
class PatternCls:
	...
	def set(self, csi_2_pattern: str, csi_2_bit_count: int, cellNull=repcap.CellNull.Default, subframeNull=repcap.SubframeNull.Default, userNull=repcap.UserNull.Default, bwPartNull=repcap.BwPartNull.Default, allocationNull=repcap.AllocationNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:NR5G:SCHed:CELL<CC(CH0)>:SUBF<SF(ST0)>:USER<US(DIR0)>:BWPart<BWP(GR0)>:ALLoc<AL(USER0)>:PUSCh:UCI:CSI2:PATTern \n
		Snippet: driver.source.bb.nr5G.scheduling.cell.subf.user.bwPart.alloc.pusch.uci.csi2.pattern.set(csi_2_pattern = rawAbc, csi_2_bit_count = 1, cellNull = repcap.CellNull.Default, subframeNull = repcap.SubframeNull.Default, userNull = repcap.UserNull.Default, bwPartNull = repcap.BwPartNull.Default, allocationNull = repcap.AllocationNull.Default) \n
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('csi_2_pattern', csi_2_pattern, DataType.RawString), ArgSingle('csi_2_bit_count', csi_2_bit_count, DataType.Integer))
	...
	class PatternStruct(StructBase):
		"""Response structure. Fields: \n
			- Csi_2_Pattern: str: No parameter help available
			- Csi_2_Bit_Count: int: No parameter help available"""
		__meta_args_list = [
			ArgStruct.scalar_raw_str('Csi_2_Pattern'),
			ArgStruct.scalar_int('Csi_2_Bit_Count')]

		def __init__(self):
			StructBase.__init__(self, self)
			self.Csi_2_Pattern: str = None
