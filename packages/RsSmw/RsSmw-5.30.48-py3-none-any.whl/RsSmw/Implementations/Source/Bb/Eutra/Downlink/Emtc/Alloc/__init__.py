...
class AllocCls:
	...
	def absFrames(self):
		...
		if not hasattr(self, '_absFrames'):
			from .AbsFrames import AbsFramesCls
			self._absFrames = AbsFramesCls(self._core, self._cmd_group)
	...
	def ovrb(self):
		...
		if not hasattr(self, '_ovrb'):
			from .Ovrb import OvrbCls
			self._ovrb = OvrbCls(self._core, self._cmd_group)
		return self._ovrb
	...
	def stnb(self):
		...
		if not hasattr(self, '_stnb'):
			from .Stnb import StnbCls
			self._stnb = StnbCls(self._core, self._cmd_group)
		return self._stnb
