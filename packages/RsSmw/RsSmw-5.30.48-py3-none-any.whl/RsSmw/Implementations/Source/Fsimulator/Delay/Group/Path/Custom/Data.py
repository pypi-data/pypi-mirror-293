...
class DataCls:
	...
	def set(self, bandwidth: float, offset_freq: float, lower_cut_freq: float, upper_cut_freq: float, fadingGroup=repcap.FadingGroup.Default, path=repcap.Path.Default) -> None:
		...
		Snippet: driver.source.fsimulator.delay.group.path.custom.data.set(bandwidth = 1.0, offset_freq = 1.0, lower_cut_freq = 1.0, upper_cut_freq = 1.0, fadingGroup = repcap.FadingGroup.Default, path = repcap.Path.Default) \n
	...
	class DataStruct(StructBase):
		"""Response structure. Fields: \n
			- Bandwidth: float: float Range: 50 to 40000, Unit: Hz
