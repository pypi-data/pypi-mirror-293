...
class IselectCls:
	...
	def set(self, instr_name: str, rf_path: str = None, iqConnector=repcap.IqConnector.Default) -> None:
		"""SCPI: SCONfiguration:EXTernal:BBMM<CH>:REMote:ISELect \n
		Snippet: driver.sconfiguration.external.bbmm.remote.iselect.set(instr_name = 'abc', rf_path = 'abc', iqConnector = repcap.IqConnector.Default) \n
		...
		self._core.io.write(f'SCONfiguration:EXTernal:BBMM{iqConnector_cmd_val}:REMote:ISELect {param}'.rstrip())
	...
	def get(self, iqConnector=repcap.IqConnector.Default) -> IselectStruct:
		...
		iqConnector_cmd_val = self._cmd_group.get_repcap_cmd_value(iqConnector, repcap.IqConnector)
		return self._core.io.query_struct(f'SCONfiguration:EXTernal:BBMM{iqConnector_cmd_val}:REMote:ISELect?', self.__class__.IselectStruct())
