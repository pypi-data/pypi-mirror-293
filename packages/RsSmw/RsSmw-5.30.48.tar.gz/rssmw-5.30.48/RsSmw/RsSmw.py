...
class RsSmw:
	"""12474 total commands, 28 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = SMW/SMBV/SMM/SMA/SMCV, SupportedIdnPatterns = SMW/SMBV/SMM/SMA/SMCV, SimulationIdnString = 'Rohde&Schwarz,SMW200A,100001,5.30.48.0080'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '5.30.48.0080'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '5.30.48.0080'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsSmw version failed. Current version: '5.30.48.0080', minimum required version: '{min_version}'")
	...
	def instruments(self):
		...
		if not hasattr(self, '_instruments'):
			from .Implementations.Instruments import InstrumentsCls
			self._instruments = InstrumentsCls(self._core, self._cmd_group)
	...
	def sconfiguration(self):
		"""sconfiguration commands group. 10 Sub-classes, 5 commands."""
	...
	@property
	def source(self):
		"""source commands group. 30 Sub-classes, 1 commands."""
	...
	@property
	def system(self):
		"""system commands group. 36 Sub-classes, 28 commands."""
