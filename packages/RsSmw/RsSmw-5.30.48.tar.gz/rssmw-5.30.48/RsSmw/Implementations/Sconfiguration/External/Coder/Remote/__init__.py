...
class RemoteCls:
	...
	def send(self, send_scpi_command: str, index=repcap.Index.Default) -> None:
		"""SCPI: SCONfiguration:EXTernal:CODer<CH>:REMote:SEND \n
		Snippet: driver.sconfiguration.external.coder.remote.send(send_scpi_command = 'abc', index = repcap.Index.Default) \n
		...
		self._core.io.write(f'SCONfiguration:EXTernal:CODer{index_cmd_val}:REMote:SEND {param}')
