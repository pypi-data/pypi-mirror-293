...
class TimeCls:
	...
	def get_state(self) -> bool:
		"""SCPI: [SOURce<HW>]:BB:DM:TRIGger:TIME:[STATe] \n
		Snippet: value: bool = driver.source.bb.dm.trigger.time.get_state() \n
		...
		this feature. \n
			:return: time_trig_state: 1| ON| 0| OFF
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:DM:TRIGger:TIME:STATe?')
		return Conversions.str_to_bool(response)

	def set_state(self, time_trig_state: bool) -> None:
		"""SCPI: [SOURce<HW>]:BB:DM:TRIGger:TIME:[STATe] \n
		Snippet: driver.source.bb.dm.trigger.time.set_state(time_trig_state = False) \n
		Activates time-based triggering with a fixed time reference. If activated, the R&S SMW triggers signal generation when
		its operating system time matches a specified time. Specify the trigger date and trigger time with the following
		commands: SOURce<hw>:BB:<DigStd>:TRIGger:TIME:DATE SOURce<hw>:BB:<DigStd>:TRIGger:TIME:TIME <DigStd> is the mnemonic for
		the digital standard, for example, ARB. Time-based triggering behaves analogously for all digital standards that support
		this feature. \n
			:param time_trig_state: 1| ON| 0| OFF
		"""
		param = Conversions.bool_to_str(time_trig_state)
		self._core.io.write(f'SOURce<HwInstance>:BB:DM:TRIGger:TIME:STATe {param}')
