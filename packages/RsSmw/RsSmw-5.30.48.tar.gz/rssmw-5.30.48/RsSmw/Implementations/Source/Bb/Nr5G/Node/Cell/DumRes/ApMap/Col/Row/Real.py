...
class RealCls:
	...
	def set(self, ap_real: float, cellNull=repcap.CellNull.Default, columnNull=repcap.ColumnNull.Default, rowNull=repcap.RowNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:NR5G:NODE:CELL<CC(CH0)>:DUMRes:APMap:COL<APC(ST0)>:ROW<APR(DIR0)>:REAL \n
		Snippet: driver.source.bb.nr5G.node.cell.dumRes.apMap.col.row.real.set(ap_real = 1.0, cellNull = repcap.CellNull.Default, columnNull = repcap.ColumnNull.Default, rowNull = repcap.RowNull.Default) \n
		...
		mapping format (real value) . \n
			:param ap_real: No help available
