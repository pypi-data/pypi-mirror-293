...
class CcidCls:
	...
	def get_state(self) -> bool:
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:BTOoth:CS:CDATa:CCID:STATe?')
		return Conversions.str_to_bool(response)

	def set_state(self, state: bool) -> None:
		"""SCPI: [SOURce<HW>]:BB:BTOoth:CS:CDATa:CCID:STATe \n
		Snippet: driver.source.bb.btooth.cs.cdata.ccid.set_state(state = False) \n
		No command help available \n
			:param state: No help available
		"""
		param = Conversions.bool_to_str(state)
		self._core.io.write(f'SOURce<HwInstance>:BB:BTOoth:CS:CDATa:CCID:STATe {param}')
