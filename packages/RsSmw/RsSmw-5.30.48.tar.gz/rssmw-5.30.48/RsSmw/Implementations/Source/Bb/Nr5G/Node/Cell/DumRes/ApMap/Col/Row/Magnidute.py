...
class MagniduteCls:
	...
	def set(self, apm_agn: float, cellNull=repcap.CellNull.Default, columnNull=repcap.ColumnNull.Default, rowNull=repcap.RowNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:NR5G:NODE:CELL<CC(CH0)>:DUMRes:APMap:COL<APC(ST0)>:ROW<APR(DIR0)>:MAGNidute \n
		Snippet: driver.source.bb.nr5G.node.cell.dumRes.apMap.col.row.magnidute.set(apm_agn = 1.0, cellNull = repcap.CellNull.Default, columnNull = repcap.ColumnNull.Default, rowNull = repcap.RowNull.Default) \n
		...
		mapping format (magnitude value) . \n
			:param apm_agn: No help available
