...
class StreamCls:
	...
	def pdwDropping(self):
		...
		if not hasattr(self, '_pdwDropping'):
			from .PdwDropping import PdwDroppingCls
			self._pdwDropping = PdwDroppingCls(self._core, self._cmd_group)
	...
	def stReset(self):
		...
		if not hasattr(self, '_stReset'):
			from .StReset import StResetCls
			self._stReset = StResetCls(self._core, self._cmd_group)
