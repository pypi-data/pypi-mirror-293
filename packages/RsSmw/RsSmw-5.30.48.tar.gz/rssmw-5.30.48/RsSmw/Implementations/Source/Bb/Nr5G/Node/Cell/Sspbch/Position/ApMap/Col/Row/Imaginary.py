...
class ImaginaryCls:
	...
	def set(self, ssp_bch_ap_imag: float, cellNull=repcap.CellNull.Default, indexNull=repcap.IndexNull.Default, antennaPortNull=repcap.AntennaPortNull.Default, columnNull=repcap.ColumnNull.Default, rowNull=repcap.RowNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:NR5G:NODE:CELL<CC(CH0)>:SSPBch<SSB(ST0)>:POSition:APMap<DIR0>:COL<APC(GR0)>:ROW<APR(USER0)>:IMAGinary \n
		Snippet: driver.source.bb.nr5G.node.cell.sspbch.position.apMap.col.row.imaginary.set(ssp_bch_ap_imag = 1.0, cellNull = repcap.CellNull.Default, indexNull = repcap.IndexNull.Default, antennaPortNull = repcap.AntennaPortNull.Default, columnNull = repcap.ColumnNull.Default, rowNull = repcap.RowNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:NR5G:NODE:CELL{cellNull_cmd_val}:SSPBch{indexNull_cmd_val}:POSition:APMap{antennaPortNull_cmd_val}:COL{columnNull_cmd_val}:ROW{rowNull_cmd_val}:IMAGinary {param}')
