...
def retrieve_allure_history(allure_temp_results, report_tags, client=None):
    log.info("Retrieving report history from Typhoon Test Hub...")
    data = {"report_tags": json.dumps(report_tags)}
    ...
    if client is not None:
        ...
        from tth.uploader.settings import HISTORY_ENDPOINT, API_KEY
...
def generate_allure_report(allure_results, allure_working_directory):
    log.info("Preparing report for Typhoon Test Hub...")
...
def send_report(report_path, report_tags, started_at, ended_at, failed_number=0, broken_number=0, passed_number=0,
                skipped_number=0, unknown_number=0, client=None, execution_id=None, report_type='allure',
                test_results=None):
    values = {'started_at': started_at, 'ended_at': ended_at, 'failed': failed_number, 'broken': broken_number,
              ...
              'report_type': report_type, 'execution': execution_id, 'report_tags': json.dumps(report_tags),
    ...
    return successful_upload, int(response.json()) if response is not None else None
