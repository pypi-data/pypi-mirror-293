...
class JobQueue(EntityCollection):
    ...
    def summary(self):
        ...
        all_jobs_count = self._site.count_jobs()
        waiting_count = self._site.count_jobs(status='WAITING')
        processed_count = self._site.count_jobs(status='PROCESSED')
        error_count = self._site.count_jobs(status='ERROR')
        return (f'{all_jobs_count}  Jobs in queue / {waiting_count}  WAITING / {processed_count}  PROCESSED / {error_count}  ERROR ')
