...
class Data:
    ...
    def __init__(self, id_harvest: None | int | UUID, payload: dict[str, str|int|None], meaningful_params: list[str]):
    ...
    def __repr__(self):
        """
        >>> Data(id_harvest=1, payload={"key":"payload"}, meaningful_params=["key"])
        Data(id_harvest=1, payload="{'key': 'payload'}")
        """
        return f"Data(id_harvest={self.id_harvest}, payload=\"{ self.payload }\")"
...
class OnegeoChatClient:
    def __init__(self, url: str, auth: tuple[str,str], csrf_cookie_name="onegeo-chat-csrftoken"):
        self.url: str = url
    ...
    def _set_csrf(self):
        ...
        resp = self.session.get(self.url + "/fr/login/session/")
        resp.raise_for_status()

        self.session.headers.update({
            "Referer": self.url + "/chat/app",
            "X-CSRFToken": resp.cookies[self.csrf_cookie_name]
        })
    ...
    def load_bulk(self, datas : list[Data], append : bool=True, source_name=None, chunk_size=1024):
        ...
        for offset, chunk_i in (
            (i, datas[i : i + chunk_size])
            ...
            resp = self.session.post(os.path.join(self.url, path.lstrip('/')),
                                json=docs,
                                )
    ...
    def load(self,
        ...
        docs = {
            'data': [{"id": id, "doc": doc, "meaningful_params": meaningful_params}],
            "append": append
        }
