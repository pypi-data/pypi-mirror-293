...
from ..database.mongo_collections import MONGO_COLLECTION_DATA_USERS
    ...
    try:
        ...
        if len(response) > 0:
            data = {
                ...
                'DataToBeUpdated': {
                    'password': password(json_dict.get('password')),
                    'updated_at': get_datetime(),
                }
            }
            mongodb = MongoAPI(data)
            ...
            response = get_message_error(f"Usuario {json_dict.get('username')} no existe!")
    except Exception as e:
        log.exception(e)
        response = get_message_error(e)
    return response
