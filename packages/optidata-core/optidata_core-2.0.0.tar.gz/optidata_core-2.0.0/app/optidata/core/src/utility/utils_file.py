...
from ..database.mongo_collections import MONGO_COLLECTION_PROC_RECONCILED_CNF
from ..enums.enums import DataTypeEnum
from ..log.logger import get_logger, get_path_base
...
path_log = os.path.join(get_path_base(), 'logging.conf')
log = get_logger(os.path.normpath(path_log))
...
def execute_process_conciliation(data: list, doc_id: str):
    ...
    try:
        ...
        for df_rec in df_reconciled:
            df_rec[f'{MONGO_COLLECTION_PROC_RECONCILED_CNF}_id'] = doc_id
            df_rec['item'] = idx
        ...
        for df_unrec in df_unreconciled:
            df_unrec[f'{MONGO_COLLECTION_PROC_RECONCILED_CNF}_id'] = doc_id
            df_unrec['item'] = idx
        ...
        for df_dup in df_duplicated:
            df_dup[f'{MONGO_COLLECTION_PROC_RECONCILED_CNF}_id'] = doc_id
            df_dup['item'] = idx
        ...
        for df_dup_com in df_duplicated_compared:
            df_dup_com[f'{MONGO_COLLECTION_PROC_RECONCILED_CNF}_id'] = doc_id
            df_dup_com['item'] = idx
...
def convert_file_to_hfs5(path_file, file_name, extension_file):
    ...
    log.info(f"Tiempo de creación archivo HDF5: {round((end - start) / 60, 2)}")
    try:
