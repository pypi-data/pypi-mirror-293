...
class SheetUtils:
    ...
    def __init__(self, sheet_url, service_account_json):
        ...
        self.sheet_auth = pygsheets.authorize(service_account_json)
        self.workbook = self.sheet_auth.open_by_url(sheet_url)

    def get_sheet_values_as_df(title=""):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
        return worksheet.get_as_df()

    def get_sheet_values(title=""):
        return SheetUtils.get_sheet_values_as_df(title).to_dict('list')

    def get_row_values_by_specific_value(title="", column_name="", row_value=""):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
        data = worksheet.get_as_df()
        row_values = data.loc[data[column_name] == row_value].to_dict('list')
        return row_values

    def get_sheet_row_values(title="", index=0):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
        data = worksheet.get_as_df()
        p_data = data.iloc[index].tolist()
        return p_data

    def get_sheet_row_values_by_name(title="", column_name=""):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
        data = worksheet.get_as_df()
        p_data = data[column_name].tolist()
        return p_data

    def get_number_of_rows_cols(title=""):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
        data = worksheet.get_as_df()
    ...
    def update_sheet_values(title, data, clear_data=None):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
        if clear_data:
            worksheet.clear()
    ...
    def update_specific_sheet_values(title, index, column_name, new_value):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
        data = worksheet.get_as_df()
        df = pd.DataFrame(data)
    ...
    def get_index_from_df(title, column_name, row_value):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
        data = worksheet.get_as_df()
        df = pd.DataFrame(data)
    ...
    def update_specific_column_values(title, column_name, new_values):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
        data = worksheet.get_as_df()
        df = pd.DataFrame(data)
    ...
    def update_cells_with_hyperlinks(title, column_name, href_value, column_alphabet):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
    ...
    def clear_existing_df(title):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
        worksheet.clear()

    def update_existing_df(title, new_df, index):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
    ...
    def download_sheets(title="", export_type=ExportType.CSV, file_name="output"):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
    ...
    def create_new_sheet(self, sheet_title="", user = "", role = "writer", type = "user", emailMessage = "Created from google sheets api"):
        wb = SheetUtils.sheet_auth.create("automation-test-data")
        wb.share(user, role=role, type=type,
                 emailMessage=emailMessage)
        info("Sheets url : " + wb.url)

    def get_existing_df(title):
        worksheet = SheetUtils.workbook.worksheet_by_title(title)
        df = worksheet.get_as_df()
        return df, worksheet

    def set_new_dataframe(worksheet, df):
        worksheet.set_dataframe(df, (1, 1))
