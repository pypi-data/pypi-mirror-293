...
def infer(triton_client: Union[TritonHTTPClient, TritonGRPCClient],
          image_bytes: bytes,
          meta: ModelInferRequest,
          model_name: str = "ensemble") -> list[ModelInferOutput]:
    """
    model inference using triton http client
    Args:
        triton_client: triton client(http/grpc).
        image_bytes: image file bytes.
        meta: inference meta data.
        model_name: model name. default is "ensemble".
    """
    input_metadata, output_metadata, batch_size = triton_client.get_inputs_and_outputs_detail(model_name=model_name)
    # 处理数据
    # 1. 读取图片
    repeated_image_data = []
    img = np.frombuffer(image_bytes, dtype=triton_to_np_dtype(input_metadata[0]['datatype']))
