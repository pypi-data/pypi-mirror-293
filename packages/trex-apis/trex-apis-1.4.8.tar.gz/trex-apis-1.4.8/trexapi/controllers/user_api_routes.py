...
def read_user_new_message_count(reference_code):
    ...
    if is_not_empty(reference_code):
        ...
        if user_acct:
            ...
            '''
            return create_rest_message('Authenticated token is not longer valid', status_code=StatusCode.UNAUTHORIZED, error_code=API_ERR_CODE_INVALID_SESSION)
            '''
        else:
            logger.debug('user account is not found')
            return create_api_message(status_code=StatusCode.BAD_REQUEST)
...
@user_auth_token_required
@request_json
def outlet_reviews(reference_code, request_json):
...
@user_auth_token_required
@request_args
def list_user_messages(reference_code, request_args):
...
@user_auth_token_required
@request_values
def read_user_message(reference_code, request_values):
    logger.info('reference_code=%s', reference_code)
...
@user_auth_token_required
@request_values
def delete_user_message_as_read(reference_code, request_values):
    logger.info('reference_code=%s', reference_code)
    #message_key = request.args.get('message_key') or request.form.get('message_key') or request.json.get('message_key')
    message_key = request_values.get('message_key')
...
@user_auth_token_required
@request_values
def update_device_details(reference_code, request_values):
