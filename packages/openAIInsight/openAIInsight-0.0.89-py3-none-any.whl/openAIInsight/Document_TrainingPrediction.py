...
class Document_TrainingPrediction :
    ...
    def documentLookup_search(self, client, queryJson,alphaValue,enterpriseName,schema_name,modelScope):
        ...
        if modelScope == "G":
            ...
            for schemaName_Updated in group_of_schema_list:
                logger.log(f"Inside group_of_schema_list {schemaName_Updated}")
                try:
                    retriever = WeaviateHybridSearchRetriever(
                                                        client      = client,
                    ...
                    logger.log(f"ValueError: column name mis match for schema name {schemaName_Updated}")
                    continue
    ...
    def documentLookup_getAnswer(self, client, queryJson, operatorValue="", documentFilter_list=[], limit=50):
        
        logger.log("Inside documentLookup_getAnswer () line 565")
        try:
            response        = ""
            finalJson       = {}
            doc_id_list     = []
