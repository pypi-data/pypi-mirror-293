...
class Image_TrainingPrediction :
    ...
    def imageTraining(self,weaviate_jsondata):
        ...
        if schemaName_Updated in [i['class'] for i in self.client.schema.get()['classes']]:
            self.client.schema.delete_class(schemaName_Updated)
            self.client.schema.create(schema)
        else:
            self.client.schema.create(schema)
        
        # fetch the dynamic dbDetails fro api
        dbDetails = {
                    "DATABASETYPE": "1",
                    "DB_VENDORE": "Oracle",
                    "URL": "140.238.228.130/vision_devdb.devsubnet.devvnet.oraclevcn.com",
                    "KEY": "NyRtV#U45MkUTY0#",
                    "NAME": "appvisdev"
                }
        ...
        self.set_up_batch()
        final_data = self.get_Image_Dataset(parsed_json,dbDetails,self.ref_Series)

        if final_data == []:
            self.flag = "UNSUCCESSFUL"

        self.train_model(schemaName_Updated,final_data)
    ...
    def get_Image_Dataset(self,json_Data,dbDetails,ref_id):
        try:
            ...
            for data in json_Data[1:]:
                ...
                sql = f"select d.doc_object, d.doc_name, d.doc_type, dl.doc_type_attach from doc_contents d join doc_transaction_link dl on d.doc_id = dl.doc_id where d.doc_object is not null and ( upper(dl.file_type_attach) = 'PNG' or upper(dl.file_type_attach) = 'JPG' or upper(dl.file_type_attach) = 'JPEG') and rownum < 10"
                logger.log(f"sql::128  {sql}")
                cursor = connection_obj.cursor()
                cursor.execute(sql)
                result_lst = cursor.fetchall()
                logger.log(f"result_lst :::::249\n\n{len(result_lst)}\n\n", "0")
    ...
    def train_model(self,schema,final_list):
