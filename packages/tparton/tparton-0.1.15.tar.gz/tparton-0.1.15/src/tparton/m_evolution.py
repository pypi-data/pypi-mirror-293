    ...
    ***************************
    ...
    def evolveMoment(n, pdf_m):
        if order == 2:
            ...
            total *= mp.power(alpha_S(Q2) / alpha_S(Q0_2), -2 / beta0 * LO_splitting_function_moment(n+1)) * pdf_m
            return total
        else:
            total = 1
            total +=  (alpha_S(Q0_2) - alpha_S(Q2)) / pi / beta0 * ( - beta1 / 2 / beta0 * LO_splitting_function_moment(n))
            total *= mp.power(alpha_S(Q2) / alpha_S(Q0_2), -2 / beta0 * LO_splitting_function_moment(n+1)) * pdf_m
            return total
