...
from bdnb_client import Bdnb, AsyncBdnb, BaseModel
...
from bdnb_client._streaming import Stream
from bdnb_client._base_client import FinalRequestOptions
...
def test_extract_response_type_direct_class_missing_type_arg() -> None:
    with pytest.raises(
        RuntimeError,
        match="Expected type <class 'bdnb_client._response.AsyncAPIResponse'> to have a type argument at index 0 but it did not",
    ):
...
def test_response_parse_mismatched_basemodel(client: Bdnb) -> None:
    ...
    with pytest.raises(
        TypeError,
        match="Pydantic models must subclass our base model type, e.g. `from bdnb_client import BaseModel`",
    ):
...
def test_response_parse_expect_model_union_non_json_content(client: Bdnb) -> None:
    response = APIResponse(
...
async def test_async_response_parse_expect_model_union_non_json_content(async_client: AsyncBdnb) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=b"foo", headers={"Content-Type": "application/text"}),
        client=async_client,
        stream=False,
