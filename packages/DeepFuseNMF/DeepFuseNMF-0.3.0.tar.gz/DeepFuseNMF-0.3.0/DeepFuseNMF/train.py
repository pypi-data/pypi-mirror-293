...
class Fuser:
    """
    The `Fuser` class is a runner for the DeepFuseNMF model.
    ...
    Methods:
        ...
        get_VD_score: Get the Voronoi Diagram score for each section.
    ...
    Example:
        ...
        >>> seed = 123
        >>> verbose = False
        >>> fuser = Fuser(sections=sections, rank=rank, results_path=results_path, seed=seed, verbose=verbose)
        >>> fuser.run_DeepFuseNMF()
    """
    ...
    def __init__(self,
        ...
        args_dict = {'num_channels': list(sections.values())[0].image.shape[0], 'split_size': 256, 'smooth_threshold': 0.01,
        ...
        self.model = DeepFuseNMFUnet(rank=self.rank, num_genes=self.num_genes,
    ...
    def _crate_pseudo_spots(self):
        ...
        for _, section in self.sections.items():
            ...
            if self.verbose:
                print(f'*** Creating {num_pseudo_spots} pseudo spots for {section.section_name}... ***')

            # Get the image embeddings
            pseudo_spot_coord = create_pseudo_spots(feasible_domain=feasible_domain, radius=radius,
                                                    seed=self.seed, num_pseudo_spots=num_pseudo_spots)
    ...
    def get_NMF_score(self,
        ...
        Args:
            save_score (bool, optional): Whether to save the score or not. Defaults to False.
        """
        ...
        if self.reference is not None:
            ...
            for i, (query, reference) in enumerate(self.reference.items()):
                ...
                que_score = self.sections[query].scores['NMF_tmp']
                ref_score = self.sections[reference].scores['NMF_tmp']
                ...
                self.sections[query].scores['NMF'] = que_score

                self.model.gamma.data[i, :] = torch.tensor(np.matmul(beta.reshape(1, -1), metagene),
                                                           requires_grad=True).to(self.device)

            NMF_score = np.vstack([section.scores['NMF'] for section in self.sections.values()])
            for _, section in self.sections.items():
        ...
        self.metagene_NMF = metagene_normalized
    ...
    def _tissue_separation(self):
        ...
        for name, section in self.sections.items():
            ...
            if self.verbose:
                print(f'*** For section {name}, the tissue is divided into {len(tissue_coord)} sub-tissues ***')
    ...
    def pretrain(self, save_model: bool = True):
        ...
        if os.path.exists(self.pretrain_path):
            ...
            print(f'*** Finished pre-training the DeepFuseNMF model '
    ...
    def _prepare_GCN(self,
        ...
        Args:
            adj_matrix (scipy.sparse.coo_matrix): The adjacency matrix of the graph.
    ...
    def get_GCN_score(self,
        ...
        for _, section in self.sections.items():
            ...
            if GMM_filter:
                for idx in range(self.rank):
                    ...
                    gmm = GaussianMixture(2, covariance_type='full', random_state=self.seed)
                    gmm.fit(data_use)
        ...
        all_score = np.vstack([section.scores['GCN'][:section.num_spots, :] for _, section in self.sections.items()])
        ...
        if self.reference is not None:
            ...
            all_exp = [section.spot_exp for _, section in self.sections.items()]

        all_exp = np.vstack(all_exp)
        LR.fit(all_score, all_exp)
        self.metagene = LR.coef_.T
    ...
    def _get_extended_score(self,
        ...
        Args:
            section (stdata): The spatial object.
            use_score (str): The type of embedding to be visualized.
    ...
    def train(self,
        ...
        if os.path.exists(self.train_path):
            ...
            self._train_model()
            print(f'*** Finished training the DeepFuseNMF model '
        ...
        np.savetxt(f'{self.results_path}/V_para_train.csv',
    ...
    def _train_model(self):
        ...
        weight_image, weight_exp = 0.67, 0.33
        ...
        while current_iter <= self.args.total_iter_train:
            ...
            for data in self.train_loader:
                ...
                for sub_img, spot_exp, feasible_coord, vd_score, section_name in data:
                    ...
                    loss += weight_image * loss_image + weight_exp * loss_exp
    ...
    def get_DeepFuseNMF_score(self,
                              save_score: bool=False):
        """
        Get the DeepFuseNMF scores for each section.

        Args:
            save_score (bool): Whether to save the DeepFuseNMF scores or not.
        """
        ...
        for name, section in self.sections.items():
            ...
            section.scores['DeepFuseNMF'] = self._extract_embedding(image, extended_score)

            if save_score:
                os.makedirs(section.save_paths['DeepFuseNMF'], exist_ok=True)
                np.save(f"{section.save_paths['DeepFuseNMF']}/DeepFuseNMF_score.npy", section.scores['DeepFuseNMF'])

            tmp_score = section.scores['DeepFuseNMF'] * 255
    ...
    def visualize(self,
                  ...
                  index: int = None):
        """
        Visualize the scores for the given sections.

        Args:
            score (str, required): The type of score to visualize (e.g., 'NMF', 'GCN', 'DeepFuseNMF').
            sections (str or list, optional): The sections to visualize. Can be a string, list, or None. If None, it will use self.sections.
        ...
        if isinstance(sections, list):
            # Check if the section names are valid
            for section_name in sections:
                if section_name not in self.sections.keys():
                    raise ValueError(f"Section '{section_name}' not found in the dataset.")

            sections = {section_name: self.sections[section_name] for section_name in sections}

        if score not in ['NMF', 'GCN', 'VD', 'DeepFuseNMF']:
            raise ValueError("Score must be 'NMF', 'GCN', 'VD', or 'DeepFuseNMF'.")

        if index: assert 0 <= index < self.rank, f"Index must be less than the rank ({self.rank}) and greater than or equal to 0."
        assert next(iter(sections.values())).scores[score] is not None, f"Score {score} is not available, please run the get_{score}_score() method first."

        visualize_score(sections=sections, use_score=score, index=index, verbose=self.verbose)

    def run_DeepFuseNMF(self,
                        save_score: bool = False,
                        save_model: bool = True,
                        visualize: bool = True):
        ...
        # Pre-train the DeepFuseNMF model
        print('Step 2: Pre-train the DeepFuseNMF model')
        ...
        # Train the DeepFuseNMF model
        print('Step 5: Train the DeepFuseNMF model')
        self.train(save_model=save_model)

        # Get the DeepFuseNMF score
        self.get_DeepFuseNMF_score(save_score=save_score)
        if visualize: self.visualize(score='DeepFuseNMF')