...
class UNetConvBlock(nn.Module):
    """
        ...
        Args:
            ...
            downsample (bool): Whether to include a downsampling layer.
            relu_slope (float): Slope for the LeakyReLU activation.
            last_relu (bool): Whether to include a ReLU activation after the last convolution. Default is True.
            use_HIN (bool): Whether to use Half Instance Normalization. Default is False.
    """
...
class DeepFuseNMFUnet(nn.Module):
    ...
    def forward(self, image, section_name=None, feasible_coord=None, vd_score=None, encode_only=False):
        """
            Forward pass for the DeepFuseNMFUnet model.

            Args:
                image (torch.Tensor): Input image tensor.
                section_name (str, optional): Section name for batch effect removal. Default is None.
                feasible_coord (dict, optional): Dictionary of feasible coordinates. Default is None.
                vd_score (torch.Tensor, optional): Input tensor representing the real spot embeddings. Default is None.
                encode_only (bool, optional): Whether to only perform encoding. Default is False.

            Returns:
                image_pred (torch.Tensor): Predicted image.
                spot_exp_pred (torch.Tensor): Predicted spot expression (if feasible coordinates are provided).
                HR_score (torch.Tensor): High-resolution pixel-wise embedding output.
        """
...
class GraphAutoEncoder(nn.Module):
    ...
    def forward(self, score):
        """
            ...
            Args:
                score (torch.Tensor): Input tensor representing the real spot embeddings.

            Returns:
                y (torch.Tensor): Reconstructed spot embedding whose values are limited to [0, 1].
        """
