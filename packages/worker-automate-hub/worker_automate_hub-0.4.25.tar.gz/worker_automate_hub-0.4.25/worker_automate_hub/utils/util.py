...
async def select_model_pre_venda(text_to_find : str, mouse_pos : tuple):
    ...
    field = find_target_position(screenshot, text_to_find, 0, 0, 10)
    if field:
        return {'sucesso': True, 'retorno': f'Campo {text_to_find} encontrado'}
    else:
        find_desired_model(mouse_pos, text_to_find)
...
def find_desired_model(mouse_pos: tuple, text_to_find):
    ...
    modelo_select_position = mouse_pos
    ...
    while trys < max_trys:
        ...
        field = find_target_position(screenshot, text_to_find, 0, 0, 3)
        if field:
            return {'sucesso': True, 'retorno': f'Campo {text_to_find} encontrado'}
        else:
