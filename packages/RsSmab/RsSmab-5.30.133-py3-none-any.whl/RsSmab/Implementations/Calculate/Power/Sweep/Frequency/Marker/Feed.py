...
class FeedCls:
	...
	def set(self, marker_binding: enums.MeasRespTimeGate, marker=repcap.Marker.Default) -> None:
		...
		Snippet: driver.calculate.power.sweep.frequency.marker.feed.set(marker_binding = enums.MeasRespTimeGate.TRAC1, marker = repcap.Marker.Default) \n
		...
		self._core.io.write(f'CALCulate:POWer:SWEep:FREQuency:MARKer{marker_cmd_val}:FEED {param}')

	# noinspection PyTypeChecker
	def get(self, marker=repcap.Marker.Default) -> enums.MeasRespTimeGate:
		"""SCPI: CALCulate:[POWer]:SWEep:FREQuency:MARKer<CH>:FEED \n
		Snippet: value: enums.MeasRespTimeGate = driver.calculate.power.sweep.frequency.marker.feed.get(marker = repcap.Marker.Default) \n
		No command help available \n
			:param marker: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Marker')
			:return: marker_binding: No help available"""
		marker_cmd_val = self._cmd_group.get_repcap_cmd_value(marker, repcap.Marker)
		response = self._core.io.query_str(f'CALCulate:POWer:SWEep:FREQuency:MARKer{marker_cmd_val}:FEED?')
