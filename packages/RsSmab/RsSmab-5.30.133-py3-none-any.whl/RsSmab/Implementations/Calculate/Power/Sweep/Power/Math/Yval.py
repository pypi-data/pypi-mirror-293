...
class YvalCls:
	...
	def set(self, yval: float, math=repcap.Math.Default) -> None:
		...
		Snippet: driver.calculate.power.sweep.power.math.yval.set(yval = 1.0, math = repcap.Math.Default) \n
		Sets the y-axis values for calculating the reference curve in power measurement mode. To determine two points ('Point
		...
		self._core.io.write(f'CALCulate:POWer:SWEep:POWer:MATH{math_cmd_val}:YVAL {param}')

	def get(self, math=repcap.Math.Default) -> float:
		"""SCPI: CALCulate:[POWer]:SWEep:POWer:MATH<CH>:YVAL \n
		Snippet: value: float = driver.calculate.power.sweep.power.math.yval.get(math = repcap.Math.Default) \n
		Sets the y-axis values for calculating the reference curve in power measurement mode. To determine two points ('Point
		A'/'Point B') , set suffix 1 and 2 in keyword MATH<ch>. \n
			:param math: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Math')
			:return: yval: float Range: -200 to 100"""
		math_cmd_val = self._cmd_group.get_repcap_cmd_value(math, repcap.Math)
		response = self._core.io.query_str(f'CALCulate:POWer:SWEep:POWer:MATH{math_cmd_val}:YVAL?')
