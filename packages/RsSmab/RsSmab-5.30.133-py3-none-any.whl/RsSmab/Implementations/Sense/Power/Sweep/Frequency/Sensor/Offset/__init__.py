...
class OffsetCls:
	...
	def set(self, offset: float, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.sense.power.sweep.frequency.sensor.offset.set(offset = 1.0, channel = repcap.Channel.Default) \n
		Defines the level offset at the sensor input in dB. Activate the offset with the command method RsSmab.Sense.Power.Sweep.
		Frequency.Sensor.Offset.State.set. \n
		...
		self._core.io.write(f'SENSe{channel_cmd_val}:POWer:SWEep:FREQuency:SENSor:OFFSet {param}')

	def get(self, channel=repcap.Channel.Default) -> float:
		"""SCPI: SENSe<CH>:[POWer]:SWEep:FREQuency:[SENSor]:OFFSet \n
		Snippet: value: float = driver.sense.power.sweep.frequency.sensor.offset.get(channel = repcap.Channel.Default) \n
		Defines the level offset at the sensor input in dB. Activate the offset with the command method RsSmab.Sense.Power.Sweep.
		Frequency.Sensor.Offset.State.set. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sense')
			:return: offset: float Range: -100 to 100"""
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SENSe{channel_cmd_val}:POWer:SWEep:FREQuency:SENSor:OFFSet?')
