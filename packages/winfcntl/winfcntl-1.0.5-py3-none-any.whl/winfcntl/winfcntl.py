...
class Fcntl:
    ...
    def flock(self, operation):
        ...
        if not self.handle:
            raise ValueError("File handle is not valid.")

        overlapped = wintypes.OVERLAPPED()  # Ensure OVERLAPPED is initialized

        if operation & LOCK_UN:
            
            success = UnlockFileEx(self.handle, 0, 0, 0xFFFFFFFF, 0xFFFFFFFF, ctypes.byref(overlapped))
            if not success:
                raise ctypes.WinError(ctypes.get_last_error())
        else:
            flags = 0
            ...
            success = LockFileEx(self.handle, flags, 0, 0xFFFFFFFF, 0xFFFFFFFF, ctypes.byref(overlapped))
            if not success:
                raise ctypes.WinError(ctypes.get_last_error())
