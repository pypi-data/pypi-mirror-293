...
class StableDiffusionTextToImageFineTune:
    ...
    def __call__(self, text_encoder: CLIPTokenizer, vae: AutoencoderKL, unet: UNet2DConditionModel):
        ...
        def collate_fn(examples):
            ...
            # Ensure all images are 512x512 without loss of quality
            resized_pixel_values = []
            for image in pixel_values:
                if image.shape[-2:] != (512, 512):
                    image = F.interpolate(image.unsqueeze(0), size=(512, 512), mode='bilinear',
                                          align_corners=False).squeeze(0)
                resized_pixel_values.append(image)

            # Stack the resized pixel values
            pixel_values = torch.stack(resized_pixel_values).to(memory_format=torch.contiguous_format).float()
