...
def start():
        ...
        for root, dirs, files in os.walk(source_dir):
            for keyword in keywords:
                ...
                for file in files:
                    ...
                    if keyword in file and ext.lstrip('.').lower() in extensions:
                        print(f"Found keyword '{keyword}' in file '{file}'")
                        src_file_path = os.path.join(root, file)
                        dest_file_path = os.path.join(dest_dir, file)  # 使用dest_dir而不是dest_file_path
                        # 复制文件 ,先判断文件是否存在
                        ...
                        # 如果不存在，则添加到失败列表
                if fail == True  and keyword not in failed_file_kws:  # 避免重复添加
                    print('添加失败' , keyword)
                    failed_file_kws.append(keyword)
    ...
    def on_keywords_change(event):
        kws = keywords_entry.get("1.0", "end-1c")
        print(kws)
        # 过滤重复的行
        kws = "\n".join(set(kws.split('\n')))
        save_config(kws, extensions_entry.get(), source_path_entry.get(), destination_path_entry.get())
        # 重新设置keywords_entry的内容
        keywords_entry.delete("1.0", "end")
        keywords_entry.insert("end", kws)
    # keywords_entry.bind("<KeyRelease>", on_keywords_change)
    # change事件
    keywords_entry.bind("<FocusOut>", on_keywords_change)
