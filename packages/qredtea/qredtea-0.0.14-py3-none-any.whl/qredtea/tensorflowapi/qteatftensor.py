...
class QteaTFTensor(_AbstractQteaBaseTensor):
    ...
    def __init__(
        ...
        if ctrl in ["N"]:
            ...
            # pylint: disable-next=no-value-for-parameter
            dim = int(tf.math.cumprod(links)[0])
    ...
    def dummy_link(example_link):
        """Return a dummy link based on an example."""
        return 1
    ...
    def zeros_like(self):
        ...
        return QteaTFTensor(self.shape, ctrl="Z", dtype=self.dtype, device=self.device)
    ...
    def set_subtensor_entry(self, corner_low, corner_high, tensor):
        ...
        * self[2:4, 4:6, 2:4] = tensor
        ...
        sub_1d = tensor.copy()
        sub_1d = sub_1d.cpu().numpy().flatten()
        ...
        self.elem = tf.reshape(self_1d, shape)
        self.convert(device=device)
    ...
    def split_svd(
        ...
        * 'GPU'; T in [DT_DOUBLE, DT_FLOAT]
        ...
        svd_ctrl = _process_svd_ctrl(
            conv_params.svd_ctrl,
            conv_params.max_bond_dimension,
            matrix.shape,
            device,
            contract_singvals,
        )
        if (matrix.dtype == tf.float16) and (self.device == "cpu"):
        ...
        if (self.dtype == tf.float16) and (self.device == "cpu"):
