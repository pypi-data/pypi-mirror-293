...
class SqlDatabase:
    ...
    def purge_att_property(self, table, field, obj, attr, name):
        ...
        if to_del > 0:
            for row in rows[:to_del]:
                cursor.execute(f"DELETE FROM {table} WHERE id=?", (table, row[0]))
    ...
    def put_attribute_alias(self, attribute_name, attribute_alias):
        ...
        cursor.execute(
            "INSERT attribute_alias SET alias=? ,name=?, device=?,updated=datetime('now', 'localtime')",
