...
def main(num : int) -> int:
    boolean : bool = any("".join(char for char in str(num) if char in list("23456789")))
    if boolean:
        return -1
    else:
        last_digit : int = 0
...
def binary_to_decimal() -> None:
        ...
        result : int = main(binary_number)
        if result == -1:
            print("You have entered a decimal number. Please enter a binary number instead.")
        else:
