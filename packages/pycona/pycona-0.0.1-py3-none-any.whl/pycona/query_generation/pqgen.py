...
class PQGen(QGenBase):
    ...
    def __init__(self, ca_system: ActiveCA = None, *, objective_function=None, time_limit=1, blimit=5000):
        """
        Initialize the PQGen with the given parameters.

        :param ca_system: The CA system.
    ...
    def generate(self):
        ...
        Y = frozenset(get_variables(self.ca.instance.bias))
        lY = list(Y)

        if len(Y) == len(self.ca.instance.X):
            B = self.ca.instance.bias
            Cl = self.ca.instance.cl
        else:
            B = get_con_subset(self.ca.instance.bias, Y)
            Cl = get_con_subset(self.ca.instance.cl, Y)
        ...
        try:
            objective = self.obj(B=B, ca_system=self.ca)
        except:
