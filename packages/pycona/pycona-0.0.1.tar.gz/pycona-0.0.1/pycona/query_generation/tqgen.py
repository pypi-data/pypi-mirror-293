...
class TQGen(QGenBase):
    ...
    def __init__(self, ca_system: ActiveCA = None, *, time_limit=2, tau=0.2, alpha=0.8, l=None):
        """
        ...
        self._tau = tau
        self._alpha = alpha
        self._lamda = l if l else len(ca_system.instance.X)
    ...
    def generate(self):
        ...
        while (ttime < self.time_limit) and (len(bias) > 0):
            ...
            Y = find_suitable_vars_subset2(l, bias, self.ca.instance.X)
    ...
    def adjust(self, l, a, answer):
        ...
        """
        if answer:
            l = min([int(math.ceil(l / a)), len(self.ca.instance.X)])
        else:
            l = int((a * l) // 1)  # //1 to round down
        if l < get_min_arity(self.ca.instance.bias):
            l = 2
        return l
