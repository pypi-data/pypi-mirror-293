...
class getter_trn_race_result(baseGetter):
    ...
    def getArrivalOrder(self, orderString:str):
        ...
        if orderString.isnumeric(): # 着順が数値の場合
            ...
            orderInt, retInComFlg = (aor.exclusion, icf.exclusion) if orderString == aor.exclusionJp else \
                                    ...
                                    (aor.raceCancel, icf.cancel_race) if orderString == aor.raceCancelJp1 or orderString == aor.raceCancelJp2 else \
                                    (int(orderString.replace('({0})'.format(icf.re_rideJp), '')), icf.re_ride) if icf.re_rideJp in orderString else \
    ...
    def getTimeSecond(self, timeString:str) -> Decimal:
        timeSecond = 0
        if timeString == '': return timeSecond
    ...
    class beautifulSoup(browserContainer.beautifulSoup):
        ...
        def createSearchResultDataFrameByBeutifulSoup(self, soup:BeautifulSoup, *args, **kwargs) -> DataFrame:
            ...
            if dataIntro:
                ...
                raceInfo.fields(trn_race_info.updinfo.key).value = bc.getUpdInfo()
            ...
            if raceTables:
                ...
                for index in range(len(raceTables)):
                    ...
                    try:
                        ...
                        # 着順
                        arrival_order, incommingFlg = bc.getArrivalOrder(drow[self.raceResultCol.arrival_order.value].text)
                        # 枠番
                        ...
                        # 通過順
                        corners = bc.getCorner(drow[self.raceResultCol.corners.value].text)
                        # 上がり3ハロン
                        last3furlong = bc.getLast3furlong(drow[self.raceResultCol.last3furlong.value].find_all('span')[0].contents)
                        # 単勝オッズ
                        win_odds = bc.getWinOdds(drow[self.raceResultCol.win_odds.value].text)
                        # 人気
                        popular = bc.getPopular(drow[self.raceResultCol.popular.value].find_all('span')[0].contents)
                        # 馬体重
                        ...
                        # 賞金
                        prize_money = bc.getPrizeMoney(drow[self.raceResultCol.prize_money.value].text)
                        ...
                        raceResult.fields(trn_race_result.updinfo.key).value = bc.getUpdInfo()
                        ...
                        mstHorse.fields(mst_horse.updinfo.key).value = bc.getUpdInfo()
                        ...
                        mstJokey.fields(mst_jockey.updinfo.key).value = bc.getUpdInfo()
                        ...
                        mstTrainer.fields(mst_trainer.updinfo.key).value = bc.getUpdInfo()

                        # 馬主マスタ追加
                        mstHowner.newRow()
                        mstHowner.fields(mst_howner.howner_id.key).value = howner_id
                        mstHowner.fields(mst_howner.howner_nm.key).value = howner_nm
                        mstHowner.fields(mst_howner.updinfo.key).value = bc.getUpdInfo()
