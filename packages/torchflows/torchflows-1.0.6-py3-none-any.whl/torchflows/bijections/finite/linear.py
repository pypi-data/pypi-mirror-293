...
from torchflows.bijections.matrices import (
...
from torchflows.utils import get_batch_shape, flatten_event, unflatten_event, flatten_batch, unflatten_batch
...
class LinearBijection(Bijection):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        log_det = self.matrix.log_det() + torch.zeros(size=batch_shape, device=x.device).to(x)
        return z, log_det
    ...
    def inverse(self, z: torch.Tensor, context: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        log_det = -self.matrix.log_det() + torch.zeros(size=batch_shape, device=z.device).to(z)
        return x, log_det
