...
def create_data_list(datestart1, dateend1, family, pathin, name, pathwork, boxsizex, boxsizey, fonction, flag_criteria, region_seleccionada, varnos):
       data_list = []
      ...
      for family in familys:
          ...
          if n_cpu==1:
           ...
           with dask.distributed.Client(processes=True, threads_per_worker=1, 
               ...
               delayed_funcs = [dask.delayed(create_and_populate_moyenne_table)(data_['family'], 
                                                 ...
                                                 data_['boxsizex'],
                                                 data_['boxsizey'],
                                                 data_['varnos'] )for data_ in data_list]
       ...
       # n_cpu=1
        if n_cpu==1:  
         ...
         print (f"in Paralle plots = {len(data_list_plot)} x {Proj}  x {fonction} x {data_list_plot[0]['varno']}")
         with dask.distributed.Client(processes=True, threads_per_worker=1, 
                                          n_workers=n_cpu, 
                                          silence_logs=40) as client:
