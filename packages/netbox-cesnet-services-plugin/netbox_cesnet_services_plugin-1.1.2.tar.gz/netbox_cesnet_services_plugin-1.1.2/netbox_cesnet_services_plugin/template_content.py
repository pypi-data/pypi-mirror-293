...
from netbox_cesnet_services_plugin.filtersets import (
    ...
    LLPDNeighborFilterSet,
    LLPDNeighborLeafFilterSet,
)
...
class LLDPNeighborPluginTemplateExtension(PluginTemplateExtension):
    ...
    def get_page(self):
        ...
        lldp_neighbors = LLDPNeighbor.objects.annotate(
            related_property=F(self.related_property)
        ).filter(**{self.related_property: related_object.id})
...
class BGPConnectionPluginTemplateExtension(PluginTemplateExtension):
    ...
    def get_page(self):
        related_object = self.context["object"]
        bgp_connections = BGPConnection.objects.annotate(
            related_property=F(self.related_property)
        ).filter(**{self.related_property: related_object.id})
...
def create_lldpneighbor_tabview(model):
    ...
    class View(generic.ObjectChildrenView):
        ...
        filterset = LLPDNeighborFilterSet
        ...
        if model == Device:
            ...
            tab = ViewTab(
                label="LLDP Neighbors",
                badge=lambda obj: LLDPNeighbor.objects.filter(
                    interface_a=obj.id
                ).count(),
                permission="netbox_cesnet_services_plugin.view_lldpneighbor",
            )
    ...
    class View(generic.ObjectChildrenView):
        ...
        if model == Tenant:
            tab = ViewTab(
                ...
                badge=lambda obj: BGPConnection.objects.annotate(
                    related_property=F("bgp_prefix__tenant")
                )
            ...
            def get_children(self, request, parent):
                childrens = (
                    self.child_model.objects.annotate(
                        related_property=F("bgp_prefix__tenant")
                    )
            ...
            def get_children(self, request, parent):
                childrens = self.child_model.objects.filter(device=parent.id).restrict(
                    request.user, "view"
                )
                return childrens
            ...
            tab = ViewTab(
                label="BGP Connections",
                badge=lambda obj: BGPConnection.objects.annotate(
                    related_property=F("next_hop__interface")
                )
            ...
            tab = ViewTab(
                label="BGP Connections",
                badge=lambda obj: BGPConnection.objects.filter(
                    bgp_prefix=obj.id
                ).count(),
                permission="netbox_cesnet_services_plugin.view_bgpconnection",
            )
            ...
            def get_children(self, request, parent):
                childrens = self.child_model.objects.filter(vrf=parent.id).restrict(
                    request.user, "view"
                )
                return childrens
