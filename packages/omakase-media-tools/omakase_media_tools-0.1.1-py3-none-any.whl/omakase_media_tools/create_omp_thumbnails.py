...
from file_log import setup_logger
import os
import sys
...
logger = setup_logger("create_thumbnails")
...
def create_thumbnails(source_video_file_path, thumbnail_directory='', interval_in_secs=10):
    ...
    try:
        ...
        logger.info(f"FFmpeg output: {stdout.decode('utf-8')}")
        ...
        print(f"An error occurred: {error_message}", file=sys.stderr)
