...
def filter_results(metric, percent_conec, readdition_threshold, clusters, recipe_results, outfile_prefix):
    percent_conec = str(percent_conec)
    qualifying_proteins_by_metric = {}
    with open(recipe_results, "rb") as f:
...
def pretty_print_clusters(clusters, output_prefix):
    # NOTE: this is added to be able to print the clusters to files for lenore
    summary_output_filename = f'{output_prefix}-pretty_clusters_summary.txt'
    with (open(summary_output_filename, 'w+')) as f:
        for i in clusters:
            f.write(i.__repr__() + '\n')
        # f.write(json.dumps([i.to_dict() for i in clusters]))
    logging.info(f"Pretty summary saved to {summary_output_filename}")

def print_reconnected_clusters(clusters, output_prefix):
    hash_to_proteins_file = f'{output_prefix}-clusters.json'
    out_dict = {}
    for i in clusters:
        out_dict[hash(i)] = {}
    ...
    with (open(hash_to_proteins_file, 'w+')) as f:
        for i in clusters:
            f.write(json.dumps({hash(i): i.proteins}) + '\n')
    '''
    logging.info(f"Clusters saved to {hash_to_proteins_file}")
...
def parse(cluster_filepath,
    ...
    G, DSD, protein_names = read_network(dsd_file, network_filepath, threshold, "Network", output_prefix)
    ...
    qualifying_proteins_by_metric = filter_results(metric, cthresh, readdition_threshold, clusters, recipe_results, output_prefix)
    ...
    pretty_print_clusters(clusters, output_prefix)
    print_reconnected_clusters(clusters, output_prefix)
...
def get_args(parser=None):
    ...
    parser.add_argument("--output-prefix", help = "Prefix with which to save results", type = str, required=False, default="./recipe")
    parser.add_argument(
...
def main(args=None):
    ...
    parse(args.cluster_filepath, args.network_filepath, args.dsd_file, args.protein_go_filepaths, args.go_db_file, args.output_prefix, args.recipe_results, args.threshold, args.connectivity_threshold, args.metric, args.readdition_threshold)
