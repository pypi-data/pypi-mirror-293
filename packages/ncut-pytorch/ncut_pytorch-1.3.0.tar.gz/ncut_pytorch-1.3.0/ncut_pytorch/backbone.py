...
class StableDiffusion(nn.Module):
    ...
    def forward(self, image, timestep=None, positive_prompt="", negative_prompt=""):
        ...
        latent = self.noise_scheduler.add_noise(latent, noise, torch.LongTensor([timestep]).to(device))
...
class StableDiffusion3(nn.Module):
    ...
    def forward(self, image, timestep=None, return_flat_dict=False):
        ...
        timestep = torch.LongTensor([timestep]).to(device)
        
        latent = self.vae.encode(image).latent_dist.sample()
        noise = torch.randn(latent.shape).to(device)
        latent = self.noise_scheduler.add_noise(latent, noise, torch.LongTensor([timestep]).to(device))
        
        out = self.transformer(latent, timestep=self.noise_scheduler.timesteps[timestep], pooled_projections=pooled_projections, encoder_hidden_states=encoder_hidden_states)
