...
class PluginManager:
    ...
    def load_config(self):
        ...
        os.environ['API_BASEPATH'] = config.get('settings', 'API_BASEPATH',
                                                fallback='http://model-register-service/cogapi/')
        os.environ['TIMER_IN_SEC'] = config.get('settings', 'TIMER_IN_SEC', fallback='10')
        os.environ['JUPYTER_USER_ID'] = config.get('settings', 'JUPYTER_USER_ID', fallback='0')

        # Validate that the environment variables are set
        missing_vars = [var for var in ['API_BASEPATH', 'TIMER_IN_SEC', 'JUPYTER_USER_ID'] if not os.getenv(var)]
        if missing_vars:
            raise ValueError(f"The following environment variables are missing or not set: {', '.join(missing_vars)}")
