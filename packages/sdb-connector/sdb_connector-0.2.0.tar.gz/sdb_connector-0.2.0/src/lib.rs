...
fn select_measurement_data(ip: &str, port: &str,
    user: &str, pw:&str, namespace: &str, db_name: &str,
    table_name: &str, run_id: &str) -> PyResult<Vec<(u64, u8, u64, u64, u16, u16, u16, u16, u16, String, String, Vec<u8>)>> {
    // Create a Tokio runtime and block on the async function
    let rt = tokio::runtime::Runtime::new().unwrap();
...
async fn select_measurement_data_async(
    ...
    table_name: &str,
    run_id: &str
) -> Result<Vec<(u64, u8, u64, u64, u16, u16, u16, u16, u16, String, String, Vec<u8>)>, Box<dyn Error>> {
    let db = connect_to_db(ip, port, user, pw, namespace, db_name).await?;
...
pub async fn process_measurement_data(result: Response) -> Result<Vec<(u64, u8, u64, u64, u16, u16, u16, u16, u16, String, String, Vec<u8>)>, Box<dyn Error>> {
    let mut data = result;
    let data: Vec<UdpTag41> = data.take(0).unwrap();
    let exploded_data: Vec<(u64, u8, u64, u64, u16, u16, u16, u16, u16, String, String, Vec<u8>)> = data
        .into_iter()
        .flat_map(|tag| {
            tag.channel.into_iter()
