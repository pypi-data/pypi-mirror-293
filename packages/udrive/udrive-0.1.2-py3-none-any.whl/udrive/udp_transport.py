...
from udrive.utils import wait_any

def noprint(x):
  pass

class UDPTransport(udrive.protocol.PacketSource, udrive.protocol.PacketSink):
...
def discover_channels(path, serial_number, callback, cancellation_token, channel_termination_token, logger):
  ...
  while not cancellation_token.is_set():
    try:
      udp_transport = udrive.udp_transport.UDPTransport(dest_addr, dest_port, logger)
