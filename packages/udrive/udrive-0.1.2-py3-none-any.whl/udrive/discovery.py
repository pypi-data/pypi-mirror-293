"""
Provides functions for the discovery of uFibre nodes
"""
...
import traceback
import struct
import udrive.protocol
import udrive.utils
import udrive.remote_object
from udrive.utils import Event, Logger
from udrive.protocol import ChannelBrokenException, TimeoutError
import appdirs
import os
...
try:
    import udrive.usbbulk_transport
    channel_types['usb'] = udrive.usbbulk_transport.discover_channels
except ImportError:
    pass

try:
    import udrive.serial_transport
    channel_types['serial'] = udrive.serial_transport.discover_channels
except ImportError:
    pass

try:
    import udrive.tcp_transport
    channel_types['tcp'] = udrive.tcp_transport.discover_channels
except ImportError:
    pass

try:
    import udrive.udp_transport
    channel_types['udp'] = udrive.udp_transport.discover_channels
except ImportError:
    pass
...
def find_all(path, serial_number,
         ...
         logger):
    """
    Starts scanning for uFibre nodes that match the specified path spec and calls
    the callback for each uFibre node that is found.
    ...
    def did_discover_channel(channel):
        ...
        try:
            ...
            try:
                ...
                cache_path = os.path.join(cache_dir, 'ufibre_schema_cache_{:08d}'.format(json_version_tag))
            except:
            ...
            try:
                if not cache_path is None:
                    with open(cache_path, 'rb') as fp:
                        json_crc16 = udrive.protocol.calc_crc16(udrive.protocol.PROTOCOL_VERSION, fp.read())
                        fp.seek(0)
            ...
            if json_data is None:
                ...
                logger.info("Downloading json data from uDrive... (this might take a while)")
                ...
                json_crc16 = udrive.protocol.calc_crc16(udrive.protocol.PROTOCOL_VERSION, json_bytes)
            ...
            json_data = {"name": "ufibre_node", "members": json_data}
            obj = udrive.remote_object.RemoteObject(json_data, None, channel, logger)

            obj.__dict__['_json_data'] = json_data['members']
            obj.__dict__['_json_crc'] = json_crc16

            device_serial_number = udrive.utils.get_serial_number_str(obj)
...
def find_any(path="usb", serial_number=None,
    ...
    """
    Blocks until the first matching uFibre node is connected and then returns that node
    """
    result = []
