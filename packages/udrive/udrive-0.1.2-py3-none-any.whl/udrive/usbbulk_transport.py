...
import traceback
import platform
from udrive.utils import TimeoutError

# Currently we identify ufibre-enabled devices by VID,PID
...
class USBBulkTransport(udrive.protocol.PacketSource, udrive.protocol.PacketSink):
  ...
  def process_packet(self, usbBuffer):
    try:
      ...
      if ex.errno == 19 or ex.errno == 32: # "no such device", "pipe error"
        raise udrive.protocol.ChannelBrokenException()
        ...
        raise udrive.protocol.ChannelDamagedException()
  ...
  while not cancellation_token.is_set():
    ...
    for usb_device in devices:
      try:
        ...
        bulk_device.init()
        channel = udrive.protocol.Channel(
        ...
        if ex.errno == 13:
          # TODO: this is an ODrive specific message and should live outside of the ufibre library
          logger.warn("I found a USB device that looks like an uDrive (bus {}, device {}) but I can't access it. Try running `sudo odrivetool udev-setup`, then unplug and replug the device.".format(usb_device.bus, usb_device.address))
