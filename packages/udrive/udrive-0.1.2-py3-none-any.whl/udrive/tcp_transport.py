...
from udrive.utils import wait_any, TimeoutError

def noprint(x):
  pass

class TCPTransport(udrive.protocol.StreamSource, udrive.protocol.StreamSink):
...
def discover_channels(path, serial_number, callback, cancellation_token, channel_termination_token, logger):
  ...
  while not cancellation_token.is_set():
    try:
      tcp_transport = udrive.tcp_transport.TCPTransport(dest_addr, dest_port, logger)
      stream2packet_input = udrive.protocol.PacketFromStreamConverter(tcp_transport)
      packet2stream_output = udrive.protocol.StreamBasedPacketSink(tcp_transport)
