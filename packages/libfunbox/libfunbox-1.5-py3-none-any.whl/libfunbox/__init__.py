...
class Funbox:
    ...
    def GetDSLLinkDetails(self) -> dict:
        ...
        return self._post("sysbus/NeMo/Intf/data:getMIBs", {"mibs":"dsl"}).json()["result"]["status"]["dsl"]
    def GetPPPInfo(self) -> dict:
        ...
        """
        return self._post("sysbus/NeMo/Intf/data:getMIBs",{"mibs":"ppp"}).json()["result"]["status"]["ppp"]["ppp_data"]
    ...
    def SetWanMode(self, vdsl: bool) -> None:
        """
        ...
        """
        assert self._post("sysbus/NMC:setWanMode", {"WanMode":"VDSL_PPP" if vdsl else "Ethernet_PPP"}).json()["result"]["status"]
    def SetWanModeWithUsername(self, vdsl: bool, username: str) -> None:
        """
        See SetWanMode, but this will also change your PPP username
        
        See warning from SetWanMode
        """
        assert self._post("sysbus/NMC:setWanMode", {"WanMode":"VDSL_PPP" if vdsl else "Ethernet_PPP", "Username": username}).json()["result"]["status"]        
    ...
    def GetWiFiAdapterInfo(self) -> dict:
        """
        This returns the bands, standards, possible channels, channels in use, and similiar
        
        Does not contain password or SSID
        """
        return self._post("sysbus/NeMo/Intf/lan:getMIBs",{"mibs": "wlanradio"}).json()["result"]["status"]["wlanradio"]
    def GetWiFiInfo(self) -> dict:
        """
        Returns things like BSSID, SSID, SSID advertisement, password, WPS data
        """
        return self._post("sysbus/NeMo/Intf/lan:getMIBs", {"mibs":"wlanvap"}).json()["result"]["status"]["wlanvap"]
    ...
    def __del__(self) -> None:
        if sys.meta_path and self.__authenticated != None:
            self.logout()
    def __enter__(self):
        return self
