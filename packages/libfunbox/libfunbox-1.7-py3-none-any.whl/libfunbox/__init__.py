...
__version__: float = 1.07
__pypi_version: float = 1.7
...
class Funbox:
    ...
    def GetDSLErrors(self, line:str="dsl0"):
        """
        Error information
        """
        return self._post(f"sysbus/NeMo/Intf/{line}:getDSLStats").json()["result"]["status"]
    ...
    def IsConnectedWithDSL(self):
        return (self.GetWANStatus()["LinkType"] == "vdsl")
    ...
    def GetForwardedPorts(self, include_upnp: bool=False) -> dict:
        """
        Get currently forwarded ports either normally or with UPnP
        """
        return self._post("sysbus/Firewall:getPortForwarding", ({"origin": "webui"} if not include_upnp else {})).json()["result"]["status"]
