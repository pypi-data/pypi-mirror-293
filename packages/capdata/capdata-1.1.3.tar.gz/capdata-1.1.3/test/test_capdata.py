...
class TestApiFunctions(unittest.TestCase):
    ...
    def test_get_vol_surface(self):
        curve_data = capdata.get_vol_surface("USDCNY_VOL_SVI", '2024-06-16 00:00:00', '2024-06-18 18:00:00', 'd')
        if curve_data is not None:
            for data in curve_data:
                print(data)
        else:
    ...
    def test_get_hist_mkt(self):
        market_data = capdata.get_hist_mkt(['2400004.IB'], '2024-08-27', '2024-08-27', ['bid', 'ask', 'open_ytm'],
                                           window=['09:00:00', '18:00:00'], freq='30m')
        print(market_data)
    ...
    def test_get_bond_list(self):
        datas = capdata.get_bond_list(['CFETS'], ['CNY'], ['国债'], ['FIXED_COUPON_BOND'], [])
        if datas is not None:
            for data in datas:
                print(data)
        else:
    ...
    def test_get_std_bond_list(self):
        datas = capdata.get_std_bond_list(['SYNTHETIC'], ['CNY'], ['TREASURY_BOND'], ['FIXED_COUPON_BOND'], [])
        if datas is not None:
            for data in datas:
                print(data)
        else:
