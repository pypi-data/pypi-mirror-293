...
class DetailEvent(object):
    ...
    def getNumVersions(self, product_name):
        ...
        prodlist = self._jdict["properties"]["products"][product_name]
        prodlist = [prod for prod in prodlist if prod["status"] != "DELETE"]
        return len(prodlist)
    ...
    def getProducts(self, product_name, source="preferred", version="preferred"):
        ...
        # remove all rows where status is DELETE
        df = df.loc[df["status"] != "DELETE"].copy()
