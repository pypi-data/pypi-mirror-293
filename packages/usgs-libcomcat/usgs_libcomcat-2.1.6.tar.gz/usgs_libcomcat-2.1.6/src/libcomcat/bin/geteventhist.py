...
from libcomcat.visualization import PRODUCT_COLORS, Timeline
...
def get_parser():
    ...
    thelp = "Create a timeline of earthquake-related products release time."

    parser.add_argument(
        "-t", "--timeline", action="store_true", help=thelp, default=False
    )

    t_b_help = (
        "Allows the user to choose the maximum amount of "
        "time in hours the timeline covers. \n"
        "Maximum of 96 hours (4 days)."
    )

    parser.add_argument("-b", "--timeline-block", type=int, help=t_b_help, default=24)
...
def main():
    ...
    if args.timeline is not None:
        timeline = Timeline(args.timeline_block * 60, dataframe, detail_event)
        timeline = timeline.create_timeline()
        if timeline is not None:
            if not Path(args.outdir).exists():
                Path(args.outdir).mkdir()
            timeline.savefig(
                f"{args.outdir}/{args.eventid}_timeline.{args.timeline_format}",
                bbox_inches="tight",
                dpi=300,
            )

            print(
                f"{args.eventid}_timeline.png saved to {args.outdir}/{args.eventid}_timeline.{args.timeline_format}."
            )
