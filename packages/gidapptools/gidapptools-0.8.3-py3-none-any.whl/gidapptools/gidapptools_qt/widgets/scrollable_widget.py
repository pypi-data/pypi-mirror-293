...
if TYPE_CHECKING:
    from gidapptools.gidapptools_qt.sub_typing import QWidgetStandardKwargsType
...
class ScrollableWidget(QWidget):
    ...
    @property
    def layout(self):
        return self._inner_scroll_widget.layout()

    def set_outer_Layout(self, layout: QLayout):
        return super().setLayout(layout)

    def setLayout(self, layout: QLayout) -> None:
        return self._inner_scroll_widget.setLayout(layout)
...
if __name__ == '__main__':
    app = QApplication()
    x = ScrollableWidget()
    x.setLayout(QVBoxLayout())
    for i in range(10):
        y = QLineEdit()
        y.setText("sdfsdf sdfdsf  sdff\n" * 2)

        y.setMinimumSize(y.sizeHint())

        pp: QVBoxLayout = x.layout
        pp.addWidget(y, 1)
    print(x.layout)
