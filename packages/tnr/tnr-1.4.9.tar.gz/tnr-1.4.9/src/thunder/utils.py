...
BASEURL = 'api.thundercompute.com:8443'
...
def get_available_gpus():
    endpoint = f"https://{BASEURL}/hosts"
    try:
...
def get_instances(token):
    endpoint = f"https://{BASEURL}/instances/list"
    try:
        response = requests.get(
            endpoint, headers={"Authorization": f"Token {token}"}, timeout=30
        )
        if response.status_code != 200:
...
def create_instance(token):
    endpoint = f"https://{BASEURL}/instances/create"
    try:
...
def delete_instance(instance_id, token):
    endpoint = f"https://{BASEURL}/instances/{instance_id}/delete"
    try:
        response = requests.post(
            endpoint, headers={"Authorization": f"Token {token}"}, timeout=30
        )
...
def start_instance(instance_id, token):
    endpoint = f"https://{BASEURL}/instances/{instance_id}/up"
    try:
        response = requests.post(
            endpoint, headers={"Authorization": f"Token {token}"}, timeout=30
        )
...
def stop_instance(instance_id, token):
    endpoint = f"https://{BASEURL}/instances/{instance_id}/down"
    try:
        response = requests.post(
            endpoint, headers={"Authorization": f"Token {token}"}, timeout=30
        )
...
def get_active_sessions(token):
    endpoint = f"https://{BASEURL}/active_sessions"
    try:
        response = requests.get(
            endpoint, headers={"Authorization": f"Token {token}"}, timeout=30
        )
...
def add_key_to_instance(instance_id, token):
    endpoint = f"https://{BASEURL}/instances/{instance_id}/add_key"
    try:
        response = requests.post(
            endpoint, headers={"Authorization": f"Token {token}"}, timeout=30
        )
...
SSH_CONFIG_PATH = os.path.join(os.path.join(os.path.expanduser("~"), '.ssh'), 'config')
def read_ssh_config():
