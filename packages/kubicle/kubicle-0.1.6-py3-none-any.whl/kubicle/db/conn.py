...
def get_sqlite_conn() -> Engine:
    db_name = os.environ.get("KUBICLE_DB_NAME", "hub.db")
    db_path = os.environ.get("KUBICLE_DB_PATH", "./.data")
    db_test = os.environ.get("KUBICLE_DB_TEST", "false") == "true"
    if db_test:
        db_name = f"kubicle_test_{int(time.time())}.db"
