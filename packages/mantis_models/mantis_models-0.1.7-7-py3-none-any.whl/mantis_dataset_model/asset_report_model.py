...
class NetworkInterface(BaseModel):
    ipv4: str
    ipv4_runtime: str = Field(regex=r"[0-9]+(.[0-9]+){3}")  # 192.168.44.11 for example
    mac: str = Field(
        regex=r"[a-zA-Z0-9]+(:[a-zA-Z0-9]+){5}"
...
class AssetNode(BaseModel):
    ...
    os: Optional[str] = Field(min_length=1)
    os_family: Optional[OSFamily] = Field(min_length=1)
    os_version: Optional[str] = Field(min_length=1)
    cpe: Optional[str] = Field(min_length=3)
