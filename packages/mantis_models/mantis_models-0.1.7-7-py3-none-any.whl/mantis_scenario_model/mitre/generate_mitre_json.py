...
# NOTE: the following imports are NOT dependencies of cr_scenario_model
...
def load_reference_mitre_data() -> Attck:
    ...
    source_enterprise_attck_json = str(
        os.path.join(CURRENT_DIR, "merged_enterprise_attck_v1.json")
    )
    source_pre_attck_json = str(os.path.join(CURRENT_DIR, "merged_pre_attck_v1.json"))
    source_mobile_attck_json = str(
        os.path.join(CURRENT_DIR, "merged_mobile_attck_v1.json")
    )
    source_ics_attck_json = str(os.path.join(CURRENT_DIR, "merged_ics_attck_v1.json"))
...
def do_generate(attack: Attck) -> None:
    ...
    class TacticTmp:
        ...
        def __eq__(self, other: "TacticTmp") -> bool:
