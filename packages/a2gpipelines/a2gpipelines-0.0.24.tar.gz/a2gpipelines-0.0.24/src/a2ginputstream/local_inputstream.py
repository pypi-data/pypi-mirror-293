...
class A2GHttpClient():
    ...
    def remove_documents(self, ikey:str, query:dict) -> int:
        try:
            ...
            if response.status_code != 200: 
                raise Exception(f"Error al obtener el inputstream {response.status_code} {response.content}")
            ...
            print(f"Operation complete, total docs deleted: {deleted_docs}")
    ...
    def clear_inputstream(self, ikey:str) -> int:
        try:
            print("Removing all data from A2G...")
            hearders = {
                "Authorization": f"A2G {self.token}",
                "ikey": ikey
            }

            response = requests.post(A2G_DATA_URL + f"/QueryData/Clear", headers=hearders, verify=False)
            if response.status_code != 200:
                raise Exception(f"Error al obtener el inputstream {response.status_code} {response.content}")
            res_object = response.json(cls=CustomJsonDecoder)
            if not res_object["success"]: raise Exception(res_object["errorMessage"])
