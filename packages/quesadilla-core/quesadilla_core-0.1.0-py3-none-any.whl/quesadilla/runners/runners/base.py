...
class BaseRunnerLoggingExtras(RunnerLoggingExtras):
...
class BaseRunner(Runner, metaclass=ABCMeta):
    ...
    config: RunnerConfig = field(kw_only=True)
    ...
    def name(self) -> str:
        return (
            f"{str(self.task_queue.namespace)}.{str(self.task_queue.name)}."
            f"runner.{str(self.id)}"
        )

    @cached_property
    def logging_extras(self) -> BaseRunnerLoggingExtras:
        return {
            "id": str(self.id),
