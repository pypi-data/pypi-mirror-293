...
class Worker(BaseWorkload):
    ...
    @cached_property
    def name(self) -> str:
        return f"{self.runner.name}.worker.{str(self.id)}"
        ...
        while not self.stopped.is_set():
            while not self.closed.is_set() and (queued_task := self.next()) is not None:
                self.logger.info(
                    ...
                    queued_task.id,
                    extra=self.get_queued_task_logging_extras(queued_task),
                )
                queued_task = queued_task.execute(self.asyncio_runner)
                self.result(queued_task)
                match queued_task.result:
                    case Success():
                        self.logger.info(
                            "Processed task %s(%s)",
                            queued_task.task_executable.name,
                    ...
                    case Failure(exc=exc):  # pragma: nobranch
