...
class CodingAgent:
    ...
    def initial_setup(self, context_files, instructions, context, role):
        ...
        if context_files:
            ...
            self.conversation_history.append({"role": "user", "content": f"These are all the supported files to provide enough context: {all_file_contents}"})
            self.conversation_history.append({"role": "assistant", "content": "Got it! I have carefully read the provided context to perform the task."})
    ...
    async def get_coding_request(self, session, is_first, title, file, techStack):
        ...
        """
        # Determine the prompt based on whether it is the first request
        if is_first:
            prompt = (
                f"Begin working on {file}. "
                ...
                f"Always maintain the default file description for {techStack}.\n"
                f"Be mindful of overlapping or redeclaration with other files. Focus solely on the original instructions and the main functionality of {file}.\n"
                f"Consider integration issues, as well as the format and structure required for {techStack}.\n"
                f"Your code must be well-organized, with a senior-level design approach.\n"
                f"If your code involves UI, ensure it's well-organized, adaptively fits, and is compelling.\n"
            )
        else:
            prompt = (
                f"{title} is complete. "
                f"Now, work on {file} and return with an exact JSON response example, ensuring that the 'code' field contains valid code without additional descriptions or any Markdown symbols.\n"
                f"Please strictly follow the exact syntax and formatting for {techStack}.\n"
                f"Always maintain the default file description for {techStack}.\n"
                f"Be mindful of overlapping or redeclaration with other files. Focus solely on the original instructions and the main functionality of {file}.\n"
                f"Consider integration issues, as well as the format and structure required for {techStack}.\n"
