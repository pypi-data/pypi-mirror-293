...
class Binomial(Distribution):
    ...
    def plot_bar(self):
        ...
        data = {'tails (0)': self.n - np.sum(self.data), 'heads (1)': np.sum(self.data)}
        plt.bar(data.keys(), data.values())
        plt.xlabel('Outcome (head/tail)')
    ...
    def __add__(self, other):
        ...
        n = self.n + other.n      
        new_binomial = Binomial(prob=self.p, size=n)
