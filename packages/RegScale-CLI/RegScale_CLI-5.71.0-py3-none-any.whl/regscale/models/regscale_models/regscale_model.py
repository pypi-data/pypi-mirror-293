...
from typing import Any, ClassVar, Dict, List, Optional, TypeVar, Union, cast, get_type_hints, Set, Tuple
...
class RegScaleModel(BaseModel, ABC):
    ...
    _parent_cache: ClassVar[TTLCache] = TTLCache(maxsize=1000, ttl=3600)  # Cache for parent-based queries
    ...
    def has_changed(self, comp_object: Optional[T] = None) -> bool:
        ...
        if getattr(self, "id", 0) in [0, None]:
            return True
        changed_fields = self.show_changes(comp_object=comp_object).keys()
        return any(field not in ["id", "dateCreated"] for field in changed_fields)
    ...
    def create_or_update(
        ...
        """
        logger.debug(f"Starting create_or_update for {self.__class__.__name__}")
    ...
    def _create_objects_from_items(
        ...
        if cls._get_objects_for_list:
            ...
            return [cls.cast_list_object(item, parent_id=parent_id, parent_module=parent_module) for item in objects]
        return [cls.cast_list_object(item, parent_id=parent_id, parent_module=parent_module) for item in items]
