...
class Defender(FlatFileImporter):
    ...
    def determine_first_seen(self, dat: dict) -> str:
        ...
        iso_string = dat.get("TIMEGENERATED", "").rstrip("Z")
    ...
    def create_asset(self, dat: Optional[dict] = None) -> Asset:
        ...
        :rtype: Asset
        """
        additional_data = json.loads(dat.get("ADDITIONALDATA", {}))
    ...
    def create_vuln(self, dat: Optional[dict] = None) -> Optional[Vulnerability]:
        ...
        """
        regscale_vuln = None
        severity = dat["SEVERITY"].lower()
        additional_data = json.loads(dat.get("ADDITIONALDATA", {}))
        hostname = additional_data.get("repositoryName", "")
        description = dat["SUBASSESSMENTNAME"]
        solution = dat["SUBASSESSMENTREMEDIATION"]
        ...
        if dat and asset_match:
            regscale_vuln = Vulnerability(
                ...
                cve=", ".join(cves) if cves else dat["SUBASSESSMENTNAME"],
                vprScore=None,
