...
class ScannerIntegration(ABC):
    ...
    def __init__(self, plan_id: int, tenant_id: int = 1):
        ...
        self.software_to_create: List[regscale_models.SoftwareInventory] = manager.list()
        self.software_to_update: List[regscale_models.SoftwareInventory] = manager.list()
        self.data_to_create: List[regscale_models.Data] = manager.list()
        self.data_to_update: List[regscale_models.Data] = manager.list()
        self.link_to_create: List[regscale_models.Link] = manager.list()
        self.link_to_update: List[regscale_models.Link] = manager.list()
        ...
        if component_name:
            ...
            if not component:
                ...
                if component.securityPlansId:
                    ...
                    component_mapping.get_or_create(bulk=True)
    ...
    def _update_processed_count(self, assets_processed: int) -> int:
        """
    ...
    def create_or_update_issue_from_finding(
        ...
        if ScannerVariables.issueCreation.lower() != "perasset":
            ...
            existing_issues: List[regscale_models.Issue] = regscale_models.Issue.find_by_other_identifier(
                self.get_finding_other_identifier(finding)
            )
            delimiter = "\n"
    ...
    def is_poam(self, finding: IntegrationFinding) -> bool:
        """
    ...
    def get_existing_issues_dict(self) -> Dict[str, regscale_models.Issue]:
        ...
        :return: A dictionary of existing issues, keyed by a combination of otherIdentifier and assetIdentifier
    ...
    def get_date_completed(
        self, finding: IntegrationFinding, issue_status: regscale_models.IssueStatus
    ) -> Optional[str]:
        """
