...
from regscale.core.app.utils.nessus_utils import cpe_xml_to_dict, software
...
class NessusIntegration(ScannerIntegration):
    ...
    def get_vulnerability_data(cls, file_vuln: Any) -> dict:
        ...
        cvss_base_score = get("cvss_base_score")
        cve = get("cve")
        ...
        # Convert cvss_base_score to float if it exists
        if cvss_base_score:
    ...
    def find_nessus_files(path: str) -> List[Path]:
        """
        Find all .nessus files in the given folder path or return a single file if provided.
        ...
        if file_path.is_file():
            return [file_path]
        return list(file_path.glob("*.nessus"))
