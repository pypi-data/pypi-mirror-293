...
from regscale.core.app.utils.nessus_utils import validate_nessus_severity, get_min_cvss_score
from regscale.core.utils.date import datetime_obj
from regscale.integrations.commercial.tenable import get_last_pull_epoch
...
from regscale.integrations.scanner_integration import IntegrationAsset, ScannerIntegration, IntegrationFinding
...
class TenableIntegration(ScannerIntegration):
    ...
    def fetch_assets(self, *args: Any, **kwargs: Any) -> Iterator[IntegrationAsset]:
        ...
        :param Any **kwargs: Arbitrary keyword arguments
        ...
        if (
            ...
            assets_iterator = self.client.exports.assets(updated_at=int(tenable_last_updated))
    ...
    def get_last_update_time(self) -> datetime.datetime:
        ...
        if filtered_assets:
            return max(datetime_obj(asset.dateLastUpdated) for asset in filtered_assets)
    ...
    def get_asset_name(self, node: Dict[str, Any]) -> str:
        """
        ...
        :rtype: str
        """
        for key in ["hostnames", "fqdns", "netbios_names"]:
            values = node.get(key, [])
            if values and values[0]:
                return values[0]
        return "Unknown Asset"

    def get_all_mac_addresses(self, node: Dict[str, Any]) -> List[str]:
        """
    ...
    def generate_notes(self, node: Dict[str, Any]) -> str:
        """
    ...
    def get_other_cloud_identifier(self, node: Dict[str, Any]) -> Optional[str]:
        """
    ...
    def map_asset_category(tenable_type: Optional[str]) -> str:
        ...
        """
        if not tenable_type:
            return "Hardware"  # Default to Hardware if type is unknown

        # List of types that are typically considered software
        software_types = ["application", "database", "web-application", "virtual-machine", "container"]

        return "Software" if tenable_type.lower() in software_types else "Hardware"
    ...
    def parse_finding(self, vuln: Dict[str, Any]) -> Optional[IntegrationFinding]:
        ...
        try:
            ...
            asset_id = asset.get("hostname", asset.get("fqdn", None))

            cve = plugin.get("cve", [])[0] if isinstance(plugin.get("cve"), list) else None
            plugin_name = plugin.get("name", [])[0] if isinstance(plugin.get("name"), list) else None
            ...
            return IntegrationFinding(
                ...
                severity=severity,
                status=status,
                asset_identifier=asset.get("uuid", ""),
                external_id=str(plugin.get("id", "")),
                first_seen=vuln.get("first_found"),
                last_seen=vuln.get("last_found"),
