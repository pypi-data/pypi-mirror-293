...
def start():
    ...
    with progress:
        ...
        global directories
        directories = set_directory_variables(task=task0, evidence_folder=config["evidenceFolder"], progress=progress)

        task1 = progress.add_task("[white]Building a required documents list from config.json...", total=3)
        # call function to define variable for use outside of function
        global docs
        docs = parse_required_docs(evidence_folder=directories[0], task=task1, progress=progress)

        task2 = progress.add_task("[white]Calculating files last modified times...", total=5)
        # call function to define variable for use outside of function
        global times
        times = get_doc_timestamps(
            evidence_folder=directories[2], directory=directories[1], task=task2, progress=progress
        )

        task3 = progress.add_task("[white]Building a required texts list from config.json...", total=3)
        # call function to define variable for use outside of function
        global texts
        texts = set_required_texts(evidence_folder=directories[0], task=task3, progress=progress)

        task4 = progress.add_task("[white]Searching evidence folder for required files...", total=4)

        # call function to define variable for use outside of function
        global folders
        folders = find_required_files_in_folder(evidence_folder=directories[2], task=task4, progress=progress)

        task5 = progress.add_task("[white]Searching for digital signatures in documents...", total=2)

        # call function to define variable for use outside of function
        global SIGRESULTS
        SIGRESULTS = signature_assessment_results(directory=folders, r_docs=docs[0], task=task5, progress=progress)

        task6 = progress.add_task("[white]Testing if required documents are present...", total=2)

        # call function to define variable for use outside of function
        global DOCRESULTS
        DOCRESULTS = document_assessment_results(directory=folders, documents=docs[1], task=task6, progress=progress)

        task7 = progress.add_task("[white]Extracting texts from required files...", total=4)

        # call function to define variable for use outside of function
        global file_texts
        file_texts = parse_required_text_from_files(evidence_folder=directories[2], task=task7, progress=progress)
        ...
        # call function to define variable for use outside of function
        global TEXTRESULTS
        TEXTRESULTS = text_assessment_results(searches=search_results, r_texts=texts, task=task9, progress=progress)

        task10 = progress.add_task("[white]Retrieving data from the evidence test projects...", total=3)

        # call function to define variable for use outside of function
        global data
        data = gather_test_project_data(api=api, evidence_folder=directories[0], task=task10, progress=progress)
        ...
        global TIMERESULTS
        TIMERESULTS = assess_doc_timestamps(timestamps=times, documents=docs[0], task=task11, progress=progress)
        ...
        create_child_assessments(api=api, project_data=data, output=html_output, task=task16, progress=progress)
...
def build_package(regscale_id: int, regscale_module: str, path: Path, progress: Progress):
    """
...
def package_builder(regscale_id: int, regscale_module: str, path: Path):
    ...
    with progress:
        ...
        try:
            ...
            ssp = api.get(url=config["domain"] + f"/api/{regscale_module}/megaAPI/{regscale_id}").json()
            module_folder_name = str(ssp["securityPlan"]["id"]) + "_" + str(ssp["securityPlan"]["systemName"])
            folder_contents_name = str(ssp["securityPlan"]["id"]) + "_Evidence_Folder_Contents"
            ...
            if ssp["attachments"]:
                ...
                for f in outter_attachments:
                    ...
                    file = api.get(
                        url=config["domain"] + f"/api/files/downloadFile/{file_id}/{module}/{stored_name}/{file_hash}"
                    ).content

                    with open(f"{path}/{module_folder_name}/{file_name}", "wb") as att:
                        att.write(file)

            else:
                app.logger.info("No Evidence at SSP level for SSP. Checking for Evidence at Control level.")
            ...
            if ssp["normalizedControls"]:
                ...
                for i in controls:
                    ...
                    for p in i["attachments"]:
                        ...
                        parent_id = p["parentId"] if p else None
                        parent_module = p["parentModule"] if p else None
                        file_hash = p["fileHash"] if p else None
                        sha_hash = p["shaHash"] if p else None
                        file_size = p["size"] if p else None
                        date_created = p["dateCreated"] if p else None
                ...
                for f in control_attachments:
                    ...
                    file_name = f["fileName"]
                    file_hash = f["fileHash"]
                    file_drop_folder = f["controlId"]
                    stored_name = f["storedName"]

                    file = api.get(
                        url=config["domain"] + f"/api/files/downloadFile/{id}/{module}/{stored_name}/{file_hash}"
                    ).content

                    with open(
                        f"{path}/{module_folder_name}/{file_drop_folder}/{file_name}",
                        "wb",
                    ) as output:
                        output.write(file)
                    with open(
                        f"{path}/{module_folder_name}/{file_drop_folder}/{file_drop_folder}_Evidence_Folder_Contents.json",
                        "a",
                    ) as file_drop:
                ...
                app.logger.info("No Control level Evidence for SSP.")
...
def set_directory_variables(task: int, evidence_folder: str, progress: Progress) -> Tuple[str, str, str]:
    """
    Set evidence folder directory variables

    :param int task: The task to update on the job_progress
...
def parse_required_docs(evidence_folder: str, task: int, progress: Progress) -> Tuple[list[dict], set[str]]:
    """
    ...
    with open(evidence_folder + os.sep + "config.json", "r", encoding="utf-8") as json_file:
...
def get_doc_timestamps(evidence_folder: str, directory: str, task: int, progress: Progress) -> list[dict]:
    """
...
def set_required_texts(evidence_folder: str, task: int, progress: Progress) -> set[str]:
    """
...
def find_required_files_in_folder(evidence_folder: str, task: int, progress: Progress) -> list[dict]:
    """
...
def signature_assessment_results(
    directory: list[dict], r_docs: list[dict], task: int, progress: Progress
) -> list[dict]:
    """
...
def document_assessment_results(
    directory: list[dict], documents: set[str], task: int, progress: Progress
) -> list[dict]:
    """
...
def parse_required_text_from_files(evidence_folder: str, task: int, progress: Progress) -> list[dict]:
    """
...
def text_string_search(f_texts: list[dict], req_texts: set[str], task: int, progress: Progress) -> list[dict]:
    """
...
def text_assessment_results(searches: list[dict], r_texts: set[str], task: int, progress: Progress) -> list[dict]:
    """
...
def gather_test_project_data(api: Api, evidence_folder: str, task: int, progress: Progress) -> list[dict]:
    """
    ...
    for item in test_data:
        ...
        response = api.get(
            url=f'{api.config["domain"]}/api/projects/{item["id"]}',
            headers={
...
def assess_doc_timestamps(timestamps: list[dict], documents: list[dict], task: int, progress: Progress) -> list[dict]:
    """
...
def build_assessment_dataframe(assessments: list[dict], task: int, progress: Progress) -> list[dict]:
    """
...
def build_score_data(assessments: list[dict], task: int, progress: Progress) -> Tuple[list[int], list[int], list[int]]:
    """
...
def build_html_table(assessments: list[dict], task: int, progress: Progress) -> list[dict]:
    """
...
def create_child_assessments(
    api: Api, project_data: list[dict], output: list[dict], task: int, progress: Progress
) -> None:
    """
    ...
    for i, project in enumerate(project_data):
        ...
        update_request = api.post(
            f'{api.config["domain"]}/api/assessments',
