...
def create_issue_from_vuln(app: Application, row: "Series", default_due_delta: int) -> "Issue":
    """
    Creates an Issue object from a Tenable vulnerability

    :param Application app: Application object
    :param Series row: Row of data from Tenable
...
def create_issue_from_row(app: Application, row: "Series", default_due_delta: int) -> "Issue":
    """
    Creates an Issue object from a Tenable vulnerability
...
def prepare_issues_for_sync(
    app: Application, df: "DataFrame", regscale_ssp_id: int
) -> Tuple[List["Issue"], List["Issue"]]:
    """
    Prepares Tenable vulnerabilities for synchronization as RegScale issues

    :param Application app: Application object
    :param DataFrame df: Dataframe of Tenable data
...
def create_regscale_issue_from_vuln(regscale_ssp_id: int, df: Optional["DataFrame"] = None) -> None:
    """
    Sync Tenable Vulnerabilities to RegScale issues

    :param int regscale_ssp_id: RegScale System Security Plan ID
    :param Optional["DataFrame"] df: Pandas dataframe of Tenable data
    :rtype: None
    """
    if df is None:
        df = DataFrame()
...
def fetch_vulns(query_id: int = 0, regscale_ssp_id: int = 0) -> Tuple[list, "DataFrame"]:
    """
...
def consume_iterator_to_file(iterator: AnalysisResultsIterator, ssp_id: int) -> None:
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        for dat in iterator:
            ...
            rec_count += 1
            if rec_count == len(iterator.page):
                page_number += 1
                executor.submit(
                    process_sc_chunk,
