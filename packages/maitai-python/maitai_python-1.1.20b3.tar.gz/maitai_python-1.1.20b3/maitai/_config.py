...
def _get_aws_instance_metadata(url, timeout=2):
    try:
        # First, try to get a token for IMDSv2
        token_url = "http://169.254.169.254/latest/api/token"
        token_headers = {"X-aws-ec2-metadata-token-ttl-seconds": "21600"}
        token_response = requests.put(token_url, headers=token_headers, timeout=timeout)
        
        if token_response.status_code == 200:
            # If we got a token, use it to request the metadata
            token = token_response.text
            headers = {"X-aws-ec2-metadata-token": token}
        else:
            headers = None
        
        # Make the actual metadata request
...
def _determine_maitai_host():
    ...
    if _get_aws_instance_metadata("http://169.254.169.254/latest/meta-data/placement/region") == "us-west-2":
        return "https://api.aws.us-west-2.trymaitai.ai"
    
    gcp_zone = _get_gcp_instance_metadata("http://metadata.google.internal/computeMetadata/v1/instance/zone")
    if gcp_zone:
...
class Config:
    ...
    def __init__(self):
        print("INITIALIZING HOST WITH", self.maitai_host)
