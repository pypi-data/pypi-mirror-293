    ...
    def create(
        ...
        *,
        ...
        if server_side_inference:
            ...
            try:
                ...
                if stream:
                    ...
                    return _process_inference_stream(response, retry_args, self.create, timing)
                # ChatCompletion only
        ...
        if not server_side_inference:
            ...
            if stream:
                return _process_openai_stream(session_id, reference_id, intent, application_ref_name, messages, response, evaluation_enabled, completion_params, timing, callback, metadata)
            else:
                timing.time_request_end = time.time()
...
def _process_inference_stream(stream: Iterable[InferenceStreamResponse], retry_args: dict, retry_func, timing: RequestTimingMetric) -> Iterable[MaitaiChunk]:
    first = True
    try:
