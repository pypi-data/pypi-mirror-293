...
def convert_type(
    data_type: Expression, constraints: list[ColumnConstraint], dialect: str = "clickhouse", openapi: bool = False, gpt: bool = False
) -> dict[str, Any]:
    ...
    if dt:
        ...
        value_type = data_type.expressions[0]
        res = convert_type(value_type, [], dialect, openapi=openapi, gpt=gpt)
        if openapi:
            res["nullable"] = True
        elif gpt:
...
def convert_schema(schema: str | dict[str, str], dialect: str = "clickhouse", openapi: bool = False, gpt: bool = False) -> dict[str, Any]:
