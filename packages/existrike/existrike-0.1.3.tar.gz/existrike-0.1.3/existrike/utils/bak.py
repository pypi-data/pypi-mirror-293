...
class ExploitManager:
    def __init__(self):
        # Ajusta o diretório base para o diretório onde o script atual está localizado
        self.base_dir = os.path.dirname(os.path.abspath(__file__))
        self.exploits_dir = os.path.join(self.base_dir, '../exploits')
        self.scans_dir = os.path.join(self.base_dir, '../scans')
        self.auxiliaries_dir = os.path.join(self.base_dir, '../auxiliaries')
        self.configurations = {}
    ...
    def search_modules(self, keyword):
        results = []
        base_dirs = {
            'exploits': self.exploits_dir,
            'scans': self.scans_dir,
            'auxiliaries': self.auxiliaries_dir
        }

        for module_type, base_dir in base_dirs.items():
#           print(f"Searching in base directory: {base_dir}")  # Debug print
            for root, dirs, _ in os.walk(base_dir):
                for dir_name in dirs:
                    module_path = os.path.join(root, dir_name)
#                   print(f"Checking module path: {module_path}")  # Debug print
            ...
            if cmd.startswith("search "):
                ...
                if results:
                    ...
                    for result in results:
                        print(result)  # Removed highlight_keyword for simplicity
                else:
