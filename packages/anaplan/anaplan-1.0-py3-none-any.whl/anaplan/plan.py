...
def sobel_activation(image):
    """
    NumPy kullanarak kenar algılama aktivasyonu yapan bir fonksiyon.
    Sobel filtresi ile kenar algılaması yapar ve sonuçları geri döner.
    
    Args:
        image (np.array): Girdi olarak kullanılan görüntü.
    
    Returns:
        np.array: Kenar algılama işlemi sonrası elde edilen görüntü.
    """
    
    # Eğer görüntü renkli ise gri tonlamaya çeviriyoruz
    if len(image.shape) == 3 and image.shape[-1] == 3:
        image_gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    else:
        image_gray = image
    
    # Sobel kenar algılama (x ve y yönlerinde)
    sobel_x = cv2.Sobel(image_gray, cv2.CV_64F, 1, 0, ksize=3)
    sobel_y = cv2.Sobel(image_gray, cv2.CV_64F, 0, 1, ksize=3)
    
    # Kenar büyüklüğünü hesaplıyoruz
    sobel_magnitude = np.sqrt(sobel_x**2 + sobel_y**2)
    
    # Görüntüyü normalize edip geri döndürüyoruz
...
def fast_activation_vector(image):
    """
    FAST algoritması ile köşe tespiti yapar ve vektör olarak döner.
    """
    ...
    # Köşe noktalarını gösteren mask oluşturuyoruz
    mask = np.zeros_like(image_gray)
    for kp in keypoints:
        x, y = map(int, kp.pt)
        mask[y, x] = 255

    # Maskeyi vektöre çeviriyoruz
...
def gabor_activation_vector(image, frequency=0.6):
    """
    Gabor filtresi kullanarak özellik çıkarır ve sonucu vektör olarak döner.
    """
    # Eğer görüntü renkli ise gri tonlamaya çevir
    if len(image.shape) == 3 and image.shape[-1] == 3:
        image_gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    ...
    # Gabor filtresi kullanarak özellik çıkarma
    ...
    # Sonucu normalize edip vektöre çeviriyoruz
...
def hog_activation_vector(image):
    
    HOG (Histogram of Oriented Gradients) özelliği çıkarır ve sonucu vektör olarak döner.
    ...
    # HOG özelliği çıkarma
    hog_features, _ = hog(image_gray, pixels_per_cell=(8, 8),
...
def canny_activation_vector(image):
    """
    Canny kenar algılama işlemi yapar ve sonucu vektör olarak döner.
    """
    # Eğer görüntü renkli ise gri tonlamaya çevir
    if len(image.shape) == 3 and image.shape[-1] == 3:
        image_gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    ...
    # Kenar algılama sonucunu vektöre çevir
...
def sift_activation_vector(image):
    """
    SIFT algoritması ile özellik çıkarır ve sonucu vektör olarak döner.
    """
    # Eğer görüntü renkli ise gri tonlamaya çevir
    if len(image.shape) == 3 and image.shape[-1] == 3:
        image_gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    ...
    # SIFT algoritmasını oluştur
    sift = cv2.SIFT_create()
    
    # Anahtar noktaları ve tanımlayıcıları bul
    keypoints, descriptors = sift.detectAndCompute(image_gray, None)
    
    # Descriptorları vektöre çevir (vektör sayısı görüntüdeki anahtar nokta sayısına bağlıdır)
...
def laplacian_activation_vector(image):
    """
    Laplacian kenar algılama işlemi yapar ve sonucu vektör olarak döner.
    """
    # Eğer görüntü renkli ise gri tonlamaya çevir
    if len(image.shape) == 3 and image.shape[-1] == 3:
        image_gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    ...
    # Normalize edip 0-255 aralığına çekiyoruz
...
def fex(
    ...
    for activation in activation_potentiation:
        ...
        if activation == 'sigmoid':
            ...
            Output += hog_activation_vector(Input)
