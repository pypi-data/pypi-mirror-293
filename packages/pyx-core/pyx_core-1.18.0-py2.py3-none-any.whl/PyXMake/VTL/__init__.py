...
class Command(object):
    ...
    def alias(*args, **kwargs):
        ...
        alias = {"stm_conda_env":"config","stm_cara_software":"cara","py2x":"f2py"}
        return alias
    ...
    def parse(cls, command, config, args):
        ...
        if config and args.method[-1].lower() not in ["-h","--help"] and args.method[0] in command:
            print("==================================")
            ...
            index = [n for n, x in enumerate(command, 1) if args.method[0] in x][0]
...
def GetSourceCode(key_opt=0):
    ...
    beos_src_files = ["Main.f90", "BEOSMain.for", "A11xst.for", "A11yst.for", "A21ast.for", "A22ast.for", "ABDetc.for", 
                                "BmtstN.for", "BuVi.for", "Chneig.for", "Chodec.for", "Chosol.for", "Cktype.for", "Cmpa11.for", "Cmpa21.for", 
                                "Cmpa22.for", "Cmpbmt.for", "Cmpmmt.for", "Cntr.for", "CripplingO.for", "CripplingT.for", "EIGVPR.FOR", 
                                "F01kbf.for", "F01KTF.FOR", "GlBuck.for", "Incs2.for", "Incs3N.for", "Iuoriv3.for", "JACOB2.FOR", "Kuorkv2.for", 
                                "Lqfit95.for", "MKLVBD.F90", "Mmtst.for", "Modes.for", "Phi.for", "Plotmesh.for", "PPROBTM95.for", "Prtout.for", 
                                "Prtvib.for", "PRTYPE.FOR", "RddataN-Ak.for", "Rdtype.for", "Redfkq.for", "Setkey_2.for", "SIMVT2003.FOR", 
                                "TCZINC.FOR", "Tfmstf.for", "TMINC.FOR", "Trnpsi.for", "Vibs2.for", "Vibs3NN.for","#PRAMS"]
...
def GetBuildCommand(make_opt=0, _format="fixed", _arch=Utility.GetArchitecture()):
    ...
    win_abq_command += "&& ( " 
    win_abq_command += "ECHO compile_fortran=['ifort', "  
    win_abq_command += "'/c','/DABQ_WIN86_64', '/extend-source', '/fpp', " 
    win_abq_command += "'/iface:cref', '/recursive', " 
    win_abq_command += "'/QxSSE3', '/QaxAVX', '/heap-arrays:1', "  
    ...
    win_abq_command += "'/reentrancy:threaded', "                
    win_abq_command += "'/Qopenmp', " # <-- Use OpenMP library
    
    try:
        # Windows version exits only on Windows (obviously)
        if sys.getwindowsversion() >= (10, 0, 0):
            # Windows 10
            win_abq_command += "'/DABQ_FORTRAN', "  
            win_abq_command += "'/Qauto', " # <-- important for thread-safety of parallel user subroutines
            win_abq_command += "'/align:array64byte', "
            win_abq_command += "'/Qpc64', " # set FPU precision to 53 bit significand
            win_abq_command += "'/Qprec-div', '/Qprec-sqrt', " # improve precision of FP divides and sqrt
            win_abq_command += "'/Qfma-', "  # disable floating point fused multiply-add
            win_abq_command += "'/fp:precise', "  # floating point model: precise 
            win_abq_command += "'/Qimf-arch-consistency:true', "  # math library consistent results
            win_abq_command += "'/Qfp-speculation:safe', "  # floating point speculations only when safe
            win_abq_command += "'/Qprotect-parens', "  # honor parenthesis during expression evaluation
            win_abq_command += "'/Qfp-stack-check', "  # enable stack overflow protection checks
        else:
            # Windows 7
            win_abq_command += "'/Qauto-scalar', "
    except:
        # We are on Linux. No Problem.
        pass
    ...
    # win_abq_command += "'/DCLUSTER', "                          # <-- Conditional compilation on Cluster
    win_abq_command += "'/include:%I']  ) >> %ABQ_ENV_FILE% "                
    win_abq_command += "&& ( "     
    win_abq_command += "ECHO link_sl=['LINK', '/nologo', '/NOENTRY', '/INCREMENTAL:NO', '/subsystem:console', '/machine:AMD64',  " 
    win_abq_command += "'/NODEFAULTLIB:LIBC.LIB', '/NODEFAULTLIB:LIBCMT.LIB', " 
    win_abq_command += "'/DEFAULTLIB:OLDNAMES.LIB', '/DEFAULTLIB:LIBIFCOREMD.LIB', '/DEFAULTLIB:LIBIFPORTMD.LIB', '/DEFAULTLIB:LIBMMD.LIB',  " 
    win_abq_command += "'/DEFAULTLIB:kernel32.lib', '/DEFAULTLIB:user32.lib', '/DEFAULTLIB:advapi32.lib',  " 
    win_abq_command += "'/FIXED:NO', '/dll', " 
    # win_abq_command+= "'/DEBUG', "                                 # <-- Debugging 
    # win_abq_command += "'/VERBOSE', "                            # <-- Show commands
    win_abq_command += "'/def:%E', '/out:%U', '%F', '%A', '%L', '%B', "
    # Overwrite CRT section in ABQ, since its implementation is flawed (or at least is not modifiable enough for our purposes).
    win_abq_command += "r'/def:"+os.getenv("pyx_abaqus_def","%E")+"', '/INCLUDE:_DllMainCRTStartup', '/FORCE', " 
    win_abq_command += "'oldnames.lib', 'user32.lib', 'ws2_32.lib', 'netapi32.lib', 'advapi32.lib', "
...
def GetIncludeDirectory(base_path, key_opt=0, make_opt=0, architecture="x64"):
    ...
    if key_opt == 0 and make_opt in range(6):
        ...
        inc = ["six.py", "ipykernel_launcher.py",         
                ...
                os.path.pathsep.join([__occ_src_path,os.path.join(__occ_name,".")]),
                ...
                os.path.pathsep.join([os.path.join(__pyc_src_path,"Plugin","Smetana","src","smetana","static","abaqus_msa_reports.py"),os.path.join("static",".")]),
                ...
                os.path.pathsep.join([os.path.join(__pyc_src_path,   "GUI","res","stm_lab_icon.png"), os.path.join(__pyc_name,     "GUI", "res",".")]),
        ...
        try: 
            ...
            import requests, jupyter_server
            ## Add requests and server site package explicitly
            inc.extend([os.path.pathsep.join([requests.__path__[0],os.path.join(requests.__name__,".")])]);
            inc.extend([os.path.pathsep.join([jupyter_server.__path__[0],os.path.join(jupyter_server.__name__,".")])]);
        except: pass
    else:
        inc = [] 
    return inc
...
def GetLinkDependency(key_opt=0, make_opt=0, architecture="x64"):
    ...
    if key_opt == 0 and make_opt in range(6):
        ...
        ## Add additional libraries
        lib += [os.path.join(__pyc_src_path,"Plugin","DELiS","src"),os.path.join(__pyc_src_path,"Plugin","Smetana"), 
                     os.path.join(__pyc_src_path,"Plugin","Smetana","src","smetana")]            
    else:
        lib = []   
    return lib
