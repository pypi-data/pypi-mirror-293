...
for k in range(n_Lattice):
    ...
    diret_cells_Lattice = 'cells_Lattice' + str(k+1)
    diret_structures    = 'Lattice' + str(k+1)

    #----------------------------------------------------
    # Verificando a existância do diretório 'Lattice' ---
    #----------------------------------------------------
    if os.path.isdir(diret + diret_cells_Lattice):
       0 == 0
    else:
       os.mkdir(diret + diret_cells_Lattice)
    #---------------------------------------
    ...
    for s in range(ions_crit_i, (ions_crit_f+1)):
        cell_Lattice = open(diret + diret_cells_Lattice + '/Cells_Lattice_' + str(s) + '.dat', "w")
        cell_Lattice.close()
    #-----------------------
                      ...
                      if (ion1 == ion2 == type and v1z == v2z == ion_z):
                         ...
                         if (angle >= angle_min and angle <= angle_max):
                            if (angle != 0.0 and angle != 180.0):
                               cell_Lattice = open(diret + diret_cells_Lattice + '/Cells_Lattice_' + str(n_ions) + '.dat', "a")
    ...
    #----------------------------------------------------------------------------------------------------
    for s in range(ions_crit_i, (ions_crit_f+1)):
        if ( os.path.getsize(diret + diret_cells_Lattice + '/Cells_Lattice_' + str(s) + '.dat') == 0 ):
           os.remove(diret + diret_cells_Lattice + '/Cells_Lattice_' + str(s) + '.dat')
    #----------------------------------------------------------------------------------
