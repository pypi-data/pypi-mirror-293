...
for k in range(n_Lattice):
    ...
    #----------------------------------------------------------------------------------------------------
    for s in range(ions_crit_i, (ions_crit_f+1)):
        cell_Lattice = open(diret + dir_cells_Lattice + '/Cells_Lattice_' + str(s) + '.dat', "w")
        cell_Lattice.close()
    #-----------------------
    ...
    for i in range(len(vx)):
        for j in range(len(vx)):
            ...
            n_ions = round(n_ions, 3);  n_ions = int(n_ions)
            #-------------------------------------------------------------------------------
               ...
               try:
                   ...
                   if (produto_triplo > 0):
                      ...
                      if (ion1 == ion2 == type and v1z == v2z == ion_z):
                         ...
                         if (dot_product < -1.0 or dot_product > 1.0):  print(dot_product)
                         angle = np.arccos(dot_product) / np.pi * 180
                         angle = round(angle, 3)                 
                         #----------------------------------------------
                         if (angle >= angle_min and angle <= angle_max):
                            if (angle != 0.0 and angle != 180.0):
                               cell_Lattice = open(diret + dir_cells_Lattice + '/Cells_Lattice_' + str(n_ions) + '.dat', "a")
    ...
    #--------------------------------------------
    for s in range(ions_crit_i, (ions_crit_f+1)):
        if ( os.path.getsize(diret + dir_cells_Lattice + '/Cells_Lattice_' + str(s) + '.dat') == 0 ):
           os.remove(diret + dir_cells_Lattice + '/Cells_Lattice_' + str(s) + '.dat')
    #--------------------------------------------------------------------------------
...
for s in range(ions_crit_i, (ions_crit_f+1)):
    ...
    if (n_Lattice == 2):  
       if os.path.isfile(diret + 'cells_Lattice1' + '/Cells_Lattice_' + str(s) + '.dat'):
          if os.path.isfile(diret + 'cells_Lattice2' + '/Cells_Lattice_' + str(s) + '.dat'):
             cont = 1
             #-------
    if (n_Lattice == 3):
       if os.path.isfile(diret + 'cells_Lattice1' + '/Cells_Lattice_' + str(s) + '.dat'):
          if os.path.isfile(diret + 'cells_Lattice2' + '/Cells_Lattice_' + str(s) + '.dat'):
             if os.path.isfile(diret + 'cells_Lattice3' + '/Cells_Lattice_' + str(s) + '.dat'):
                cont = 1
                #-------

    if (cont == 1 and n_Lattice == 2):
       #----------------------------------------
       print(f'Analisando redes com {s} atomos')
       #----------------------------------------

       #------------------------------------------------------------------------------------------------------
       SLattice1 = np.loadtxt(diret + 'cells_Lattice1' + '/Cells_Lattice_' + str(s) + '.dat'); SLattice1.shape
       SLattice2 = np.loadtxt(diret + 'cells_Lattice2' + '/Cells_Lattice_' + str(s) + '.dat'); SLattice2.shape
       #------------------------------------------------------------------------------------------------------

       #-------------------------------------------------------------------------------------------------------------------
       if (n_Lattice == 2):  Structures = open(diret + 'Matching_Lattices' + '/Matching_Lattices_' + str(s)  + '.dat', "w")
       if (n_Lattice == 3):  Structures = open(diret + 'Matching_Lattices' + '/Matching_Lattices_' + str(s)  + '12.dat', "w")
       #------------------------------------------------------------------------------------------------------------------------------------------------------------
...
for s in range(ions_crit_i, (ions_crit_f+1)):
    ...
    if (cont == 1 and n_Lattice == 3):
       ...
       #----------------------------------------------------------------------------------------------------------------
       SLattice1 = np.loadtxt(diret + 'Matching_Lattices' + '/Matching_Lattices_' + str(s)  + '12.dat'); SLattice1.shape
       SLattice3 = np.loadtxt(diret + 'cells_Lattice3' + '/Cells_Lattice_' + str(s) + '.dat'); SLattice3.shape
       #------------------------------------------------------------------------------------------------------
       Structures = open(diret + 'Matching_Lattices' + '/Matching_Lattices_' + str(s)  + '.dat', "w")
       #---------------------------------------------------------------------------------------------------------------------------------------------------------------
       v1Ax = SLattice1[:,0]; v1Ay = SLattice1[:,1]; d1A = SLattice1[:,2]; v1Bx = SLattice1[:,3];  v1By = SLattice1[:,4];  d1B = SLattice1[:,5];  ang1 = SLattice1[:,6]
...
print(f' ')
print(f'========================================================================')
print(f'Passo 4: Escrevendo os arquivos POSCAR para as redes dos {n_Lattice} materiais ===')
print(f'========================================================================')
...
if (n_test == 1):
   for s in range(len(dir_matchings)):
       ...
       ni_m = dir_temp_m.replace('Matching_Lattices_', '')
       print(f'Analisando redes com {ni_m} atomos')
       #-------------------------------------------
...
print(f' ')
print(f'===============================================================')
print(f'Passo 5: Escrevendo os arquivos POSCAR das Heteroestruturas ===')
print(f'===============================================================')  
   ...
   for s in range(len(dir_matchings)):
       ...
       if (n_structures != 0):
          ...
          for i in range(n_structures):
              ...
              #-----------------------------------
              if os.path.isdir(dir_files + '/' + dir_o + '/' + dir_temp_m):
                 0 == 0
              else:
                 os.mkdir(dir_files + '/' + dir_o + '/' + dir_temp_m)
              #--------------------------------------------------------
              diret2 = dir_files + '/' + dir_o + '/' + dir_temp_m + '/'
              #--------------------------------------------------------
              if (loop_ht == 1):
                 if os.path.isdir(dir_files + '/' + dir_o + '/' + dir_temp_m + '/' + dir_loop):
                    0 == 0
                 else:
                    os.mkdir(dir_files + '/' + dir_o + '/' + dir_temp_m + '/' + dir_loop)
                 #-----------------------------------------------------------------------
...
n_structures = 0
#---------------
files0 = os.listdir(dir_files + '/' + dir_o)
#---------------------------
for i in range(len(files0)):
    files1 = os.listdir(dir_files + '/' + dir_o + '/' + files0[i])
...
print(f' ')
print(f'========================================')
print(f'Passo 6: Excluindo celulas não-unitárias')
print(f'========================================')
...
for s in range(len(files0)):
    ...
    files = os.listdir(dir_files + '/' + dir_o + '/' + files0[s])

    for i in range(len(files)):
        #--------------------------
        structure = Poscar.from_file(dir_files + '/' + dir_o + '/' + files0[s] + '/' + files[i]).structure   # Lendo o arquivo POSCAR
        ...
        Poscar(reduced_structure).write_file(dir_files + '/' + dir_o + '/' + files0[s] + '/' + 'temp_' + str(i+1) + '.vasp')

        poscar1 = open(dir_files + '/' + dir_o + '/' + files0[s] + '/' + files[i], "r")
        poscar2 = open(dir_files + '/' + dir_o + '/' + files0[s] + '/' + 'temp_' + str(i+1) + '.vasp', "r")
        ...
        #---------------------------------------------------
        if (nion2 < nion1):
           os.remove(dir_files + '/' + dir_o + '/' + files0[s] + '/' + files[i])
           n_exclusion += 1

    for i in range(len(files)):
        os.remove(dir_files + '/' + dir_o + '/' + files0[s] + '/' + 'temp_' + str(i+1) + '.vasp')
...
for i in range(len(files0)):
    ...
    n_structures += len(files1) 
    #--------------------------
    if (len(files1) == 0): shutil.rmtree(dir_files + '/' + dir_o + '/' + files0[i])
#-----------------------------------------------------------------------------------
files0 = os.listdir(dir_files + '/' + dir_o)
#-------------------------------------------
if (len(files0) == 0):
...
print(f' ')
print(f'========================================')
print(f'Passo 7: Excluindo Redes semelhantes ===')
print(f'========================================')
...
for s in range(len(files0)):
    ...
    for i in range(len(files)):
        #----------------------------------------------------------------------------
        if os.path.exists(dir_files + '/' + dir_o + '/' + files0[s] + '/' + files[i]):
           #--------------------------------------------------------------------------------------------------
           structure1 = Poscar.from_file(dir_files + '/' + dir_o + '/' + files0[s] + '/' + files[i]).structure
           #--------------------------------------------------------------------------------------------------
           poscar = open(dir_files + '/' + dir_o + '/' + files0[s] + '/' + files[i], "r")
           VTemp = poscar.readline().split()
           ...
           for j in range(len(files)):
               if (i != j):
                  #-----------------------------------------------------------------------------
                  if os.path.exists(dir_files + '/' + dir_o + '/' + files0[s] + '/' + files[j]):
                     #--------------------------------------------------------------------------
                     structure2 = Poscar.from_file(dir_files + '/' + dir_o + '/' + files0[s] + '/' + files[j]).structure
                     #---------------------------------------------------------------------------------------------------
                     matcher = StructureMatcher()                   # Criando um objeto StructureMatcher
                     if matcher.fit(structure1, structure2):        # Verificando se as estruturas são similares
                        #----------------------------------------------------------------------------------------
                        poscar = open(dir_files + '/' + dir_o + '/' + files0[s] + '/' + files[j], "r")
                        VTemp = poscar.readline().split()
                        ...
                        #----------------
                        if (dAB1 < dAB2):
                           os.remove(dir_files + '/' + dir_o + '/' + files0[s] + '/' + files[j])
                           n_exclusion += 1
                        #----------------
                        if (dAB1 > dAB2):
                           if os.path.exists(dir_files + '/' + dir_o + '/' + files0[s] + '/' + files[i]):
...
#========================================================
print(f'--------------------------------------')
print(f'{n_exclusion2} células foram filtradas/excluidas')
print(f'{n_structures} células restantes')
#========================================================
...
for i in range(len(files0)):
    number += 1
    #------------------------------------------------------
    temp_nion = files0[i].replace('Matching_Lattices_', '')
    #------------------------------------------------------
    files1 = os.listdir(dir_files + '/' + dir_o + '/' + files0[i])
    #-------------------------------------------------------------
    for j in range(len(files1)):
        shutil.copyfile(dir_files + '/' + dir_o + '/' + files0[i],  dir_files + '/' + dir_o + '/' + str(temp_nion) + 'atoms_POSCAR_' + str(number))
    #-------------------------------------------------------
    shutil.rmtree(dir_files + '/' + dir_o + '/' + files0[i])
    #-------------------------------------------------------
