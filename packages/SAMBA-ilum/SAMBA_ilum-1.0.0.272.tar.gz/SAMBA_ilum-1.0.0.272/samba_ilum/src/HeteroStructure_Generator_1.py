       ...
       if (n_structures != 0):
          ...
          for i in range(n_structures):
              ...
              if (loop_ht == 1):
                 ...
                 #--------------------------------------------------------------------------
                 diret0 = dir_files + '/' + dir_o + '/' + dir_loop + '/'
                 diret2 = dir_files + '/' + dir_o + '/' + dir_loop + '/' + dir_temp_m + '/'
...
n_structures = 0
#---------------
files0 = os.listdir(diret0)
#--------------------------
for i in range(len(files0)):
    files1 = os.listdir(diret0 + files0[i])
...
for s in range(len(files0)):
    ...
    files = os.listdir(diret0 + files0[s])

    for i in range(len(files)):
        #--------------------------
        structure = Poscar.from_file(diret0 + files0[s] + '/' + files[i]).structure   # Lendo o arquivo POSCAR
        ...
        Poscar(reduced_structure).write_file(diret0 + files0[s] + '/' + 'temp_' + str(i+1) + '.vasp')

        poscar1 = open(diret0 + files0[s] + '/' + files[i], "r")
        poscar2 = open(diret0 + files0[s] + '/' + 'temp_' + str(i+1) + '.vasp', "r")
        ...
        #---------------------------------------------------
        if (nion2 < nion1):
           os.remove(diret0 + files0[s] + '/' + files[i])
           n_exclusion += 1

    for i in range(len(files)):
        os.remove(diret0 + files0[s] + '/' + 'temp_' + str(i+1) + '.vasp')
...
for i in range(len(files0)):
    ...
    n_structures += len(files1) 
    #--------------------------
    if (len(files1) == 0): shutil.rmtree(diret0 + files0[i])
#-----------------------------------------------------------
files0 = os.listdir(diret0)
#-------------------------------------------
if (len(files0) == 0):
...
for s in range(len(files0)):
    ...
    for i in range(len(files)):
        #------------------------------------------------------
        if os.path.exists(diret0 + files0[s] + '/' + files[i]):
           #---------------------------------------------------------------------------
           structure1 = Poscar.from_file(diret0 + files0[s] + '/' + files[i]).structure
           #---------------------------------------------------------------------------
           poscar = open(diret0 + files0[s] + '/' + files[i], "r")
           VTemp = poscar.readline().split()
           ...
           for j in range(len(files)):
               if (i != j):
                  #------------------------------------------------------
                  if os.path.exists(diret0 + files0[s] + '/' + files[j]):
                     #--------------------------------------------------------------------------
                     structure2 = Poscar.from_file(diret0 + files0[s] + '/' + files[j]).structure
                     #----------------------------------------------------------------------------------
                     matcher = StructureMatcher()                   # Criando um objeto StructureMatcher
                     if matcher.fit(structure1, structure2):        # Verificando se as estruturas são similares
                        #----------------------------------------------------------------------------------------
                        poscar = open(diret0 + files0[s] + '/' + files[j], "r")
                        VTemp = poscar.readline().split()
                        ...
                        #----------------
                        if (dAB1 < dAB2):
                           os.remove(diret0 + files0[s] + '/' + files[j])
                           n_exclusion += 1
                        #----------------
                        if (dAB1 > dAB2):
                           if os.path.exists(diret0 + files0[s] + '/' + files[i]):
...
for i in range(len(files0)):
    ...
    for j in range(len(files1)):
        number += 1 
        shutil.copyfile(diret0 + files0[i] + '/' + files1[j],  diret0 + 'POSCAR_' + str(number))
    #-------------------------------------------------------------------------------------------
    shutil.rmtree(diret0 + files0[i])
    #--------------------------------
