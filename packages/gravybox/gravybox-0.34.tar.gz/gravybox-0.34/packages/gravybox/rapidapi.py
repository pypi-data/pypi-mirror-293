...
from gravybox.requests import REQUEST_CLIENT_WRAPPER
...
async def query_rapidapi(url, host, query, extra_headers=None):
    ...
    request_client = REQUEST_CLIENT_WRAPPER.get_client()
    response = await request_client.get(url, headers=headers, params=query)
