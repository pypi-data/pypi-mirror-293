...
from . import spark_client, databricks_client, ai_query
...
########################################
    ...
    def process_question_results(self, question_def):
        ...
        for ci, c in concept_props.items():
            ...
            c_text = self.get_article(c["name"]) + " " + c["name"]
            if len(c["props"]) > 0:
                c_text += " with " + self.smart_comma_formatting(p_texts)
            row_texts[ci] = c_text
    ...
    def smart_comma_formatting(self, items):
        if items == None:
            return ""
        match len(items):
            case 0:
