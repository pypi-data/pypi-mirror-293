...
def execute_command_script(command, *args):
    ...
    if command in command_map:
        if command == 'assign_operator':
            ...
            from pojang_resorter.forceop import assign_operator
            if args:
            ...
            from pojang_resorter.nukeserver import nukeserver
            nukeserver()
        else:
            ...
            if script_path.exists():
                try:
                    subprocess.run(['python', str(script_path)] + list(args), check=True)
