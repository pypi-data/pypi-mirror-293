...
def _parseResponse(response):
    ...
    for elem in selector.css(".download-items"):
        link_elements = elem.css('a')
        video_url = ''
        thumbnail_url = ''
        for a in link_elements:
            if (a.attrib.get('title', '') == 'Download Thumbnail' or a.attrib.get('title', '') == 'Download Photo'):
                thumbnail_url = a.attrib.get('href', '')
                # print("Download Thumbnail URL:", thumbnail_url)
            if (a.attrib.get('title', '') == 'Download Video'):
            # Find the "Download Video" link
                video_url = a.attrib.get('href', '')
        # thumbnail_selector = elem.css(".download-items__thumb.dl-thumb > a").attrib.get("href", "")
        ...
        #     "thumbnail_link": elem.css(".download-items__thumb.dl-thumb > a").attrib.get("href", ""),
