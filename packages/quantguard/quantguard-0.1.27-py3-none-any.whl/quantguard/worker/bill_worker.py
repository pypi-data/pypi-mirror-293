...
class BillWorker(Worker):
    ...
    async def sync_position(self):
        ...
        for db_position in db_positions:
            if db_position.quantity != 0 and db_position.base_asset not in new_position_base_asset:
                item = copy.deepcopy(db_position)
                item.quantity = 0
                item.created_at = positions[0].created_at if len(positions) > 0 else int(time.time() * 1000)
                item.liquidation_price = item.liquidation_price if item.liquidation_price else ""
                result = PositionDao().insert(item)
                logger.info(f"exchange {self.name} position inset zero result: {result}")
