...
class SQAPI:
    ...
    def get(self, endpoint, poll_status_interval=None, *args, **kwargs) -> Get:
        result = Get(endpoint, sqapi=self, *args, **kwargs)
    ...
    def export(self, endpoint, include_columns=None, poll_status_interval=1.0, *args, **kwargs):
        result = Export(endpoint, sqapi=self, page=None, results_per_page=None, include_columns=include_columns, *args, **kwargs)
        result = self.poll_task_status(result, poll_status_interval=poll_status_interval)
        return result
    ...
    def poll_task_status(self, result, poll_status_interval=1.0):
        if isinstance(poll_status_interval, (int, float)):
            ...
            while True:
                ...
                status = self.get(task_data.get('status_url')).execute(verbosity=0).json()
                ...
                if status.get('status') == "done" and status.get('result_available'):
                    result = self.get(task_data.get('result_url'))
                    break
