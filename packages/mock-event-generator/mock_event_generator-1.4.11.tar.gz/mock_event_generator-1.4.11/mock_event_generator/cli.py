...
from .replay import calculate_offset, replay_superevents
...
def replay(
    ...
    """Replay a set of G-events continuously and upload them to GraceDB."""
    if search is None and not original_search:
        search = 'MDC'
        ...
        async with trio.open_nursery() as nursery:
            for sevent in replay_superevents(source_client, start, end):
                ...
                logger.info(
                    ...
                    f'at time {sevent.t_0:.1f}, waiting {delay:.1f}s...'
                )
