...
class LanguageValuesModel(BaseModel):
    code: BaseValue
    currentLanguage: JsonValue
    masterId: BaseValue
    translationId: BaseValue
    translationStatus: JsonValue
...
class Alram(BaseModel):
    ...
    sended_at: Optional[datetime] = None
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        if self.sended_at:
            self.sended_at = self.sended_at.strftime('%Y-%m-%d %H:%M:%S')
        if self.reservated_at:
            self.reservated_at = self.reservated_at.strftime('%Y-%m-%d %H:%M:%S')
...
class Statistics(BaseModel):
    ...
    registerationTime: Optional[str] = None

    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        if self.registerationTime is None:
            now = datetime.now()

            self.registerationTime = int(time.mktime(now.timetuple()) * 1000 + now.microsecond / 1000)
...
# cid 구분 펑션
def determine_cid(self, value) -> CIDType:
