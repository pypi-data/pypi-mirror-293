...
class CommonBusiness:
    ...
    @classmethod
    def service(self, id: str, title: str):
        def decorator(func):
            return func
        
        self.permissions['id'] = id
        self.permissions['title'] = title

        return decorator

    @classmethod
    def permission(self, id: str, title: str, default: bool = True):
        def decorator(func):
            if not hasattr(func, '__permission_info__'):
        ...
        self.permissions['children'].append({
            'id': id,
            'title': title,
            'default': default
        })
    ...
    def notifyPermissions(self):
        self.producer_send_message(1, json.dumps(self.permissions))
...
def build_router(business:CommonBusiness, prefix:str) -> Tuple[FastAPI, APIRouter]:
        
    # 서비스 생성 시 lifespan 이벤트 핸들러 정의
    async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:

        business.notifyPermissions()
