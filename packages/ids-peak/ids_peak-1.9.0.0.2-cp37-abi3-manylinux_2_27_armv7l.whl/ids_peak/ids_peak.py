...
class RegisterNode(Node):
    ...
    def Write(self, bytes: "VectorUInt8") -> "None":
        r"""
...
class DataStream(EventSupportingModule):
    ...
    * Let the API allocate and free the memory for the buffer, using AllocAndAnnounceBuffer(). Then, the memory
    ...
    def WaitForFinishedBuffer(self, timeout_ms: "Timeout") -> "Buffer":
        ...
        Additionally the blocking wait can be aborted by calling 'KillWait'.

        Notes: After the buffer has been received it has to be requeued using
              'QueueBuffer' once the buffer data has been copied or processed.
              Until the buffer is requeued it is not available to the
              acquisition engine. Once the acquisition engine runs out of
              buffers no more images can be received.
    ...
    def KillWait(self) -> "None":
        r"""
        Tells the data stream to kill a wait for a finished buffer.

        Kills one call to 'WaitForFinishedBuffer' or queues the kill request
        to be applied to the next call to 'WaitForFinishedBuffer' immediately.
