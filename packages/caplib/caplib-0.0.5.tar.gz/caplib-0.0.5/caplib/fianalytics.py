...
def build_bond_sprd_curve(build_settings,
    ...
    pb_input = dqCreateProtoBondCreditSpreadCurveBuildingInput(create_date(as_of_date),
                                                               ...
                                                               curve_name, 
                                                               to_ir_yield_curve_building_method(build_method), calc_jacobian)
...
def vanilla_bond_pricer(instrument, pricing_date, mkt_data, pricing_settings, risk_settings):
    ...
    '''
    pb_input = dqCreateProtoVanillaBondPricingInput(create_date(pricing_date), instrument, mkt_data, pricing_settings, risk_settings,
                                                    False, b'', b'', b'', b'')
...
def yield_to_maturity_calculator(calculation_date, compounding_type, bond, forward_curve, price, price_type, frequency):
    ...
    if forward_curve is None:
        forward_curve = create_flat_ir_yield_curve(calculation_date, 'USD', 0.0)
        
    pb_input = dqCreateProtoYieldToMaturityCalculationInput(create_date(calculation_date), to_compounding_type(compounding_type), 
