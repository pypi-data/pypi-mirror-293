...
from aioredis3.client import Redis as aioredis3
...
class aioredis3Manager(object):
    _redis_db__conn_map = {}

    def __init__(self, host='127.0.0.1', port=6379, db=0, username='', password=''):
        self._key = (host, port, db, username, password,)
        if self._key not in self.__class__._redis_db__conn_map:
            self.__class__._redis_db__conn_map[self._key] = aioredis3(host=host, port=port, db=db, username=username,
                                                                     password=password, max_connections=1000, decode_responses=True)
        self.redis = self.__class__._redis_db__conn_map[self._key]

    def get_redis(self) -> aioredis3:
        """
        :rtype :redis5.Redis
        """
        return self.redis
...
class aioredis3Mixin(object):
    @property
    @decorators.cached_method_result
    def aioredis3_db_filter_and_rpc_result(self):
        # aioredis3 包已经不再更新了,推荐使用redis包的asyncio中的类
        # return aioredis3Manager(**_get_redis_conn_kwargs_by_db(BrokerConnConfig.REDIS_DB_FILTER_AND_RPC_RESULT)).get_redis()
