...
from fastboost.utils.mongo_util import MongoMixin

from fastboost.concurrent_pool import CustomThreadPoolExecutor
from fastboost.concurrent_pool.flexible_thread_pool import FlexibleThreadPoolMinWorkers0
from fastboost.utils.redis_manager import RedisMixin
from fastboost.utils.redis_manager import aioredis3Mixin
...
class AioAsyncResult(aioredis3Mixin):
...
if __name__ == '__main__':
    ...
    async def is_pending(self):
        is_exists = await self.aioredis3_db_filter_and_rpc_result.exists(self.task_id)
    ...
    async def status_and_result(self):
        if not self._has_pop:
            t1 = time.time()
            redis_value = await self.aioredis3_db_filter_and_rpc_result.blpop(self.task_id, self.timeout)
            self._has_pop = True
            if redis_value is not None:
                status_and_result_str = redis_value[1]
                self._status_and_result = Serialization.to_dict(status_and_result_str)
                await self.aioredis3_db_filter_and_rpc_result.lpush(self.task_id, status_and_result_str)
                await self.aioredis3_db_filter_and_rpc_result.expire(self.task_id, self._status_and_result['rpc_result_expire_seconds'])
                return self._status_and_result
            return None
        return self._status_and_result
