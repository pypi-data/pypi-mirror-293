...
from expAscribe.model.functional import MgCSL
...
def voting(meta_graphs:List[np.ndarray], priori_knowledge:np.ndarray) -> np.ndarray:
    # Implement voting algorithm here
    model = Voter(meta_graphs,priori_knowledge)
...
def ensemble(data:pd.DataFrame) -> List[np.ndarray]:
    ...
    lin = lingam.ICALiNGAM()
    lin.fit(data)
    glin = lin.adjacency_matrix_
    glin = remove_indirect_links(glin)
    glin = binarize(glin)
    candidates.append(glin)

    g, edges = fci(datasrc1)
