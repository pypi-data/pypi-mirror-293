...
def kink_regression_2d(file_path,xl,xh,iplot=True):
    ...
    if iplot:
        ...
        plt.scatter(data['X'], data['Y'],color='#3BA997' ,alpha=0.5, label='Samples')
        plt.plot(x_vals, y_vals, color='#B291B5', label='Piecewise Linear Fit')
        plt.axvline(best_knot_x, color='grey', linestyle='--', label='Estimated Knot')
        plt.xlabel('X')
        plt.ylabel('Y')
