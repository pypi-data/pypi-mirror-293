...
class DiscordMessageImageGenerator():
    def __init__(self):
        self.chrome_driver = start_chrome(False)

    def __go_to_website(self, url):
        go_to_and_wait_loaded(self.chrome_driver, url)
    ...
    def generate(self, username: str, user_avatar_url: str, message: str, badge_text: str = None, datetime_text: str = None, image_url: str = None, output_filename: str = None):
        ...
        self.chrome_driver.execute_script(js_code)

        # Focus on textarea
        textarea = self.chrome_driver.find_elements(By.ID, TEXT_AREA_ID)[0]
        textarea.click()
        ...
        self.__go_to_website(WEB_URL + '/json')
        ...
        active_element = self.chrome_driver.switch_to.active_element
        active_element.click()
        actions.send_keys(Keys.DELETE).perform()
        active_element.send_keys(Keys.CONTROL, 'v')
        # Press the save button
        save_button = self.chrome_driver.find_element(By.XPATH, "//button[contains(text(), 'Save')]")
        save_button.click()
        # TODO: This could be a 'wait-for-element-change' waiting,
        # by now it is manually chosen
        sleep(2)
        message_element = self.chrome_driver.find_element(By.XPATH, '//div[@class="discord-message"]')

        # Hide bot badge
        if not badge_text:
            self.chrome_driver.execute_script('document.getElementsByClassName("discord-application-tag")[0].style.display = "none";')
        else:
            self.chrome_driver.execute_script('document.getElementsByClassName("discord-application-tag")[0].innerHTML = "' + badge_text + '";')
        ...
        self.chrome_driver.execute_script('document.getElementsByClassName("discord-message-timestamp pl-1")[0].innerHTML = "' + datetime_text + '";')
        
        # Save the screenshot with the requested name
        if not output_filename:
            output_filename = create_tmp_filename('tmp_discord_image.pn')
