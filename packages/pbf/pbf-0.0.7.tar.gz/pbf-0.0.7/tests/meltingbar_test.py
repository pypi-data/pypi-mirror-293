...
class MeltingBarTest(unittest.TestCase):
    def test(self):
        ...
        tsetup.ambientTemperature = Ts
        tsetup.setMaterials( { "powder" : material, "structure" : material } )
        tsetup.degree = 3
        tsetup.addDirichletBC(pbf.temperatureBC(0, Tl))
        tsetup.addDirichletBC(pbf.temperatureBC(1, solution))
        tsetup.addPostprocessor(pbf.thermalVtuOutput("outputs/pbftests/meltingbar", functions=[(solution, "Analytical")]))

        # Solution
        tstate0 = pbf.makeThermalState(tsetup, grid, powderHeight=10.0*units.mm)
        tstate1 = pbf.computeThermalProblem(tsetup, tstate0, deltaT=1.0*units.s, duration=100.0*units.s)
