...
for i in range(nLayers):
    # initialize moving heat source on the ith layer
    laserTrack = [pbf.LaserPosition(xyz=[x0, 0.0, layerThickness * (i + 1)], time=0.0, power=laserPower),
                  pbf.LaserPosition(xyz=[x1, 0.0, layerThickness * (i + 1)], time=singleTrackTime * 1, power=laserPower),
                  pbf.LaserPosition(xyz=[x1, hatching, layerThickness * (i + 1)], time=singleTrackTime * 1 + dwellTime, power=0),
                  pbf.LaserPosition(xyz=[x1, hatching, layerThickness * (i + 1)], time=singleTrackTime * 1 + dwellTime,
                                    power=laserPower),
                  pbf.LaserPosition(xyz=[x0, hatching, layerThickness * (i + 1)], time=singleTrackTime * 2 + dwellTime,
                                    power=laserPower)]
    heatSource = pbf.volumeSource(laserTrack, laserBeam, depthSigma=0.045 * units.mm)
    tsetup.addSource(heatSource)

    tstate1, mstate1 = pbf.computeThermomechanicalProblem(tsetup, msetup, tstate0, mstate0, timestep, layerTime)
    # update states for the next layer
    tstate0 = tstate1 #updateThermalState(tstate1)
    mstate0 = mstate1