...
inline ElementProcessor<3> postprocessPlasticity( const MechanicalHistory<3>& historyAccessor )
{
    auto evaluate = [&historyAccessor]( auto& shapes, auto&, auto&, auto target )
    {
        ...
        target[4] = S13;
        target[5] = S23;
        target[6] = std::sqrt( 0.5 * ( D1 + D2 + D3 ) + 3.0 * S );
        target[7] = history.effectivePlasticStrain;

    };

    auto outputData = []( const AbsBasis<3>& ) -> Output
    {
        return { .name = "StressAndJ2AndPlasticStrain",
                                   ...
                                   size_t vtuinterval )
{
                            ...
                            const MechanicalState<D>& state )
    {
        if( state.index % vtuinterval == 0 )
        {
            ...
            //};
            //                    
            //auto materialMeshProvider = cellmesh::createGrid( array::makeSizes<D>( 1 ), PostprocessTopologies::Volumes );
