...
from ._internal.ffi import enums, api
...
class RobotModel(UnmanagedObject):
  ...
  def __init__(self, **kwargs):
    ...
    for frame_type in enums.FrameType.values:
  ...
  def __repr__(self):
    ...
    for i, elem_metadata in enumerate(metadata):
      ...
      if elem_metadata.is_dof and type(elem_metadata) != ActuatorMetaData:
        ret += f'    {i:<4}  {str(elem_metadata):<15}  {"true":<6}  {0.0:>9.4f}\n'
      else:
        ret += f'    {i:<4}  {str(elem_metadata):<15}  {"false":<6}  {masses[mass_idx]:>9.4f}\n'
        mass_idx += 1
  ...
  def base_frame(self) -> 'npt.NDArray[np.float64]':
    ...
    code = api.hebiRobotModelGetBaseFrame(self, frame, _row_major.value)
  ...
  def __update_cached_frame_counts(self):
    ...
    self.__update_cached_masses(self._frame_count_map[enums.FrameTypeCenterOfMass])
  ...
  def add_end_effector(self, end_effector_type: str):
    ...
    body = api.hebiRobotModelElementCreateEndEffector(end_effector_enum.value, None, None, 0, None, _row_major.value)
    return self.__try_add(body)
  ...
  def get_forward_kinematics(self,
    ...
    api.hebiRobotModelGetForwardKinematics(self, frame_type_enum.value, to_double_ptr(positions), frames, _row_major.value)
  ...
  def get_forward_kinematics_mat(self, frame_type: str, positions: 'VectorType', output: 'npt.NDArray[np.float64] | None' = None):
    """Generates the forward kinematics for the given robot model.
  ...
  def get_end_effector(self, positions: 'VectorType', output: 'npt.NDArray[np.float64] | None' = None):
    ...
    api.hebiRobotModelGetForwardKinematics(self,
                                           enums.FrameTypeEndEffector.value,
                                           positions.ctypes.data_as(c_double_p),
                                           output.ctypes.data_as(c_double_p),
                                           _row_major.value)
    return output
  ...
  def __get_jacobians(self, frame_type, positions, rows: int, cols: int, order=_row_major, out: 'npt.NDArray[np.float64] | None' = None):
  ...
  def get_jacobians_mat(self, frame_type: str, positions: 'VectorType', output: 'npt.NDArray[np.float64] | None' = None):
    """Generates the Jacobian for each frame in the given kinematic tree.
  ...
  def get_jacobian_end_effector(self, positions, output=None):
    ...
    self.__get_jacobians(enums.FrameTypeEndEffector, to_double_ptr(positions), rows, cols, out=output)
  ...
  def get_grav_comp_efforts(self,
    ...
    if jacobians is None:
      jacobians = self.get_jacobians_mat('CoM', positions)

    # jacobians (frames, 6, dof)
    # wrench (6, frames)
    # efforts (dof)
    np.einsum('ijk,ji->k', jacobians, wrench, out=comp_torque)
  ...
  def get_dynamic_comp_efforts(self,
    ...
    if output is None:
      efforts = np.zeros(self.dof_count, dtype=np.float64)
    else:
      efforts = output
    ...
    # (#frames x 3)
    cmd_frames_last = self.get_forward_kinematics_mat('CoM', cmd_positions_last)[:, :3, 3]
    cmd_frames_now = self.get_forward_kinematics_mat('CoM', cmd_positions)[:, :3, 3]
    cmd_frames_next = self.get_forward_kinematics_mat('CoM', cmd_positions_next)[:, :3, 3]

    # Build wrench vector and calculate compensatory torques
    if jacobians is None:
      jacobians = self.get_jacobians_mat('CoM', fbk_positions)

    wrench: 'npt.NDArray[np.float64]' = np.zeros((len(masses), 6), dtype=np.float64)

    # Calculate XYZ accelerations of the CoM
    # (frames x 3)
    #accel = (cmd_frames_last-cmd_frames_now)/(dt**2)+(cmd_frames_next-cmd_frames_now)/(dt**2)
    accel = (cmd_frames_last+cmd_frames_next-2*cmd_frames_now)*dt_s_inv

    # Set translational part of wrench vector (rotational stays zero)
    np.multiply(accel, masses[:, None], out=wrench[:, :3])

    # jacobians (frames, 6, dof)
    # wrench (frames, 6)
    # efforts (dof)

    np.einsum('ijk,ij->k', jacobians, wrench, optimize=True, out=efforts)
...
class PositionObjective(_ObjectiveBase):
  def __init__(self, frame_type: str, *, xyz: 'npt.NDArray[np.float64]', idx: int = 0, weight: float = 1.0):
    self._x: float = xyz[0]
    self._y: float = xyz[1]
    self._z: float = xyz[2]
    self._frame_type = _kinematics.parse_frame_type(frame_type)
    self._idx = idx
...
def endeffector_position_objective(xyz: 'VectorType', weight: float = 1.0):
  ...
  return PositionObjective('endeffector', xyz=xyz, weight=weight)


class SO3Objective(_ObjectiveBase):
  def __init__(self, frame_type: str, *, rotation: 'npt.NDArray[np.float64]', idx: int = 0, weight: float = 1.0):
    self._frame_type = _kinematics.parse_frame_type(frame_type)
...
def endeffector_so3_objective(rotation: 'VectorType', weight: float = 1.0):
  ...
  return SO3Objective('endeffector', rotation=rotation, weight=weight)


class TipAxisObjective(_ObjectiveBase):
  def __init__(self, frame_type: str, *, axis: 'npt.NDArray[np.float64]', idx: int = 0, weight: float = 1.0):
    self._frame_type = _kinematics.parse_frame_type(frame_type)
...
def endeffector_tipaxis_objective(axis: 'VectorType', weight: float = 1.0):
  ...
  return TipAxisObjective('endeffector', axis=axis, weight=weight)
