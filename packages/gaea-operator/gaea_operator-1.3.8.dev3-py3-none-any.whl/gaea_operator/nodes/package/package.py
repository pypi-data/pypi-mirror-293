...
def package(args):
    ...
    # 4. 下载 ensemble 模型
    bcelogger.info(f"Dumping model {current_ensemble_name} to {args.output_model_uri}")
    ...
    new_sub_models.pop(model_names["template_transform_model_name"].local_name)
    ...
    for name in sub_extra_models:
        ...
        if model_name.local_name in new_sub_models or \
                model_name.local_name == model_names["transform_model_name"].local_name:
            new_sub_models[model_name.local_name] = str(artifact_name.version)
        if model_name.local_name in new_extra_models:
    ...
    # 9. 上传 ensemble 模型
    ensemble_model_uri = os.path.join(args.output_model_uri,
