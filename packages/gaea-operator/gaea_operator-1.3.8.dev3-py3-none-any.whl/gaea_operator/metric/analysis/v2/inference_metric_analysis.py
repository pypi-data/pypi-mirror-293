...
class InferenceMetricAnalysis(object):
    ...
    def set_labels(self, labels: List):
        ...
        for label in labels:
            ...
            if "parent_id" not in label or label["parent_id"] is None:
                ...
                label["parentID"] = int(label["parent_id"])
        ...
        for item in references:
            ...
            for anno in item["annotations"]:
                for idx in range(len(anno["labels"])):
                    ...
                    if label["id"] not in self.label_id2index:
                        continue
                    if "labels" in label and len(label["labels"]) > 0:
                        for inner_label in label["labels"]:
                            if isinstance(inner_label["id"], str):
                                inner_label["id"] = int(inner_label["id"])
                            # 如果多属性标注属性id不在label,则跳过
                            if inner_label["id"] not in self.label_inner_id2index[label["id"]]:
                                continue

                            array_item[self.label_inner_id2index[label["id"]][inner_label["id"]]] = 1
                            array_item[self.label_id2index[label["id"]]] = inner_label["id"]
                        continue
