...
class SparkDsTFDs():
    ...
    def get_dicts(self):
        ...
        for name in self.input_names:
            ...
            if hasattr(self.model.get_layer(name),"_input_tensor"): #keras 3
                ...
                shape_dict[name] = self.model.get_layer(name).shape[1:]
    ...
    def convert(self,spark_df,partition_col,partition_values,batch_size,num_parallel_calls=None):

        def _get_tensors_from_partition(partition_id):
            pdf = spark_df.filter(F.col(partition_col) == F.lit(partition_id.numpy().item())).toPandas()
        ...
        dataset = (
            tf.data.Dataset.from_tensor_slices(partition_values)
            .interleave(
...
class KerasOnSparkPredict:
    ...
    def predict(self, spark_df, model, use_spark_files=True,local_path = home_path, maxRecordsPerBatch=100):
