...
class ResultFileManager(object):
...
class format_residue_single(ResultFileManager):
    def __init__(self,output_path="."):
        ...
        if not os.path.isdir(output_path):
            print("The directory output_path does not exist.")
    ...
    def append_write(self,ac,seq,pred_prob,pred_class):
        with open(self.path+ac+".daruma","w") as f:
            out = "\n".join(["{} {} {:.4f} {}".format(i,res,p,c) for i,res,p,c in zip(range(1,len(seq)+1),seq,pred_prob,pred_class)])
            f.write(">{}\n{}\n".format(ac,out))


class format_residue_multi(ResultFileManager):
    def __init__(self,output_path):
        super(format_residue_multi,self).__init__(output_path)
...
class format_4lines(ResultFileManager):
    def __init__(self,output_path):
        super(format_4lines,self).__init__(output_path)
    ...
    def append_write(self,ac,seq,pred_prob,pred_class):
        with open(self.path,"a") as f:
           f.write('>{}\n{}\n{}\n{}\n'.format(ac, ",".join(seq), ",".join(map(lambda x:"{:.4f}".format(x),pred_prob)), ",".join(map(str,pred_class))))
