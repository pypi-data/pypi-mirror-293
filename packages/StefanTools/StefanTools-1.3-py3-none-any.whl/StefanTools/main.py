...
limit=1.7976931348623157*(10**308) #if that number is measured in seconds, it'd be 5.6965457920194048343346769082566e300 years
    ...
    class int():
        ...
        def no(msg="")->list:
            if msg.endswith(": ") or msg=="":
                ...
                msg+=": "
            try:
                a=inputimeout(msg,timeout=limit)
                a=int(a)
                return [1,a]
            except ValueError:
                return [0,a]
        ...
        def yes(msg="")->int:
            ...
            while True:
                try:
                    ...
                    a=inputimeout(timeout=limit).strip()
                    a=int(a)
                    return a
                except ValueError:
...
class format:
    class number:
        ...
        @staticmethod
        def text(number:int|str,ordinal:bool=False):
            ...
            number=int(number)
            p=inflect.engine()
            if ordinal: return p.ordinal(number)
            else:       return p.number_to_words(number)
...
def fraction(first_number:int|float,second_number:int|float,multiply_by:int|float=1.0)->float:
    ...
    if isinstance(first_number,str) and first_number.isdigit():
        console.print(f"The first number {first_number} is a string. It has been turned into an integer.")
        first_number=int(first_number)
    if isinstance(second_number,str) and second_number.isdigit():
        console.print(f"The second number {second_number} is a string. It has been turned into an integer.")
...
delay_between_lines:float=0.0,auto_color:dict={})->None:
    ...
    Args:
        text (optional): The text to print. If it's a list, it will be treated as if you put a comma between elements in a normal print() statement. If it's neither a list or a string, it will be turned to a string. Defaults to "".
