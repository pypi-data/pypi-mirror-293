...
from chalk.parsed.utils import validate_namespaced_features
...
def convert_type_to_gql(
    ...
    if isinstance(t, NamedQuery):
        if t.output is None and t.input is None:
            raise ValueError("Named query must have either an input or an output.")

        input = validate_namespaced_features(t.input)
        output = validate_namespaced_features(t.output)
        ...
        return UpsertNamedQueryGQL(
            ...
            input=list(input) if input is not None else None,
    ...
    if isinstance(t, Feature):
        ...
        return UpsertFeatureGQL(
            ...
            if t.window_materialization_parsed is None
            else UpsertWindowMaterializationGQL(
                namespace=t.window_materialization_parsed.namespace,
                groupBy=[
                    UpsertFeatureReferenceGQL(
                        underlying=_get_feature_id(f, root_fqn=False),
                        path=[_get_path_component(p) for p in f.path or []],
                    )
                    for f in t.window_materialization_parsed.group_by
                ],
                bucketDuration=float(t.window_materialization_parsed.bucket_duration_seconds),
                aggregation=t.window_materialization_parsed.aggregation,
                aggregateOn=UpsertFeatureReferenceGQL(
                    underlying=_get_feature_id(t.window_materialization_parsed.aggregate_on, root_fqn=False),
                    path=[_get_path_component(p) for p in t.path or []],
                )
                if t.window_materialization_parsed.aggregate_on is not None
                else None,
                dtype=serialize_dtype(t.window_materialization_parsed.pyarrow_dtype),
            ),
