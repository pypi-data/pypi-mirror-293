...
from lanstudio.app.models.attachment_file import AttachmentFile
from lanstudio.app.models.comment import Comment
from lanstudio.app.models.department import Department
from lanstudio.app.models.notification import Notification
from lanstudio.app.models.project import Project
from lanstudio.app.models.task import Task

from lanstudio.app.services import (
    assets_service,
    base_service,
    breakdown_service,
    entities_service,
...
from lanstudio.app.utils import cache, date_helpers, events, fs, fields
from lanstudio.app.stores import file_store
...
def generate_auto_mandays_count(new_data, task, count):
    ...
    if count == "stop":
        ...
        if wipdate is None:
            # Fallback to default if wipdate is missing
            wipdate = dt.utcnow()
        try:
            if isinstance(wipdate, str):
            ...
            date1 = dt.strptime("0000-00-00T00:00:00", "%Y-%m-%dT%H:%M:%S")
        date2 = current_date
        # Calculate time difference in days
        time_difference = (date2.date() - date1.date()).days
        print(time_difference, "*******345-AUGUST")
        if mandays_count is None:
            mandays_count = 0
        # Update mandays_count with the calculated days
        new_data['mandays_count'] = int(time_difference + mandays_count)
    return new_data
