    ...
    async def run(self) -> bool:
        ...
        while self.running:
            try:
                ...
                field = str(why)
                self._stats(field)
                # TODO: strategy, add same task to be executed later on
                # TODO: using a scheduled queue, not just a simple queue
                self.input_queue.push(task, monotonic() + 5)
                await self.idle()
            finally:
