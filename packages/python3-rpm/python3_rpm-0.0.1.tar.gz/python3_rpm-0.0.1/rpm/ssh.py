...
class SSHClient(object):
    ...
    def _exec(self, cmd,promt_response,results):
        ...
        try:
            if promt_response:
                ...
                results.append(ExecResult(exit_status,std_output,std_err))
                return ExecResult(exit_status,std_output,std_err)
        except Exception as e:
            results.append(ExecResult(255, "", str(e)))
            return ExecResult(255, "", str(e))

    def exec(self, cmd,promt_response=[],timeout=1200):
        try:
            if not self.is_active():
                raise RuntimeError("ssh transport is not active.")
            results=[]
            t=threading.Thread(target=self._exec,args=(cmd,promt_response,results))
            t.start()
            t.join(timeout=timeout)
            if t.is_alive():
                log.error(f"{self.__ip}:{self.__port}: {timeout} has passed but thread is still alive.kill and return None.")
                raise RuntimeError("{timeout} has passed but thread is still alive.kill and return None.")
            else:
                return results[0]
        except RuntimeError() as e:
            log.error(f"{self.__ip}:{self.__port} | ssh transport is not active:Error.")
            return ExecResult(255,"",str(e))
        except TimeoutError as e1:
            log.error(f"{self.__ip}:{self.__port} | timeout to run cmd:{cmd}.",exc_info=True)
            return ExecResult(255,"",str(e1))
        except Exception as e2:
            log.error(f"when run cmd: {cmd}, meets exception.",exc_info=True)
            return ExecResult(255,"",str(e2))

    @func_set_timeout(1800)
    def _scp_to_remote(self, local_path, remote_path):
        log.info(f" {self.__ip}:{self.__port} | Begin to copy file from local {local_path} to remote host {remote_path}.")
        self.__sftp.put(local_path,remote_path)
        rs=self.exec(f"ls {remote_path}")
        if rs.exit_status_code == 0:
            log.info(f" {self.__ip}:{self.__port} | Success to copy file from local {local_path} to remote host{remote_path}: OK.")
            return True
        else:
            log.error(
    ...
    def scp_to_remote(self, local_path, remote_path):
        try:
            ...
            log.error(f"{self.__ip}:{self.__port} | failed to run copy file from local {local_path} to remote {remote_path} for ssh transport is not active..", exc_info=True)
            return False
        except TimeoutError:
            log.error(f"{self.__ip}:{self.__port} | timeout to run copy file from local {local_path} to remote {remote_path}.", exc_info=True)
            return False
        except Exception:
    ...
    def _scp_file_to_local(self, remote_path, local_path):
        log.info(f" {self.__ip}:{self.__port} | Begin to copy file from remote {remote_path} to local host {local_path}.")
        ...
        if rs.returncode == 0:
            log.info(f" {self.__ip}:{self.__port} | Success to copy file from remote {remote_path} to local host{local_path}:OK.")
            return True
        else:
            log.error(
