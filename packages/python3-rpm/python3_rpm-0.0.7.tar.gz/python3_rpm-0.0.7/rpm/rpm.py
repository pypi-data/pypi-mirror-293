...
class RpmService():
    ...
    def get_param_list(self,cmd):
        ...
        params = []
        output = ssh.exec(
                cmd + " --help 2>&1 |grep -Ei '^\s*-[a-zA-Z]'|awk -F ' ' '{print $1}'|awk -F ',' '{print $1} |awk -F '=' '{print $1}'' \n\n",
        ...
        if len(output) > 0:
            for line in output.split("\n"):
                if line.startswith("--"):
                    params.append(line.strip())
        output = ssh.exec(
            cmd + " --help 2>&1 |grep -E '^\s*-[a-zA-Z0-9],\s*--[a-zA-Z1-9\=_-]*\s'|awk -F ',' '{print $2}' |awk -F ' ' '{print $1} |awk -F '=' '{print $1}'' \n\n",
