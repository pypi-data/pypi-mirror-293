...
class DrawImages:
    ...
    def __init_glb_map(self, sub_ax=None):
        """
        初始化全球地图
        :return:
        """
        if sub_ax is None:
            sub_ax = self.ax
        sub_ax.coastlines(resolution="110m", zorder=2, color="black")
        # self.ax.add_feature(cfeature.LAKES, alpha=0.5, zorder=2)
        # self.ax.add_feature(cfeature.RIVERS, zorder=2)
        # self.ax.add_feature(cfeature.LAND, zorder=2, edgecolor="black")
        # self.ax.add_feature(cfeature.OCEAN, zorder=2, edgecolor="black")
        # self.ax.add_feature(cfeature.BORDERS, linewidth=0.5, zorder=2, edgecolor="black")

    def init_map(self, add_chn_border=True, add_province=True, add_city=False, add_cnty=False, single_province=None,
                 line_color="black"):
        """
        初始化地图
        :param add_chn_border:
        :param line_color:
        :param add_province:
        :param add_city:
        :param add_cnty:
        ...
        self.__init_chn_map(add_chn_border=add_chn_border, add_province=add_province, add_city=add_city,
                            add_cnty=add_cnty, line_color=line_color,
    ...
    def set_title(self, y, title, family="helvetica", size=22):
        ...
        self.ax.set_title(title, y=y, fontdict=font_family)
    ...
    def set_review_number(self):
        self.ax.text(0.75, -0.08, f"审图号：{self.review_number}", zorder=10, transform=self.ax.transAxes,
