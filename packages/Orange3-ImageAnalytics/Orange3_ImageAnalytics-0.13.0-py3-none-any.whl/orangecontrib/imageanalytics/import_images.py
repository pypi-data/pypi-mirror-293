    ...
    if image_meta:
        if categories and len(categories) > 1:
            cat_var = Orange.data.DiscreteVariable.make(
                _tr.m[17, "category"], values=list(sorted(categories.values()))
            )
        else:
            cat_var = None
        # Image name (file basename without the extension)
        imagename_var = Orange.data.StringVariable.make(_tr.m[18, "image name"])
        # Full fs path
        ...
        size_var = Orange.data.ContinuousVariable.make(_tr.m[19, "size"])
        size_var.number_of_decimals = 0
        width_var = Orange.data.ContinuousVariable.make(_tr.m[20, "width"])
        width_var.number_of_decimals = 0
        height_var = Orange.data.ContinuousVariable.make(_tr.m[21, "height"])
