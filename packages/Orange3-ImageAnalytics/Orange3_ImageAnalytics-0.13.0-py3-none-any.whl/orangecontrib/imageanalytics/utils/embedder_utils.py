...
class ImageLoader:
    ...
    def session(self):
        ...
        try:
            ...
            log.info(
                (f"Cache file creation/opening failed with: '{str(ex)}'. " + "Using requests.Session instead of cached session.")
            )
    ...
    def _load_image_from_url_or_local_path(self, file_path):
        ...
        if urlparts.scheme in ("http", "https"):
            try:
                ...
                log.warning(_tr.m[22, "Image skipped"], exc_info=True)
                return None
        elif urlparts.scheme in ("ftp", "data"):
            try:
                file = urlopen(file_path)
            except (URLError,) + ftplib.all_errors:
                log.warning(_tr.m[23, "Image skipped"], exc_info=True)
                return None
        else:
            file = file_path
        ...
        try:
            ...
            log.warning(_tr.m[24, "Image skipped"], exc_info=True)
            return None
