...
MODELS = {
    "inception-v3": {
        "name": "Inception v3",
        "description": _tr.m[2, "Google's Inception v3 model trained on ImageNet."],
    ...
    "painters": {
        "name": "Painters",
        "description": _tr.m[3, "A model trained to predict painters from artwork\nimages."],
    ...
    "deeploc": {
        "name": "DeepLoc",
        "description": _tr.m[4, "A model trained to analyze yeast cell images."],
    ...
    "vgg16": {
        "name": "VGG-16",
        "description": _tr.m[5, "16-layer image recognition model trained on\nImageNet."],
    ...
    "vgg19": {
        "name": "VGG-19",
        "description": _tr.m[6, "19-layer image recognition model trained on\nImageNet."],
        "target_image_size": (224, 224),
        "layers": ["penultimate"],
        "order": 3,
        "batch_size": 500,
    ...
    "openface": {
        "name": "openface",
        "description": (_tr.m[7, "Face recognition model trained on FaceScrub and\n"] + _tr.m[8, "CASIA-WebFace datasets."]),
        "target_image_size": (256, 256),
        "layers": ["penultimate"],
        "order": 6,
        "batch_size": 500,
    ...
    "squeezenet": {
        "name": "SqueezeNet",
        "description": (_tr.m[9, "Deep model for image recognition that achieves \n"] + (_tr.m[10, "AlexNet-level accuracy on ImageNet with \n"] + _tr.m[11, "50x fewer parameters."])),
...
class ImageEmbedder:
    ...
    def _get_model_settings_confidently(self) -> Dict[str, Any]:
        ...
        if self.model not in MODELS.keys():
            model_error = _tr.m[12, "'{:s}' is not a valid model, should be one of: {:s}"]
    ...
    def __call__(
        ...
        if isinstance(data, Table):
            assert col is not None, _tr.m[13, "Please provide a column for image path"]
    ...
    def prepare_output_data(
        ...
        if np.any(skipped_images_bool):
            ...
            skipped_images.name = _tr.m[14, "Skipped images"]
        ...
        if np.any(embedded_images_bool):
            ...
            embedded_images.name = _tr.m[15, "Embedded images"]
        else:
