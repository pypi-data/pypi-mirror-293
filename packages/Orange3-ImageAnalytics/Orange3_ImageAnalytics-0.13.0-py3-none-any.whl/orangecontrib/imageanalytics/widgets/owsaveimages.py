...
class OWSaveImages(OWWidget, ConcurrentWidgetMixin):
    ...
    class Inputs:
        data = Input(_tr.m[126, "Data"], Table)
    ...
    def __init__(self):
        ...
        self.cb_image_attr = gui.comboBox(
            ...
            label=_tr.m[129, 'Image attribute'],
        ...
        gui.checkBox(
            ...
            master=self,
            value="use_scale",
            label=_tr.m[130, "Scale images to "],
            callback=self.setting_changed
        )
        ...
        # buttons
        self.bt_save = gui.button(self.buttonsArea, self, _tr.m[133, "Save"], callback=self.save_file)
        gui.button(self.buttonsArea, self, _tr.m[134, "Save as ..."], callback=self.save_file_as)
    ...
    def save_file_as(self):
        ...
        self.bt_save.setText(_tr.e(_tr.c(138, "Save as {os.path.basename(dirname)}")))
        self._update_messages()
        self.reset_queue()
        self.save_file()
    ...
    def get_save_filename(self):
        ...
        while True:
            dlg = QFileDialog(
                None, _tr.m[139, "Select directory to save"], filename)
            ...
            if not os.path.exists(filename) or QMessageBox.question(
                    self, _tr.m[140, "Overwrite file?"],
                    (_tr.e(_tr.c(141, "Folder {os.path.split(filename)[1]} already exists.\n")) + _tr.m[142, "Overwrite?"])) == QMessageBox.Yes:
                return filename
