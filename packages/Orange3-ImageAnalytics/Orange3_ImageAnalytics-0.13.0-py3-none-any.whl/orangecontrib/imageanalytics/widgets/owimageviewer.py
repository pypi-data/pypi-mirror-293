...
class OWImageViewer(OWWidget):
    ...
    class Inputs:
        data = Input(_tr.m[106, "Data"], Orange.data.Table)

    class Outputs:
        selected_data = Output(_tr.m[107, "Selected Images"], Orange.data.Table)
        data = Output(_tr.m[108, "Data"], Orange.data.Table)
    ...
    UserAdviceMessages = [
        Message(
            (_tr.m[109, "Pressing the 'Space' key while the thumbnail view has focus and "] + _tr.m[110, "a selected item will open a window with a full image"]),
