...
class OffsetCls:
	...
	def set(self, areg_fe_ota_offset: int, channel=repcap.Channel.Default, rxIndex=repcap.RxIndex.Default) -> None:
		...
		Snippet: driver.source.areGenerator.frontend.cfe.rx.ota.offset.set(areg_fe_ota_offset = 1, channel = repcap.Channel.Default, rxIndex = repcap.RxIndex.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:FRONtend:CFE{channel_cmd_val}:RX{rxIndex_cmd_val}:OTA:OFFSet {param}')

	def get(self, channel=repcap.Channel.Default, rxIndex=repcap.RxIndex.Default) -> int:
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:CFE<CH>:RX<ST>:OTA:OFFSet \n
		Snippet: value: int = driver.source.areGenerator.frontend.cfe.rx.ota.offset.get(channel = repcap.Channel.Default, rxIndex = repcap.RxIndex.Default) \n
		Specifies the length of the gap between frontend and target. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cfe')
			:param rxIndex: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Rx')
		...
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		rxIndex_cmd_val = self._cmd_group.get_repcap_cmd_value(rxIndex, repcap.RxIndex)
		response = self._core.io.query_str(f'SOURce<HwInstance>:AREGenerator:FRONtend:CFE{channel_cmd_val}:RX{rxIndex_cmd_val}:OTA:OFFSet?')
