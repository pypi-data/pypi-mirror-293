...
class AtsCls:
	...
	def set(self, areg_fe_ats: float, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.areGenerator.frontend.fe.ats.set(areg_fe_ats = 1.0, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:FRONtend:FE{channel_cmd_val}:ATS {param}')

	def get(self, channel=repcap.Channel.Default) -> float:
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:FE<CH>:ATS \n
		Snippet: value: float = driver.source.areGenerator.frontend.fe.ats.get(channel = repcap.Channel.Default) \n
		Sets the angle between frontend and radar sensor. Reference point for the definition of the angle is the center of the
		frontend. The angle describes the deviation of the position of the frontend from the 0° center position of the field of
		view of the radar.
			INTRO_CMD_HELP: We recommend that you zero in regular intervals (at least once a day) , if: \n
		...
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:AREGenerator:FRONtend:FE{channel_cmd_val}:ATS?')
