...
class ConnectCls:
	...
	def set(self, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.areGenerator.frontend.fe.connect.set(channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:FRONtend:FE{channel_cmd_val}:CONNect')

	def set_with_opc(self, channel=repcap.Channel.Default, opc_timeout_ms: int = -1) -> None:
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:FE<CH>:CONNect \n
		Snippet: driver.source.areGenerator.frontend.fe.connect.set_with_opc(channel = repcap.Channel.Default) \n
		Triggers a connection procedure to connect the R&S AREG800A with the external frontend in the network. \n
		...
		self._core.io.write_with_opc(f'SOURce<HwInstance>:AREGenerator:FRONtend:FE{channel_cmd_val}:CONNect', opc_timeout_ms)
