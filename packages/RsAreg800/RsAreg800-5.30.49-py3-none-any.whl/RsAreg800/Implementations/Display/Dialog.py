...
class DialogCls:
	...
	def close(self, dialog_id: str) -> None:
		...
		Closes the specified dialog. \n
			:param dialog_id: To find out the dialog identifier, use the query method RsAreg800.Display.Dialog.id. The DialogName part of the query result is sufficient.
		"""
	...
	def get_id(self) -> str:
		...
		Returns the dialog identifiers of the open dialogs in a string separated by blanks. \n
			:return: dialog_id_list: Dialog identifiers are string without blanks. Blanks are represented as $$. Dialog identifiers DialogID are composed of two main parts: DialogName[OptionalParts]
				- DialogName: Meaningful information, mandatory input parameter for the commands::DISPlay:DIALog:OPEN:DISPlay:DIALog:CLOSe
	...
	def open(self, dialog_id: str) -> None:
		...
		Opens the specified dialog. \n
			:param dialog_id: To find out the dialog identifier, use the query method RsAreg800.Display.Dialog.id. The DialogName part of the query result is mandatory.
		"""
		param = Conversions.value_to_quoted_str(dialog_id)
