...
class NsRatioCls:
	...
	def set(self, ns_ratio: float, channel=repcap.Channel.Default) -> None:
		...
		Sets an upper limit for the relative noise content in fixed noise filter mode (method RsAreg800.Sense.Power.FilterPy.
		TypePy.set) . This value determines the proportion of intrinsic noise in the measurement results. \n
			:param ns_ratio: No help available
	...
	def get(self, channel=repcap.Channel.Default) -> float:
		...
		TypePy.set) . This value determines the proportion of intrinsic noise in the measurement results. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sense')
			:return: ns_ratio: No help available"""
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
