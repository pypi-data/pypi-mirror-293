...
class BgInfoCls:
	...
	def get(self, board: str = None) -> str:
		...
		Queries information on the modules available in the instrument, using the variant and revision state. \n
			:param board: Module name, as queried with the command method RsAreg800.Diagnostic.BgInfo.catalog. To retrieve a complete list of all modules, omit the parameter. The length of the list is variable and depends on the instrument equipment configuration.
			:return: bg_info: List of comma-separated entries, one entry per module. Each entry for one module consists of four parts that are separated by space characters."""
	...
	def get_catalog(self) -> List[str]:
		...
		Queries the names of the assemblies available in the instrument. \n
			:return: catalog: List of all assemblies; the values are separated by commas The length of the list is variable and depends on the instrument equipment configuration.
		"""
