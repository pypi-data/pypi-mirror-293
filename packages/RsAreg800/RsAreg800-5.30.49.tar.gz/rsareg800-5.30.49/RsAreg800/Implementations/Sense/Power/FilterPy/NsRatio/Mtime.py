...
class MtimeCls:
	...
	def set(self, mtime: float, channel=repcap.Channel.Default) -> None:
		...
		the filter. The filter type is set with command method RsAreg800.Sense.Power.FilterPy.TypePy.set. \n
			:param mtime: No help available
	...
	def get(self, channel=repcap.Channel.Default) -> float:
		...
		the filter. The filter type is set with command method RsAreg800.Sense.Power.FilterPy.TypePy.set. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sense')
			:return: mtime: No help available"""
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
