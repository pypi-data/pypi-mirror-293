...
class RtsCls:
	...
	def set(self, areg_fe_rts: float, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.areGenerator.frontend.cfe.rts.set(areg_fe_rts = 1.0, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:FRONtend:CFE{channel_cmd_val}:RTS {param}')

	def get(self, channel=repcap.Channel.Default) -> float:
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:CFE<CH>:RTS \n
		Snippet: value: float = driver.source.areGenerator.frontend.cfe.rts.get(channel = repcap.Channel.Default) \n
		Sets the rotation angle between frontend and sensor. Reference point for the definition of the angle is the center of the
		frontend. The rotation describes the deviation of the position of the frontend from a 90° angle to the direct line of
		sight of the sensor. For TRX-type or custom frontends this parameter has currently no impact, since it is a single sensor
		and no sensor array. \n
		...
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:AREGenerator:FRONtend:CFE{channel_cmd_val}:RTS?')
