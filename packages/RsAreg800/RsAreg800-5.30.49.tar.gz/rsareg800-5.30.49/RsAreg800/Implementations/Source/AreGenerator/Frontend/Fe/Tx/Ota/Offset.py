...
class OffsetCls:
	...
	def set(self, areg_fe_ota_offset: int, channel=repcap.Channel.Default, txIndexNull=repcap.TxIndexNull.Default) -> None:
		...
		Snippet: driver.source.areGenerator.frontend.fe.tx.ota.offset.set(areg_fe_ota_offset = 1, channel = repcap.Channel.Default, txIndexNull = repcap.TxIndexNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:FRONtend:FE{channel_cmd_val}:TX{txIndexNull_cmd_val}:OTA:OFFSet {param}')

	def get(self, channel=repcap.Channel.Default, txIndexNull=repcap.TxIndexNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:FE<CH>:TX<ST0>:OTA:OFFSet \n
		Snippet: value: int = driver.source.areGenerator.frontend.fe.tx.ota.offset.get(channel = repcap.Channel.Default, txIndexNull = repcap.TxIndexNull.Default) \n
		Specifies the length of the gap between frontend and target. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Fe')
			:param txIndexNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Tx')
		...
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		txIndexNull_cmd_val = self._cmd_group.get_repcap_cmd_value(txIndexNull, repcap.TxIndexNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:AREGenerator:FRONtend:FE{channel_cmd_val}:TX{txIndexNull_cmd_val}:OTA:OFFSet?')
