...
class RmvCls:
	...
	def set(self, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.areGenerator.frontend.fe.rmv.set(channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:FRONtend:FE{channel_cmd_val}:RMV')

	def set_with_opc(self, channel=repcap.Channel.Default, opc_timeout_ms: int = -1) -> None:
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:FE<CH>:RMV \n
		Snippet: driver.source.areGenerator.frontend.fe.rmv.set_with_opc(channel = repcap.Channel.Default) \n
		Removes the configuration of the connected QAT-type, FE-type or custom frontend. \n
		...
		self._core.io.write_with_opc(f'SOURce<HwInstance>:AREGenerator:FRONtend:FE{channel_cmd_val}:RMV', opc_timeout_ms)
