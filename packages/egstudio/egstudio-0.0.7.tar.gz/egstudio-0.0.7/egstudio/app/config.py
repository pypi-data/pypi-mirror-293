...
from egstudio.app.utils import dbhelpers
from egstudio.app.utils.env import envtobool, env_with_semicolon_to_list
...
TMP_DIR = os.getenv("TMP_DIR", os.path.join(tempfile.gettempdir(), "egstudio"))
...
JOB_QUEUE_NOMAD_PLAYLIST_JOB = os.getenv(
    "JOB_QUEUE_NOMAD_PLAYLIST_JOB", "egstudio-playlist"
)
JOB_QUEUE_NOMAD_NORMALIZE_JOB = os.getenv("JOB_QUEUE_NOMAD_NORMALIZE_JOB", "")
JOB_QUEUE_NOMAD_HOST = os.getenv("JOB_QUEUE_NOMAD_HOST", "egstudio-nomad-01.egstudio")
...
LDAP_BASE_DN = os.getenv("LDAP_BASE_DN", "cn=Users,dc=egstudio,dc=local")
LDAP_GROUP = os.getenv("LDAP_GROUP", "")
LDAP_DOMAIN = os.getenv("LDAP_DOMAIN", "egstudio.local")
