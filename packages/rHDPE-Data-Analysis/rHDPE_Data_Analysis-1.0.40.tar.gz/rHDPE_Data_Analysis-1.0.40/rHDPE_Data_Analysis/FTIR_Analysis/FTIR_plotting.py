...
def plot_data( directory, output_directory, file_data, data, first_derivative_data, second_derivative_data, third_derivative_data, m_peaks, m_peaks_array, y_peaks, y_peaks_array, shiny, shiny_samples_to_plot, shiny_split, shiny_specimen, shiny_mean, savefig = False ):
        ...
        for i in samples_to_plot:
                ...
                for ind, j in enumerate( mask ):
                    ...
                    if (ind in specimen_mask) or all_specimens:
                        ...
                        if deriv0:
                            ...
                            shiny_de.append( np.array( data[0] )[wn_mask].tolist() )
                            shiny_de.append( np.array( data[1][j] )[wn_mask].tolist() )
            ...
            if mean:
                ...
                if deriv0:
                    ...
                    shiny_de.append( np.array( data[0] )[wn_mask].tolist() )
                    shiny_de.append( np.array( data[2][index] )[wn_mask].tolist() )
