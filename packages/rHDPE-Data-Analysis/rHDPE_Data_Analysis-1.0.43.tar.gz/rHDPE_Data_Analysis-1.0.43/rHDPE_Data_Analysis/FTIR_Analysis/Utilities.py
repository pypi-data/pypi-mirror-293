...
def pp_percentage_model( directory, output_directory, file_data, data ):
    ...
    sample_mask = [11, 14, 10, 4, 13, 21, 23, 18, 22, 20, 2, 3, 17, 16, 19, 1, 15, 12, 6, 5, 7, 9, 8, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 601, 602, 603, 604, 605, 701, 702, 703, 704, 705, 706, 500, 502, 504, 506, 508, 510, 512, 514, 516, 518]
    # sample_mask = samples_present
    ...
    range_mask = np.where( (np.array( data[0] ) >= 3050) & (np.array( data[0] ) < 3100) )

    for i in range( len( data[1] ) ):

        mini = np.array( data[1][i] )[range_mask].min()

        data[1][i] = ((np.array( data[1][i] ) - mini) / (np.array( data[1][i] ).max() - mini)).tolist()

    ######
    # Scaling data so that all spectra have a peak magnitude of 0.1 or less at 720.

    range_mask = np.where( (np.array( data[0] ) >= 712) & (np.array( data[0] ) < 726) )

    for i in range( len( data[1] ) ):

        value = np.array( data[1][i] )[range_mask].max()

        if value > 0.1:

            data[1][i] = np.array( data[1][i] ) * 0.1 / value
    ...
    df.to_csv( output_directory + "FTIR/Sandbox/PP_Percentage_Analysis/Features/PP_Predictions.csv" )
    ...
    range_mask_1 = np.where( (np.array( data[0] ) >= 3090) & (np.array( data[0] ) < 3110) )
    range_mask_2 = np.where( (np.array( data[0] ) >= 1374) & (np.array( data[0] ) < 1379) )
    range_mask_3 = np.where( (np.array( data[0] ) >= 717) & (np.array( data[0] ) < 722) )

    for i in range( len( data[1] ) ):

        value_1 = np.array( data[1][i] )[range_mask_1].min()
        value_2 = np.array( data[1][i] )[range_mask_2].max()
        value_3 = np.array( data[1][i] )[range_mask_3].max()
...
def crystallinity_determination( directory, output_directory, file_data, data ):
    ...
    range_mask_1 = np.where( (np.array( data[0] ) >= 755) & (np.array( data[0] ) < 765) )
    range_mask_2 = np.where( (np.array( data[0] ) >= 715) & (np.array( data[0] ) < 725) )
    range_mask_3 = np.where( (np.array( data[0] ) >= 725) & (np.array( data[0] ) < 735) )

    for i in range( len( data[1] ) ):

        i_a = np.array( data[1][i] )[range_mask_3].max() - np.array( data[1][i] )[range_mask_1].min()
        i_b = np.array( data[1][i] )[range_mask_2].max() - np.array( data[1][i] )[range_mask_1].min()
    ...
    for i in range( len( data[1] ) ):
        ...
        i_a = np.array( data[1][i] )[range_mask_2].max() - np.array( data[1][i] )[range_mask_3].max()
    ...
    range_mask_1 = np.where( (np.array( data[0] ) >= 1500) & (np.array( data[0] ) < 1525) )
    range_mask_2 = np.where( (np.array( data[0] ) >= 1460) & (np.array( data[0] ) < 1468) )
    range_mask_3 = np.where( (np.array( data[0] ) >= 1470) & (np.array( data[0] ) < 1480) )
