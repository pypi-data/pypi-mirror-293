...
class Header(dict[str, bytes]):
    ...
    @classmethod
    def read(cls: type[Header],
             src: BinaryIO,
             expect: RecordType | None = None) -> Header:
...
class Reader:
    ...
    def open(self) -> None:
        ...
        try:
            ...
            try:
                self.connections = [
                    self.read_connection() for _ in range(conn_count)
                ]
                self.chunk_infos = [
                    self.read_chunk_info() for _ in range(chunk_count)
                ]
            except ReaderError as err:
            ...
            self.indexes = {
                x.id: sorted(indexes[x.id])
                for x in self.connections
            }

            self.connections = [
                Connection(*x[0:5], len(self.indexes[x.id]), *x[6:])
                for x in self.connections
            ]
        except ReaderError:
            self.close()
    ...
    def start_time(self) -> int:
        ...
        return min(
            x.start_time
            for x in self.chunk_infos) if self.chunk_infos else 2**63 - 1

    @property
    def end_time(self) -> int:
        """Timestamp in nanoseconds after the latest message."""
        return max(x.end_time
                   for x in self.chunk_infos) if self.chunk_infos else 0

    @property
    def message_count(self) -> int:
        """Total message count."""
        return reduce(lambda x, y: x + y,
                      (x.msgcount for x in self.topics.values()), 0)
    ...
    def topics(self) -> dict[str, TopicInfo]:
        ...
        for topic, group in groupby(
            ...
            msgcount = reduce(
                lambda x, y: x + y,
                (y.connection_counts.get(x.id, 0) for x in connections
            ...
            topics[topic] = TopicInfo(
                msgtypes.pop() if len(
                    msgtypes := {x.msgtype
                                 for x in connections}) == 1 else None,
                msgdefs.pop() if len(
                    msgdefs := {x.msgdef
                                for x in connections}) == 1 else None,
                msgcount,
    ...
    def read_connection(self) -> Connection:
        ...
        callerid = header.get_string(
            'callerid') if 'callerid' in header else None
        latching = None
        if 'latching' in header:
            latching_value = header.get_string('latching')
            if latching_value.lower() == "true":
                latching = 1
            elif latching_value.isdigit():
                latching = int(latching_value)
            else:
                latching = None
    ...
    def read_chunk_info(self) -> ChunkInfo:
        ...
        return ChunkInfo(
            ...
            end_time,
            {
                read_uint32(self.bio): read_uint32(self.bio)
    ...
    def read_index_data(self, pos: int,
                        indexes: dict[int, list[IndexData]]) -> None:
        ...
        idx = 0
        while idx < size:
            time = deserialize_uint32(
                buf, idx)[0] * 10**9 + deserialize_uint32(buf, idx + 4)[0]
        ...
        for entry in heapq.merge(*indexes):
            ...
            if self.current_chunk[0] != entry.chunk_pos:
                ...
                rawbytes = chunk_header.decompressor(
                    read_bytes(self.bio, chunk_header.datasize))
