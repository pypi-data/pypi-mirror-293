...
from querybuilder import Select, Column


math_operators = '< <= > >= <> != ='


def math_query(q: Select, c: Column, operator: str, v: any):
    v = q.add_parameter(v)
    return {
        '=': q.where(c == v),
        '<': q.where(c < v),
...
def number_filter_parser(query: Select, column: Column, filter_string: str) -> Select:
    exp = pp.Opt(pp.one_of(math_operators), '=') + pp.common.number + pp.LineEnd()
    tokens = exp.parse_string(filter_string)

    return math_query(query, column, tokens[0], tokens[1])


def date_filter_parser(query: Select, column: Column, filter_string: str) -> Select:
    ...
    return math_query(query, column, tokens[0], value)


def datetime_filter_parser(query: Select, column: Column, filter_string: str) -> Select:
...
def boolean_filter_parser(query: Select, column: Column, filter_string: str) -> Select:
    return query.where(column == boolean_parser(filter_string))
