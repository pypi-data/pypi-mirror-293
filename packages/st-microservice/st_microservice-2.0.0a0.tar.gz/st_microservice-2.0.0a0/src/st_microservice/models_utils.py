...
def database_model(
    ...
    def database_model_sub[T: type[BaseModel]](cls: T) -> T:
        ...
        for dc_field in dc_fields:
            ...
            setattr(cls, field_name, table_column)  # After dataclass processing, reset class attibutes
            dataclass_fields[field_name] = dc_field

        metadata = ModelMetaData(registry, table_name, primary_keys, database_table, dataclass_fields)
...
class Relation:
    def __init__(self, model: type[BaseModel], rel_model: type[BaseModel], join_on: dict[str, qb.Column]):
        self.model = model
        self.rel_model = rel_model
        self.join_on = join_on  # key is model's column name and value is rel_model's Column object
...
def setup_relation(model: type[BaseModel], relation_name: str, rel_model: type[BaseModel], **join_on):
    ...
    main_field_names = model.__metadata__.table.columns.keys()
    ref_fields = rel_model.__metadata__.table.columns.values()
