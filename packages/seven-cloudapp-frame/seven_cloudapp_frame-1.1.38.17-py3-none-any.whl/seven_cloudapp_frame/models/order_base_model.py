...
@LastEditTime: 2024-08-29 18:28:44
            ...
            if invoke_result_data.success == True:
                ...
                if len(order_data)>0:
                    ...
                    if len(all_order_list) >0:
                        ...
                        black_status = user_base_model.check_pull_black(user_info_dict,act_info_dict["is_black"],act_info_dict["refund_count"],all_order_list,2)
            ...
            if invoke_result_data.success == True:
                ...
                if len(order_data) > 0:
                    ...
                    for item in order_data:
                        for order in item["sku_order_list"]:
                            if str(order["sku_info"]["product_id"]) in goods_ids_list:
                                ...
                                order["product_pic"] = order["sku_info"]["product_pic"]
                                del order["sku_info"]
                    ...
                    for order in reward_order_list:
                        try:
                            ...
                            if sku_order:
                                ...
                                order["spec"] = sku_order['spec'] # 规格信息
                                order["ext_json"] = {}
                                order["ext_json"]['post_tel'] = order['post_tel']
                                order["ext_json"]['post_receiver'] = order['post_receiver']
                                order["ext_json"]['province'] = order['post_addr']["province"]
                                order["ext_json"]['city'] = order['post_addr']["city"]
                                order["ext_json"]['town'] = order['post_addr']["town"]
                                order["ext_json"]['street'] = order['post_addr']["street"]
                                order["ext_json"]['post_detail'] = order['post_addr']["detail"]
                                order["ext_json"]["mask_post_receiver"] = order['mask_post_receiver']
                                order["ext_json"]["mask_post_tel"] = order['mask_post_tel']
                                order["ext_json"]["mask_detail"] = order['mask_post_addr']['detail']
                                order["ext_json"]["open_address_id"] = order['open_address_id']
                            ...
                            if order["sub_order_id"] not in pay_order_no_list and int(order["order_status"]) in reward_order_status_list and order["after_sale_info"]["refund_status"] != '3': # 订单状态;1 待确认/待支付-订单创建完毕;105-已支付; 2-备货中； 101-部分发货; 3-已发货（全部发货）;4- 已取消;5 已完成（已收货）;
                                ...
                                third_main_order = third_main_order_dict.get(str(order['order_id']))
                                if third_main_order:
                                    third_main_order.buy_num += third_pay_order.buy_num
                                    third_main_order.pay_price += third_pay_order.pay_price
                                    third_main_order_dict[str(order['order_id'])] = third_main_order
                                else:
                    ...
                    if len(pay_order_list) > 0:
                        ...
                        third_main_order_model.add_list(list(third_main_order_dict.values()))
                        result, message = db_transaction.commit_transaction(True)
                        if result == True:
                            asset_base_model = AssetBaseModel(context=self.context)
                            invoke_result_data.data = {}
