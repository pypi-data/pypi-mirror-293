...
The idea is to express units in terms of 
"""

from ..errors import UnitsError, InputMissingError, InputError
from ..tools import (
    format_docstring, alias,
)
...
KNOWN_CONVERSION_SYMBOLS = {
    ...
    'power'           : 'W',
    'number_density'  : 'm-3',     'n': 'number_density', 'nr': 'number density',
...
# DICTIONARY OF PHYSICAL CONSTANTS DEFINED BY A VALUE AND ANY OF THE ABOVE UNITS (SI UNITS)
PHYSICAL_CONSTANTS_SI = {
    'amu'    : (1.66054e-27, 'M'),  # atomic mass unit
    'c'      : (2.99792E8,   'u'),  # speed of light
    'kB'     : (1.38065E-23, 'J K-1'),  # boltzmann constant
    'eps0'   : (8.85419E-12, 'permittivity'),  # permittivity of free space
    'mu0'    : (1.25664E-6,  'permeability'),  # permeability of free space
    'qe'     : (1.60218E-19, 'q'),  # elementary charge
    'me'     : (9.10938E-31, 'M'),  # electron mass
    'qme'    : (1.75882E11,  'q M-1'), # q / m_e
    'h'      : (6.260701E-34, 'J s'),  # planck constant (not hbar)
    'm_proton' : (1.67262E-27, 'M'),  # proton mass
    'eV'     : (1.602176634E-19, 'J'),  # electron volt
    'eV kB-1': (11604.518121550082, 'K'),  # eV / kB
    # [TODO] allow physical constants to be involved in the conversion factor notation, in general.
}
# aliases:
for _key, _alias in (('qe', 'q_e'), ('me', 'm_e'), ('m_proton', 'm_p')):
    PHYSICAL_CONSTANTS_SI[_alias] = PHYSICAL_CONSTANTS_SI[_key]
...
@format_docstring(helpstr=_helpstr, sub_indent=DEFAULTS.TAB)
class UnitsManager():
    '''Manages units, including conversion factors and physical constants in different unit systems.

    :param units: str
        the default unit system to convert to ("si" or "raw")
    ...
    def init_sic(self):
        ...
        b = self.bases
        self.sic = dict(kg=b['M'], m=b['l'], s=b['t'], A=b['q'] / b['t'], K=b['K'])
    ...
    # # # CONVERTING UNITS # # #
    def __call__(self, ustr, units=None, convert_from='raw'):
        ''' return quantity or conversion factor in the specified unit system.
        :param ustr: physical constant or quantity (e.g. "c" for speed of light or "M" for mass units)
        :param units: unit system to convert to ("si" or "raw")
        :param convert_from: unit system to convert from ("si" or "raw").
            Ignored if ustr corresponds to a physical constant (e.g. "c") rather than a conversion factor (e.g. "mass")
        ...
        if convert_from == units:
            return 1
        elif set((convert_from, units)) == set(("si", "raw")):
            raw_to_si = self.get_sic_factor(ustr)
            if units == "si":   # and convert_from == "raw"
                return raw_to_si
            else: # units == "raw" and convert_from == "si"
                return 1 / raw_to_si
        else:
    ...
    def get_sic_factor(self, ustr):
        ...
        try:
            ...
            return self.calc_sic_factor(ustr)
    ...
    def __repr__(self):
        ...
        bases = [f"{key}={format(value, f'.{prec}e')}" for key, value in self.bases.items()]
        return f'{type(self).__name__}(units={self.units!r}, {", ".join(bases)})'
