...
from ...tools import simple_setdefaultvia_property, alias
...
class CrossTable():
    ...
    def __repr__(self):
        ...
        contents = [f'with len={len(self)}',
                    f'T in range [{Tmin:.2e}, {Tmax:.2e}] K',
                    f'cross in range [{cmin:.2e}, {cmax:.2e}] m^-2',
...
class CrossMapping(SymmetricPairMapping):
    ...
    def __str__(self):
        ...
        contents = ', '.join(f'{key}: {val}' for key, val in zip(keys, vals))
