...
def take_perp_to(B, A):
    '''return A after subtracting the component of A parallel to B.
    Equivalent to A - (A dot Bhat) Bhat. Note that B is the first argument.
    '''
    Bhat = B / magnitude(B)
...
def take_parallel_to(B, A):
    ...
    Equivalent to (A dot Bhat) Bhat. Note that B is the first argument.
    '''
    Bhat = B / magnitude(B)
...
class VectorArithmeticLoader(QuantityLoader):
    ...
    def get_dot(self, var, *, _match=None, **_known_vals):
        ...
        with self.using(component=tuple(components)):
            Aval = _known_vals.get(f'_{A}', None)
            if Aval is None:
                Aval = self(A)
            Bval = _known_vals.get(f'_{B}', None)
            if Bval is None:
    ...
    def get_mod(self, var, *, _match=None, **_known_vals):
        ...
        vdotv = self(f'{A}_dot_{A}{components}', **_known_vals)
        return vdotv ** 0.5
    ...
    def get_mod2(self, var, *, _match=None):
        ...
        if components is None: components = ''
        return self(f'{A}_dot_{A}{components}')
    ...
    def get_cross(self, var, *, _match=None, **_known_vals):
    ...
    def get_hat(self, var, *, _match=None):
    ...
    def get_perp(self, var, *, _match=None):
        ...
        A = self(A)
        B = self(B)
        return self.take_perp_to(B, A)

    # {A}_par_{B}
    @known_pattern(r'(.+)_par_(.+)', deps=[0,1], ignores_dims=['component'])
    def get_parallel(self, var, *, _match=None):
