...
class EppicBasesLoader(AllBasesLoader, SimpleDerivedLoader):
    ...
    def get_m(self):
        ...
        return xr.DataArray(self.fluid.m * self.u('M'), attrs=dict(units=self.units))

    @known_var(load_across_dims=['fluid'])
    def get_q(self):
        '''charge, of a "single particle". For protons, == +1 elementary charge'''
        return xr.DataArray(self.fluid.q * self.u('q'), attrs=dict(units=self.units))
    ...
    def get_nusn(self):
        ...
        '''
        return xr.DataArray(self.fluid['coll_rate'] * self.u('Hz'), attrs=dict(units=self.units))
    ...
    def get_ds(self):
        ...
        dx = self.input_deck.get_dspace('x') * self.u('length')
    ...
    def get_n0(self):
        ...
        return xr.DataArray(n0 * self.u('n'), attrs=dict(units=self.units))
    ...
    def get_E_ext(self):
        ...
        return xr.DataArray(Ex, attrs=dict(units=self.units))
    ...
    def get_vdist(self):
        ...
        def vdist_loader():
            ...
            units = self.u("speed")
            result = self.fluid.assign_velocity_coords(result, units)
    ...
    def get_B(self):
        ...
        return xr.DataArray(Bx, attrs=dict(units=self.units))
    ...
    def _get_T_neutral(self):
        ...
        def Tn_loader():
            ...
            T = m * vth**2 / self.u('kB')
            return xr.DataArray(T, attrs=dict(units=self.units))
