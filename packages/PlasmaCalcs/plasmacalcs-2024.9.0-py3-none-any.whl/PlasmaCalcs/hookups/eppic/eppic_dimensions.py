...
class EppicDist(DictlikeFromKeysAndGetitem, Fluid):
    ...
    def __init__(self, m, q, n0, *, N=None, name=None, i=None, params=dict()):
    ...
    # # # SERIALIZING # # #
    def to_dict(self):
        '''returns a dictionary containing all the information from the EppicDist object.'''
        result_dict = super().to_dict()   # Fluid.to_dict() handles name, m, q, and some other info.
        result_dict["n0"] = self.n0
        result_dict["N"] = self.N
    ...
    def assign_velocity_coords(self, array, units=1):
        '''assign velocity dims and coords, based on self.get_velocity_coords().
        array must have same shape as implied by velocity dims and coords.
        returns an xarray with proper details for PlasmaCalcs.

        This function creates a *new* xarray based on array, and velocity dims & coords are >0 dimensional.
        This is not like assign_{dim}_coord functions, which assign 0D coord to an existing xarray.
        '''
        ndim = np.ndim(array)
        assert ndim == 3, f"expected 3D, but got ndim={ndim}"
        coords = self.get_velocity_coords(units)
        array = xr.DataArray(array, coords=coords, dims=("vx", "vy", "vz"))
        return array
...
class EppicNeutral(Fluid):
    ...
    def __init__(self, m, *, name='neutral', i=0, vth=None, **kw_super):
        self.vth = vth
        super().__init__(name, m=m, q=0, i=i, **kw_super)
...
''' --------------------- EppicRun and EppicRunList --------------------- '''

class EppicRun(Run):
    '''coordinate referring to a single run in eppic, in the dimension space of possibly multiple runs.'''
    def is_valid(self):
        '''returns whether self is a valid eppic run.
        Here, checks the following:
            - self.path is a directory
            - eppic*.i is an existing file in self.path.  (* can be anything, e.g. eppic_0.i is fine.)
        '''
        if not os.path.isdir(self.path):
            return False
        for f in os.listdir(self.path):
            if f.startswith('eppic') and f.endswith('.i'):
                if os.path.isfile(os.path.join(self.path, f)):
                    return True
        return False

class EppicRunList(RunList):
    '''list of EppicRun objects.'''
    value_type = EppicRun
