...
from cmem_plugin_sparql_anything.bin import sparql_anything_jar_path
from cmem_plugin_sparql_anything.constants import POLICY_TEMPLATE, \
    QUERY_PARAMETER_DESCRIPTION, DEFAULT_SPARQL, SPARQL_ANYTHING_ERROR_PATTERN
...
""",
    ...
    parameters=[
        ...
        PluginParameter(
            name="graph",
            label="Graph",
            description="Graph in which query result is replaced and stored",
...
class SPARQLAnything(WorkflowPlugin):
    ...
    def post_result_to_graph(self, data: bytes) -> None:
        ...
        post_streamed(graph=self.graph, file=io.BytesIO(data), replace=True)
    ...
    def _run_query(self, resource: str) -> bytes:
        ...
        with (
            ...
            cmd = (
                ...
                f" -jar {sparql_anything_jar_path} -q {query_file.name}"
            )
