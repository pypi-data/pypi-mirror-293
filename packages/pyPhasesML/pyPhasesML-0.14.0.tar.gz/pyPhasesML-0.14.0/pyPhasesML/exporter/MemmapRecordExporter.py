...
class MemmapRecordExporter(DataExporter):
    ...
    def _getRecordSegmentIndex(self, segment):
        if self.segmentLength is None:
            return segment
        
        segmentLength = self.segmentLength
        recordStartInSegments = self.recordLengths_cumulative/segmentLength
    ...
    def __getitem__(self, index):
        ...
        if self.segmentLength is None:
            ...
            start = self.recordLengths_cumulative[recIndex] + int(segIndex * self.segmentLength)
            end = start + self.segmentLength
    ...
    def __len__(self):
        if self.segmentLength is None:
            ...
            return sum(self.recordLengths) // self.segmentLength
