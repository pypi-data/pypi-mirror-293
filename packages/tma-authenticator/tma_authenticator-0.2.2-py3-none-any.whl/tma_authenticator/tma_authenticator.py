...
class TMAAuthenticator():
    ...
    def __init__(self,
        ...
        self.user_model = user_model or UserDB
    ...
    async def verify_token(self, authorization: str, extra_tokens_validation: List[str] = None) -> T:
        try:
            ...
            if decoded_data['initData'] != self.admin_password:
                ...
                if not db_user:
                    matched_count, upserted_id = await self.storage_provider.insert_user(
                        user_data=user.model_dump()
                    )
                    return self.user_model(id=str(upserted_id), **user.model_dump())
                else:
                    attributes_to_compare = ['tg_language', 'first_name', 'last_name', 'username']
                    for attr in attributes_to_compare:
                        if getattr(user, attr) != db_user[attr]:
                            ...
                            break
                    return self.user_model(id=db_user['id'], **user.model_dump())
            else:
                db_user = await self.storage_provider.retrieve_user(search_query={'tg_id': user.tg_id})
                if db_user:
                    ...
                    raise HTTPException(status_code=status.HTTP_404_UNAUTHORIZED,
                                        detail="User to impersonalize was not found.",
                                        headers={"Authorization": "Bearer"})
