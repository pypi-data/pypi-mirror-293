...
mono_attributes = {'Hex': {'mass': {'03X': 72.0211, '02X': 42.0106, '15X': 27.9949, '13A': 60.0211, '24A': 60.0211, '15A': 134.057859,
                  ...
                  'HexA3S': {'mass': {'02X': 42.0106, '02A': 213.97839, '24X': 116.01099, '24A': 139.9779, 'HexA3S': 255.98889},
                           'atoms': {'02X': [1, 2], '02A': [3, 4, 5, 6], '24X': [1, 2, 5, 6], '24A': [3, 4], 'HexA3S': [1, 2, 3, 4, 5, 6]}},
                  ...
                  'HexNAc4S': {'mass': {'04A': 60.0211, '24A': 139.9779, '35A': 153.9936, '03A': 169.9885, '25A': 184.0041, '25X': 99.0321,
                                      '02A': 199.9991, '24X': 223.0151, '14A': 211.015044, '15A': 255.041259, '15X': 27.994941, '04X': 223.0151, '35X': 129.0426,
                  ...
                  'HexOS': {'mass': {'02X': 42.0106, '03X': 151.9779, '15X': 27.9949, '13A': 139.9779, '24A': 139.9779, '04A': 139.9779, '35A': 153.9936, '25A': 184.0041,
                            ...
                            'atoms': {'02X': [1, 2], '03X': [1, 2, 3], '15X': [1], '13A': [2, 3], '24A': [3, 4], '04A': [5, 6], '35A': [4, 5, 6], '25A': [3, 4, 5, 6],
                                      '02A': [3, 4, 5, 6], '03A': [4, 5, 6], '15A': [2, 3, 4, 5, 6], '24X': [1, 2, 5, 6], '04X': [1, 2, 3, 4], '35X': [1, 2, 3], 'HexOS': [1, 2, 3, 4, 5, 6]}},
...
def atom_mods_init(subg, present_breakages, terminals, terminal_labels):
  ...
  for bond, bond_label in present_breakages.items():
    ...
    elif bond_label == 'glycosite':
      if bond[0] in subg.nodes():
        atomic_mod_dict[bond[0]][3] = 2
...
def generate_atomic_frags(nx_mono, global_mods, special_residues, allowed_X_cleavages, max_cleavages = 3, fragment_masses = [],
  ...
  for i,subg in enumerate(subgraphs):
    ...
    other_terminals = [x for x in subg.nodes if x in all_other_terminals] 
...
def input_to_graph(input_dict): 
    if 'peptide' not in input_dict and 'glycosites' not in input_dict:
...
def get_glycan_cleavages(gp,subg):
    """Return Domon-Costello labels for all glycans on a glycopeptide"""
    all_glycan_prefixes = set([x[0] for x in gp.nodes() if x[0]!='0'])
    subg_glycan_prefixes = set([x[0] for x in subg.nodes() if x[0]!='0'])
    all_mods = []
    for prefix in all_glycan_prefixes:
        if prefix in subg_glycan_prefixes:
...
def peptide_to_RF_nomenclature(peptide,pep_subg,iupac = False):
    ...
    if iupac == True:
        ...
        return RF_cleavages,''.join([x[1] for x in sorted(subg_peptide_labels.items(),key=lambda x:x[0][-1])])    
    else:
        return RF_cleavages
...
@rescue_glycans
def CandyCrumbs(input_dict, fragment_masses, mass_threshold,
  ...
  if 'peptide' in input_dict:
    for observed_mass in fragment_masses:
      ...
      for frag_subg in fragment_properties[-1]:
        ...
        dc_names = get_glycan_cleavages(nx_mono,frag_subg)
  ...
  for i,frag_dc_names in enumerate(filtered_dc_names):
    if frag_dc_names:
      ...
      final_hits = [[y for y in filtered_properties if y[5]==x][0] for x in frag_dc_names]
