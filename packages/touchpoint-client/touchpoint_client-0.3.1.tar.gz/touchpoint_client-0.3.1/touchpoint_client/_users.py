...
def _parse_result(
    ...
    try:
        if response.status_code in (404,) and (
            not error_code or response.json()["code"] == error_code
        ):
            if error_on_404:
...
class _Users:
    ...
    @use_error_details
    def user(
        self, user_id: str, *, error_details: bool = False, **kwargs
    ) -> Optional[dict]:
    ...
    def update_user(
        ...
        **kwargs,
        ...
        assert user_id, "user_id is required"
        assert (
            account_role_id or personal_settings is not None
        ), "account_role or personal_settings must not be empty"
    ...
    def create_group(
        self, name: str, *, error_details: bool = False, **kwargs
    ) -> List[dict]:
        r"""Получить список пользователей
    ...
    @use_error_details
    def group(
        self, group_id: str, *, error_details: bool = False, **kwargs
    ) -> Optional[dict]:
    ...
    @use_error_details
    def update_group(
        self, group_id: str, *, name: str, error_details: bool = False, **kwargs
    ) -> Optional[dict]:
    ...
    @use_error_details
    def role(
        self, role_id: str, *, error_details: bool = False, **kwargs
    ) -> List[dict]:
        """
        Получить список ролей на аккаунт
...
class _UsersAsync:
    ...
    @use_error_details
    async def projects_permissions(
        self, *, error_details: bool = False, **kwargs
    ) -> List[str]:
        """
        Получить все разрешения на проект
    ...
    async def update_projects_role(
        ...
        **kwargs,
        ...
        resp = await self._client.patch(
            f"projects/roles/{role_id}", json=body, **kwargs
        )
        return cast(dict, _parse_result(resp, (200, 201)))

    @use_error_details
    async def remove_projects_role(
        self, role_id: str, *, error_details: bool = False, **kwargs
    ) -> None:
        """
        Получить список ролей на аккаунт
