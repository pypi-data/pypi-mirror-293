...
def init_bruker_dll(bruker_dll_file_name: str = BRUKER_DLL_FILE_NAME):
    ...
    bruker_dll.tims_set_num_threads(utils.MAX_THREADS)
...
def read_bruker_binary(
    ...
    if compression_type == 1:
        process_frame_func = utils.threadpool(
            process_frame,
            thread_count=1
        )
    else:
        process_frame_func = utils.threadpool(process_frame)
    process_frame_func(
        range(1, len(frames)),
        tdf_bin_file_name,
        tims_offset_values,
...
class TimsTOF(object):
    ...
    def save_as_hdf(
        ...
        with h5py.File(full_file_name, hdf_mode) as hdf_root:
            ...
            utils.create_hdf_group_from_dict(
    ...
    def _import_data_from_hdf_file(
        ...
        with h5py.File(bruker_d_folder_name, "r") as hdf_root:
            ...
            self.__dict__ = utils.create_dict_from_hdf_group(
                hdf_root["raw"],
                mmap_arrays,
                bruker_d_folder_name,
            )
...
@utils.pjit(thread_count=1, include_progress_callback=False)
...
def save_as_mgf(
    ...
    with open(full_file_name, "w") as infile:
        for index in utils.progress_callback(
