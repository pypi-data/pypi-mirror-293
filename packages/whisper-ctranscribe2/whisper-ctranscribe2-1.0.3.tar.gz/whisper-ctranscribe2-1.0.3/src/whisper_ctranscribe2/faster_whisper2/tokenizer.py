...
class Tokenizer:
    ...
    def __init__(
        ...
        if multilingual:
            ...
            if style not in _STYLES:
                raise ValueError(
                    "'%s' is not a valid transcription style (accepted transcription styles: %s)"
                    % (style, ", ".join(_STYLES))
                )

            self.task = self.tokenizer.token_to_id("<|%s|>" % task)
            self.style = self.tokenizer.token_to_id("<|%s|>" % style)
    ...
    def subtitle(self) -> int:
        return self.tokenizer.token_to_id("<|subtitle|>")
    
    @cached_property
    def verbatim(self) -> int:
        return self.tokenizer.token_to_id("<|verbatim|>")
    ...
    def sot_sequence(self) -> List[int]:
        ...
        if self.style is not None and self.task == "transcribe":
            sequence.append(self.style)   
