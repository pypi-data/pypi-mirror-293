            ...
            while expect_changes:
                ...
                while n_articles < new_n_articles:
                    ...
                    if author_tag is not None:
                        ...
                        logger.info(f"found {len(images)=} in article")
                        for img in images:
                            try:
                    ...
                    try:
                        ...
                        if not src:
                            ...
                            if len(source) > 0:
                                ...
                                logger.info(f"source {outerHTML=}")
                        ...
                        full_local_url = BasePlugin.get_local_url(src, session_meta["url"], allow_external=True)
                        if full_local_url:
                            # TODO: getting image from browser works somehow but
                            #   requires image type detection, quality check, crossOrigin understading etc
                            #   So for now let's do not in optimal way
