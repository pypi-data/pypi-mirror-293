...
@click.option(
    ...
    required=False,
    type=click.STRING,
    help="Base model to use. Model must be compatible with base model. If base model is not specified, model will be used.",
)
@click.option(
    "--finetuning_type",
...
@click.option(
    ...
    + click.style("NOTE: Currently not supported!", italic=True),
)
@click.option(
...
# ******************************* TRAINNIG CONFIGURATION OPTIONS *******************************
@click.option(
    "--epochs",
    cls=CategorizedMutuallyExclusiveOption,
    required=False,
...
@click.option(
    ...
    required=False,
    is_flag=True,
    help="Whether or not to load the best model found during training at the end of training. When this option is enabled, the best checkpoint which recorded lower loss will always be saved. See save_total_limit for more.",
    category="Training configuration options",
)
@click.option(
    "--save_strategy",
...
@click.option(
    ...
    cls=CategorizedMutuallyExclusiveOption,
    required=False,
    default=DEFAULT_LLM_PARAMS.use_deepspeed,
    show_default=True,
    help="Use DeepSpeed for training. Will be ignored if any PEFT method is enabled.",
    category="Compute optimization configuration options",
    mutually_exclusive=["use_peft"],
)
@click.option(
