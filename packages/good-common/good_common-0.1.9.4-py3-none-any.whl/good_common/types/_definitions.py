...
REGEXP_TRACKING_PARAMS = re.compile(
    r"""
        ^(__s|_branch_match_id|_bta_\w+|_ga|_hs_\w+|_ke|_kx|\w*clid\w*|campid|\w*cmp\w*|\w*_cid|
        \w*cid|t|mod|[\w\-]*source[\w\-]*|src|at_\w+|device\w*|\w*tag\w*|\wlink\w*|taid|customid|r|mod|
        dm_i|ef_id|epik|gbraid|gclsrc|gdffi|gdfms|sh|gdftrk|hsa_\w+|ic_id|igshid|matomo_\w+
        |mc_\w+|mkcid|mkevt|mkrid|mkt_\w+|mkwid|msclkid|mtm_\w+|ncid|nr_email_refer|ns_\w+
        |ocid|pcrid|piwik_\w+|pk_(campaign|keywords|kwd)|redirect_\w+|ref|s_kwcid|sb_referer_host
        |si|sms_\w+|spm|sr_share|srcid|toolid|trk_\w+|utm_\w+|utmz|vero_\w+|wbraid|z|xto|_\w+|cvid|eir)$|
        (?:dc|fbc|gc|twc|yc|ysc)lid|(?:click|gbra|msclk|igsh|partner|wbra)id|(?:ads?|mc|ga|gs|itm|mc|mkt|ml|mtm|oly|pk|utm|vero)_|
        (?:\b|_)(?:aff|affi|affiliate|campaign|cl?id|eid|ga|gl|kwd|keyword|medium|ref|referr?er|session|source|uid|xtor)
    """,
    re.IGNORECASE,
)
...
DOMAIN_RULES = RegexRuleSet(
    {
        ...
        f"{match_any_subdomains}washingtonpost.com": {"non_canonical": {"pwapi_token"}},
