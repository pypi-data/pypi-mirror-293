...
class Patient():
	...
	def __init__(self, 
		...
		self.ipss                           = self.collect_patient_reported_outcomes("ipss")
		self.epic                           = self.collect_patient_reported_outcomes("epic")
	...
	def collect_patient_reported_outcomes(self, pro_type : str, xlsx_workbook=None):
		...
		if xlsx_workbook == None:
			path_to_pro_xlsx = "/media/rasmus/3tb_backup/ufhpti_all_data/Jacksonville_data/PRX31_{}_DATA.xlsx".format(pro_type.upper())
		...
		time_points_patient = [pro_datasheet["B{}".format(time_point_row)].value.split("_")[0] for time_point_row in this_patient_rows]
		...
		for column_id, column_header in column_headers.items():
			...
			for time, row in zip(time_points_patient, this_patient_rows):
				...
				patient_metadata[column_header][time] = this_value
