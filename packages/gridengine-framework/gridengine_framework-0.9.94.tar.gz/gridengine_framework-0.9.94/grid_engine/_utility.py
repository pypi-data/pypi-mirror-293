...
def generate_images(dimensions, cdata, cell_size, grid_id, animate=False, display=True):
    ...
    if i == len(cells)-1 and len(cells) < total_cell_count:
        remaining_cells = total_cell_count - len(cells)
        remaining_rows = remaining_cells%(dimensions[0]//cell_size)
        new_height = dimensions[1] - (remaining_rows*cell_size)
        dimensions = (dimensions[0], new_height)
    image = Image.new('F', dimensions, (255, 255, 255))
    ...
    for i, cell in enumerate(cells):
        ...
        draw.rectangle((x, y, x+cell_size, y+cell_size), fill=color)
        if i == len(cells)-1 and len(cells) < total_cell_count:
            print('Failed to draw all cells.')
