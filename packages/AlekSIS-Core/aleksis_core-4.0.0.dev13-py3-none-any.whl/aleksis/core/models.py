...
class CalendarEvent(CalendarEventMixin, ExtensiblePolymorphicModel, RecurrenceModel):
    ...
    def get_objects(
        ...
        qs = cls.objects.with_occurrences().instance_of(cls)
        if start and end:
            qs = qs.filter(Q(datetime_start__lte=end) | Q(date_start__lte=end.date())).filter(
                # With rrule, filter recurrences
                Q(odatetime__gte=start, odatetime__lte=end)
                | Q(odate__gte=start.date(), odate__lte=end.date())
                # Without rrule, filter the whole date(time)
                | (
                    Q(rrule__isnull=True)
                    & (Q(datetime_end__gte=start) | Q(date_end__gte=start.date()))
                )
            )
        return qs
    ...
    def save(self, *args, **kwargs):
        ...
        if self.recurrences:
            ...
            self.rrule = serialize(self.recurrences.rrules[0])
...
class PersonalEvent(CalendarEvent):
    ...
    def get_objects(
        ...
        qs = super().get_objects(request=request, params=params, start=start, end=end)
        if request:
            qs = qs.filter(
