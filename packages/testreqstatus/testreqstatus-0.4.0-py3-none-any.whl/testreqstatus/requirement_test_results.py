...
class RequirementTestResults:
    ...
    def __init__(
        self,
        test_requirements: Path | str | dict[str, list[str]],
        test_results: Path | str,
    ):
        """
        ...
        if isinstance(test_requirements, (Path, str)):
            self.requirements = RequirementTests.from_file(test_requirements)
        elif isinstance(test_requirements, dict):
            self.requirements = RequirementTests(test_requirements)
        else:
            self.requirements = test_requirements

        if isinstance(test_results, Path) or Path(test_results).exists():
            self.results = JUnitXml.fromfile(test_results)
        else:
            self.results = JUnitXml.fromstring(test_results)
