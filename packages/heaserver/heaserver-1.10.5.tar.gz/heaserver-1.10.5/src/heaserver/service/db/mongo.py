...
class Mongo(Database):
    ...
    async def upsert_admin(self, obj: root.DesktopObject | root.DesktopObjectDict, collection: str,
                           mongoattributes: Mapping[str, Any] | None=None, default_content: Optional[IO] = None) -> Any:
        """
        ...
        try:
            ...
            if mongoattributes is None and not obj_.id:
                ...
                if inserted_result and default_content is not None:
                    fs = self.__new_gridfs_bucket(collection)
                    await fs.upload_from_stream_with_id(ObjectId(inserted_result.inserted_id), obj_.display_name, default_content)
                return inserted_result.inserted_id
            else:
                upserted_result = await coll.replace_one({'_id': ObjectId(obj_.id)} if mongoattributes is None else mongoattributes, obj_.to_dict(), upsert=True)
                if upserted_result and default_content is not None:
                    fs = self.__new_gridfs_bucket(collection)
                    await fs.upload_from_stream_with_id(ObjectId(upserted_result.upserted_id), obj_.display_name, default_content)
                return upserted_result.upserted_id
    ...
    async def update_admin(self, obj: root.DesktopObject | root.DesktopObjectDict, collection: str, default_content: Optional[IO] = None) -> UpdateResult | None:
        ...
        try:
            ...
            coll = self._get_collection(collection)
            result = await coll.replace_one({'_id': ObjectId(obj_.id)}, obj_dict)
            if result and default_content is not None:
                fs = self.__new_gridfs_bucket(collection)
    ...
    async def delete_admin(self, collection: str, mongoattributes=None) -> DeleteResult | None:
        ...
        try:
            q = Mongo.__replace_object_ids(mongo_expr(None,
                                                      ...
                                                      mongoattributes))
            logger.debug('Query is %s', q)
            object_ids_to_delete: list[ObjectId] = []
            async for result in coll.find(q):
                object_ids_to_delete.append(result['_id'])
            result_ = await coll.delete_many(q)
            fs = self.__new_gridfs_bucket(collection)
            try:
