...
def app_context(db_manager_cls: Type[MicroserviceDatabaseManager],
    ...
    with TestEnvExitStack() as context_manager, closing(db_manager_cls()) as external_db_, \
        ...
        with ThreadPoolExecutor() as pool:
            ...
            pool_futures = []
            pool_futures.append(pool.submit(external_db_.insert_all, external_desktop_objects, content))
            for b_ in bridge_dbs:
                pool_futures.append(pool.submit(b_.insert_all, bridge_desktop_objects, content))
            exceptions = []
            for pool_future in pool_futures:
                try:
                    pool_future.result()
                except Exception as e:
                    exceptions.append(e)
            if exceptions:
                raise exceptions[0]  # Use exception groups when we start using Python 3.11
