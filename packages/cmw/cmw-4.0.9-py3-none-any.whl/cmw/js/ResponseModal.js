...
function createOverlay() {
    ...
    overlay.style.zIndex = 1000 + modalCount * 2;
...
function ResponseModal(content, dismiss_callback = undefined) {
    ...
    contentDiv.querySelectorAll("*").forEach(node => {
        if (node.tagName === "FORM" && node.hasAttribute("dismiss-modal")) {
            showDismissButton = false;
            node.addEventListener("submit", dissmissModal);
        }
    });
    ...
    const observer = new MutationObserver(mutationsList => {
        mutationsList.forEach(mutation => {
            mutation.addedNodes.forEach(node => {
                ResponseModalSetupFormSubmitListener(node.querySelectorAll("*"));
            });
        });
    });
