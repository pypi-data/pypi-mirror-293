...
class Opendrive2Apollo(Convertor):
  def __init__(self, input_file_name, output_file_name = None, manual_speed_limit = -1) -> None:
    self.xodr_map = Map()
    self.xodr_map.load(input_file_name, manual_speed_limit)
    ...
    self.junction_straight_lane_manager = LaneManager()
  ...
  def set_parameters(self, only_driving = True, reverse = False,
                     additional_junction_points=None, reverse_boundary=False,
                     enable_junction_forward_neighbors=False):
    self.only_driving = only_driving
    self.reverse = reverse
    self.additional_junction_points = additional_junction_points
    self.reverse_boundary = reverse_boundary
    self.enable_junction_forward_neighbors = enable_junction_forward_neighbors
  ...
  def add_road_sample(self, pb_lane, lane, lane_section, xodr_road):
    cur_lane_id = int(lane.lane_id)
    total_s = lane.center_line[0].s
    # print("-----------Lane: {}".format(lane.lane_id))
    for i, point3d in enumerate(lane.center_line):
      lane_width = lane.get_width_by_s(point3d.s)
      left_dist = math.sqrt((point3d.x - lane_section.leftmost_boundary(self.reverse_boundary)[0][i].x) *
                            (point3d.x - lane_section.leftmost_boundary(self.reverse_boundary)[0][i].x) +
                            (point3d.y - lane_section.leftmost_boundary(self.reverse_boundary)[0][i].y) *
                            (point3d.y - lane_section.leftmost_boundary(self.reverse_boundary)[0][i].y))
      right_dist = math.sqrt((point3d.x - lane_section.rightmost_boundary(self.reverse_boundary)[0][i].x) *
                             (point3d.x - lane_section.rightmost_boundary(self.reverse_boundary)[0][i].x) +
                             (point3d.y - lane_section.rightmost_boundary(self.reverse_boundary)[0][i].y) *
                             (point3d.y - lane_section.rightmost_boundary(self.reverse_boundary)[0][i].y))
  ...
  def create_lane(self, xodr_road, lane_section, idx, lane):
    ...
    # add road sample
    self.add_road_sample(pb_lane, lane, lane_section, xodr_road)
    # add neighbor
  ...
  def add_road_section_boundary(self, pb_road_section, lane_section):
    ...
    leftmost_boundary, leftmost_length = lane_section.leftmost_boundary(self.reverse_boundary)
    rightmost_boundary, rightmost_length = lane_section.rightmost_boundary(self.reverse_boundary)
    ...
    for idx, lane_section in enumerate(xodr_road.lanes.lane_sections):
      ...
      for lane in lane_section.left:
        ...
        if pb_lane is not None:
          ...
          if lane_marker.direction != "turn" and xodr_road.junction_id == "-1":  # not in junction && straight lane
          ...
          # [cjh][2024.7.23] Add straight lane in junction to another lane manager
          elif lane_marker.direction != "turn" and xodr_road.junction_id != "-1":  # in junction && straight lane
            self.junction_straight_lane_manager.add_lane(pb_lane.id.id, lane_marker)
  ...
  def add_junction_forward_neighbors(self):
    for k, v in self.junction_straight_lane_manager.id_to_marker.items():
      # print("lane_id: {}, lane_marker: {}".format(k, v))
      pass
    # print("lane_manager size: {}".format(len(self.lane_manager)))
    left_count = 0
    ...
    lane_width = 4
    for pb_lane in self.pb_map.lane:
      if pb_lane.id.id not in self.junction_straight_lane_manager.id_to_marker:
        continue

      pb_boundary_type = map_lane_pb2.LaneBoundaryType.DOTTED_WHITE
      del pb_lane.left_boundary.boundary_type[:]
      boundary_type = pb_lane.left_boundary.boundary_type.add()
      boundary_type.s = 0
      boundary_type.types.append(pb_boundary_type)

      del pb_lane.right_boundary.boundary_type[:]
      boundary_type = pb_lane.right_boundary.boundary_type.add()
      boundary_type.s = 0
      boundary_type.types.append(pb_boundary_type)

      lane_marker = self.junction_straight_lane_manager.get_lane_marker(pb_lane.id.id)

      start_point = lane_marker.start_point
      end_point = lane_marker.end_point
      left_forward_start_point = copy.deepcopy(start_point)
      left_forward_end_point = copy.deepcopy(end_point)
      right_forward_start_point = copy.deepcopy(start_point)
      right_forward_end_point = copy.deepcopy(end_point)
      left_forward_lane_id = ""
      right_forward_lane_id = ""
      if lane_marker.direction == "left_to_right":
        left_forward_start_point.y += lane_width
        left_forward_end_point.y += lane_width
        right_forward_start_point.y -= lane_width
        right_forward_end_point.y -= lane_width
        left_forward_lane_id = self.junction_straight_lane_manager.get_lane_id(LaneMarker(
          "left_to_right", left_forward_start_point, left_forward_end_point))
        right_forward_lane_id = self.junction_straight_lane_manager.get_lane_id(LaneMarker(
          "left_to_right", right_forward_start_point, right_forward_end_point))
      elif lane_marker.direction == "right_to_left":
        left_forward_start_point.y -= lane_width
        left_forward_end_point.y -= lane_width
        right_forward_start_point.y += lane_width
        right_forward_end_point.y += lane_width
        left_forward_lane_id = self.junction_straight_lane_manager.get_lane_id(LaneMarker(
          "right_to_left", left_forward_start_point, left_forward_end_point))
        right_forward_lane_id = self.junction_straight_lane_manager.get_lane_id(LaneMarker(
          "right_to_left", right_forward_start_point, right_forward_end_point))
      elif lane_marker.direction == "top_to_bottom":
        left_forward_start_point.x += lane_width
        left_forward_end_point.x += lane_width
        right_forward_start_point.x -= lane_width
        right_forward_end_point.x -= lane_width
        left_forward_lane_id = self.junction_straight_lane_manager.get_lane_id(LaneMarker(
          "top_to_bottom", left_forward_start_point, left_forward_end_point))
        right_forward_lane_id = self.junction_straight_lane_manager.get_lane_id(LaneMarker(
          "top_to_bottom", right_forward_start_point, right_forward_end_point))
      elif lane_marker.direction == "bottom_to_top":
        left_forward_start_point.x -= lane_width
        left_forward_end_point.x -= lane_width
        right_forward_start_point.x += lane_width
        right_forward_end_point.x += lane_width
        left_forward_lane_id = self.junction_straight_lane_manager.get_lane_id(LaneMarker(
          "bottom_to_top", left_forward_start_point, left_forward_end_point))
        right_forward_lane_id = self.junction_straight_lane_manager.get_lane_id(LaneMarker(
          "bottom_to_top", right_forward_start_point, right_forward_end_point))

      # print("id: {}, left/right forward lane_id: {}/{}".format(pb_lane.id.id, left_forward_lane_id,
      #                                                          right_forward_lane_id))

      if right_forward_lane_id != "" and right_forward_lane_id not in pb_lane.right_neighbor_forward_lane_id:
        right_count += 1

        pb_lane.right_neighbor_forward_lane_id.add().id = right_forward_lane_id
      if left_forward_lane_id != "" and left_forward_lane_id not in pb_lane.left_neighbor_forward_lane_id:
        left_count += 1
        pb_lane.left_neighbor_forward_lane_id.add().id = left_forward_lane_id
  ...
  def convert_roads(self):
    ...
    if self.enable_junction_forward_neighbors:
      self.add_junction_forward_neighbors()
  ...
  def construct_junction_polygon(self, xodr_junction):
    ...
    if self.additional_junction_points is not None:
      for junction_id, x, y in self.additional_junction_points:
        if xodr_junction.junction_id == junction_id:
          print(f"Adding point ({x}, {y}) to junction {junction_id}")
