...
class Speed:
  ...
  def parse_from(self, raw_speed, manual_speed_limit=-1):
    if raw_speed is not None:
      ...
      self.max_v = convert_speed(self.raw_max_v, self.unit, manual_speed_limit)
...
class Lane:
  ...
  def parse_from(self, raw_lane, manual_speed_limit=-1):
    ...
    self.speed.parse_from(raw_speed, manual_speed_limit)
...
class LaneSection:
  ...
  def parse_from(self, raw_lane_section, manual_speed_limit=-1):
    ...
    if left is not None:
      for raw_lane in left.iter('lane'):
        ...
        lane.parse_from(raw_lane, manual_speed_limit)
        self.add_left_lane(lane)

    # center
    center = raw_lane_section.find("center")
    raw_lane = center.find('lane')
    self.center.parse_from(raw_lane, manual_speed_limit)
  ...
  def leftmost_boundary(self, reverse_boundary=False):
    # [2024.7.23][cjh] Add reverse_boundary for reverse map
    if reverse_boundary:
  ...
  def rightmost_boundary(self, reverse_boundary=False):
    # [2024.7.23][cjh] Add reverse_boundary for reverse map
    if reverse_boundary:
      for lane in self.left:
        if lane.lane_type == "driving":
...
class Lanes:
  ...
  def parse_from(self, raw_lanes, manual_speed_limit=-1):
    # laneOffset
    ...
    for raw_lane_section in raw_lanes.iter("laneSection"):
      ...
      lane_section.parse_from(raw_lane_section, manual_speed_limit)
