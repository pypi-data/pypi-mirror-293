...
class Map:
  ...
  def parse_roads(self, raw_roads, manual_speed_limit=-1):
    for raw_road in raw_roads:
      road = Road()
      road.parse_from(raw_road, manual_speed_limit)
  ...
  def load(self, filename, manual_speed_limit):
    ...
    self.parse_roads(raw_roads, manual_speed_limit)
