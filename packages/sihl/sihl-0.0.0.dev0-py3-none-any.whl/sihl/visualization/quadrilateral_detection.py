...
def _(head, config, input, target, features) -> List[np.ndarray]:
    ...
    if prediction is not None:
        ...
        scores = head.get_saliency(features).to("cpu").numpy()
        # print(scores.shape)
        # inputs = [features[level] for level in head.levels]
        # scores = head.get_features(inputs)[0].max(dim=2)[0]
    ...
    for batch, image in enumerate(images):
        ...
        if target is not None:
            for label, quad in zip(target["classes"][batch], target["quads"][batch]):
