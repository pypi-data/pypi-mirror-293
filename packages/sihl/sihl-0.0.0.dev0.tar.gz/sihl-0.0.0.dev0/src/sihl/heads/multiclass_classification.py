...
class MulticlassClassification(nn.Module):
    ...
    def forward(self, inputs: List[Tensor]) -> Tuple[Tensor, Tensor]:
        scores, classes = self.net(inputs[self.level]).softmax(dim=1).max(dim=1)
    ...
    def training_step(
        ...
        if self.is_ordinal:
            target = soft_label(target, self.num_classes)
...
def soft_label(labels: Tensor, num_labels: int) -> Tensor:
    target = torch.arange(num_labels, dtype=torch.float32, device=labels.device)
    return torch.softmax(-(target.unsqueeze(0) - labels.unsqueeze(1)).abs(), dim=1)
