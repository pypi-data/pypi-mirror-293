...
#         }
# 
#         # After defining month_map, get the current month
#         self.current_month = self.data[(self.data['year'] == self.max_year) & (self.data['period'] != 'Average')].sort_values(by='period', key=lambda x: x.apply(self.__normalize_month))['period'].iloc[-1]
# 
#     def __normalize_month(self, month_input):
#         """Normalize various month inputs to standard month names used in the dataframe."""
...
#         return content if content else None
# 
#     def CPI_U(self, series_id='CPIAUCNS', verbose=True):
#         """
#         Fetches and processes Consumer Price Index (CPI) data for all urban consumers.
# 
#         Args:
#             series_id (str): The series ID for CPI data to query. Defaults to 'CPIAUCNS', which represents the all-items Consumer Price Index for all urban consumers in the United States.
#             verbose (bool): If True, displays the data in a tabular format using a custom Table class. If False, returns the data as a DataFrame. Defaults to True.
# 
#         Returns:
#             pandas.DataFrame or None: If `verbose` is False, returns a DataFrame containing the processed CPI data. If `verbose` is True, displays the data in tabular form and returns None.
#             Returns None if no data is available or the HTTP request fails.
#         """    	
#         make_method = getattr(self.asset, 'make')
#         url = make_method(series_id=series_id)
#         html_content = self._make_request(url)
...
#             end_date = obj1.date()
#             obj2 = cpi.CUUR0000AA0.Data(end_date=end_date)
#             chart_title = "CPI All Items"
#             data = obj2.all_items_index()
#             if verbose:
#                 rows = self._split_dataframe(data)
...
# 
# 
# engine = Engine(cpi_asset)
# cpi_dataset = engine.CPI_U(series_id='CPIAUCNS', verbose=False)
# inflation_adjustment = InflationAdjustment(cpi_dataset)
# 
# 
# 
# def __dir__():
