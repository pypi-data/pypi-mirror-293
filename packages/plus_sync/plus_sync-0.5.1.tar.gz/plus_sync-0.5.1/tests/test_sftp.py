...
def test_simple_sftp(plus_sync_cmd, plus_sync_initialized, sftp_fixture):
    ...
    result = plus_sync_cmd(['list-remotes'])
    assert result.exit_code == 0
    ...
    with sftp_fixture.serve_content(all_content):
        sync_tester(plus_sync_cmd, subject_files, 'test_sftp', input='pw\n')
