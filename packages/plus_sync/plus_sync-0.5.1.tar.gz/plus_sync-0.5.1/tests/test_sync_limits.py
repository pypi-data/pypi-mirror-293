...
def test_limit_subjects_sync(plus_sync_cmd, plus_sync_initialized, rclone_config, plus_sync_subjects_in_folder, hashed):
    ...
    synced = plus_sync_cmd(['sync', 'test_rclone', *limit_param_list, hash_param])
...
@pytest.mark.parametrize('hashed', [True, False])
def test_limit_subjects_wrong_subject(
    plus_sync_cmd, plus_sync_initialized, rclone_config, plus_sync_subjects_in_folder, hashed
):
    subjects, tmpdir_path = plus_sync_subjects_in_folder(3, 'testdata')
    plus_sync_cmd(['add', 'rclone', 'test_rclone', 'test', str(Path(tmpdir_path, 'testdata')), '--globs', '*.txt'])
    ...
    synced = plus_sync_cmd(['sync', 'test_rclone', '--only-subject=abc', hash_param])
