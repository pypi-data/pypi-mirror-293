...
def _stable_fields(value: Any) -> list[tuple[str, type | str]]:
    ...
    field_names = [
        ...
        if not _is_transient_field(field)
    ]

    # FIXME better support lazy fields
    if value.__class__.__name__ == "JobFunction":
        from databricks.bundles.jobs.models.task import Task

        field_names.append(("tasks", list[Task]))
...
def _is_transient_field(field: Field) -> bool:
    return field.metadata.get("_transient", False)
