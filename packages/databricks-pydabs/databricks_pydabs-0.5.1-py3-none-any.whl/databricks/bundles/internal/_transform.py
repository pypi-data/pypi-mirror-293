...
def _transform_variable_or_list(
    cls: Type[_T], value: VariableOrList[Any]
) -> VariableOrList[_T]:
...
def _transform_variable_or_dict(
    cls: Type[_T], value: VariableOrDict[Any]
) -> VariableOrDict[_T]:
    if isinstance(value, Variable):
        return value
...
def _transient_field(default=dataclasses.MISSING, default_factory=dataclasses.MISSING):
    """
    Data class field that is not serialized
    """

    metadata = {"_transient": True}

    if default != dataclasses.MISSING:
        return field(default=default, metadata=metadata)
    elif isinstance(default_factory, typing.Callable):
    ...
    if origin := get_origin(tpe):
        if origin == Union or origin == UnionType:
            ...
            if len(args) == 2 and args[1] == type(None):
                return args[0]
            elif len(args) == 2 and args[0] == type(None):
                return args[1]
