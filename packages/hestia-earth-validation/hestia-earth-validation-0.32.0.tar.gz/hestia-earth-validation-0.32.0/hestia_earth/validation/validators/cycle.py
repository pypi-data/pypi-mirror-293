...
from .animal import (
    validate_has_animals, validate_duplicated_feed_inputs
)
...
def validate_maximum_cycleDuration(cycle: dict):
    ...
    return not cycleDuration or not is_crop or not max_cycleDuration or cycleDuration <= int(max_cycleDuration) or {
        'level': 'error',
        'dataPath': '.cycleDuration',
        'message': f"should be <= {max_cycleDuration}",
        'params': {
            'comparison': '<=',
            'limit': max_cycleDuration,
            'exclusive': False,
