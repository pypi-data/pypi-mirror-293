...
def get_boards(board_name_list, *args, **kwargs):
    """Given a list of boards, return :class:`pysharty.Board` objects.

    Args:
        board_name_list (list): List of board names to get, eg: ['soy', 'pol']

    Returns:
        dict of :class:`pysharty.Board`: Requested boards.
    """
...
def get_all_boards(*args, **kwargs):
    """Returns every board on soyjak.party.

    Returns:
        dict of :class:`pysharty.Board`: All boards.
    """
...
class Board(object):
    """Represents a soyjak.party board.

    Attributes:
        name (str): Name of this board, such as "soy" or "pol".
        title (string): Board title, such as "International Politics".
    ...
    def __init__(self, board_name, https=False, session=None):
        """Creates a :mod:`pysharty.Board` object.

        Args:
            board_name (string): Name of the board, such as "soy" or "pol".
            https (bool): Whether to use a secure connection to soyjak.party.
        ...
        self._requests_session.headers['User-Agent'] = 'py-sharty/%s' % __version__
    ...
    def get_thread(self, thread_id, update_if_cached=True, raise_404=False):
        """Get a thread from soyjak.party via sharty API.
        ...
        Returns:
            :class:`pysharty.Thread`: Thread object
        """
    ...
    def get_threads(self, page=1):
        ...
        Returns:
            list of :mod:`pysharty.Thread`: List of Thread objects representing the threads on the given page.
        """
    ...
    def get_all_threads(self, expand=False):
        ...
        Returns:
            list of :mod:`pysharty.Thread`: List of Thread objects representing every thread on this board.
        """
        if not expand:
