...
from ._helpers import corporation_details_button_html
...
class CorporationListJson(
    ...
    def _render_details(self, row):
        return corporation_details_button_html(row)
    ...
    def filter_queryset(self, qs):
        ...
        qs = self._apply_search_filter(
            qs, 0, "corporation__corporation__corporation_name"
        )

        qs = self._apply_search_filter(
            qs, 1, "corporation__corporation__alliance__alliance_name"
        )
...
@login_required
@permission_required("metenox.basic_access")
def corporation_details(request):
    """Render details of a corporation"""
