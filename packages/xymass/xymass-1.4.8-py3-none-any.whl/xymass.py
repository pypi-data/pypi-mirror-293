...
def sample_imf(size,model,**params):
    ...
    if not 'm_min' in params:
        params['m_min']=0.1*u.M_sun
    if not 'm_max' in params:
        params['m_max']=150.*u.M_sun
    ...
    if model=='salpeter':
        ...
        mass,k_salpeter=sampler.pl(size,params['m_min'].to(params['m_max'].unit).value,params['m_max'].value,params['alpha'])
    ...
    if model=='lognormal':
        ...
        erf1=scipy.special.erf((np.log10(params['mean'])*np.log(10.)-np.log(params['m_min'].to(u.M_sun).value))/np.sqrt(2.)/np.log(10.)/params['std'])
        erf2=scipy.special.erf((np.log10(params['mean'])*np.log(10.)-np.log(params['m_max'].to(u.M_sun).value))/np.sqrt(2.)/np.log(10.)/params['std'])
        ...
        mass=sampler.lognormal(size,params['m_min'].to(params['m_max'].unit).value,params['m_max'].value,params['mean'],params['std'])
    ...
    if model=='kroupa':#sample from kroupa IMF, 3 separate power laws with indices -alpha1, -alpha2, -alpha3, break masses at m1_break and m2_break
        ...
        if not 'm1_break' in params:
            params['m1_break']=0.08*u.M_sun
        if not 'm2_break' in params:
            params['m2_break']=0.5*u.M_sun
