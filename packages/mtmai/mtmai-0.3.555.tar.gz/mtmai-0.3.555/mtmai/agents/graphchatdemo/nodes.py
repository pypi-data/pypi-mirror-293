...
# async def my_generator(state: MainState):
#     messages = [
#         aisdk.data(
#             [
#                 {
...
class Nodes:
    ...
    async def entry(self, state: MainState, config: RunnableConfig):
        ...
        # 初始化客户端状态
        state_ret["uistate"] = UiState(
            items=[UiStateItem(comp_type="msg", props={"content": "content1"})]
        )
    ...
    async def chat_node(self, state: MainState, config: RunnableConfig):
        thread_id = config.get("configurable").get("thread_id")
        messages = state.messages
        logger.info("msgs %s, user_input: %s", len(messages), state.user_input)
        ...
        if user_input == "/1":
            ...
            return {
                "wait_user": True,
                "uiappendcomponents": UiAppendComponent(
                    items=[UiAppendComponentItem(component="GraphFlowView", props={})]
                ),
            }
        # if user_input == "/rag":
        #     # 命令测试
