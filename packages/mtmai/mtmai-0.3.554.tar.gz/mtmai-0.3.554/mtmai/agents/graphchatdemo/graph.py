    ...
    async def chat(
        ...
        async for event in wf.astream_events(
            ...
            if kind == "on_chain_stream":
                if name == "chat":
                    if data:
                        uiStateDelta = data["chunk"].get("uiStateDelta")
                        if uiStateDelta:
                            logger.info("node: {%s}, uiDelta: %s", name, uiStateDelta)
                            yield aisdk.data(
                                {
                                    "dataType": "uiStateDelta",
                                    "data": uiStateDelta,
                                }
                            )
                        append_components = data["chunk"].get("uiappendcomponents")
                        if append_components:
                            yield aisdk.data(
                                {
                                    "dataType": "uiappendcomponents",
                                    "data": append_components,
                                }
                            )
                # if name == "ask_human":
                #     if data:
