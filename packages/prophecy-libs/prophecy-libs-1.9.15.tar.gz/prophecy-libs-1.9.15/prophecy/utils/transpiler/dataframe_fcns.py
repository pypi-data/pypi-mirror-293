...
def parseXML(df, xmlStringColumnName, childName, attributeOrChildValues, spark):
    result = spark.sparkContext._jvm.io.prophecy.alteryx.SparkFunctions.parseXML(df._jdf, xmlStringColumnName, childName, attributeOrChildValues, spark._jsparkSession)
    return DataFrame(result, ScalaUtil.getAbiLib().sqlContext)

def buildJson(df, groupByColumns, nameFieldColumn, valueFieldColumn, outputColumn, spark):
    result = spark.sparkContext._jvm.io.prophecy.alteryx.SparkFunctions.buildJson(df._jdf, groupByColumns, nameFieldColumn, valueFieldColumn, outputColumn, spark._jsparkSession)
    return DataFrame(result, ScalaUtil.getAbiLib().sqlContext)
...
def buildJsonFromPrimitiveCols(df, groupByColumns, nameFieldColumn, stringFieldColumnName, intFieldColumnName, floatFieldColumnName, boolFieldColumnName, outputColumn, spark):
    result = spark.sparkContext._jvm.io.prophecy.alteryx.SparkFunctions.buildJsonFromPrimitiveCols(df._jdf, groupByColumns, nameFieldColumn, stringFieldColumnName, intFieldColumnName, floatFieldColumnName, boolFieldColumnName, outputColumn, spark._jsparkSession)
    return DataFrame(result, ScalaUtil.getAbiLib().sqlContext)
