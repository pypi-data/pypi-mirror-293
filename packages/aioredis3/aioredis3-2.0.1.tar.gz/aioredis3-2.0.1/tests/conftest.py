...
import aioredis3
from aioredis3.client import Monitor
from aioredis3.connection import (
    HIREDIS_AVAILABLE,
    HiredisParser,
    PythonParser,
    parse_url,
...
async def _get_info(redis_url):
    client = aioredis3.Redis.from_url(redis_url)
...
def create_redis(request, event_loop):
    """Wrapper around aioredis3.create_redis."""
    ...
    async def f(url: str = request.config.getoption("--redis-url"), **kwargs):
        ...
        pool = aioredis3.ConnectionPool(parser_class=parser_class, **url_options)
        client: aioredis3.Redis = aioredis3.Redis(connection_pool=pool)
        if single:
        ...
        def teardown():
            async def ateardown():
                ...
                except aioredis3.ConnectionError:
...
async def wait_for_command(client: aioredis3.Redis, monitor: Monitor, command: str):
