...
class TestEncoding:
    ...
    async def test_simple_encoding(self, r_no_decode: aioredis3.Redis):
    ...
    async def test_simple_encoding_and_decoding(self, r: aioredis3.Redis):
        unicode_string = chr(3456) + "abcd" + chr(3421)
    ...
    async def test_memoryview_encoding(self, r_no_decode: aioredis3.Redis):
        unicode_string = chr(3456) + "abcd" + chr(3421)
    ...
    async def test_memoryview_encoding_and_decoding(self, r: aioredis3.Redis):
        unicode_string = chr(3456) + "abcd" + chr(3421)
        unicode_string_view = memoryview(unicode_string.encode("utf-8"))
    ...
    async def test_list_encoding(self, r: aioredis3.Redis):
        unicode_string = chr(3456) + "abcd" + chr(3421)
...
class TestCommandsAreNotEncoded:
    ...
    async def test_basic_command(self, r: aioredis3.Redis):
        await r.set("hello", "world")


class TestInvalidUserInput:
    async def test_boolean_fails(self, r: aioredis3.Redis):
        with pytest.raises(aioredis3.DataError):
            await r.set("a", True)  # type: ignore

    async def test_none_fails(self, r: aioredis3.Redis):
        with pytest.raises(aioredis3.DataError):
            await r.set("a", None)  # type: ignore

    async def test_user_type_fails(self, r: aioredis3.Redis):
        class Foo:
            def __str__(self):
                return "Foo"
