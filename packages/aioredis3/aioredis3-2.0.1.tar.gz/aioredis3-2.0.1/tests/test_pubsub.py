...
class TestPubSubAutoDecoding:
    ...
    async def test_pattern_message_handler(self, r: aioredis3.Redis):
        p = r.pubsub(ignore_subscribe_messages=True)
    ...
    async def test_context_manager(self, r: aioredis3.Redis):
...
class TestPubSubRedisDown:
    async def test_channel_subscribe(self, r):
        r = aioredis3.Redis(host="localhost", port=6390)
        p = r.pubsub()
