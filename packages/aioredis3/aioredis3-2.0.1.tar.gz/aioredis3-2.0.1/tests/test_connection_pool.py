...
import aioredis3
from aioredis3.connection import Connection, to_bool
...
class TestRedisAutoReleaseConnectionPool:
    @pytest.fixture
    async def r(self, create_redis) -> aioredis3.Redis:
    ...
    @staticmethod
    async def create_two_conn(r: aioredis3.Redis):
    ...
    @staticmethod
    def has_no_connected_connections(pool: aioredis3.ConnectionPool):
        return not any(
            x.is_connected
            for x in pool._available_connections + list(pool._in_use_connections)
        )

    async def test_auto_disconnect_redis_created_pool(self, r: aioredis3.Redis):
    ...
    async def test_do_not_auto_disconnect_redis_created_pool(self, r2: aioredis3.Redis):
    ...
    async def test_close_override(self, r: aioredis3.Redis, auto_close_conn_pool):
    ...
    async def test_negate_auto_close_client_pool(
        self, r: aioredis3.Redis, auto_close_conn_pool
    ):
        r.auto_close_connection_pool = auto_close_conn_pool
        new_conn = await self.create_two_conn(r)
...
class TestConnectionPool:
    def get_pool(
        ...
        connection_class=aioredis3.Connection,
    ):
        connection_kwargs = connection_kwargs or {}
        pool = aioredis3.ConnectionPool(
    ...
    async def test_max_connections(self, master_host):
        ...
        await pool.get_connection("_")
        with pytest.raises(aioredis3.ConnectionError):
            await pool.get_connection("_")
    ...
    def test_repr_contains_db_info_tcp(self):
        ...
        pool = self.get_pool(
            connection_kwargs=connection_kwargs, connection_class=aioredis3.Connection
        )
        expected = (
    ...
    def test_repr_contains_db_info_unix(self):
        ...
        pool = self.get_pool(
            ...
            connection_class=aioredis3.UnixDomainSocketConnection,
        )
        expected = (
...
class TestBlockingConnectionPool:
    def get_pool(self, connection_kwargs=None, max_connections=10, timeout=20):
        connection_kwargs = connection_kwargs or {}
        pool = aioredis3.BlockingConnectionPool(
...
class TestConnectionPoolURLParsing:
    def test_hostname(self):
        pool = aioredis3.ConnectionPool.from_url("redis://my.host")
        assert pool.connection_class == aioredis3.Connection
        assert pool.connection_kwargs == {
            "host": "my.host",
        }

    def test_quoted_hostname(self):
        pool = aioredis3.ConnectionPool.from_url("redis://my %2F host %2B%3D+")
        assert pool.connection_class == aioredis3.Connection
        assert pool.connection_kwargs == {
            "host": "my / host +=+",
        }
    ...
    def test_port(self):
        pool = aioredis3.ConnectionPool.from_url("redis://localhost:6380")
        assert pool.connection_class == aioredis3.Connection
        assert pool.connection_kwargs == {
            "host": "localhost",
            "port": 6380,
    ...
    def test_username(self):
        pool = aioredis3.ConnectionPool.from_url("redis://myuser:@localhost")
        assert pool.connection_class == aioredis3.Connection
        assert pool.connection_kwargs == {
            "host": "localhost",
            "username": "myuser",
    ...
    def test_quoted_username(self):
        pool = aioredis3.ConnectionPool.from_url(
    ...
    def test_password(self):
        pool = aioredis3.ConnectionPool.from_url("redis://:mypassword@localhost")
        assert pool.connection_class == aioredis3.Connection
        assert pool.connection_kwargs == {
            "host": "localhost",
            "password": "mypassword",
    ...
    def test_username_and_password(self):
        pool = aioredis3.ConnectionPool.from_url("redis://myuser:mypass@localhost")
        assert pool.connection_class == aioredis3.Connection
        assert pool.connection_kwargs == {
            "host": "localhost",
            "username": "myuser",
    ...
    def test_db_as_argument(self):
        pool = aioredis3.ConnectionPool.from_url("redis://localhost", db=1)
        assert pool.connection_class == aioredis3.Connection
        assert pool.connection_kwargs == {
            "host": "localhost",
            "db": 1,
    ...
    def test_db_in_path(self):
        pool = aioredis3.ConnectionPool.from_url("redis://localhost/2", db=1)
        assert pool.connection_class == aioredis3.Connection
        assert pool.connection_kwargs == {
            "host": "localhost",
            "db": 2,
    ...
    def test_db_in_querystring(self):
        pool = aioredis3.ConnectionPool.from_url("redis://localhost/2?db=3", db=1)
        assert pool.connection_class == aioredis3.Connection
        assert pool.connection_kwargs == {
            "host": "localhost",
            "db": 3,
    ...
    def test_invalid_extra_typed_querystring_options(self):
        with pytest.raises(ValueError):
            aioredis3.ConnectionPool.from_url(
                "redis://localhost/2?socket_timeout=_&" "socket_connect_timeout=abc"
            )

    def test_extra_querystring_options(self):
        pool = aioredis3.ConnectionPool.from_url("redis://localhost?a=1&b=2")
        assert pool.connection_class == aioredis3.Connection
        assert pool.connection_kwargs == {"host": "localhost", "a": "1", "b": "2"}

    def test_calling_from_subclass_returns_correct_instance(self):
        pool = aioredis3.BlockingConnectionPool.from_url("redis://localhost")
        assert isinstance(pool, aioredis3.BlockingConnectionPool)

    def test_client_creates_connection_pool(self):
        r = aioredis3.Redis.from_url("redis://myhost")
        assert r.connection_pool.connection_class == aioredis3.Connection
    ...
    def test_invalid_scheme_raises_error(self):
        with pytest.raises(ValueError) as cm:
            aioredis3.ConnectionPool.from_url("localhost")
        assert str(cm.value) == (
...
class TestConnectionPoolUnixSocketURLParsing:
    def test_defaults(self):
        pool = aioredis3.ConnectionPool.from_url("unix:///socket")
        assert pool.connection_class == aioredis3.UnixDomainSocketConnection
        assert pool.connection_kwargs == {
            "path": "/socket",
        }
    ...
    @skip_if_server_version_lt(REDIS_6_VERSION)
    def test_username(self):
        pool = aioredis3.ConnectionPool.from_url("unix://myuser:@/socket")
        assert pool.connection_class == aioredis3.UnixDomainSocketConnection
        assert pool.connection_kwargs == {
            "path": "/socket",
            "username": "myuser",
    ...
    def test_password(self):
        pool = aioredis3.ConnectionPool.from_url("unix://:mypassword@/socket")
        assert pool.connection_class == aioredis3.UnixDomainSocketConnection
        assert pool.connection_kwargs == {
            "path": "/socket",
            "password": "mypassword",
    ...
    def test_db_as_argument(self):
        pool = aioredis3.ConnectionPool.from_url("unix:///socket", db=1)
        assert pool.connection_class == aioredis3.UnixDomainSocketConnection
        assert pool.connection_kwargs == {
            "path": "/socket",
            "db": 1,
    ...
    def test_db_in_querystring(self):
        pool = aioredis3.ConnectionPool.from_url("unix:///socket?db=2", db=1)
        assert pool.connection_class == aioredis3.UnixDomainSocketConnection
        assert pool.connection_kwargs == {
            "path": "/socket",
            "db": 2,
    ...
    def test_extra_querystring_options(self):
        pool = aioredis3.ConnectionPool.from_url("unix:///socket?a=1&b=2")
        assert pool.connection_class == aioredis3.UnixDomainSocketConnection
...
class TestSSLConnectionURLParsing:
    def test_host(self):
        pool = aioredis3.ConnectionPool.from_url("rediss://my.host")
        assert pool.connection_class == aioredis3.SSLConnection
        assert pool.connection_kwargs == {
            "host": "my.host",
        }
    ...
    def test_cert_reqs_options(self):
        ...
        class DummyConnectionPool(aioredis3.ConnectionPool):
...
class TestConnection:
    async def test_on_connect_error(self):
        ...
        # 9999 databases ;)
        bad_connection = aioredis3.Redis(db=9999)
        # an error should be raised on connect
        with pytest.raises(aioredis3.RedisError):
    ...
    async def test_busy_loading_disconnects_socket(self, r):
        ...
        """
        with pytest.raises(aioredis3.BusyLoadingError):
    ...
    async def test_read_only_error(self, r):
        ...
        with pytest.raises(aioredis3.ReadOnlyError):
            await r.execute_command("DEBUG", "ERROR", "READONLY blah blah")

    def test_connect_from_url_tcp(self):
        connection = aioredis3.Redis.from_url("redis://localhost")
    ...
    def test_connect_from_url_unix(self):
        connection = aioredis3.Redis.from_url("unix:///path/to/socket")
        pool = connection.connection_pool
    ...
    async def test_connect_no_auth_supplied_when_required(self, r):
        ...
        """
        with pytest.raises(aioredis3.AuthenticationError):
