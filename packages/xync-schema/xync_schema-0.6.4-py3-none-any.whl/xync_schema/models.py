...
class Curex(Model):
    ...
    countries: fields.ManyToManyRelation[Country] = fields.ManyToManyField("models.Country", through="curexcountry", backward_key="curexs")
...
class User(BaseUser):  # tg user
    ...
    ref: fields.ForeignKeyNullableRelation["User"] = fields.ForeignKeyField("models.User", related_name="proteges", null=True)
    ref_id: int | None
...
class Pm(Model):
    ...
    exs: fields.ManyToManyRelation[Ex] = fields.ManyToManyField("models.Ex", through="pmex")  # no need. use pmexs[.exid]
    # orders: fields.ReverseRelation["Order"]
...
class Fiat(Model):
    ...
    @classmethod
    def pydListItem(cls, max_recursion: int = 1, backward_relations: bool = False, exclude: tuple[str] = (), include: tuple[str] = (), force: bool = False) -> type[PydanticModel]:
        return super().pydListItem(2, False, ('pmcur__exs', 'user'))
