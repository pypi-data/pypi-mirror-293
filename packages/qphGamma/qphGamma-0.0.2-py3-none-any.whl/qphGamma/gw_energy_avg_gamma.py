...
import vasp_param
import vasp_dos
import fermi_gas_dos
import fermi_gas_params
...
class GWAvgGamma:
    ...
    def __init__(self, file, dos_file=None, n=-1, core_E=-100000) -> None:
        ...
        Args:
            ...
            E_gamma_occ (2D array - nkpts*nbands x 3): GW data of KS energies - Ef, scattering rate, and occupation values sorted in ascending order of KS energies
        ...
        params = vasp_param.VaspParam(file)
        gw_data = gw_gamma.GWGamma(file, self.n, self.core_E)
    ...
    def get_vasp_dos(self):
        ...
        if self.dos_file != None:
            data = vasp_dos.VaspDOS(self.dos_file, self.Te, self.N)
            interp_vasp_dos = interp1d(
                data.E, data.dos, fill_value=(0, np.nan), bounds_error=False
            )
            # mu is set to 0 eV
    ...
    def gw_gamma_occ_vasp_dos(self):
        E_gamma_occ = self.E_gamma_occ
        E = self.E_gamma_occ[:, 0]
        gamma = self.E_gamma_occ[:, 1]
        occ = self.E_gamma_occ[:, 2] / 2
    ...
    def get_fg_dos(self):
        ...
        fg_params = fermi_gas_params.FGParams(self.Te, self.N, self.V)
        fg_dos = fermi_gas_dos.FermiGasDOS(E_range, self.V)
