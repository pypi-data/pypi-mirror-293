...
from app.crud.genericapp.user_dao import UserDao
from app.model.genericapp.user import UserModel
...
class Token(Auth):
    ...
    async def login(self, form_data: OAuth2PasswordRequestForm, request: Request):
        ...
        if id_conn is not None:
            ...
            user = UserModel(key=passwd, username=username)
            id_user = await UserDao().login(user)
    ...
    async def login_web(self, form_data: OAuth2PasswordRequestForm, request: Request):
        ...
        if id_conn is not None:
            ...
            user = UserModel(key=passwd, email=email)
            id_user = await UserDao().login(user)
    ...
    async def _get_token(self, id_user, id_login, id_conn) -> str:
        if id_user is not None:
            ...
            model = UserModel(id=id_user)
            scope = await UserDao().scope(model)
