...
def generate(
    ...
    try:
        if overwrite_cache or overwrite_system:
            generate_cache(schema_file, database_name)

        if overwrite_authorization or overwrite_system:
            generate_authorization(schema_file, database_name)

        if overwrite_token or overwrite_system:
            generate_token(schema_file, database_name)

        if overwrite_audit or overwrite_system:
            generate_audit(schema_file, database_name)

        if overwrite_conn or overwrite_system:
            generate_conn(schema_file, database_name)

        if overwrite_login or overwrite_system:
            generate_login(schema_file, database_name)

        if overwrite_payload or overwrite_system:
            generate_payload(schema_file, database_name)

    except Exception as e:
        print("ERROR: Generator cache -> generate: " + str(e))


def generate_cache(schema_file, database_name, with_listener=False):
    try:
        ...
        path_out = os.path.join(APP_PATH, f"{c.SERVICE}")

        funtionality_name = St.text_to_snake(c.API_FUNTIONALITY)
        role_name = St.text_to_snake(c.API_ROLE)

        funtionality_class = St.text_to_pascal(c.API_FUNTIONALITY)
        role_class = St.text_to_pascal(c.API_ROLE)

        file_name_conn = St.text_to_snake(c.API_CONNECTION)
        class_name_conn = St.text_to_pascal(c.API_CONNECTION)
...
def generate_authorization(schema_file, database_name):
    try:
        path_schema = os.path.join(THIS_PATH, c.ROUTE_SCHEMAS_GENERATOR)

        content = Fl.read_file(path_schema, c.SCHEMA_AUTHORIZATION_FILE)

        path_service = os.path.join(APP_PATH, f"{c.SERVICE}")
        path_middleware = os.path.join(path_service, f"{c.MIDDLEWARE}")
        path_out = os.path.join(path_middleware, f"{c.AUTH}")

        file_name_user = St.text_to_snake(c.API_USER)
        class_name = St.text_to_pascal(c.API_USER)
...
def generate_token(schema_file, database_name):
    try:
        path_schema = os.path.join(THIS_PATH, c.ROUTE_SCHEMAS_GENERATOR)
...
def generate_audit(schema_file, database_name):
    try:
        path_schema = os.path.join(THIS_PATH, c.ROUTE_SCHEMAS_GENERATOR)
        ...
        file_name_request = St.text_to_snake(c.API_AUDIT_REQUEST)
        class_name_request = St.text_to_pascal(c.API_AUDIT_REQUEST)

        file_name_request_not_logged = St.text_to_snake(c.API_AUDIT_REQUEST_NOT_LOGGED)
        class_name_request_not_logged = St.text_to_pascal(
            c.API_AUDIT_REQUEST_NOT_LOGGED
        )

        file_name_response = St.text_to_snake(c.API_AUDIT_RESPONSE)
        class_name_response = St.text_to_pascal(c.API_AUDIT_RESPONSE)
...
def generate_conn(schema_file, database_name):
    try:
        path_schema = os.path.join(THIS_PATH, c.ROUTE_SCHEMAS_GENERATOR)
        ...
        file_name_login = St.text_to_snake(c.API_LOGIN)
        class_name_login = St.text_to_pascal(c.API_LOGIN)
...
def generate_login(schema_file, database_name):
    try:
        path_schema = os.path.join(THIS_PATH, c.ROUTE_SCHEMAS_GENERATOR)
...
def generate_payload(schema_file, database_name):
    try:
        path_schema = os.path.join(THIS_PATH, c.ROUTE_SCHEMAS_GENERATOR)
        ...
        file_name_login = St.text_to_snake(c.API_AUDIT_PAYLOAD)
        class_name_login = St.text_to_pascal(c.API_AUDIT_PAYLOAD)
