...
from genapp.script.generator_ import generate
    ...
    for folder in BASE_STRUCTURE.keys():
        for subfolder in BASE_STRUCTURE[folder]:
            ...
            if os.path.exists(template_folder):
                ...
                click.echo(f"No se encontraron archivos de plantilla en: {template_folder}")
...
def model_update(project_name):
    ...
    if type == "PostgreSQL":
        db_instance = PostgresSQLInstance(username, passwd, host, port, db_name, db_alias)
    ...
    generate(db_instance, project_name, overwrite_model=True, overwrite_schema=True)
...
def set_database(project_name):
    ...
    db_user = click.prompt("Introduce el usuario de la base de datos principal", type=str)
    db_password = click.prompt("Introduce la contraseña de la base de datos principal", type=str)
    db_host = click.prompt("Introduce el host de la base de datos principal", type=str)
    db_port = click.prompt("Introduce el puerto de la base de datos principal", type=str)
    db_name = click.prompt("Introduce el nombre de la base de datos principal", type=str)
    db_alias = click.prompt("Introduce el nombre de fichero para la base de datos principal", type=str)
    ...
    db_type_choice = click.prompt("Introduce el número correspondiente al tipo de base de datos", type=click.Choice(["1", "2", "3"]))
    ...
    with open(env_production_path, "w") as env_file:
        ...
        # Añadir las variables adicionales al archivo .env
