...
from genapp.script.generator.lib.sqlacodegen_v2.models import (
    ColumnAttribute,
    JoinType,
    Model,
    ModelClass,
...
from genapp.script.generator.lib.sqlacodegen_v2.utils import (
...
"""
ATL 20240212: 
- En las relaciones MANY_TO_MANY se hace uso de listas en DeclarativeGenerator
- Las variables de los atributos estaran en formato snake_case
...
class TablesGenerator(CodeGenerator):
    valid_options: ClassVar[set[str]] = {
        "noindexes", "noconstraints", "nocomments"}
    ...
    def render_column(
        ...
        if isinstance(column.server_default, DefaultClause):
            ...
            args.append(
                render_callable("Computed", repr(expression),
                                kwargs=computed_kwargs)
            )
    ...
    def render_constraint(self, constraint: Constraint | ForeignKey) -> str:
        ...
        if isinstance(constraint, ForeignKey):
            ...
            args.append(repr(get_compiled_expression(
                constraint.sqltext, self.bind)))
    ...
    def fix_column_types(self, table: Table) -> None:
        ...
        for constraint in table.constraints.copy():
            if isinstance(constraint, CheckConstraint):
                sqltext = get_compiled_expression(
                    constraint.sqltext, self.bind)
                ...
                if match:
                    ...
                    if colname_match:
                        colname = colname_match.group(3)
                        items = match.group(2)
                        # Cambio de table.c[colname].type a type(table.c[colname])
                        if isinstance(type(table.c[colname]), String):
                            table.constraints.remove(constraint)
...
class DeclarativeGenerator(TablesGenerator):
    ...
    def generate_relationships(
        ...
        pk_column_names = {
            col.name for col in source.table.primary_key.columns}
    ...
    def generate_model_name(self, model: Model, global_names: set[str]) -> None:
        if isinstance(model, ModelClass):
            ...
            # )
            preferred_name = St.text_to_snake(model.table.name)
            preferred_name = St.text_to_pascal(preferred_name)
    ...
    def generate_column_attr_name(
        ...
        # Text to snake for uppercase name attributes
        # ATL
    ...
    def render_relationship(self, relationship: RelationshipAttribute) -> str:
        ...
        # Many to Many = List
        # ATL
        elif relationship.type is RelationshipType.MANY_TO_MANY:
            kwargs["uselist"] = True
        #
...
class DataclassGenerator(DeclarativeGenerator):
    ...
    def render_relationship(self, relationship: RelationshipAttribute) -> str:
        rendered = super().render_relationship(
            relationship).partition(" = ")[2]
        kwargs: dict[str, Any] = {}
