...
from app.crud.genericapp.connection_dao import ConnectionDao
from app.crud.genericapp.funtionality_dao import FuntionalityDao
...
from app.model.genericapp.connection import (
    ConnectionModel,
)
...
class Conn:
    ...
    async def _update_conn(ip, id_conn) -> None:
        try:
            ...
            await ConnectionDao().update(dto)
    ...
    async def _new_conn_id(ip) -> int:
        try:
            ...
            conn = await ConnectionDao().insert(dto)
    ...
    async def analyze_request_not_logged(request, id_conn) -> None:
        try:
            dto = AuditRequestNotLoggedModel(id_conn=id_conn)
            firts_attemp = datetime.utcnow() - timedelta(
                seconds=Conn.ANALYZE_REQUEST_NOT_LOGGED_TIME
            )

            attempts = await AuditRequestNotLoggedDao().last_requests_not_logged(
                dto, firts_attemp
            )

            attempts = Conn._filter_distinct_attemps(attempts, ConnKeys.URL)

            funtionalities = await FuntionalityDao().get_no_auth_funtionalities()
    ...
    async def analyze_login(request, id_conn) -> None:
        try:
            dto = LoginModel(id_conn=id_conn)
            firts_attemp = datetime.utcnow() - timedelta(
                seconds=Conn.ANALYZE_LOGIN_TIME
            )

            attempts, _ = await LoginDao().last_logins(dto, firts_attemp)
    ...
    async def block_conn(request, id_conn, reason=None) -> None:
        ...
        try:
            ...
            if conn[0] == ConnKeys.ALLOW:
                ...
                dto = ConnectionModel(id=id_conn, block=ConnKeys.BLOCKED, reason=reason)
                await ConnectionDao().update(dto)
