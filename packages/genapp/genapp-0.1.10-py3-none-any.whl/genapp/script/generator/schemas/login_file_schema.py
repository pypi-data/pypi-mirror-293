...
class Login:
    ...
    async def audit(id_conn, status, request, username, passwd, id_user=None):
        try:
            ...
            login = await API_LOGIN_CLASS_KEYDao().insert(dto)
    ...
    async def new_conn(request, id_login, new_id_conn):
        try:
            dto = API_LOGIN_CLASS_KEYModel(id=id_login)
            old_login, _ = await API_LOGIN_CLASS_KEYDao().query(dto)
