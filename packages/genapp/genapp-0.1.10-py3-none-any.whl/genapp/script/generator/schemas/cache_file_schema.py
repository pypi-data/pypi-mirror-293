...
from app.CRUD_KEY.DATABASE_NAME_KEY.API_ROLE_KEY_dao import API_ROLE_CLASS_KEYDao

from app.CRUD_KEY.SCHEMA_KEY.SCHEMA_DATABASE_KEY import API_FUNTIONALITY_CLASS_KEY
...
async def load_connection_data():
    try:
        ...
        result = await API_CONNECTION_CLASS_KEYDao().get_last_connections(
            timestamp_delta
        )
...
async def update_role_permissions() -> bool:
    try:
        result = await API_ROLE_CLASS_KEYDao().get_all_permissions()
...
async def update_role_scope() -> bool:
    try:
        result = await API_ROLE_CLASS_KEYDao().get_all_scopes()
...
async def update_funtionality_permissions() -> bool:
    try:
        result = await API_FUNTIONALITY_CLASS_KEYDao().get_all_permissions()
    ...
    try:
        ...
        with funtionalities_cache_lock:
            ...
            for i, key in enumerate(keys):
                if key in data:
                    data = data.get(key)
                    if isinstance(data, API_FUNTIONALITY_CLASS_KEY):
                        if i + 1 < len(
                            keys
...
async def update_funtionalities() -> bool:
    try:
        result = await API_FUNTIONALITY_CLASS_KEYDao().get_all_funtionalities()
