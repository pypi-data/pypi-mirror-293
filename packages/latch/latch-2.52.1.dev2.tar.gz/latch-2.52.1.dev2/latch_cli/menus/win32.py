...
def raw_input(f: Callable[P, T]) -> Callable[P, T]:
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        ...
        # dont print characters read to console screen
        enable_echo_input = 0b0100
        # put characters in the buffer as they are typed instead of on carriage return
        enable_line_input = 0b0010
        # dont process ctrl + c, etc., just put it in the buffer
        enable_processed_input = 0b0001
...
def select_tui(
    ...
    def render(
        ...
        for i in range(start, start + max_per_page):
            if i >= len(options):
                break
            ...
            if i == cur:
                ...
                old = set_attrs()
                common.print(f"{prefix}{name}")
                reset_attrs(old)
            else:
                common.print(f"{indent}{name}")
