...
def get_text_list_length(text_list):
    l = len(''.join(text_list))
    return l


def cut_by_time(text, admission_time="2024-01-01 00:00:00", avg_len=256):
    ...
    for start_index, end_index, seg in seg_list:
        ...
        if timeline:
            results.append((timeline, [seg], start_index))
        else:
            if results and get_text_list_length(results[-1][1]) < avg_len:
                results[-1][1].append(seg)
            else:
                results.append((last_timeline, [seg], start_index))
        if end_index < len(text):
            results[-1][1].append(text[end_index])
    return results
