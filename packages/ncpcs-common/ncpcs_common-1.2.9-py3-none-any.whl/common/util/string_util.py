...
def cut_by_time(text, admission_time="2024-01-01 00:00:00", avg_len=256):
    ...
    for start_index, end_index, seg in seg_list:
        ...
        if timeline and timeline.base_time_convert:
            ...
            if not results or len(results[-1][1]) < avg_len:
                results[-1][1].append(seg)
            else:
                results.append((None, [seg], start_index))
...
for timeline, text_list, start_index in cut_by_time(content, '2020-12-01 00:00:00'):
    if timeline:
        print("原时间：" + timeline.base_time + ", 转换时间：" + timeline.base_time_convert)
    text = ''.join(text_list)
    print(text)
    print(content[start_index: start_index + len(text)])