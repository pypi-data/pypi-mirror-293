...
def ji(a='',mode='',c=ct.jiset,end='\n'):
    ...
    if mode =='':
        ...
        info = inspect.getframeinfo(frame)
        if 'f' in mode:
            print(ji_round[a][1]+f'At Line {info.lineno} '+f"Tag '{a}' Passed : "+str(ji_round[a][0])+f" From {info.filename}"+ct.set,end=end)
        else:
            print(ji_round[a][1]+f'At Line {info.lineno} '+f"Tag '{a}' Passed : "+str(ji_round[a][0])+ct.set,end=end)
