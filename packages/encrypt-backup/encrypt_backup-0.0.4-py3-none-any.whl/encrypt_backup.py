...
def compress_and_encrypt_folder(backup_file_path, encrypted_7zip_path, public_key_path, base_path=None):
    ...
    with open("backup/backup_info.json", "w") as f:
        backup_data = {
            "raw_folder": backup_folder,
            "version": 4
        }
        f.write(str(backup_data))
    zip_command = ['7z', 'a', '-t7z', '-mx=9', 'tmp.7z', backup_folder]
...
def restore_backup_file(encrypted_7zip_path, private_key_path):
    ...
    backup_info = json.loads(open("backup/backup_info.json", "r").read())
    if backup_info.get("version", 0) < 4:
        __invalid_data_warning()
