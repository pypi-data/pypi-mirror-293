...
def _add_ranking_column(df):
    ...
    alignment_scores = df.get('alignment_score')
    alignment_norm = (alignment_scores - alignment_scores.mean()) / alignment_scores.std()
    alignment_rank = alignment_scores.rank(method='average', ascending=False, na_option='bottom')

    propriety_scores = df.get('propriety_score')
    propriety_norm = (propriety_scores - propriety_scores.mean()) / propriety_scores.std()
    propriety_rank = propriety_scores.rank(method='average', ascending=False, na_option='bottom')

    norm = elo_norm * 0.65 + alignment_norm * 0.35
    ranking = norm.rank(method='average', ascending=False, na_option='bottom')

    alt_model_ranking = (elo_rank + alignment_rank).rank(method='average', ascending=True, na_option='bottom')
