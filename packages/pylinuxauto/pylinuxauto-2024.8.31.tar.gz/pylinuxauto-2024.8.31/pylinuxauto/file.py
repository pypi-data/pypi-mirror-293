...
def __check_not_includes(
        not_includes, endwith, file, files_list, include, startwith, abs_path, root
):
    ...
    if not_includes:
        for not_include in not_includes:
            if not_include in file:
                continue
            __check_startwith(endwith, file, files_list, include, startwith, abs_path, root)
    else:
        __check_startwith(endwith, file, files_list, include, startwith, abs_path, root)


def __check_startwith(endwith, file, files_list, include, startwith, abs_path, root):
    """Filter criteria for function 'find_files' parameter 'startwith'"""
    if not startwith:
        __check_endwith(endwith, file, files_list, include, abs_path, root)
    else:
        if file.startswith(startwith):
            __check_endwith(endwith, file, files_list, include, abs_path, root)


def __check_endwith(endwith, file, files_list, include, abs_path, root):
    """Filter criteria for function 'find_files' parameter 'endwith'"""
    if not endwith:
        __check_include(file, files_list, include, abs_path, root)
    else:
        if file.endswith(endwith):
            __check_include(file, files_list, include, abs_path, root)


def __check_include(file, files_list, include, abs_path, root):
    """Filter criteria for function 'find_files' parameter 'include'"""
    if not include:
        __check_abspath(file, files_list, abs_path, root)
    else:
        if include in file:
            __check_abspath(file, files_list, abs_path, root)


def __check_abspath(file, files_list, abs_path, root):
...
def file_name_and_format(position, _format=None):
    ...
    :return: 名称和格式
    """
    files = find_files(path=position, endwith=_format)
    for file in files:
