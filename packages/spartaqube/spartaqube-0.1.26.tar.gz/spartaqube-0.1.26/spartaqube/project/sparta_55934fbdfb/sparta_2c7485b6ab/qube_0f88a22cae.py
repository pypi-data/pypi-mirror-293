...
from project.sparta_b972c86658.sparta_741716a68d.qube_4982ec8d00 import sparta_b18959185f
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_812851d531(json_data,user_obj):
...
def sparta_b3f2d6b889(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_c78218442a(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_b18959185f(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_b18959185f(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_e3a3537f62(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_b18959185f(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_7984cac64c(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_b3f2d6b889(F,A.type_ticket,None)
...
def sparta_6372226b78(json_data,user_obj):
	...
	return{_A:1}
def sparta_a89b9564a3(json_data):
	...
	return{_A:1}
def sparta_64510e0deb(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}