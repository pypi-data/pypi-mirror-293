...
from project.sparta_55934fbdfb.sparta_4a675430f1 import qube_7d653d27fe as qube_7d653d27fe
from project.sparta_55934fbdfb.sparta_09f9b5034d.qube_8780a55f46 import Email as Email
def sparta_091e7a614b(function):
	def A(request,*E,**C):
		A=request;B=_B
		...
		if not B:
			D=C.get(_H)
			if D is not None:F=qube_7d653d27fe.sparta_8e6096dfe6(D);login(A,F)
		return function(A,*E,**C)
	return A
def sparta_2dd23776f5(function):
	def A(request,*C,**D):
	...
	return A
def sparta_cc30a05535(function):
	def A(request,*B,**C):
	...
	return A
def sparta_896ea4d199(function):
	C=function
	def A(request,*D,**E):
		A=request;F=_C
		try:
			G=json.loads(A.body);H=json.loads(G[_I]);I=H[_H];B=qube_7d653d27fe.sparta_8e6096dfe6(I)
...
def sparta_c13c6dc4e0():A=socket.gethostname();B=socket.gethostbyname(A);return B
def sparta_a5a9d5cd95(json_data):
	D='ip_addr';A=json_data;del A[_J];del A[_K]
	try:A[D]=sparta_c13c6dc4e0()
	except:A[D]=-1
...
def sparta_be3ec3bff5():return conf_settings.IS_GUEST_CODE_REQUIRED
def sparta_811ebe5391(guest_code):
	if GuestCodeGlobal.objects.filter(guest_id=guest_code,is_active=_B).count()>0:return _B
	return _C
def sparta_5f467f9067(guest_code,user_obj):
	...
	return _C
def sparta_5ad8bf48b5(user):
	A=UserProfile.objects.filter(user=user)
	if A.count()==1:return A[0].is_banned
	else:return _C
def sparta_d93b29300c(email,captcha):
	D=sparta_d26395362f(captcha)
	if D[_A]!=1:return{_A:-1,_D:_G}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_L}
	A=B[0];C=str(uuid.uuid4());A.token_reset_password=C;A.save();sparta_cb4688196a(A.user,C);return{_A:1}
def sparta_cb4688196a(user_obj,token_reset_password):B=user_obj;A=Email(B.username,[B.email],'Reset Password','Reset Password Message');A.addOneRow('Reset code','Copy the following code to reset your password');A.addSpaceSeparator();A.addOneRow(token_reset_password);A.send()
def sparta_7bb7bff37c(captcha,token,email,password):
	D=sparta_d26395362f(captcha)
	if D[_A]!=1:return{_A:-1,_D:_G}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_L}
