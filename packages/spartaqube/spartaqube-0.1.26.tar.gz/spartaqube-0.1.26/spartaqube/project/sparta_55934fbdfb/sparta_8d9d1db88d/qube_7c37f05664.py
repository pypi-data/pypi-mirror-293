...
from project.sparta_55934fbdfb.sparta_d2d53723e4 import qube_6d51e15bd2
from project.sparta_55934fbdfb.sparta_8d9d1db88d import qube_003b77d1ed as qube_003b77d1ed
from project.sparta_55934fbdfb.sparta_d2d53723e4.qube_856bc612d1 import Connector as Connector
from project.sparta_55934fbdfb.sparta_ce3abe84ce.qube_e97887f5fc import sparta_9bc2dc9979,sparta_865a89042b
from project.sparta_55934fbdfb.sparta_8d9d1db88d import qube_a7e7841ac9 as qube_a7e7841ac9
...
def sparta_1e883729ad(user_obj):
	A=qube_73cd480ba9.sparta_e5902ed20f(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_fbb179184d(json_data,user_obj):
	D=user_obj;E=sparta_1e883729ad(D)
...
def sparta_5646f35edb():return{_A:1,'available_engines':qube_6d51e15bd2.sparta_5646f35edb()}
def sparta_ad5d205b73(json_data,user_obj):
	...
	return{_A:1}
def sparta_5daaeb17f8(json_data):
...
def sparta_e640e29f76(json_data):
...
def sparta_944830ad3a(json_data,user_obj):A=json_data;B=datetime.now().astimezone(UTC);C=str(uuid.uuid4());D=DBConnector.objects.create(connector_id=C,host=A[_S],port=A[_T],user=A[_U],password_e=qube_6d51e15bd2.sparta_a9a113348c(A[_k]),database=A[_V],oracle_service_name=A[_W],keyspace=A[_X],library_arctic=A[_Y],database_path=A[_Z],csv_path=A[_a],csv_delimiter=A[_b],read_only=A[_c],json_url=A[_d],socket_url=A[_e],redis_db=A[_f],token=A[_l],organization=A[_m],lib_dir=A[_n],driver=A[_o],trusted_connection=A[_p],dynamic_inputs=json.dumps(A[_I]),py_code_processing=A[_g],db_engine=A[_N],name=A[_J],description=A[_K],date_created=B,last_update=B,last_date_used=B);E=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);DBConnectorUserShared.objects.create(db_connector=D,user=user_obj,date_created=B,share_rights=E,is_owner=_C);return{_A:1}
def sparta_f3ebeca219(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_1e883729ad(C)
...
def sparta_b472d21afd(json_data,user_obj):
	C=json_data[_H];A=sparta_bf27d23e57(C,user_obj)
...
def sparta_87f617e05b(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_q];D=[];E=sparta_bf27d23e57(H,user_obj)
...
def sparta_11a7911226(json_data,db_connector_obj):
...
def sparta_ab3b21dc94(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_q,'0'))==1;B=sparta_bf27d23e57(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_11a7911226(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_z]
	...
	return{_A:1,_E:sparta_865a89042b(E)}
def sparta_6e4d22711f(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_q,_D);K=A.get(_N,_B);B=sparta_bf27d23e57(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_11a7911226(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
...
def sparta_9c68632c04(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_3f676c746c(json_data,user_obj):
	A=json_data;print('json_data load_table_preview_explorer');print(A);D=A[_H];F=A.get(_O,_B);G=int(A.get(_q,'0'))==1;B=sparta_bf27d23e57(D,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_11a7911226(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_E:sparta_865a89042b(E)}
def sparta_f10d00ce6d(json_data,user_obj):
	A=json_data;J=A[_L];D=_B
	if J:D=A[_t];D=qube_003b77d1ed.sparta_37618ee279(D)
...
def sparta_68d798e189(json_data,user_obj):
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if J.is_admin or J.has_write_rights:
				M=B[_L];C=_B
				if M:C=B[_t];C=qube_003b77d1ed.sparta_37618ee279(C)
	...
	return{_A:1}
def sparta_ebf36ef0c5(json_data,user_obj):0
def sparta_5dc2dc817e(json_data,user_obj):
	D=user_obj;F=sparta_1e883729ad(D)
...
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables):
	...
	return B
def sparta_8216981f24(json_data,user_obj):
			...
			if N.count()>0:
				...
				if b:0
					...
					for B in A.data_source_list:
						...
						if I:H.append(B[Z])
							...
							if _I in B:
								c=B[_H];Q=sparta_bf27d23e57(c,P);R=[]
				...
				if len(H)>0:
					if E is not _B:
						...
						for B in A.data_source_list:
							I=B[Y]
							if I:W=B[Z];i=h[W];B[_E]=sparta_865a89042b(sparta_9bc2dc9979(i,variable_name=W))
...
def sparta_d6dc1c9116(json_data,user_obj):
	...
	return{_A:1}
def sparta_657c72ac83(session_id):
	...
	for(A,F)in E.items():
		...
		if C is not _B:
			...
			if A==_R:B[A]=D
			else:B[A]=sparta_865a89042b(sparta_9bc2dc9979(D,variable_name=A))
	return B
def sparta_e29e444c40(session_id):A=sparta_657c72ac83(session_id);B=sparta_6b4a00247d(b_return_type_id=_C);C=json.loads(A[_AS])[_E][0][0];D=[A for A in B if A['ID']==C][0][_AT];return{_A:1,_AU:A,_AV:A.get(_R,dict()),_Q:D}
def sparta_9c4dda4311(user_obj,session_id,widget_id):
	...
	if E.count()>0:
		...
		if not C:
			...
			if H.count()>0:C=_C
	if C:B=A.chart_params;B[_R]=json.loads(B[_R]);B[_AB]=json.loads(B[_E])[_AB];F=sparta_657c72ac83(session_id);I=sparta_6b4a00247d(b_return_type_id=_C);J=json.loads(F[_AS])[_E][0][0];K=[A for A in I if A['ID']==J][0][_AT];L=F.get(_R,dict());B[_R].update(L);return{_A:1,_AU:F,_AV:B,_Q:K}
	else:return{_A:-1,_F:'You do not have access to this template'}
def sparta_2b10ce7d58(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_1e883729ad(K)
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_003b77d1ed.sparta_4d3134ef4d(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_36d79254f5(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_M:A}
					...
					try:
						if qube_003b77d1ed.sparta_37618ee279(F)==A.widget_password_e:return{_A:1,_M:A}
						else:return{_A:3,_F:_AA,_M:A}
					except:return{_A:3,_F:_AA,_M:A}
			elif C.is_authenticated:
				G=sparta_1e883729ad(C)
	...
	return{_A:-1}
def sparta_8b51600314(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_1e883729ad(C)
	...
	return{_A:1,_i:_D}
def sparta_2b13b37b74(plot_db_chart_obj):
	...
	return C
def sparta_0d2f36e07d(json_data,user_obj):
	B=user_obj;D=sparta_1e883729ad(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_L];C=_B
				if N:C=B[_t];C=qube_003b77d1ed.sparta_37618ee279(C)
...
def sparta_e3ca4ef370(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_L];C=_B
				if K:C=B[_t];C=qube_003b77d1ed.sparta_37618ee279(C)
	...
	return{_A:1}
def sparta_1c0092a603(json_data,user_obj):
	...
	return{_A:1}
def sparta_625f42cd1a(user_obj):
	B=user_obj;C=sparta_1e883729ad(B)
	...
	return E
def sparta_a206b39a62(json_data,user_obj):
	try:A=sparta_8b51600314(json_data[_AW],user_obj);B=A[_i];return{_A:1,_i:B}
	except:pass
	return{_A:-1}
def sparta_5143b7af25(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_8b51600314(A[_AW],B);D=C[_i]
	if D:E=C[_M];A[_G]=E.plot_chart_id;F=sparta_8216981f24(A,B);return{_A:1,_E:[A[_E]for A in F[_A9]]}
	return{_A:-1}
def sparta_428e558f44(json_data,user_obj):
	B='Error quantstats';A=json_data;C=A['service']
	if C=='quantstats':
		try:return qube_a7e7841ac9.sparta_c90d5992aa(A,user_obj)
		except Exception as D:print(B);print(traceback.format_exc());print(B);return{_A:-1,_F:str(D)}
	return{_A:1}
def sparta_997830b331(json_data,user_obj):
...
def sparta_ab6b640aba(json_data,user_obj):
