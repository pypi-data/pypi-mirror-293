...
def sparta_0014aab5a8(textOutputArr):
	A=textOutputArr
	...
	return A
def sparta_ceefd38be4(input2JsonEncode,dateFormat=_A):
...
def sparta_e4ea52b4ed(file_path):
	A=file_path
...
def sparta_ca96cb7ff5(df):
	...
	return E
def sparta_865a89042b(data_df):return sparta_ca96cb7ff5(data_df)
def sparta_9bc2dc9979(input_data,variable_name=_A):
	B=variable_name;A=sparta_8c765dffed(input_data)
	if A is not _A:
...
def sparta_8c765dffed(input_data):
	...
	try:
		...
		if isinstance(A,(list,tuple)):
			if len(A)>0:
				if not sparta_75bef6607c(A[0]):
					A=[sparta_8c765dffed(A)for A in A];C=pd.concat(A,axis=1)
					if len(C.columns)!=len(set(C.columns)):C=sparta_5137455286(C)
					return C
			return pd.DataFrame(A)
		if A.__class__.__name__.lower()=='ndarray':return pd.DataFrame(A)
		if sparta_75bef6607c(A):return pd.DataFrame([A])
