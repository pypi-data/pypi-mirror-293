...
def sparta_387010c313(is_owner=_A):return{_F:is_owner,_B:_E,_C:_E,_D:_E}
def sparta_eae9a067e8():return{_F:_A,_B:_A,_C:_A,_D:_A}
def sparta_a1e142e465(user_obj,portfolio_obj):
	B=portfolio_obj;A=user_obj
	if B.user==A:return sparta_387010c313(_E)
	F=qube_73cd480ba9.sparta_e5902ed20f(A);E=[A.userGroup for A in F]
	...
	if D.count()==0:return sparta_eae9a067e8()
	G=D[0];C=G.ShareRights
	if C.is_delete:return sparta_eae9a067e8()
	return{_F:_A,_B:C.is_admin,_C:C.has_write_rights,_D:C.has_reshare_rights}
def sparta_fc639f7b98(user_obj,universe_obj):
	B=universe_obj;A=user_obj
	if B.user==A:return sparta_387010c313()
	F=qube_73cd480ba9.sparta_e5902ed20f(A);E=[A.userGroup for A in F]
