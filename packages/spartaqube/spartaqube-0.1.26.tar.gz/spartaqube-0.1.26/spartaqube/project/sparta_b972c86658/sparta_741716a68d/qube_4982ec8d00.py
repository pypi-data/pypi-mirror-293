...
from spartaqube_app.secrets import sparta_b5e7d19668
from project.sparta_55934fbdfb.sparta_e88354acd0.qube_40f43a81f9 import sparta_aa15c07b10
import pytz
UTC=pytz.utc
class dotdict(dict):__getattr__=dict.get;__setattr__=dict.__setitem__;__delattr__=dict.__delitem__
def sparta_b18959185f(appViewsModels):
	A=appViewsModels
	...
	return A
def sparta_f8d0732994(thisText):A=os.path.dirname(os.path.dirname(os.path.abspath(__file__)));A=A+str('/log/log.txt');B=open(A,'a');B.write(thisText);B.writelines('\n');B.close()
def sparta_1f2805a4b9(request):A=request;return{'appName':'Project','user':A.user,'ip_address':A.META['REMOTE_ADDR']}
def sparta_b5922e623f():return conf_settings.PLATFORM
def sparta_1fd141aa50():
...
def sparta_48cfdf4b1d(request):
	...
	try:
		...
		if G.count()==0:AppVersioning.objects.create(last_check_date=E);A=_A
			...
			D=G[0];H=D.last_check_date;I=E-H;J=sparta_aa15c07b10();K=D.last_available_version_pip
			if not J==K:A=_A
...
def sparta_d26395362f(captcha):
	D='errorMsg';B='res';A=captcha
	try:
		if A is not _C:
			if len(A)>0:
				E=sparta_b5e7d19668()['CAPTCHA_SECRET_KEY'];F=f"https://www.google.com/recaptcha/api/siteverify?secret={E}&response={A}";C=requests.get(F)
				if int(C.status_code)==200:
					G=json.loads(C.text)
					if G['success']:return{B:1}
	except Exception as H:return{B:-1,D:str(H)}
	return{B:-1,D:'Invalid captcha'}
def sparta_17f17b601c(password):
...
def sparta_6204b13016(user):
	...
	A[_D]=sparta_1fd141aa50();return A
def sparta_0c29dc6fd7(user):A=dict();A[_D]=sparta_1fd141aa50();return A
def sparta_48dc756841():
	try:socket.create_connection(('1.1.1.1',53));return _A
	except OSError:pass
	return _B
def sparta_a43e70f682():A=socket.gethostname();B=socket.gethostbyname(A);return A,B