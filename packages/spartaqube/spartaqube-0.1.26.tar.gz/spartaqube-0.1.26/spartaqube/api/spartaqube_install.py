...
def is_asgi_available(port:int) -> bool:
    try:
        # Create a socket object
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            # Try to connect to the specified port
...
def is_asgi_busy(port:int) -> bool:
    return not is_asgi_available(port)
...
def get_default_port() -> int:
    try:
        ...
        return int(loaded_data_dict['default_port'])
    except:
        return 8664
    
def get_default_asgi_port() -> int:
    try:
        current_path = os.path.dirname(__file__)
...
def start_asgi_server(asgi_port=None, silent=False, is_blocking=True):
    ...
    while True:
        ...
        if is_asgi_available(asgi_port):
            break
...
def start_server(port, silent=False, b_open_browser=False, is_blocking=True):
    '''
    runserver at port
    '''
...
def entrypoint(port=None, force_startup=False, asgi_port=None, silent=False, b_open_browser=False):
    '''
    ...
    if asgi_port is None:
        asgi_port = get_default_asgi_port()
