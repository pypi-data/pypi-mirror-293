...
from project.sparta_c8e521c5f3.sparta_1506e63207 import qube_e6cec87b28 as qube_e6cec87b28
...
def sparta_afd1c616b4(user_obj):
	A=qube_05804de984.sparta_598db0393a(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_24675792fb(json_data,user_obj):
	D=user_obj;E=sparta_afd1c616b4(D)
...
def sparta_3cdd98a293():return{_A:1,'available_engines':qube_a61878050d.sparta_3cdd98a293()}
def sparta_e673a5ef59(json_data,user_obj):
	...
	return{_A:1}
def sparta_00e135be65(json_data):
...
def sparta_d97b6e3798(json_data):
...
def sparta_e815e8134d(json_data,user_obj):A=json_data;B=datetime.now().astimezone(UTC);C=str(uuid.uuid4());D=DBConnector.objects.create(connector_id=C,host=A[_S],port=A[_T],user=A[_U],password_e=qube_a61878050d.sparta_acabf12e6f(A[_k]),database=A[_V],oracle_service_name=A[_W],keyspace=A[_X],library_arctic=A[_Y],database_path=A[_Z],csv_path=A[_a],csv_delimiter=A[_b],read_only=A[_c],json_url=A[_d],socket_url=A[_e],redis_db=A[_f],token=A[_l],organization=A[_m],lib_dir=A[_n],driver=A[_o],trusted_connection=A[_p],dynamic_inputs=json.dumps(A[_I]),py_code_processing=A[_g],db_engine=A[_N],name=A[_J],description=A[_K],date_created=B,last_update=B,last_date_used=B);E=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);DBConnectorUserShared.objects.create(db_connector=D,user=user_obj,date_created=B,share_rights=E,is_owner=_C);return{_A:1}
def sparta_b354048186(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_afd1c616b4(C)
...
def sparta_b3d002ab52(json_data,user_obj):
	C=json_data[_H];A=sparta_e683178a4a(C,user_obj)
...
def sparta_9c339bca74(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_q];D=[];E=sparta_e683178a4a(H,user_obj)
...
def sparta_ecc9e20039(json_data,db_connector_obj):
...
def sparta_739e145143(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_q,'0'))==1;B=sparta_e683178a4a(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_ecc9e20039(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_z]
	...
	return{_A:1,_E:sparta_9e6cb636e2(E)}
def sparta_d20fa4a62c(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_q,_D);K=A.get(_N,_B);B=sparta_e683178a4a(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_ecc9e20039(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
...
def sparta_d74f05b4eb(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_3bc61f3894(json_data,user_obj):
	A=json_data;print('json_data load_table_preview_explorer');print(A);D=A[_H];F=A.get(_O,_B);G=int(A.get(_q,'0'))==1;B=sparta_e683178a4a(D,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_ecc9e20039(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_E:sparta_9e6cb636e2(E)}
def sparta_14bf2b10cd(json_data,user_obj):
	A=json_data;J=A[_L];D=_B
	if J:D=A[_t];D=qube_e74e1c855f.sparta_153392700c(D)
...
def sparta_7af3d503c7(json_data,user_obj):
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if J.is_admin or J.has_write_rights:
				M=B[_L];C=_B
				if M:C=B[_t];C=qube_e74e1c855f.sparta_153392700c(C)
	...
	return{_A:1}
def sparta_4b9d0220df(json_data,user_obj):0
def sparta_48068218f0(json_data,user_obj):
	D=user_obj;F=sparta_afd1c616b4(D)
...
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables):
	...
	return B
def sparta_61b8785673(json_data,user_obj):
			...
			if N.count()>0:
				...
				if b:0
					...
					for B in A.data_source_list:
						...
						if I:H.append(B[Z])
							...
							if _I in B:
								c=B[_H];Q=sparta_e683178a4a(c,P);R=[]
				...
				if len(H)>0:
					if E is not _B:
						...
						for B in A.data_source_list:
							I=B[Y]
							if I:W=B[Z];i=h[W];B[_E]=sparta_9e6cb636e2(sparta_321a3dbf8f(i,variable_name=W))
...
def sparta_3f3c197267(json_data,user_obj):
	...
	return{_A:1}
def sparta_ecbe2717de(session_id):
	...
	for(A,F)in E.items():
		...
		if C is not _B:
			...
			if A==_R:B[A]=D
			else:B[A]=sparta_9e6cb636e2(sparta_321a3dbf8f(D,variable_name=A))
	return B
def sparta_3048b692d7(session_id):A=sparta_ecbe2717de(session_id);B=sparta_fff1b75e5e(b_return_type_id=_C);C=json.loads(A[_AS])[_E][0][0];D=[A for A in B if A['ID']==C][0][_AT];return{_A:1,_AU:A,_AV:A.get(_R,dict()),_Q:D}
def sparta_1855e57519(user_obj,session_id,widget_id):
	...
	if E.count()>0:
		...
		if not C:
			...
			if H.count()>0:C=_C
	if C:B=A.chart_params;B[_R]=json.loads(B[_R]);B[_AB]=json.loads(B[_E])[_AB];F=sparta_ecbe2717de(session_id);I=sparta_fff1b75e5e(b_return_type_id=_C);J=json.loads(F[_AS])[_E][0][0];K=[A for A in I if A['ID']==J][0][_AT];L=F.get(_R,dict());B[_R].update(L);return{_A:1,_AU:F,_AV:B,_Q:K}
	else:return{_A:-1,_F:'You do not have access to this template'}
def sparta_ae3c9d4709(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_afd1c616b4(K)
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_e74e1c855f.sparta_6c1f361bd7(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_ed37f36c8a(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_M:A}
					...
					try:
						if qube_e74e1c855f.sparta_153392700c(F)==A.widget_password_e:return{_A:1,_M:A}
						else:return{_A:3,_F:_AA,_M:A}
					except:return{_A:3,_F:_AA,_M:A}
			elif C.is_authenticated:
				G=sparta_afd1c616b4(C)
	...
	return{_A:-1}
def sparta_f1f3eaa0f3(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_afd1c616b4(C)
	...
	return{_A:1,_i:_D}
def sparta_073b7cba80(plot_db_chart_obj):
	...
	return C
def sparta_2ece95a1df(json_data,user_obj):
	B=user_obj;D=sparta_afd1c616b4(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_L];C=_B
				if N:C=B[_t];C=qube_e74e1c855f.sparta_153392700c(C)
...
def sparta_23fd78dc7f(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_L];C=_B
				if K:C=B[_t];C=qube_e74e1c855f.sparta_153392700c(C)
	...
	return{_A:1}
def sparta_f12956c233(json_data,user_obj):
	...
	return{_A:1}
def sparta_76a161dcab(user_obj):
	B=user_obj;C=sparta_afd1c616b4(B)
	...
	return E
def sparta_5ddcab1dd8(json_data,user_obj):
	try:A=sparta_f1f3eaa0f3(json_data[_AW],user_obj);B=A[_i];return{_A:1,_i:B}
	except:pass
	return{_A:-1}
def sparta_8f0f698c1e(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_f1f3eaa0f3(A[_AW],B);D=C[_i]
	if D:E=C[_M];A[_G]=E.plot_chart_id;F=sparta_61b8785673(A,B);return{_A:1,_E:[A[_E]for A in F[_A9]]}
	return{_A:-1}
def sparta_a54299b608(json_data,user_obj):
	B='Error quantstats';A=json_data;C=A['service']
	if C=='quantstats':
		try:return qube_e6cec87b28.sparta_81b91486ec(A,user_obj)
		except Exception as D:print(B);print(traceback.format_exc());print(B);return{_A:-1,_F:str(D)}
	return{_A:1}
def sparta_530e554b20(json_data,user_obj):
...
def sparta_ccb78e4769(json_data,user_obj):
