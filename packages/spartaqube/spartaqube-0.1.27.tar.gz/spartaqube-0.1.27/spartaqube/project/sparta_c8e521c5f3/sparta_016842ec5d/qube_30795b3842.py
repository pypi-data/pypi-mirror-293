...
def sparta_a99d8bc579(is_owner=_A):return{_F:is_owner,_B:_E,_C:_E,_D:_E}
def sparta_f2c9153f43():return{_F:_A,_B:_A,_C:_A,_D:_A}
def sparta_a898785974(user_obj,portfolio_obj):
	B=portfolio_obj;A=user_obj
	if B.user==A:return sparta_a99d8bc579(_E)
	F=qube_05804de984.sparta_598db0393a(A);E=[A.userGroup for A in F]
	...
	if D.count()==0:return sparta_f2c9153f43()
	G=D[0];C=G.ShareRights
	if C.is_delete:return sparta_f2c9153f43()
	return{_F:_A,_B:C.is_admin,_C:C.has_write_rights,_D:C.has_reshare_rights}
def sparta_3a52583f3b(user_obj,universe_obj):
	B=universe_obj;A=user_obj
	if B.user==A:return sparta_a99d8bc579()
	F=qube_05804de984.sparta_598db0393a(A);E=[A.userGroup for A in F]
