...
from project.sparta_c8e521c5f3.sparta_5e07beffc7 import qube_08b4b127fd as qube_08b4b127fd
def sparta_b990d479d2():return str(uuid.uuid4())
def sparta_9485b3b142(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_fe3a8ca3c5(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_4c02948466(path):
	with open(path,'a'):os.utime(path,None)
def sparta_8d28b9f722(json_data,userObj):
	A=json_data;print('CREATE RESOURCE');print(A);C=A[_B];D=sparta_fe3a8ca3c5(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_4c02948466(B)
	...
	return{_A:1}
def sparta_3a31229822(json_data,userObj):
	...
	return{_A:1}
def sparta_bfd97aba4e(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_fe3a8ca3c5(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_4a0ac87cdc(json_data,userObj):A=json_data[_B];B=qube_08b4b127fd.sparta_9944893aa4(A);C={_A:1,'folderStructure':B};return C
def sparta_d8cdba3b47(json_data,userObj):
	A=json_data
	try:
...
def sparta_75168df132(json_data,userObj):
	...
	return{_A:1}
def sparta_9d595188f6(json_data,userObj):
	...
	return{_A:1}
def sparta_5e133d3d20(json_data,userObj):
...
def sparta_45933f223d(json_data,userObj):
	...
	return{_A:-1}
def sparta_5a97eacd3e(json_data,userObj):
