...
from project.sparta_3ca0064d9b.sparta_e277eee10c.qube_b6b724ff27 import sparta_c155880f90
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_0ab04b3ea8(json_data,user_obj):
...
def sparta_515d6e30cf(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_084f19ad73(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_c155880f90(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_c155880f90(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_dc8009f771(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_c155880f90(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_79f1981b20(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_515d6e30cf(F,A.type_ticket,None)
...
def sparta_1200e3c99f(json_data,user_obj):
	...
	return{_A:1}
def sparta_e870fd39ab(json_data):
	...
	return{_A:1}
def sparta_4e540af8c3(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}