...
from project.sparta_c8e521c5f3.sparta_7a80ac5fed import qube_abe72da03e as qube_abe72da03e
from project.sparta_c8e521c5f3.sparta_ba231ca67b.qube_6218d70beb import Email as Email
def sparta_25d3a2863e(function):
	def A(request,*E,**C):
		A=request;B=_B
		...
		if not B:
			D=C.get(_H)
			if D is not None:F=qube_abe72da03e.sparta_7fa6655433(D);login(A,F)
		return function(A,*E,**C)
	return A
def sparta_f8d86bc838(function):
	def A(request,*C,**D):
	...
	return A
def sparta_e992d28216(function):
	def A(request,*B,**C):
	...
	return A
def sparta_ae2d50807f(function):
	C=function
	def A(request,*D,**E):
		A=request;F=_C
		try:
			G=json.loads(A.body);H=json.loads(G[_I]);I=H[_H];B=qube_abe72da03e.sparta_7fa6655433(I)
...
def sparta_d4dd60ca94():A=socket.gethostname();B=socket.gethostbyname(A);return B
def sparta_4691583bc7(json_data):
	D='ip_addr';A=json_data;del A[_J];del A[_K]
	try:A[D]=sparta_d4dd60ca94()
	except:A[D]=-1
...
def sparta_7c7a42d896():return conf_settings.IS_GUEST_CODE_REQUIRED
def sparta_b7c0bfe7cf(guest_code):
	if GuestCodeGlobal.objects.filter(guest_id=guest_code,is_active=_B).count()>0:return _B
	return _C
def sparta_98847e71ac(guest_code,user_obj):
	...
	return _C
def sparta_142b5154a7(user):
	A=UserProfile.objects.filter(user=user)
	if A.count()==1:return A[0].is_banned
	else:return _C
def sparta_069779294f(email,captcha):
	D=sparta_f29a5d8d9b(captcha)
	if D[_A]!=1:return{_A:-1,_D:_G}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_L}
	A=B[0];C=str(uuid.uuid4());A.token_reset_password=C;A.save();sparta_924a23e89f(A.user,C);return{_A:1}
def sparta_924a23e89f(user_obj,token_reset_password):B=user_obj;A=Email(B.username,[B.email],'Reset Password','Reset Password Message');A.addOneRow('Reset code','Copy the following code to reset your password');A.addSpaceSeparator();A.addOneRow(token_reset_password);A.send()
def sparta_d213c3e808(captcha,token,email,password):
	D=sparta_f29a5d8d9b(captcha)
	if D[_A]!=1:return{_A:-1,_D:_G}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_L}
