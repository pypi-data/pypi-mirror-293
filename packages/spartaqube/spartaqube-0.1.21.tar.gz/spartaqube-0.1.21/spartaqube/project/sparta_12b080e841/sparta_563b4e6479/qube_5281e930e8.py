...
def sparta_277489d51f(is_owner=_A):return{_F:is_owner,_B:_E,_C:_E,_D:_E}
def sparta_441cd6e717():return{_F:_A,_B:_A,_C:_A,_D:_A}
def sparta_47a8bc7789(user_obj,portfolio_obj):
	B=portfolio_obj;A=user_obj
	if B.user==A:return sparta_277489d51f(_E)
	F=qube_325cb84778.sparta_823cffda87(A);E=[A.userGroup for A in F]
	...
	if D.count()==0:return sparta_441cd6e717()
	G=D[0];C=G.ShareRights
	if C.is_delete:return sparta_441cd6e717()
	return{_F:_A,_B:C.is_admin,_C:C.has_write_rights,_D:C.has_reshare_rights}
def sparta_d096210c13(user_obj,universe_obj):
	B=universe_obj;A=user_obj
	if B.user==A:return sparta_277489d51f()
	F=qube_325cb84778.sparta_823cffda87(A);E=[A.userGroup for A in F]
