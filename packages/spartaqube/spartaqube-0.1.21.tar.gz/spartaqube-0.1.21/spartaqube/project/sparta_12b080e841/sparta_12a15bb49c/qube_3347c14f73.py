...
from project.sparta_509f9a5b62.sparta_76f284387c.qube_cb1c81c00b import sparta_cbc18a5346
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_38595f589a(json_data,user_obj):
...
def sparta_087164f7e0(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_72da25db01(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_cbc18a5346(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_cbc18a5346(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_c69e894cd9(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_cbc18a5346(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_1363b9de49(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_087164f7e0(F,A.type_ticket,None)
...
def sparta_bbf70fc7b0(json_data,user_obj):
	...
	return{_A:1}
def sparta_c72445440d(json_data):
	...
	return{_A:1}
def sparta_16fb16ca94(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}