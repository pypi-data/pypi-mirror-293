...
def sparta_97e1e195fb(json_data,user_obj):
	...
	for A in K:
		B=sparta_cbc18a5346(model_to_dict(A));Q=int(A.typeObject)
		if not A.is_seen:print('Add 1');E+=1
		for R in P:B.pop(R,None)
		C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.date_created.astimezone(UTC);B[N]=G;B[O]=sparta_4d751ed888(H)
		if Q==0:0
	...
	for A in L:
		...
		B=sparta_cbc18a5346(model_to_dict(A));C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.dateCreated.astimezone(UTC);B[N]=G;B['type_object']=-2;B[O]=sparta_4d751ed888(H);F.append(B)
...
def sparta_4d751ed888(dateCreated):
	...
	return str(A)
def sparta_e55c9d6024(json_data,user_obj):
