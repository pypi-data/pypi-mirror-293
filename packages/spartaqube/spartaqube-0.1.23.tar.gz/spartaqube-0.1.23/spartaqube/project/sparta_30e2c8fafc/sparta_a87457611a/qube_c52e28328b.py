...
def sparta_51a1eded52(is_owner=_A):return{_F:is_owner,_B:_E,_C:_E,_D:_E}
def sparta_2ca6e1f274():return{_F:_A,_B:_A,_C:_A,_D:_A}
def sparta_6a0bd89ed5(user_obj,portfolio_obj):
	B=portfolio_obj;A=user_obj
	if B.user==A:return sparta_51a1eded52(_E)
	F=qube_65da3c3f33.sparta_6efceea516(A);E=[A.userGroup for A in F]
	...
	if D.count()==0:return sparta_2ca6e1f274()
	G=D[0];C=G.ShareRights
	if C.is_delete:return sparta_2ca6e1f274()
	return{_F:_A,_B:C.is_admin,_C:C.has_write_rights,_D:C.has_reshare_rights}
def sparta_db3a252561(user_obj,universe_obj):
	B=universe_obj;A=user_obj
	if B.user==A:return sparta_51a1eded52()
	F=qube_65da3c3f33.sparta_6efceea516(A);E=[A.userGroup for A in F]
