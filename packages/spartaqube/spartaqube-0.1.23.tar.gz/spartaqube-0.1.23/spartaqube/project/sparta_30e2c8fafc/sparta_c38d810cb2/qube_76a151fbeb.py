...
def sparta_7287757aec(json_data,userObj):
...
def sparta_8fb2279dba(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_65da3c3f33.sparta_6efceea516(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_65da3c3f33.sparta_6efceea516(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_75e2a179c9(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_65da3c3f33.sparta_6efceea516(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_65da3c3f33.sparta_6efceea516(A);C=[A.userGroup for A in H]
...
def sparta_e6d66502f6(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';J=json_data;B=user_obj;print('json_data SHARE OBJECTS');print(J);L=datetime.now().astimezone(UTC);P=qube_65da3c3f33.sparta_6efceea516(B);M=[A.userGroup for A in P];U=J['member2ShareArr'];I=int(J[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_65da3c3f33.sparta_6efceea516(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_65da3c3f33.sparta_6efceea516(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_5742b579ff(json_data,user_obj):
