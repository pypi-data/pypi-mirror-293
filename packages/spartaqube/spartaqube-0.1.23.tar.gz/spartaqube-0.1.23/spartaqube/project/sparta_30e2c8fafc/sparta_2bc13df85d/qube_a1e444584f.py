...
from project.sparta_30e2c8fafc.sparta_2bc13df85d import qube_9d3cf4d0e7 as qube_9d3cf4d0e7
...
def sparta_55939d6e7d(user_obj):
	...
	return B
def sparta_923e0026e1(json_data,user_obj):
	D=user_obj;E=sparta_55939d6e7d(D)
...
def sparta_20c3804d27():return{_A:1,'available_engines':qube_84ad741dd2.sparta_20c3804d27()}
def sparta_5edc5e5587(json_data,user_obj):
	...
	return{_A:1}
def sparta_ef3dbc4e80(json_data):
...
def sparta_06d347047e(json_data):
...
def sparta_c504fa9446(json_data,user_obj):A=json_data;B=datetime.now().astimezone(UTC);C=str(uuid.uuid4());D=DBConnector.objects.create(connector_id=C,host=A[_S],port=A[_T],user=A[_U],password_e=qube_84ad741dd2.sparta_b2ba6350ea(A[_k]),database=A[_V],oracle_service_name=A[_W],keyspace=A[_X],library_arctic=A[_Y],database_path=A[_Z],csv_path=A[_a],csv_delimiter=A[_b],read_only=A[_c],json_url=A[_d],socket_url=A[_e],redis_db=A[_f],token=A[_l],organization=A[_m],lib_dir=A[_n],driver=A[_o],trusted_connection=A[_p],dynamic_inputs=json.dumps(A[_I]),py_code_processing=A[_g],db_engine=A[_N],name=A[_J],description=A[_K],date_created=B,last_update=B,last_date_used=B);E=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);DBConnectorUserShared.objects.create(db_connector=D,user=user_obj,date_created=B,share_rights=E,is_owner=_C);return{_A:1}
def sparta_4d3cbf2976(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_55939d6e7d(C)
...
def sparta_2028e57ccf(json_data,user_obj):
	C=json_data[_H];A=sparta_79e0bbd6f2(C,user_obj)
...
def sparta_31bfb4c5a7(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_q];D=[];E=sparta_79e0bbd6f2(H,user_obj)
...
def sparta_c6aad178aa(json_data,db_connector_obj):
...
def sparta_96908f7898(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_q,'0'))==1;B=sparta_79e0bbd6f2(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_c6aad178aa(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_z]
	...
	return{_A:1,_E:sparta_fb1f3aaed4(E)}
def sparta_a4f746c00a(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_q,_D);K=A.get(_N,_B);B=sparta_79e0bbd6f2(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_c6aad178aa(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
...
def sparta_9ff5c82028(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_05083becb3(json_data,user_obj):
	A=json_data;print('json_data load_table_preview_explorer');print(A);D=A[_H];F=A.get(_O,_B);G=int(A.get(_q,'0'))==1;B=sparta_79e0bbd6f2(D,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_c6aad178aa(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_E:sparta_fb1f3aaed4(E)}
def sparta_4a52bd884e(json_data,user_obj):
	A=json_data;J=A[_L];D=_B
	if J:D=A[_t];D=qube_720376529a.sparta_d213ed1378(D)
...
def sparta_94ab59f069(json_data,user_obj):
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if J.is_admin or J.has_write_rights:
				M=B[_L];C=_B
				if M:C=B[_t];C=qube_720376529a.sparta_d213ed1378(C)
	...
	return{_A:1}
def sparta_7444f95fe7(json_data,user_obj):0
def sparta_c5abf394d1(json_data,user_obj):
	D=user_obj;F=sparta_55939d6e7d(D)
...
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables):
	...
	return B
def sparta_ddea6beef6(json_data,user_obj):
			...
			if N.count()>0:
				...
				if b:0
					...
					for B in A.data_source_list:
						...
						if I:H.append(B[Z])
							...
							if _I in B:
								c=B[_H];Q=sparta_79e0bbd6f2(c,P);R=[]
				...
				if len(H)>0:
					if E is not _B:
						...
						for B in A.data_source_list:
							I=B[Y]
							if I:W=B[Z];i=h[W];B[_E]=sparta_fb1f3aaed4(sparta_389be66ef5(i,variable_name=W))
...
def sparta_0a99aa7682(json_data,user_obj):
	...
	return{_A:1}
def sparta_33c287c727(session_id):
	...
	for(A,F)in E.items():
		...
		if C is not _B:
			...
			if A==_R:B[A]=D
			else:B[A]=sparta_fb1f3aaed4(sparta_389be66ef5(D,variable_name=A))
	return B
def sparta_178f37e65d(session_id):A=sparta_33c287c727(session_id);B=sparta_96a28d4f7b(b_return_type_id=_C);C=json.loads(A[_AS])[_E][0][0];D=[A for A in B if A['ID']==C][0][_AT];return{_A:1,_AU:A,_AV:A.get(_R,dict()),_Q:D}
def sparta_1404d096dd(user_obj,session_id,widget_id):
	...
	if E.count()>0:
		...
		if not C:
			...
			if H.count()>0:C=_C
	if C:B=A.chart_params;B[_R]=json.loads(B[_R]);B[_AB]=json.loads(B[_E])[_AB];F=sparta_33c287c727(session_id);I=sparta_96a28d4f7b(b_return_type_id=_C);J=json.loads(F[_AS])[_E][0][0];K=[A for A in I if A['ID']==J][0][_AT];L=F.get(_R,dict());B[_R].update(L);return{_A:1,_AU:F,_AV:B,_Q:K}
	else:return{_A:-1,_F:'You do not have access to this template'}
def sparta_01181ef065(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_55939d6e7d(K)
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_720376529a.sparta_9805f823b5(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_9c64b54611(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_M:A}
					...
					try:
						if qube_720376529a.sparta_d213ed1378(F)==A.widget_password_e:return{_A:1,_M:A}
						else:return{_A:3,_F:_AA,_M:A}
					except:return{_A:3,_F:_AA,_M:A}
			elif C.is_authenticated:
				G=sparta_55939d6e7d(C)
	...
	return{_A:-1}
def sparta_8a016f94f4(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_55939d6e7d(C)
	...
	return{_A:1,_i:_D}
def sparta_d0553c1849(plot_db_chart_obj):
	...
	return C
def sparta_323268afcb(json_data,user_obj):
	B=user_obj;D=sparta_55939d6e7d(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_L];C=_B
				if N:C=B[_t];C=qube_720376529a.sparta_d213ed1378(C)
...
def sparta_d8f80288e6(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_L];C=_B
				if K:C=B[_t];C=qube_720376529a.sparta_d213ed1378(C)
	...
	return{_A:1}
def sparta_5c4278dfdb(json_data,user_obj):
	...
	return{_A:1}
def sparta_564b51f7f4(user_obj):
	B=user_obj;C=sparta_55939d6e7d(B)
	...
	return E
def sparta_5235aeee84(json_data,user_obj):
	try:A=sparta_8a016f94f4(json_data[_AW],user_obj);B=A[_i];return{_A:1,_i:B}
	except:pass
	return{_A:-1}
def sparta_bc36cee727(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_8a016f94f4(A[_AW],B);D=C[_i]
	if D:E=C[_M];A[_G]=E.plot_chart_id;F=sparta_ddea6beef6(A,B);return{_A:1,_E:[A[_E]for A in F[_A9]]}
	return{_A:-1}
def sparta_cc2abb0bb0(json_data,user_obj):
	B='Error quantstats';A=json_data;C=A['service']
	if C=='quantstats':
		try:return qube_9d3cf4d0e7.sparta_038e0f9e7b(A,user_obj)
		except Exception as D:print(B);print(traceback.format_exc());print(B);return{_A:-1,_F:str(D)}
	return{_A:1}
def sparta_c8950d155e(json_data,user_obj):
...
def sparta_a10c91bccc(json_data,user_obj):
