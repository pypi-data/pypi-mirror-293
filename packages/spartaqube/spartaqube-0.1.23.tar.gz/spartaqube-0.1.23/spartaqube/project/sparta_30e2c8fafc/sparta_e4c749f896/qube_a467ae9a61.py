...
from project.sparta_9cf3fbfd16.sparta_527aa3c9b5.qube_8fc5e0b148 import sparta_a69f3c7c14,sparta_46527cb0cc,sparta_ea8df0282b
from project.sparta_30e2c8fafc.sparta_4f7fa902d8.qube_b0ccd99581 import sparta_ad5e6e0cd1
def sparta_38b5016fbb(json_data,user_obj):
...
def sparta_fab1c609c4(message,typeCase=0,companyName=None):
	...
	if C.count()>0:
		...
		A.send()
def sparta_bd47a101d4(json_data,userObj):
	...
	return B
def sparta_c5ffe18b5f(json_data,userObj):
	...
	return A
def sparta_d89fb1ca48(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_ad5e6e0cd1(F):return{_A:-1,_B:'Invalid current code'}
	...
	return{_A:1}
def sparta_708e9cf40f(json_data,userObj):A=userObj;C=json_data['base64image'];K=hashlib.sha256((str(A.id)+'_'+A.email+str(datetime.now())).encode(_D)).hexdigest();D,E=C.split(';base64,');F,L=D.split('/');G=F.split(':')[-1];B=UserProfile.objects.get(user=A);H=datetime.now();I=Avatar.objects.create(avatar=G,image64=E,date_created=H);B.avatar=I;B.save();J={_A:1};return J
def sparta_1001828985(json_data,userObj):B=json_data['bDarkTheme'];A=UserProfile.objects.get(user=userObj);A.is_dark_theme=B;A.save();C={_A:1};return C
def sparta_15013004d2(json_data,userObj):B=json_data['theme'];A=UserProfile.objects.get(user=userObj);A.editor_theme=B;A.save();C={_A:1};return C
def sparta_1fdb5f0d46():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_55b9a6f6d9(json_data):
	A=json_data;C=A[_F];E=A[_G];B=A[_H];F=A[_Q]
	if not sparta_ea8df0282b(E):return{_A:-1,_B:_I}
...
def sparta_3b8177bf8c(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_G];G=sparta_a69f3c7c14(E)
	if G[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_46527cb0cc(F):return{_A:-1,_B:_I}
...
def sparta_5868e34167(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_G];H=A[_T];E=A[_H];I=A[_Q];J=sparta_a69f3c7c14(F)
	if J[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_46527cb0cc(G):return{_A:-1,_B:_I}
