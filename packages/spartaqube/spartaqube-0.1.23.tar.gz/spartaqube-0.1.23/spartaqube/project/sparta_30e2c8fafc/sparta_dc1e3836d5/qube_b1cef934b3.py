...
from project.sparta_9cf3fbfd16.sparta_527aa3c9b5.qube_8fc5e0b148 import sparta_1aeec6cce0
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_e7b6ecfe5c(json_data,user_obj):
...
def sparta_2f03710a3b(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_f6f2af56e8(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_1aeec6cce0(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_1aeec6cce0(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_54fb1dd9ac(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_1aeec6cce0(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_a06642d047(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_2f03710a3b(F,A.type_ticket,None)
...
def sparta_efc0471cf6(json_data,user_obj):
	...
	return{_A:1}
def sparta_f349523a94(json_data):
	...
	return{_A:1}
def sparta_220edd9863(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}