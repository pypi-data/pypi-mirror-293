...
from project.sparta_40ba2b27bc.sparta_3a19c63ec4 import qube_11d3c65eb8
from project.sparta_40ba2b27bc.sparta_ffa6b80cbc import qube_212881a4b3 as qube_212881a4b3
from project.sparta_40ba2b27bc.sparta_3a19c63ec4.qube_1db30fe9c4 import Connector as Connector
from project.sparta_40ba2b27bc.sparta_dc05d02cc2.qube_0377b9b446 import sparta_4cc463fb2a,sparta_64d58aadc8
from project.sparta_40ba2b27bc.sparta_ffa6b80cbc import qube_7f939a7ff9 as qube_7f939a7ff9
...
def sparta_557092ff98(user_obj):
	A=qube_3228fef6bb.sparta_9cd3e03d0b(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_921b5d9da3(json_data,user_obj):
	D=user_obj;E=sparta_557092ff98(D)
...
def sparta_d65f9815ba():return{_A:1,'available_engines':qube_11d3c65eb8.sparta_d65f9815ba()}
def sparta_933e4e94ff(json_data,user_obj):
	...
	return{_A:1}
def sparta_8e609383b1(json_data):
...
def sparta_d6dc27c7ad(json_data):
...
def sparta_ab0126dd89(json_data,user_obj):A=json_data;B=datetime.now().astimezone(UTC);C=str(uuid.uuid4());D=DBConnector.objects.create(connector_id=C,host=A[_S],port=A[_T],user=A[_U],password_e=qube_11d3c65eb8.sparta_9e30bdd5c0(A[_k]),database=A[_V],oracle_service_name=A[_W],keyspace=A[_X],library_arctic=A[_Y],database_path=A[_Z],csv_path=A[_a],csv_delimiter=A[_b],read_only=A[_c],json_url=A[_d],socket_url=A[_e],redis_db=A[_f],token=A[_l],organization=A[_m],lib_dir=A[_n],driver=A[_o],trusted_connection=A[_p],dynamic_inputs=json.dumps(A[_I]),py_code_processing=A[_g],db_engine=A[_N],name=A[_J],description=A[_K],date_created=B,last_update=B,last_date_used=B);E=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);DBConnectorUserShared.objects.create(db_connector=D,user=user_obj,date_created=B,share_rights=E,is_owner=_C);return{_A:1}
def sparta_f6e2731a7a(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_557092ff98(C)
...
def sparta_47a15f51ae(json_data,user_obj):
	C=json_data[_H];A=sparta_b8fd45dd04(C,user_obj)
...
def sparta_d5a7137239(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_q];D=[];E=sparta_b8fd45dd04(H,user_obj)
...
def sparta_a4e2c7a4c6(json_data,db_connector_obj):
...
def sparta_683b39c9f6(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_q,'0'))==1;B=sparta_b8fd45dd04(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_a4e2c7a4c6(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_z]
	...
	return{_A:1,_E:sparta_64d58aadc8(E)}
def sparta_66a489951e(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_q,_D);K=A.get(_N,_B);B=sparta_b8fd45dd04(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_a4e2c7a4c6(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
...
def sparta_e7dd33d37b(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_a1c25cad51(json_data,user_obj):
	A=json_data;print('json_data load_table_preview_explorer');print(A);D=A[_H];F=A.get(_O,_B);G=int(A.get(_q,'0'))==1;B=sparta_b8fd45dd04(D,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_a4e2c7a4c6(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_E:sparta_64d58aadc8(E)}
def sparta_9ee54e43b6(json_data,user_obj):
	A=json_data;J=A[_L];D=_B
	if J:D=A[_t];D=qube_212881a4b3.sparta_589cdde3e9(D)
...
def sparta_997d447910(json_data,user_obj):
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if J.is_admin or J.has_write_rights:
				M=B[_L];C=_B
				if M:C=B[_t];C=qube_212881a4b3.sparta_589cdde3e9(C)
	...
	return{_A:1}
def sparta_13562060cc(json_data,user_obj):0
def sparta_56abc368e7(json_data,user_obj):
	D=user_obj;F=sparta_557092ff98(D)
...
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables):
	...
	return B
def sparta_0b40229da9(json_data,user_obj):
			...
			if N.count()>0:
				...
				if b:0
					...
					for B in A.data_source_list:
						...
						if I:H.append(B[Z])
							...
							if _I in B:
								c=B[_H];Q=sparta_b8fd45dd04(c,P);R=[]
				...
				if len(H)>0:
					if E is not _B:
						...
						for B in A.data_source_list:
							I=B[Y]
							if I:W=B[Z];i=h[W];B[_E]=sparta_64d58aadc8(sparta_4cc463fb2a(i,variable_name=W))
...
def sparta_27ad687f95(json_data,user_obj):
	...
	return{_A:1}
def sparta_3bdc4e756c(session_id):
	...
	for(A,F)in E.items():
		...
		if C is not _B:
			...
			if A==_R:B[A]=D
			else:B[A]=sparta_64d58aadc8(sparta_4cc463fb2a(D,variable_name=A))
	return B
def sparta_102f77f2c1(session_id):A=sparta_3bdc4e756c(session_id);B=sparta_5f86688418(b_return_type_id=_C);C=json.loads(A[_AS])[_E][0][0];D=[A for A in B if A['ID']==C][0][_AT];return{_A:1,_AU:A,_AV:A.get(_R,dict()),_Q:D}
def sparta_6cbaa07ff6(user_obj,session_id,widget_id):
	...
	if E.count()>0:
		...
		if not C:
			...
			if H.count()>0:C=_C
	if C:B=A.chart_params;B[_R]=json.loads(B[_R]);B[_AB]=json.loads(B[_E])[_AB];F=sparta_3bdc4e756c(session_id);I=sparta_5f86688418(b_return_type_id=_C);J=json.loads(F[_AS])[_E][0][0];K=[A for A in I if A['ID']==J][0][_AT];L=F.get(_R,dict());B[_R].update(L);return{_A:1,_AU:F,_AV:B,_Q:K}
	else:return{_A:-1,_F:'You do not have access to this template'}
def sparta_7c21e2e500(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_557092ff98(K)
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_212881a4b3.sparta_88c60b886b(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_52b6a93721(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_M:A}
					...
					try:
						if qube_212881a4b3.sparta_589cdde3e9(F)==A.widget_password_e:return{_A:1,_M:A}
						else:return{_A:3,_F:_AA,_M:A}
					except:return{_A:3,_F:_AA,_M:A}
			elif C.is_authenticated:
				G=sparta_557092ff98(C)
	...
	return{_A:-1}
def sparta_a0da48c65e(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_557092ff98(C)
	...
	return{_A:1,_i:_D}
def sparta_e0017a61c9(plot_db_chart_obj):
	...
	return C
def sparta_2cb2f5d873(json_data,user_obj):
	B=user_obj;D=sparta_557092ff98(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_L];C=_B
				if N:C=B[_t];C=qube_212881a4b3.sparta_589cdde3e9(C)
...
def sparta_3bd2579c13(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_L];C=_B
				if K:C=B[_t];C=qube_212881a4b3.sparta_589cdde3e9(C)
	...
	return{_A:1}
def sparta_43be8cd703(json_data,user_obj):
	...
	return{_A:1}
def sparta_b4f3a67669(user_obj):
	B=user_obj;C=sparta_557092ff98(B)
	...
	return E
def sparta_0ac0753e77(json_data,user_obj):
	try:A=sparta_a0da48c65e(json_data[_AW],user_obj);B=A[_i];return{_A:1,_i:B}
	except:pass
	return{_A:-1}
def sparta_1dcc8adbe2(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_a0da48c65e(A[_AW],B);D=C[_i]
	if D:E=C[_M];A[_G]=E.plot_chart_id;F=sparta_0b40229da9(A,B);return{_A:1,_E:[A[_E]for A in F[_A9]]}
	return{_A:-1}
def sparta_15946803ec(json_data,user_obj):
	B='Error quantstats';A=json_data;C=A['service']
	if C=='quantstats':
		try:return qube_7f939a7ff9.sparta_44ae4e2f30(A,user_obj)
		except Exception as D:print(B);print(traceback.format_exc());print(B);return{_A:-1,_F:str(D)}
	return{_A:1}
def sparta_f1d662b6c5(json_data,user_obj):
...
def sparta_42023828b3(json_data,user_obj):
