...
def sparta_143030ae85(json_data,user_obj,group_id=None):
	...
	F=UserGroup.objects.create(name=E,group_id=C,user_creator=B,date_created=A,last_update=A);UserGroupUser.objects.create(user_group=F,user=B,b_admin=_B,date_created=A,last_update=A);sparta_dae77f5888(G,F,A,B);H={_A:1,_C:C};return H
def sparta_dae77f5888(membersArr,user_group_obj,dateCreation,userMe):
...
def sparta_6da497c8a3(user_obj):A=user_obj;B=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_B,user_group__is_delete=0).all();C=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_D,user_group__is_delete=0).all();D={_E:B,_F:C};return D
def sparta_9cd3e03d0b(user_obj):return UserGroupUser.objects.filter(is_delete=0,user=user_obj,user_group__is_delete=0)
def sparta_08a4e244a4(json_data,user_obj):A=sparta_6da497c8a3(user_obj);B=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_E]];C=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_F]];return{_A:1,_E:B,_F:C}
def sparta_96bc2a8a54(json_data,user_obj):
	...
	B={_A:-1};return B
def sparta_20c7d69f4c(json_data,user_obj):
	...
	if D.count()>0:
		...
		if H.count()>0:A.name=F;A.save();I=datetime.now();sparta_dae77f5888(G,A,I,C)
	J={_A:1};return J
def sparta_cd39109d3e(json_data,user_obj):
	...
	I={_A:1};return I
def sparta_b2f1ea4e3b(json_data,user_obj):
	...
	D={_A:1};return D
def sparta_36d02c0d3c(json_data,user_obj):
	...
	H={_A:1};return H
def sparta_cb2ec8a21e(json_data,user_obj):
