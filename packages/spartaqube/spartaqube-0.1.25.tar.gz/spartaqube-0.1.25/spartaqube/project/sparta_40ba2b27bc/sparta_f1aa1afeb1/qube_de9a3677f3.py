...
from project.sparta_d7b94e3744.sparta_cc6e9d39f9.qube_707ef4ff5f import sparta_217a4c5ca1
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_3dd1543ce6(json_data,user_obj):
...
def sparta_ffdc7b779a(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_125906d125(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_217a4c5ca1(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_217a4c5ca1(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_12fbc0c1eb(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_217a4c5ca1(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_257dcaa93f(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_ffdc7b779a(F,A.type_ticket,None)
...
def sparta_6cab4683fc(json_data,user_obj):
	...
	return{_A:1}
def sparta_749c3645f8(json_data):
	...
	return{_A:1}
def sparta_35ed92202f(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}