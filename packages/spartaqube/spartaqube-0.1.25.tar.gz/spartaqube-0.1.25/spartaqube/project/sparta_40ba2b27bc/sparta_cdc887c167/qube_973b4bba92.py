...
from project.sparta_40ba2b27bc.sparta_141720ebb3 import qube_7fa924d199 as qube_7fa924d199
def sparta_50ddf78e18(json_data,userObj):
...
def sparta_1c3b48c049(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_3228fef6bb.sparta_9cd3e03d0b(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_3228fef6bb.sparta_9cd3e03d0b(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_6e297f6fe6(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_3228fef6bb.sparta_9cd3e03d0b(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_3228fef6bb.sparta_9cd3e03d0b(A);C=[A.userGroup for A in H]
...
def sparta_57cd7e44af(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';J=json_data;B=user_obj;print('json_data SHARE OBJECTS');print(J);L=datetime.now().astimezone(UTC);P=qube_3228fef6bb.sparta_9cd3e03d0b(B);M=[A.userGroup for A in P];U=J['member2ShareArr'];I=int(J[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_3228fef6bb.sparta_9cd3e03d0b(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_3228fef6bb.sparta_9cd3e03d0b(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_d006171c97(json_data,user_obj):
