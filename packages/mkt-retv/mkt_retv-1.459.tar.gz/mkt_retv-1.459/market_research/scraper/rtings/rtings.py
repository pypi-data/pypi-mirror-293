...
class Rtings(Scraper):
    ...
    def get_data(self, urls: list, export_excel=True):
        with Pool(cpu_count()) as pool:
            results = pool.map(self._process_url, urls)

        score_df = pd.concat([res['scores'] for res in results], axis=0)
        measurement_df = pd.concat([res['measurement'] for res in results], axis=0)
        comments_df = pd.concat([res['comments'] for res in results], axis=0)
    ...
    def _process_url(self, url):
        try:
            score_df = self._get_score(url, format_df=True)
            measurement_df = self._get_measurement_results(url)
            comments_df = self._get_comments(url, format_df=True)
    ...
    def _get_score(self, url: str, format_df=True):
        """
        return type
        -> dict|pd.DataFrame
        """
    ...
    def _get_comments(self, url: str, format_df=True, min_sentence_length=0):
        """
    ...
    def _get_measurement_results(self, url: str) -> pd.DataFrame:
        driver = self.web_driver.get_chrome()
        maker = url.split("/")[-2]
        product = url.split("/")[-1]
        url = url.lower()
        ...
        for test_group in test_groups:
            ...
            scores_header = '(><)_'.join(header_element.stripped_strings)
            ...
            for test_value in test_values:
                ...
                results_list.append([maker, product, category, scores_header, label, result_value])
