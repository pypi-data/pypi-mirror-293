...
class ModelScraper_s(Scraper):
    ...
    def get_models_info(self, format_df: bool = True, temporary_year_marking=False):
        ...
        print(f"Collecting models with {cpu_count()} processes")
        ...
        # Pool for fetching models
        with Pool(cpu_count()) as pool:
            pool.starmap(self._get_models, [(url, url_series_dict) for url in url_series_set])

        print("Number of total models:", len(url_series_dict)) 
        print("Collecting specs")

        # Pool for fetching global specs
        with Pool(cpu_count()) as pool:
            pool.starmap(self._get_global_spec, [(url_model, dict_models, key) for key, url_model in url_series_dict.items()])
        ...
        if format_df:
            ...
            pd.read_json(self.output_folder /'s_scrape_model_data.json', orient='records', lines=True)
        ...
        for cnt_try in range(try_total):
            ...
            try:
                ...
                for url_element in url_elements:
                    ...
                    label = self.file_manager.get_name_from_url(model_url)
                    dict_url_models[label] = model_url.strip()
                break
            except Exception as e:
                if cnt_try + 1 == try_total:
    ...
    def _get_global_spec(self, url: str, dict_models, key: str):
        """
        ...
        for cnt_try in range(try_total):
            try:
                ...
                time.sleep(3)
                try:
                    model_text = driver.find_element(By.XPATH,
                ...
                try:
                    ...
                    try:
                        ...
                        if self.tracking_log:
                            ...
                            self.file_manager.make_dir(dir_model)
                            driver.save_screenshot(f"./{dir_model}/{stamp_url}_Price extraction failed_{stamp_today}.png")
                        price_now = float('nan')
                ...
                time.sleep(self.wait_time)
                if self.tracking_log:
                    driver.save_screenshot(f"./{dir_model}/{stamp_url}_2_after_click_specification_{stamp_today}.png")
