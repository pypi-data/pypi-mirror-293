...
class DataVisualizer(BaseVisualizer):
    def __init__(self, data_frame:pd.DataFrame=None, output_folder="results", sns_style="whitegrid"):
        super().__init__(output_folder=output_folder)
        sns.set_style(sns_style)
        if data_frame is None:
            data_frame = pd.read_json('https://github.com/xikest/research_market_tv/s_scrape_model_data.json', orient='records', lines=True)
        self.data_cleanup = DataCleaner(data_frame)

    def generate_price_map(self):
        price_data = self.data_cleanup.get_price_data().copy()
        unique_years = price_data['year'].unique()

        series_info = price_data[['series', 'display_type', 'year']].drop_duplicates().sort_values(by=['year', 'series'])
        series_by_year = {year: {series: display_type for series, display_type in zip(series_data['series'], series_data['display_type'])}
                          for year in series_info['year'].unique()
                          for series_data in [series_info[series_info['year'] == year]]}

        color_palette = ['#636EFA', '#00CC96', '#EF553B']
        year_to_color = {year: color_palette[i] for i, year in enumerate(unique_years)}
        marker_shapes = ['circle', 'square', 'diamond', 'pentagon', 'star', 'hexagon', 'cross']
        series_to_marker = {i: marker_shapes[i % len(marker_shapes)] for i in range(len(series_info))}

        fig = go.Figure()

        for year in unique_years:
            year_data = price_data[price_data['year'] == year]
            series_display_dict = series_by_year.get(year)

            for index, (series, display_type) in enumerate(series_display_dict.items()):
                series_data = year_data[year_data['series'] == series]
                ...
                fig.add_trace(go.Scatter(
                    ...
                    marker=dict(
                        ...
                        symbol=series_to_marker[index],
                        opacity=0.8,
                        line=dict(width=3, color='black')
                    ),
                    text=series_data['description'],
                    hoverinfo='text',
                    name=f'{series} [{display_type}]',
                    visible='legendonly'
                ))
                ...
                fig.add_trace(go.Scatter(
                    ...
                    error_y=dict(
                        type='data',
                        symmetric=False,
                        arrayminus=[0] * len(price_data),
                        array=year_data['price_gap'],
                        color=year_to_color[year],
                        thickness=1,
                        width=5
                    ),
        ...
        size_categories = sorted(price_data['size'].unique())
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                range=[0, max(price_data['price_original'].max(), price_data['price'].max()) + 1000],
                showgrid=True
            ),
            width=1000,
            height=800,
    ...
    def generate_heatmap(self, display_types=None, save_filename=None, title="SONY Spec", cmap="Blues", figsize=(8, 8),
                         show_colorbar=False, selected_columns=None):
        if selected_columns is None:
            selected_columns = [
                'high peak luminance', 'peak luminance', 'contrast enhancement', 'pixel contrast booster',
                'dynamic contrast enhancer', 'xr backlight master drive', 'color enhancement', 'triluminos',
                'viewing angle (x-wide angle)', 'x-wide angle', 'anti reflection (x-anti reflection)', 'x-anti reflection',
                'picture processor', '4k processor', 'bravia core calibrated mode', 'sony pictures core calibrated mode',
                'netflix calibrated mode', 'prime video calibrated mode', 'auto genre picture mode',
                'features for playstation5 auto genre picture mode', 'auto hdr tone mapping',
                'features for playstation5 auto hdr tone mapping', 'multi-view'
            ]

        cleaned_data = self.data_cleanup.get_cleaned_data()
        available_columns = [col for col in selected_columns if col in cleaned_data.columns]
        missing_columns = [col for col in selected_columns if col not in cleaned_data.columns]
        ...
        cleaned_data = cleaned_data[available_columns].replace('-', 0).fillna(0)
        cleaned_data = cleaned_data.applymap(lambda x: len(x.split(",")) if isinstance(x, str) else x)
        index_names = cleaned_data.index.names
        cleaned_data = cleaned_data.reset_index().drop_duplicates().set_index(index_names).sort_index(ascending=True)
        self.cleaned_data = cleaned_data

        fig, ax = plt.subplots(figsize=figsize)
        sns.heatmap(cleaned_data.T, cmap=cmap, cbar=show_colorbar, ax=ax, vmax=1)
        plt.xticks(rotation=90)
        plt.title(title)

        save_filename = save_filename or f"heatmap_for_{title}.png"
        plt.savefig(self.output_folder / save_filename, bbox_inches='tight')
        plt.show()
