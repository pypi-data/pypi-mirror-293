...
from pyzrpc.meta import CONFIG_MONGO_FIELD_NAME_KEY, CONFIG_MONGO_FIELD_SERVICE_IPADDR_KEY, \
    ...
    CONFIG_MONGO_FIELD_WORK_MAX_PROCESS_KEY, CONFIG_MONGO_FIELD_TASK_ID_KEY
...
class TaskRun:
    ...
    def run(cls_path: str, config: dict, boyd: dict):
        ...
        _build = WorkBuild()
        _cls = _build.build(cls_path=cls_path, rpc_proxy=_rpc_proxy, logger=_logger, rabbitmq=_rabbitmq)
...
class _WorkStart:
    ...
    def work_start(service, config):
        ...
        _build = WorkBuild()
        _cls = _build.build(cls_path=service.__file__, rpc_proxy=_rpc_proxy, logger=_logger, rabbitmq=_rabbitmq)
