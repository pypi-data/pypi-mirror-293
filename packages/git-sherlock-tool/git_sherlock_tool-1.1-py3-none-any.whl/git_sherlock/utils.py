...
def analyze_repo(url):
    loading_anim(lambda: clone_repo(url))
    repo_name = extract_repo_name(url)
    #url = create_download_link(url)
    remove_excessive_rows(temp_dir)       # Files are possibly sent before cutting down extras
    time.sleep(3)
    try:
        display_summary_and_tree(url, repo_name)
        ...
        if file_path.is_file() and (file_path.name.lower() == 'readme.md' or file_path.name.lower() == 'readme.rst'):
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as file:
                ...
                if content == '':
                    ...
                    return gemini_summarize_readme(content)
                ...
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as file:
                    ...
                    if content == '':
                        ...
                        try:
                            ...
                            gemini_result = generate_summaries_and_list_functions_for_single_file(content, prompt)
                            time.sleep(3)
