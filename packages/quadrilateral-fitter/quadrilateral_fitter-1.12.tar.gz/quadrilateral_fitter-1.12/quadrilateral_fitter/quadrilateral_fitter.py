...
class QuadrilateralFitter:
    def __init__(self, polygon: np.ndarray | tuple | list | Polygon):
        ...
        if isinstance(polygon, Polygon):
            ...
            if isinstance(_polygon, LineString):
                ...
                _polygon = self._patch_linestring(linestring=_polygon)
        ...
        if isinstance(self.convex_hull_polygon, LineString):
            warn("Convex Hull of the polygon seems to be a LineString, trying to convert it to a Polygon.")
            print(f"Original polygon: {_polygon}")
            # Extract unique coordinates
            unique_coords = np.unique(_polygon.exterior.coords, axis=0)

            # Recalculate the convex hull using SciPy
            hull = ConvexHull(unique_coords)
            hull_points = unique_coords[hull.vertices]

            if len(hull_points) > 2:
                self.convex_hull_polygon = Polygon(hull_points)
            else:
                warn(
                    "Convex Hull calculation with SciPy also resulted in a degenerate case, defaulting to LineString.")
                self.convex_hull_polygon = LineString(hull_points)

            # If convex hull is still a LineString, patch it to a Polygon
        if isinstance(self.convex_hull_polygon, LineString):
            self.convex_hull_polygon = self._patch_linestring(self.convex_hull_polygon)
    ...
    def __simplify_polygon(self, polygon: Polygon|GeometryCollection, max_sides: int | None,
        ...
        if isinstance(polygon, Polygon):
            ...
            print(f"Polygon Centroid: {polygon.centroid}")
            print(f"Polygon Convex Hull: {polygon.convex_hull}")
            print(f"Polygon is valid: {polygon.is_valid}")
            print(f"Polygon Vertices: {polygon.exterior.coords}")
            raise TypeError(f"Expected Polygon or GeometryCollection, got {type(polygon)}.")
    ...
    def _patch_linestring(self, linestring: LineString) -> Polygon:
        """
        Converts a LineString to a Polygon by defining a tight bounding box around it.

        :param linestring: A LineString object.
        :return: A Polygon object.
        """
        # Extract the line coordinates
        line_coords = np.array(linestring.coords, dtype=np.float32)
