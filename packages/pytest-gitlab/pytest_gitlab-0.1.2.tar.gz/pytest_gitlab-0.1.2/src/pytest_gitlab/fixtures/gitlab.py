...
from pytest_gitlab.docker import *  # noqa
...
def reset_gitlab(gl: gitlab.Gitlab) -> None:  # noqa C901
    ...
    for project in gl.projects.list():
        for deploy_token in project.deploytokens.list():
            logging.info(
                f'Deleting deploy token: {deploy_token.username!r} in ' f'project: {project.path_with_namespace!r}'
            )
    ...
    for group in gl.groups.list():
        ...
        for deploy_token in group.deploytokens.list():
            logging.info(
                f'Deleting deploy token: {deploy_token.username!r} in ' f'group: {group.path_with_namespace!r}'
            )
            helpers.safe_delete(deploy_token)
...
def check_is_alive():
    ...
    def _check(
        *,
        ...
        logging.info(
            f'Checking if GitLab container is up. ' f'Have been checking for {minutes} minute(s), {seconds} seconds ...'
        )
...
def gitlab_token(
    ...
    setup_time = time.perf_counter() - start_time
    minutes, seconds = int(setup_time / 60), int(setup_time % 60)
    logging.info(f'GitLab container is now ready after {minutes} minute(s), {seconds} seconds')

    return set_token(gitlab_container_name, fixture_dir=fixture_dir, gitlab_url=gitlab_url)
...
def make_merge_request(project):
    ...
    def _make_merge_request(*, source_branch: str, create_pipeline: bool = False):
        ...
        mr_branch = project.branches.create({'branch': source_branch, 'ref': project.default_branch})
...
""",
        ...
        for _ in range(60):
            ...
            if mr.detailed_merge_status == 'checking' or mr.detailed_merge_status == 'unchecked':  # noqa PLR1714
                time.sleep(0.5)
            else:
                break
