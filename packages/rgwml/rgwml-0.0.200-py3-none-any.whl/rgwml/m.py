...
class m:
    ...
    def google_auth(self, invoking_function_name, request, google_client_id, sqlite_db_path, post_authentication_redirect_url,
        ...
        self.send_telegram_message("here_222", telegram_bot_preset_name)
        request_body_json = request.json
        token = request_body_json.get('token')
        #send_telegram_message(f"GA TOKEN: {token}", telegram_bot_preset_name)
        self.send_telegram_message("here", telegram_bot_preset_name)
        try:
            if token:
                ...
                if idinfo:
                    ...
                    self.send_telegram_message("creating tables", telegram_bot_preset_name)
                    # Ensure the necessary tables are created
                    create_tables(sqlite_db_path)
                    self.send_telegram_message("Table created", telegram_bot_preset_name)
                    conn = sqlite3.connect(sqlite_db_path)
                    cursor = conn.cursor()
                    cursor.execute(
                    ...
                    self.send_telegram_message("Table insertions done", telegram_bot_preset_name)
                    ...
                    self.send_telegram_message("respone prepared", telegram_bot_preset_name)
                    return response
                else:
