...
class m:
    ...
    def google_auth(self, request, google_client_id, sqlite_db_path, post_authentication_redirect_url,
        ...
        try:
            ...
            error_message = f"[ERROR::functions.py|{function_name}] Error: {e}"
            self.insert_log(error_message, sqlite_db_path)
            self.send_telegram_message(error_message, telegram_bot_preset_name)
            return HTTPResponse(status=500, body=json.dumps({"error": error_message}))
    ...
    def insert_log(self, message, sqlite_db_path):
