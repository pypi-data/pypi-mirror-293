...
class m:
    ...
    def google_auth(self, invoking_function_name, request, google_client_id, sqlite_db_path, post_authentication_redirect_url,
                    telegram_bot_preset_name, cookie_domain, access_control_allow_origin_url):
        ...
        this.send_telegram_message("here_222", telegram_bot_preset_name)
        request_body_json = request.json
        token = request_body_json.get('token')
        #send_telegram_message(f"GA TOKEN: {token}", telegram_bot_preset_name)
        this.send_telegram_message("here", telegram_bot_preset_name)
        try:
            if token:
                ...
                if idinfo:
                    ...
                    this.send_telegram_message("creating tables", telegram_bot_preset_name)
                    # Ensure the necessary tables are created
                    create_tables(sqlite_db_path)
                    this.send_telegram_message("Table created", telegram_bot_preset_name)
                    conn = sqlite3.connect(sqlite_db_path)
                    cursor = conn.cursor()
                    cursor.execute(
                    ...
                    this.send_telegram_message("Table insertions done", telegram_bot_preset_name)
                    ...
                    response.set_cookie('authToken', token, httponly=True, secure=True, path='/', domain=cookie_domain, samesite='None')
                    this.send_telegram_message("respone prepared", telegram_bot_preset_name)
                    return response
                else:
            ...
            insert_log(error_message, sqlite_db_path)
            this.send_telegram_message(error_message, telegram_bot_preset_name)
            return jsonify({"error": error_message}), 500


    def logout(self, invoking_function_name, request):
        try:
            response.delete_cookie('authToken')
            return json.dumps({'message': 'Logged out'})
        except Exception as e:
            error_message = f"[ERROR::{invoking_function_name}] Error: {e}"
            self.insert_log(error_message, sqlite_db_path)
            self.send_telegram_message(error_message, telegram_bot_preset_name)
            return json.dumps({"error": error_message})

    def insert_log(self, invoking_function_name, message, sqlite_db_path):
    ...
    def send_telegram_message(self, message, preset_name):
        ...
        try:
            ...
            self.insert_log(f"Failed to send Telegram message: {e}", sqlite_db_path)

    def check_execution_time(self, func):
        def wrapper(*args, **kwargs):
            ...
            if execution_time > 0.5:
                error_message = (
                    f"[ERROR] {func.__name__} Execution time exceeded 0.5 seconds: "
                    f"{execution_time:.2f} seconds"
                )
                self.insert_log(error_message, kwargs.get('sqlite_db_path'))
                self.send_telegram_message(error_message, kwargs.get('telegram_bot_preset_name'))
            return result
        return wrapper

    def send_secure_data(self, invoking_function_name, request, secure_data, sqlite_db_path, telegram_bot_preset_name, google_client_id, access_control_allow_origin_url):
        ...
        try:
            secure_token = request.get_cookie('authToken')
            self.send_telegram_message(f"TOKEN: {secure_token}", telegram_bot_preset_name)
            ...
            http_response = HTTPResponse(status=200, body=json.dumps(response_data))
            http_response.set_header("Access-Control-Allow-Origin", access_control_allow_origin_url)
