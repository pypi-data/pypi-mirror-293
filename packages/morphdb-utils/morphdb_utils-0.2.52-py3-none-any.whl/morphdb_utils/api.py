...
def _read_configuration_from_env() -> EnvVars:
    ...
    if not database_id_value:
        raise MorphApiError("Internal error: database_id is not set")
    if not base_url_value:
        raise MorphApiError("Internal error: base_url is not set")
    if not team_slug_value:
        raise MorphApiError("Internal error: team_slug is not set")
    if not api_key_value:
        raise MorphApiError("Internal error: api_key is not set")
...
def __execute_sql_impl(
    ...
    try:
        ...
        for chunk in pd.read_csv(
            ...
            chunks.append(chunk)
        df: DataFrame = pd.concat(chunks, axis=0)
        return df
    except Exception as e:
        raise MorphApiError(f"{e}")
...
def __get_data_path_impl(
    ...
    resources = context._search_meta_objects_by_path(filepath)
    ...
    if cell_type == "file":
        ...
        if specific_dirpath is not None:
            ...
            abs_dir_path_ = (
                ...
                if "output_paths" in resource and len(resource["output_paths"]) > 0
                else None
            )
            if abs_dir_path_ is None:
                abs_dir_path_ = os.path.join(
                    project_root, ProjectConfig.PRIVATE_DIR, resource["name"]
                )
            if abs_dir_path_.endswith("{ext()}"):
                abs_dir_path_ = os.path.dirname(abs_dir_path_)
                if abs_dir_path_.endswith("{name}"):
                    abs_dir_path_ = abs_dir_path_.replace("{name}", resource["name"])
...
def ref(reference: str) -> RefResponse:
    ...
    resource = context._search_meta_object_by_name(reference)
    if not resource:
    ...
    return RefResponse(
        ...
        connection_slug=resource["connection"] if "connection" in resource else None,
    )
    ...
    *args: LoadDataParams, **kwargs: Dict[str, Union[str, int, bool]]
    ...
    if args and isinstance(args[0], RefResponse):
        ...
        if arg_dict["type"] == "sql":  # type: ignore
            ...
            omitted_args = {k: v for k, v in arg_dict.items() if k != "type"}  # type: ignore
            if not omitted_args["filepath"].startswith(MORPH_STORAGE_PREFIX):  # type: ignore
                filepath = os.path.join(
                    project_root,
                    os.path.realpath(omitted_args["filepath"]),  # type: ignore
                )
            ...
            if len(errors) > 0:
                raise MorphApiError(f"Failed to load morph functions: {errors}")
            resource = context._search_meta_object_by_name(str(reference))
            filepath = resource["id"].split(":")[0] if resource else None
            if not filepath:
                return None
            if not filepath.startswith(MORPH_STORAGE_PREFIX):
