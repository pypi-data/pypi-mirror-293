...
from morph.task.utils.profile import PostgresqlConnection
...
class PostgresqlConnector:
    def __init__(
        ...
        self.ssh_server: Optional[SSHTunnelForwarder] = None
        self.engine = None
    ...
    def _start_ssh_tunnel(self):
        ...
        ssh_pkey_str = (
            normalize_newlines(self.connection.ssh_private_key)
            if self.connection.ssh_private_key
            else None
        )
        private_key_file = io.StringIO(ssh_pkey_str)
        ssh_pkey = paramiko.RSAKey.from_private_key(private_key_file)
        remote_bind_address = (
            self.connection.host,
            int(self.connection.port) if self.connection.port else 5432,
        )
        ...
        self.ssh_server = SSHTunnelForwarder(
            ...
            local_bind_address=("localhost", 10002),
        )
