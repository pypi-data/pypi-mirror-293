...
class Connector:
    ...
    def __init__(
        ...
        if connection is None:
            connection = ProfileYaml.find_connection(connection_slug)
        if connection is None:
            raise FileNotFoundError("No connections found in profiles.yaml")
    ...
    def execute_sql(self, sql: str) -> pd.DataFrame:
        if isinstance(self.connection, PostgresqlConnection):
            ...
            if self.is_cloud:
                ...
                access_token = BigqueryApi.get_access_token_from_service_account(
                    service_account_info=self.connection.keyfile_json.to_dict()
                )
            result = BigqueryUsecase(
                project_id=self.connection.project,
                access_token=access_token,
