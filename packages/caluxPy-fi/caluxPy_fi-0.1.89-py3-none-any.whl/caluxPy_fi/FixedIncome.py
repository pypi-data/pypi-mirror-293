    ...
    def __init__(self, settlement_date, issuance_date, maturity_date, coupon, ytm, face_value, issuer, methodology, periodicity, payment_type, coupon_type, 
        ...
        try:
            if self.maturity_date and self.settlement_date and self.maturity_date >= self.settlement_date:
                ...
                if not self.a_maturity or (self.coupon != 0 and self.coupon_type != 'normal'):
                ...
                try:
                    self._nper = math.trunc(Support.ln_nper(self.issuance_date, self.maturity_date, self.periodicity, self.coupon, self.coupon_type, self.a_maturity)) - 1
                except Exception as e:
                ...
                if self.payment_type in ['amortized', 'amortizado', 'pik']:
                    try:
                        self.notional_value, self.amortization_amount, self.adjusted_value, self.dates_results = self.amortizationModule(payment_type = self.payment_type, 
