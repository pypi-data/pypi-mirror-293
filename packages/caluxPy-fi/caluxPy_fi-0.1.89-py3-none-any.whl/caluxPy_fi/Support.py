...
class Support ():
    ...
    @staticmethod
    def ln_nper(issuance, maturity, fper, coupon, type, a_maturity):
        
        excep = False
        if coupon == 0:
            if type not in ['normal', 'none']:
                excep = True
        
        if not a_maturity and not excep:
    ...
    def days360(start_date,end_date,method_eu=False):
        ...
        if start_day == 31 or (not method_eu and start_month == 2 and (start_day == 29 or (start_day == 28 and not Support.is_leap_year(start_year)))):
            start_day = 30
