...
def get_format(commandline = None, output_file = None, config = None, reportheaders = None):
    log.debug(f"Getting format based on commandline: '{commandline}', output_file: '{output_file}', config: '{config}', reportheaders: '{reportheaders}'")
    ...
    if output_file:
        ...
        if ext:
            ext = ext[1:]
            log.debug(f"Extension of output file is {ext}")
...
class Report:
    ...
    def __init__(self, report_file, template:Template=None, templatefile:str="", default_template:str="", templatemapper:dict={}):
        self.timer = Timer()
        log.debug(f"Retrieving file markdown")
        ...
        log.debug(f"Finished report init")
    ...
    def _change_template_headers(self, templateheaders, reportheaders):
        if 'change_template' in reportheaders:
            log.debug("change_template header detected")
    ...
    def process_file(self, mdfile, format=None, default_format=None, output=None, default_output_file=None, watcher=None):
        ...
        log.debug(f"Begin processing file")
        if watcher:
            watcher.clear_paths()
            watcher.set_running()
        ...
        log.debug(f"{self.timer.time()}: Creating output formatter")
        ...
        log.debug("Setting reportbody imports")
        ...
        log.debug(f"{self.timer.time()}: Begin processing report body")
