    ...
    def __init__(self, templateheaders=dict(), timer=None, watcher=None):
        ...
        if not latexheaders['table-styles']:
            ...
            for name, style in ts.items():
                log.debug("Adding style {}".format(name))
            ...
            log.debug("NO TABLE STYLES: {}".format(
    ...
    def output(self, markdown, outputfile=''):
        ...
        if self.docformat and self.docformat != 'pdf':
            ...
            with open(outputfile, 'w') as fh:
                fh.write(output)
            log.info("Finished writing Markdown")

            return output

        log.debug("Writing PDF")
        log.debug(f"Latex template file: {self.templatefile}")
        log.debug(f"Output file: {outputfile}")
        ...
        log.info("Finished writing PDF file")
