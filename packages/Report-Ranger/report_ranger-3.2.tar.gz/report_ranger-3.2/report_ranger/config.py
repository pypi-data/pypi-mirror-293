...
def get_config(arg_file = None):
    ...
    final_config['templatemapper']["sample"] = os.path.join(os.path.dirname(os.path.realpath(report_ranger.__file__)),"../sample/sample-template/rr-sample-template.md") # Add in the sample template
    final_config['templatemapper']["plain"] = os.path.join(os.path.dirname(os.path.realpath(report_ranger.__file__)),"../sample/plain-template/rr-plain-template.md") # Add in the sample template
    ...
    for envvar in envvars.keys():
        log.info(f"Getting envvar {envvar}")
        var = os.getenv(envvar)
        log.info(f"Got {var}")
        if var != None:
            log.info(f"Setting config variable {envvars[envvar]} to {var} from environment variable {envvar}")
