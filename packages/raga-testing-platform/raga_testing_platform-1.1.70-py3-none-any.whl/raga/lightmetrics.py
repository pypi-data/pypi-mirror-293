...
def lightmetrics_model_upload(test_session:TestSession, file_path:str, name:str, version:str,  events, description:str,  api_version="v1"):
    if api_version == "v2":
        lightmetrics_model_upload_v2(test_session, file_path, name, events, description, version)
    else:
...
def lightmetrics_model_upload_v2(test_session:TestSession, file_path:str, name:str, events, description:str, version:str, override:bool=True):
    from raga.constants import (INVALID_RESPONSE, INVALID_RESPONSE_DATA,
                                REQUIRED_ARG_V2)
    assert isinstance(file_path, str) and file_path, f"{REQUIRED_ARG_V2.format('file_path', 'str')}"
    assert isinstance(name, str) and name, f"{REQUIRED_ARG_V2.format('name', 'str')}"
