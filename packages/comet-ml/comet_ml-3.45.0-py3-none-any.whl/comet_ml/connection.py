...
def _get_api_key_header(api_key: str) -> Dict[str, str]:
...
def _prepare_upload_params_and_headers(
    ...
    headers.update(_get_api_key_header(options.api_key))
...
def check_upload_images_limit_reached_error_response(
    ...
    try:
        data = response.json()
        code = data.get("sdk_error_code")
        if code is not None and code == IMAGE_UPLOAD_LIMIT_REACHED:
            upload_limits_guard.image_upload_limit_exceeded()
            return True
    except Exception:
        LOGGER.debug(msg="Failed to check image file upload limit")
    return False


def check_upload_embeddings_limit_reached_error_response(
    response: requests.Response, upload_limits_guard: FileUploadLimitsGuard
) -> bool:
    try:
        data = response.json()
        code = data.get("sdk_error_code")
        if code == EMBEDDINGS_LIMIT_REACHED:
            upload_limits_guard.embedding_upload_limit_exceeded()
            return True
    except Exception:
        LOGGER.debug(msg="Failed to check embedding file upload limit")
    return False


def check_upload_histogram_limit_reached_error_response(
    response: requests.Response, upload_limits_guard: FileUploadLimitsGuard
) -> bool:
    try:
        data = response.json()
        code = data.get("sdk_error_code")
        if code == HISTOGRAM_LIMIT_REACHED:
            upload_limits_guard.histogram_upload_limit_exceeded()
            return True
    except Exception:
        LOGGER.debug(msg="Failed to check histogram file upload limit")
    return False
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            if custom_error_msg:
                ...
                raise ImagesUploadLimitReachedException()
            elif check_upload_embeddings_limit_reached_error_response(
                response, upload_limits_guard
            ):
                raise EmbeddingsUploadLimitReachedException()
            elif check_upload_histogram_limit_reached_error_response(
                response, upload_limits_guard
            ):
                raise HistogramsUploadLimitReachedException()
