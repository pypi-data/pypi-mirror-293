...
from ._typing import AudioNormalization, Number
...
def convert_dict_to_string(user_data: Mapping) -> str:
    try:
...
def convert_to_string_truncated(
    ...
    if len(value) > size:
        LOGGER.warning(CONVERT_UTILS_CONVERT_TO_STRING_TRUNCATED_WARNING, value)
        indicator = " [truncated]"
...
def convert_to_string_value(user_data: Any, source: Optional[str] = None) -> str:
