...
class Streamer(BaseStreamer):
    ...
    def _loop(self):
        ...
        try:
            ...
            if self._message_batch_metrics.accept(self._send_metric_messages_batch):
                LOGGER.debug("Metrics batch has been sent")

            if self._message_batch_stdout.accept(self._send_stdout_messages_batch):
                LOGGER.debug("stdout/stderr batch has been sent")

            # attempt to send artifact remote assets batch
            if self._artifact_remote_assets_batch_enabled:
                if self._artifact_remote_assets_batch.accept(
                    self._send_artifact_remote_assets_batch
                ):
    ...
    def _flush_batches(self):
        ...
        LOGGER.debug("Streamer: start flushing batches")
        ...
        if self._artifact_remote_assets_batch_enabled:
            if not self._artifact_remote_assets_batch.empty():
                if not self._artifact_remote_assets_batch.accept(
                    self._send_artifact_remote_assets_batch, unconditional=True
                ):
                    success = False
        ...
        LOGGER.debug(
            "Streamer: flushing batches completed, successful: %s, elapsed: %r",
            success,
            elapsed,
        )
        return success
...
class FallbackStreamer(BaseStreamer):
    ...
    def wait_for_finish(
        ...
        LOGGER.debug("FallbackStreamer: wait_for_finish started")
        ...
        if create_offline_zip:
            LOGGER.debug("FallbackStreamer: trying to create offline ZIP")
            ...
            LOGGER.debug(
                "FallbackStreamer: offline ZIP created: %r, online streamer finished successfully: %r",
                archive_created,
        ...
        LOGGER.debug(
            ...
            "offline ZIP enabled: %r, offline ZIP created: %r",
