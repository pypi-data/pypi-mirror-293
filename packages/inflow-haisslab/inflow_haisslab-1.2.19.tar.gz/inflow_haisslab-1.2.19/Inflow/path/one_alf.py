...
def auto_rename_session_files_to_alf(selected_folder,dry_run = True, return_tables = False):
    ...
    origin_pathes = find_files(selected_folder, relative = False, levels = -1, get= "files" )#, re_pattern = '(?:.*\.tif.*)|(?:.*\.tdms)')
    new_pathes = path_to_alf(origin_pathes, force = True)
    conflicts, new_pathes = check_file_rename_conflicts(origin_pathes,new_pathes)
    returns = {}
    if len(conflicts):
        with pd.option_context('display.max_rows', 500, 'display.max_columns', 500, 'display.width', 1000, "expand_frame_repr" , False,'max_colwidth', None):
...
def apply_name_change(src_list,new_list,dry_run = True):
    ...
    if dry_run :
       logger.warning("The apply_name_change function was called with option dry_run = True. It will only show you what files it would have renamed, without actually renaming them.")
    ...
    for src_file , new_file in zip(src_list,new_list) :
        ...
        if new_file == 'AUTO-DELETE':
            if not dry_run :
                os.remove(src_file)
            continue
        if new_file == 'NOT_INCLUDED' : #will not be returned, so will not be used for file registration, but not deleted either
            continue
        if not dry_run :
...
def path_to_alf(filepath, force = False):
    ...
    if session_path is None :
        raise ValueError(f"The path is not a valid path of a session. Make sure you have a path matching : {one.alf.spec.path_pattern()}")
    ...
    if not force and one.alf.spec.is_valid(filepath, one.alf.spec.FULL_ABSOLUTE_SPEC):
        return '' #no file name change. We do not return None to stay compatible with string type columns of dataframes
...
def make_homogeneous_empty_dataset_from_filelist(files, session_id, repository_name, dry_run = True):
    ...
    session_details = cnx.to_session_details(cnx.alyx.rest("sessions", 'read', session_eid, no_cache = True), as_mode = "remote")
    ...
    for existing_dataset in session_details["data_dataset_session_related"]:
        ...
        if all(booleans) == True : #all keys are matching, the dataset already exists, returning it.
            logger.info(f"The dataset {d['collection']} - {d['dataset_type']} was already existing. Using it to attach files instead of creating a new one.")
...
def add_files_to_empty_homogeneous_dataset(files, dataset_dict ,dry_run = True):
    ...
    if dataset_dict is None :
        if not dry_run :
            raise ValueError("The dataset_dict returned by make_homogeneous_empty_dataset_from_filelist before calling add_files_to_empty_homogeneous_dataset seemed to be None, and dry run was not set to True. Cannot proceed.")
    ...
    for file in files :
        ...
        if not file_already_existing :
            d = {"dataset" : dataset_dict["id"] if dataset_dict is not None else "",
...
def register_files(file_list, session_id, repository_name = None, dry_run = True):
    ...
    if repository_name is None :
        ...
        if len(roots) > 1 :
            raise ValueError("More than one data repository was found for the files given. The function cannot register files to more tha one session at once. Please change the file_list input.")
        ...
        for repo in cnx.alyx.rest("data-repository","list",no_cache = True):
            ...
            if value == root:
                ...
                found = True
                logger.info(f"Found a common data-repository in the filepaths to use for registering : {repository_name}")
                break
        if not found :
            raise ValueError("No existing data repository was found for the location of the files you are trying to register. Either check their location and move them, or add a new data repository")

    if dry_run :
       logger.warning("The register_files function was called with option dry_run = True. It will only show you what files it would have registered, without actually registering them.")
       logger.warning("If things look ok, you can set it to False to actually do the process.")
    
    results = [one.alf.files.full_path_parts(file, as_dict = True, assert_valid = False, absolute = True) for file in file_list]
    ...
    for collection_name, collection_selection in fileparts_df.groupby("collection",dropna=False):
        for filenames, files_collections in collection_selection.groupby(["object","attribute"],dropna=False):
            if files_collections.isna().all().all() : continue # if file is realy not supporting the ALF standard, we won't be able to recompose it with one.alf.spec.to_full_path, so we won't include it in addible datasets anyway
            if files_collections.iloc[0]["collection"] == "preprocessing_saves" : continue
            if files_collections.iloc[0]["collection"] == "figures" : continue
            ...
            if sum(bools) == len(bools) : # all files have been reconstitued correctly and correspond to existing files
                ...
                new_dataset = make_homogeneous_empty_dataset_from_filelist(files,session_id,repository_name,dry_run = dry_run)
