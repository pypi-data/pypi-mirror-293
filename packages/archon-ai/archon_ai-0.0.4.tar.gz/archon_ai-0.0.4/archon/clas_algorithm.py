...
class CLAS:
    ...
    def get_model_power_ranking(self):
        if os.path.exists(self.search_config["power_ranking_save_path"]):
            print(f"Loading power ranking from file: {self.search_config['power_ranking_save_path']}")
            ...
            os.makedirs(os.path.dirname(self.search_config["power_ranking_save_path"]), exist_ok=True)
            os.makedirs(os.path.dirname(self.search_config["answers_save_path"]), exist_ok=True)

            # Create configs for every model in power ranking and baseline model
            for model in self.search_config["models_available"] + [self.search_config["baseline_model"]]:
                model_config = {
                    "name": model,
                ...
                save_path = append_json(os.path.join(self.search_config["answers_save_path"], model)) # Save the config to correct folder
                self.create_archon_config(model_config, save_path=save_path)
                print(f"Created Archon config for {model}, saved to: {self.get_save_path(model_config)}")
            ...
            if self.search_config["baseline_model"] not in self.search_config["models_available"]:
                model_power_ranking.remove(parse_model_name(self.search_config["baseline_model"]))
            ...
            with open(self.search_config["power_ranking_save_path"], 'w') as file:
                ...
                print("Saved power ranking to file:", self.search_config["power_ranking_save_path"])
    ...
    def create_archon_config(self, archon_config_dict, save_path=None):
        ...
        def add_critic_layer(critic_model):
            return [{
                ...
                "max_context_length": self.general_model_config["max_context_length"],
                "samples": 1
            }]
        ...
        archon_config["layers"].append([{
            ...
            "model_type": self.MODEL_TO_API_DICT[parse_model_name(archon_config_dict["model_power_ranking"][i])],
            "temperature": self.general_model_config["temperature"],
            "max_context_length": self.general_model_config["max_context_length"],
            "samples": 1
        ...
        for fuser_layer_key in ["fuser_layer_1", "fuser_layer_2", "fuser_layer_3"]:
            if archon_config_dict[fuser_layer_key] > 0:
                ...
                archon_config["layers"].append([{
                    ...
                    "model_type": self.MODEL_TO_API_DICT[archon_config_dict["model_power_ranking"][j]],
                    "temperature": self.general_model_config["temperature"],
                    "max_context_length": self.general_model_config["max_context_length"],
                    "samples": 1
        ...
        if archon_config_dict["add_final_fuser"]:
            archon_config["layers"].append([{
                ...
                "model_type": self.MODEL_TO_API_DICT[archon_config_dict["final_fuser_model"]],
                "temperature": self.general_model_config["temperature"],
                "max_context_length": self.general_model_config["max_context_length"],
                "samples": 1
            }])
    ...
    def run_archon_config(self, model_top_k_choice, sample_top_k_choice, fuser_layer_1_choice, fuser_layer_2_choice, fuser_layer_3_choice):
        # Map the choices to the closest valid value
        model_top_k_choice = self.find_nearest_value(model_top_k_choice, self.search_config["model_top_k_choices"])
        sample_top_k_choice = self.find_nearest_value(sample_top_k_choice, self.search_config["sample_top_k_choices"])
        fuser_layer_1_choice = self.find_nearest_value(fuser_layer_1_choice, self.search_config["fuser_layer_1_choices"])
        fuser_layer_2_choice = self.find_nearest_value(fuser_layer_2_choice, self.search_config["fuser_layer_2_choices"])
        fuser_layer_3_choice = self.find_nearest_value(fuser_layer_3_choice, self.search_config["fuser_layer_3_choices"])

        print(f"Running with Model Top-K: {model_top_k_choice}, Sample Top-K: {sample_top_k_choice}, "
        ...
        if total_inference_calls > self.search_config["maximum_inference_calls"]:
            ...
            _, model_to_score_dict = self.run_benchmark(archon_json, self.search_config["dataset_sample_for_search"])
    ...
    def perform_random_search(self):
        ...
        while len(archon_save_path_to_score) < self.search_config["random_search_iterations"]:
            random_choices = {
                "model_top_k": np.random.choice(self.search_config["model_top_k_choices"]),
                "sample_top_k": np.random.choice(self.search_config["sample_top_k_choices"]),
                "fuser_layer_1": np.random.choice(self.search_config["fuser_layer_1_choices"]),
                "fuser_layer_2": np.random.choice(self.search_config["fuser_layer_2_choices"]),
                "fuser_layer_3": np.random.choice(self.search_config["fuser_layer_3_choices"]),
            }
        ...
        for model_top_k_choice in self.search_config["model_top_k_choices"]:
            for sample_top_k_choice in self.search_config["sample_top_k_choices"]:
                for fuser_layer_1_choice in self.search_config["fuser_layer_1_choices"]:
                    for fuser_layer_2_choice in self.search_config["fuser_layer_2_choices"]:
                        for fuser_layer_3_choice in self.search_config["fuser_layer_3_choices"]:
                            archon_score = self.run_archon_config(
                                model_top_k_choice, sample_top_k_choice,
                                fuser_layer_1_choice, fuser_layer_2_choice, fuser_layer_3_choice
                            )
    ...
    def perform_bayesian_optimization(self):
        archon_save_path_to_score = {}
        pbounds = {
            "model_top_k_choice": (min(self.search_config["model_top_k_choices"]), max(self.search_config["model_top_k_choices"])),
            "sample_top_k_choice": (min(self.search_config["sample_top_k_choices"]), max(self.search_config["sample_top_k_choices"])),
            "fuser_layer_1_choice": (min(self.search_config["fuser_layer_1_choices"]), max(self.search_config["fuser_layer_1_choices"])),
            "fuser_layer_2_choice": (min(self.search_config["fuser_layer_2_choices"]), max(self.search_config["fuser_layer_2_choices"])),
            "fuser_layer_3_choice": (min(self.search_config["fuser_layer_3_choices"]), max(self.search_config["fuser_layer_3_choices"]))
        }
        ...
        for result in sorted_results:
            choices = {
                "model_top_k": self.find_nearest_value(result["params"]["model_top_k_choice"], self.search_config["model_top_k_choices"]),
                "sample_top_k": self.find_nearest_value(result["params"]["sample_top_k_choice"], self.search_config["sample_top_k_choices"]),
                "fuser_layer_1": self.find_nearest_value(result["params"]["fuser_layer_1_choice"], self.search_config["fuser_layer_1_choices"]),
                "fuser_layer_2": self.find_nearest_value(result["params"]["fuser_layer_2_choice"], self.search_config["fuser_layer_2_choices"]),
                "fuser_layer_3": self.find_nearest_value(result["params"]["fuser_layer_3_choice"], self.search_config["fuser_layer_3_choices"]),
            }
    ...
    def display_clas_results(self, clas_results):
        table_data = []
        for i, (archon_config, score) in enumerate(sorted(clas_results.items(), key=lambda x: x[1], reverse=True)):
            with open(archon_config, 'r') as file:
                archon_json = json.load(file)
            table_data.append([
                i + 1,
                archon_config.split("search_run_search_config_")[1][16:].replace(".json",""),
                score,
                self.calculate_total_inference_calls(archon_json)
            ])
        print(tabulate(table_data, headers=["Rank", "Archon Config", "Score", "Inference Calls"], tablefmt="grid"))

    def clas_algorithm(self):
        ...
        for archon_config_tuple in top_10_archon_save_paths:
            ...
            archon_json["save_path"] = archon_json["save_path"].replace(".json", "_final_ranking.json")
            ...
            _, model_to_score_dict = self.run_benchmark(archon_json, self.search_config["dataset_sample_for_final_ranking"])
            archon_save_path_to_score_complete_dataset[archon_json["save_path"]] = list(model_to_score_dict.values())[0]

        self.display_clas_results(archon_save_path_to_score_complete_dataset)
...
def main():
    parser = argparse.ArgumentParser(description="Run the CLAS algorithm with a given search configuration.")
    parser.add_argument('--search-config', type=Path, required=True, help="Path to the JSON file containing the search configuration.")
    ...
    base_output_path = os.path.join(search_config['base_output_directory'], search_config_name)
    search_config["power_ranking_save_path"] = os.path.join(base_output_path, "power_ranking_config.json")
    search_config["answers_save_path"] = os.path.join(base_output_path, "configs", "individual_model_configs")
    search_config["save_directory"] = os.path.join(base_output_path, "model_generations")
    ...
    # Initialize and run the CLAS algorithm
    clas = CLAS(search_config=search_config, general_model_config=general_model_config)
    clas.clas_algorithm()
