...
import yaml
from mix_eval.utils import parse_mix_eval_options, construct_mix_eval_prompt_freeform, construct_mix_eval_prompt_multichoice
from code_contests.utils import get_python_solutions
import os

IMAGE_TAGS = ["<image>", "[Image]"]
PROMPT = "Q: Write python code to solve the following coding problem that obeys the constraints and passes the example test cases. The output code needs to read from and write to standard IO. Please wrap your code answer using ```:"
...
class MtBenchBenchmark(Benchmark):
    ...
    def load_dataset(self):
        question_file = (
            "mt_bench/FastChat/fastchat/llm_judge/data/mt_bench/question.jsonl"
        )
...
class ArenaHardAutoBenchmark(Benchmark):
    ...
    def load_dataset(self):
        question_file = "arena_hard_auto/arena_questions.jsonl"
        self.dataset = datasets.load_dataset("json", data_files=question_file)["train"]
        if self.dataset_sample < 1.0:
            self.dataset = self.dataset.select(
                range(int(len(self.dataset) * self.dataset_sample))
...
class MixEvalBenchmark(Benchmark):
    ...
    def load_dataset(self):
        ...
        self.dataset = datasets.concatenate_datasets([dataset['free_form'], dataset['multiple_choice']])
    ...
    def save_answers(self, output_path, answers=None):
        ...
        free_form_path = os.path.join(dir_name, base_filename.replace(".jsonl", "_free_form.jsonl"))
        mult_choice_path = os.path.join(dir_name, base_filename.replace(".jsonl", "_mult_choice.jsonl"))

        free_form_answers = [a for a in answers if a["problem_type"] == "free-form"]
        mult_choice_answers = [a for a in answers if a["problem_type"] == "multiple-choice"]
...
class MixEvalHardBenchmark(Benchmark):
    ...
    def save_answers(self, output_path, answers=None):
        ...
        # Update the file paths to be inside the created directory
        base_filename = os.path.basename(output_path)
        free_form_path = os.path.join(dir_name, base_filename.replace(".jsonl", "_free_form_hard.jsonl"))
        mult_choice_path = os.path.join(dir_name, base_filename.replace(".jsonl", "_mult_choice_hard.jsonl"))
