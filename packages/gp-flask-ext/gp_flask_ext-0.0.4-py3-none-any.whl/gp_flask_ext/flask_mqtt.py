    ...
    if config.get("blueprint", True):
        ...
        def events():
            def generate():
                ...
                try:
                    while True:
                        message = _mqtt.poll(tag, timeout=1)
                        if message is None:
                            continue
                        yield f"data: {json.dumps(message)}\n\n"
                finally:
                    logger.info(f"Closing session: {tag}")
                    _mqtt.stop_session(tag)
        ...
        def sse():
            return render_template("mqtt/sse.html", topic=_mqtt.subscribe_topic)
