...
class MqttClient:
    ...
    def __init__(self, config, user_data=None):
        ...
        # 设置用户数据
        _mqtt.user_data_set(self.user_data)
        # 设置 TLS
        if self.tls:
    ...
    def on_message(self, client, userdata, message: MQTTMessage):
        logger.debug("message: {} {}", message.topic, message.payload.decode())
    ...
    def on_connect_fail(self, client: Client, userdata):
        logger.info("Connect failed with result code {}", self)

    def poll(self, tag="Default"):
