...
import collections.abc as abc2, enum, inspect as ins, zipfile as zi, tkinter as tk, dis, types as ty, typing as tp, typing_extensions as tpe, string as st, functools as ft, uuid as uu, re
...
class TypeVar(type):
    if sys.version_info >= (3, 13):
        def __new__(cls, name: str, *constraints, bound = None, covariant = None, contravariant = None, default = tp.NoDefault, inferVariance = False):
            return tp.TypeVar(name, *constraints, bound = bound, covariant = covariant, contravariant = contravariant, default = default, infer_variance = inferVariance)
    elif sys.version_info >= (3, 12):
        def __new__(cls, name: str, *constraints, bound = None, covariant = None, contravariant = None, inferVariance = False):
            return tp.TypeVar(name, *constraints, bound = bound, covariant = covariant, contravariant = contravariant, infer_variance = inferVariance)
    else:
        def __new__(cls, name: str, *constraints, bound = None, covariant = None, contravariant = None):
            return tp.TypeVar(name, *constraints, bound = bound, covariant = covariant, contravariant = contravariant)
"""
...
TypeVarExtended = tpe.TypeVar
"\\@since 0.3.26rc3. `typing_extensions.TypeVar`"
Callback = Callable
...
###### VERSION ASCENDING (forced Python 3.9 or above cuz string-subscripted type notations) ######

# since Python 3.5.2
NamedTuple = tp.NamedTuple
...
IO = _IO[_T_sb]
"\\@since 0.3.26rc3. [`typing.IO`](https://docs.python.org/3/library/typing.html#abcs-for-working-with-io) (≥ 3.9)"
TextIO = _TextIO
...
class ZipExtFile(zi.ZipExtFile):
    "\\@since 0.3.26rc2. File-like object for reading an archive member. Returned by `ZipFile.open()`. See [`zipfile.ZipFile`](https://docs.python.org/3/library/zipfile.html#zipfile.ZipExtFile)"
    ...
...
"\\@since 0.3.26rc2. An enumerator class for `__buffer__` method. Equal to `inspect.BufferFlags`"
