...
which might no longer be developed, comparing to this module. \\
Documentation: https://aveyzan.glitch.me/tense/py.html

Submodules (revitalized on 0.3.26rc3):
...
import time, warnings, types as ty, typing as tp, os
...
_LenGetItemOperable = tc.TypeOrFinalVarType[tc.PickSequence[_T]]
...
_HaveCodeType = _uni[
    ...
    str, bytes, bytearray, ty.MethodType, ty.FunctionType, tc.CodeType, type, tc.AnyCallable
]
...
class _ProbabilityLength(tc.IntegerFlag):
    "\\@since 0.3.26rc2. Internal class for class `tense.Tense`"
...
class _BisectMode(tc.IntegerFlag):
    "\\@since 0.3.26rc2. Internal class for method `Tense.bisect()`"
    BISECT_LEFT = 0
    BISECT_RIGHT = 1

class _InsortMode(tc.IntegerFlag):
...
class Tense(NennaiAbroads, NennaiStringz, NennaiRandomize, Time, Math, tc.Positive[str], tc.Negative[str], tc.Invertible[str]):
    """
    ...
    def __str__(self):
        ...
        try:
            ...
            subcl = f"'{NennaiAbroads.__name__}', '{NennaiRandomize.__name__}, '{NennaiStringz.__name__}', '{Math.__name__}', '{Time.__name__}'"
        return f"""
    ...
    def timeit(
        self,
        statement: __tc.Optional[__tc.Union[__tc.AnyCallable, str]] = None,
        /,
        setup: __tc.Optional[__tc.Union[__tc.AnyCallable, str]] = None,
        timer: __tc.Callable[..., float] = TIMEIT_TIMER_DEFAULT,
    ...
    @_cm
    def bisect(self, seq: __tc.LenGetItemOperable[_T], item: _T_richComparable, low: int = 0, high: _opt[int] = None, mode: _BisectMode = BISECT_RIGHT, key: _opt[_cal[[_T], _T_richComparable]] = None):
    ...
    @_cm
    def first(self, seq: _LenGetItemOperable[_T1], condition: _opt[_cal[[_T1], bool]] = None, /, default: _T2 = None):
        """
        \\@since 0.3.26rc2
        ```ts \\
        "class method" in class Tense
        ...
        return default
    @_cm
    def last(self, seq: _LenGetItemOperable[_T1], condition: _opt[_cal[[_T1], bool]] = None, /, default: _T2 = None):
        """
        \\@since 0.3.26rc2
        ```ts \\
        "class method" in class Tense
        ...
        return default
    @_cm
    def any(self, seq: _LenGetItemOperable[_T], condition: _opt[_cal[[_T], bool]] = None, /):
        """
        \\@since 0.3.26rc2
        ```ts \\
        "class method" in class Tense
        ...
        return a
    @_cm
    def all(self, seq: _LenGetItemOperable[_T1], condition: _opt[_cal[[_T1], bool]] = None, /, default: _T2 = None):
        """
        \\@since 0.3.26rc2
        ```ts \\
        "class method" in class Tense
    ...
    def probability(self, *vf: _ProbabilityType[int], length: _ProbabilityLengthType = PROBABILITY_COMPUTE):
        ...
        Aveyzan's case). Since 0.3.26rc1 you can also append `tense.types_collection.FinalVar` instance, which holds an integer.
        """
        # explanation:
        ...
        return self.pick(a1)
    @_cm
    def until(self, desiredString: _LenGetItemOperable[str], /, message: _opt[str] = None, caseInsensitive: bool = True):
        """
        \\@since 0.3.25
        ```ts \\
        "class method" in class Tense
...
class _ChangeVarState(tc.IntegerFlag):
...
class _ColorStyling(tc.IntegerFlag):
...
class _ColorAdvancedStyling(tc.IntegerFlag):
...
class Color(tc.ModuloOperable, tc.UnaryOperable):
    """
    \\@since 0.3.26rc1 \\
    \\@lifetime ≥ 0.3.26rc1
    ```ts \\
    ...
    **Warning**: 24-bit colors remains an experimental thing. Rather use 8-bit \\
    ...
    def __prepare_return(self):
        ...
        if _s != "\033[":
            ...
            _s = self.__text
        return _s
    def __init__(self, text: str, /, bits: _Bits = 8, foregroundColor: _Color = None, backgroundColor: _Color = None, underlineColor: None = None) -> None: # slash since 0.3.26rc2
        """
        ...
        At all it shouldn't be used, as 'underline color' isn't in standard. Due to that since 0.3.26rc2 any other \\
        type than `None` throws an error. See https://en.wikipedia.org/wiki/ANSI_escape_code#3-bit_and_4-bit \\
        for color palette outside 24-bit colors
        """
