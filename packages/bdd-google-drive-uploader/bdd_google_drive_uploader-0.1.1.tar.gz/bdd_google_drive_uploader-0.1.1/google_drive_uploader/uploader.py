...
class GoogleDriveUploader:
    def __init__(
        ...
        self.drive_manager = GoogleDriveManager(self.auth_manager.creds)
    ...
    def upload_to_drive(
        ...
        if drive_id:
            target_folder_id = self.drive_manager.find_folder_by_path(
                drive_id, drive_folder_path
            )
            if target_folder_id:
                print(f"Ready to upload to folder ID: {target_folder_id}")
                self.file_uploader.upload_file(
                    target_folder_id, upload_file_path, mimetype
                )
            else:
