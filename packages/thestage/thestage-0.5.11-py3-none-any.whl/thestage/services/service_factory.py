...
class ServiceFactory:
    ...
    def get_storage_service(self, config_provider: Optional[ConfigProvider] = None) -> StorageService:
    ...
    def get_thestage_api_client(self) -> TheStageApiClient:
        if not self.__thestage_api_client:
            self.__thestage_api_client = TheStageApiClient(api_url=self.__config_provider.get_full_config().main.config_api_link)
    ...
    def get_task_service(self, config_provider: Optional[ConfigProvider] = None) -> TaskService:
        return TaskService(
            thestage_api_client=self.get_thestage_api_client(),
            config_provider=config_provider if config_provider else self.__config_provider,
        )
