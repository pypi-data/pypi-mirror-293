...
app = typer.Typer(no_args_is_help=True, help=__("Help manage configuration settings"))
...
def config_get():
    ...
    service_factory = validate_config_and_get_service_factory(no_dialog=True)
...
@app.command(name='set', no_args_is_help=True, help=__("Update a configuration setting"))
def config_set(
    token: str = typer.Option(
            None,
            "--api-token",
            "-t",
            help=__("Initialize or update API token"),
            is_eager=False,
            is_flag=True,
        ),
):
...
def config_clear():
    ...
    """
    app_logger.info(f'Start config from {get_current_directory()}')
    service_factory = validate_config_and_get_service_factory(no_dialog=False)
