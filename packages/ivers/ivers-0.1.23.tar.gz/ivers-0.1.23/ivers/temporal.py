...
def leaky_endpoint_split(df: DataFrame, split_size: float, smiles_column: str, endpoint_date_columns: Dict[str, str], exclude_columns: List[str]) -> Tuple[DataFrame, DataFrame]:
    ...
    for endpoint in endpoint_date_columns.keys():
        ...
        # Ensure that SMILES identified for testing any endpoint are excluded from the training sets of those endpoints
        endpoint_specific_train_df = df[~df[smiles_column].isin(all_test_compounds)]
        ...
        train_df = endpoint_specific_train_df[endpoint_specific_train_df[endpoint].notnull()]
        
        test_dfs.append(test_df)
        train_dfs.append(train_df)

    # Concatenate all training and testing DataFrames to avoid duplication
    all_train_df = pd.concat(train_dfs).drop_duplicates()
    all_test_df = pd.concat(test_dfs).drop_duplicates()

    # Group by SMILES and apply aggregation to prevent any accidental leakage
