...
def allforfree_endpoint_split(df: DataFrame, split_size: float, smiles_column: str,
    ...
    # Convert the relevant date columns to datetime format and handle aggregation
    #for endpoint, date_column in endpoint_date_columns.items():
    #    df[date_column] = pd.to_datetime(df[date_column])

    # Group by SMILES and handle aggregation for each date column
    for endpoint, date_column in endpoint_date_columns.items():
        if aggregation == 'min':
            df['tmp_date'] = df.groupby(smiles_column)[date_column].transform('min')
        elif aggregation == 'max':
            df['tmp_date'] = df.groupby(smiles_column)[date_column].transform('max')
        elif aggregation == 'avg':
            df['tmp_date'] = df.groupby(smiles_column)[date_column].transform('mean')
        else:
            raise ValueError("Invalid aggregation method. Use 'min', 'max', or 'avg'.")

        # Update compound_dates for determining test compounds
        unique_dates = df.groupby(smiles_column)['tmp_date'].first()
        for compound, date in unique_dates.items():
    ...
    # Create initial splits based on the updated 'tmp_date' column
    ...
    aggregation_rules = {col: 'mean' if df_train[col].dtype in [np.float64, np.int64] and col not in exclude_columns and col != smiles_column 
                    else 'first' for col in df_train.columns}
