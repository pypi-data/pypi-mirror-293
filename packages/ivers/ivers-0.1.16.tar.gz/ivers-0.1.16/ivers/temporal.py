...
def allforfree_endpoint_split(df_list: List[pd.DataFrame], split_size: float, smiles_column: str, date_column: Dict[str, str]) -> Tuple[List[pd.DataFrame], List[pd.DataFrame]]:
    """
    ...
    Additionally, ensure each test set is as close to the desired size as possible by adding compounds in order of date until the size is met.
    ...
    # Second pass: Create initial splits
    for df in df_list:
    ...
    # Third pass: Adjust test set sizes
    for df, original_test_df in zip(df_list, test_dfs):
        target_size = int(len(df) * split_size)
        current_size = len(original_test_df)
        df_sorted = df.sort_values(by=date_column, ascending=False)
        if current_size < target_size:
            additional_needed = target_size - current_size
            additional_test_compounds = df_sorted[df_sorted[smiles_column].isin(set(df_sorted[smiles_column]) - set(original_test_df[smiles_column]))]
            additional_test_df = additional_test_compounds.head(additional_needed)
            adjusted_test_df = pd.concat([original_test_df, additional_test_df])
        else:
            adjusted_test_df = original_test_df.head(target_size)

        adjusted_test_dfs.append(adjusted_test_df)
        train_df = df[~df[smiles_column].isin(adjusted_test_df[smiles_column])]
