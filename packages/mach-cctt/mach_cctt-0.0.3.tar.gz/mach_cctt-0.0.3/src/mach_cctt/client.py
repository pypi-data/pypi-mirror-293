...
class MachClient:
    ...
    def _handle_response(
        ...
        match response.status_code:
            ...
            case 422:
                ...
                raise pydantic.ValidationError("Invalid request")
    ...
    def deployments(self) -> dict:
        ...
        chains = frozenset(config.rpc_urls.keys()) & frozenset(deployments.keys())

        for chain in frozenset(deployments.keys()) - chains:
            print(f"Warning: {chain=} RPC URL missing from config")
    ...
    def chains(self) -> frozenset:
        return frozenset(self.deployments.keys())

    def symbols(self, chain: str) -> frozenset:
    ...
    def get_token_balances(self, wallet_address: str) -> dict[str, dict[str, int]]:
        ...
        response = self.session.get(MachClient.token_balances, json=data)

        return self._handle_response(response)["balances"]
