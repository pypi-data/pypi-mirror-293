...
import bs4
import httpx
from .types import HittingStats, PitchingStats, StatCast
...
class Client:
    ...
    def soup(self) -> bs4.BeautifulSoup:
        return bs4.BeautifulSoup(self._content, "html.parser")
    ...
    async def get_stats(self, session: httpx.AsyncClient) -> StatCast:
        async with self.player.semaphore:
            ...
            self.get_hitting()
            self.get_pitching()
            return self.player.statcast

    def get_stats_sync(self) -> StatCast:
