...
# @unittest.mock.patch('digiflow.requests.get')
def test_request_handler_no_records_in_state(tmp_path):
    """Behavior when server down"""

    # arrange
    list_label = 'oai-record-test'
    a_state = 'zkw_ingest_busy'
    client_name = '127.0.0.1'
    client = df_r.Client(list_label, '1.2.3.4', '9999')
    exc_msg = f'connection timeout after {client.timeout_secs} seconds'
    # srv_rsp = f'{df_rs.DATA_EXHAUSTED_MARK.format(a_state, list_label)}'.encode()
    list_dir = tmp_path / 'lists'
    list_dir.mkdir()
    handler_info = df_r.HandlerInformation(list_dir, None)
    hndlr = df_r.RecordRequestHandler(handler_info, list_label, '1.2.3.4', '9999')
    # mock_resp = unittest.mock.Mock()
    # mock_resp.status_code = 404
    # mock_resp.headers = {'Content-Type': 'text/xml'}
    # mock_resp.content = srv_rsp
    ...
    # act
    with pytest.raises(df_r.RecordsServiceException) as rse:
        hndlr.get_next_record(list_label, client_name,
                              requested_state=a_state, set_state='zkw_ingest_done')
        # client.get_record(get_record_state=a_state, set_record_state='done')
