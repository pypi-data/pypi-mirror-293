...
def study(keyword: str, description: str, author_nickname: str):
    ...
    '''만일, 학습 시킨 사람이 출력이 안되게 하고 싶으시다면 author_nickname을 None으로 설정해주세요.'''
    ...
    if korcen.check(description) or korcen.check(keyword) or korcen.check(keyword + description) or korcen.check(description + keyword):
        return "나쁜 말은 배우기 싫어요....\n`©️korcen을 사용하여 필터링 되었습니다."
...
def study_say(keyword: str):
    ...
    basic_word = {
        ...
        'shii_study': f'`pip install -U shii_study`'
        }
    if keyword in basic_word:
        return basic_word[keyword]
    else:
        if info:
            if info['author_nickname'] == None:
