...
from .param import calender_days  # 假设此部分没有问题
...
def date2str(date, date_format='%Y-%m-%d'):
    ...
    if isinstance(date, datetime64):
        return date2str(str(date.astype('datetime64[D]').astype(datetime.date)),date_format)
...
def get_trading_days_diff(start_date, end_date):
    ...
    return max(
        0, int(calender_days[str(e_date)] - calender_days[str(s_date)])
    )
