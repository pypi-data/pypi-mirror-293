...
class OCR:
    ...
    def __get_session(self):
        ...
        else:  # If you use environment variables to authorize
            self.__session = boto3.Session()
            return self.__session

    def __get_s3_resource(self):
        if self.__s3_resource is None:
            self.__s3_resource = self.__get_session().resource("s3")
        return self.__s3_resource

    def __get_s3_client(self):
        if self.__s3_client is None:
            self.__s3_client = self.__get_session().client("s3")
        return self.__s3_client

    def __step_function(self):
        if self.__sf_client is None:
            self.__sf_client = self.__get_session().client("stepfunctions")
        return self.__sf_client

    def login(self, role_arn: str, external_id: str, region: str = "us-east-1"):
        """
    ...
    def login_via_okta(
        ...
        self.__session = okta.get_aws_session()
