...
class MNISTDataLoader:
    ...
    def display_sample_data(self, data: DataLoader, num_samples) -> None:
        """
        ...
        plot_images(images=images[:num_samples], cls_true=labels[:num_samples], label_names=self.class_names, title="Sample MNIST Images")
    ...
    def load_data(self, split_train_val_proportion, train_size, test_size):
        ...
        if train_size is None:
            ...
            train_size_full = int(split_train_val_proportion[0] * train_size)
            val_size = train_size - train_size_full

        train_subset, val_subset = random_split(train_dataset, [train_size_full, val_size])

        if train_size is not None and train_size < len(train_subset):
            train_indices = np.random.choice(len(train_subset), train_size_full, replace=False)
