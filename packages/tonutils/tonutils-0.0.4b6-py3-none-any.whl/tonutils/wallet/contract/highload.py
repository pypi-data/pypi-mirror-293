...
from ..utils import generate_query_id_and_created_at
...
class HighloadWalletV3(Wallet):
    ...
    def raw_create_transfer_msg(
        ...
        assert self.timeout < (1 << 22), "Too big timeout"
        assert self.timeout > 5, "Too small timeout"

        created_at, query_id = generate_query_id_and_created_at()

        value, actions = self.pack_actions(query_id, messages)
        message_to_send = self.create_internal_msg(
            dest=self.address,
            body=actions,
            value=value,
        )
        ...
        signing_message = (
            ...
            .store_ref(message_to_send.serialize())
            .store_uint(3, 8)
    ...
    @classmethod
    def pack_actions(cls, query_id: int, messages: List[WalletMessage]) -> Tuple[int, Cell]:
        """
        ...
        """
        messages_per_pack = 253

        if len(messages) > messages_per_pack:
            rest = cls.pack_actions(query_id, messages[messages_per_pack:])
            messages = messages[:messages_per_pack] + [rest]

        value, list_cell = 0, Cell.empty()
        ...
        for msg in messages:
            ...
            msg = (
                ...
                .store_ref(
                    begin_cell()
                    .store_cell(msg.message.serialize())
                    .end_cell()
                )
                .end_cell()
            )
