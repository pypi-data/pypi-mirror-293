...
def information_gain(df, split_attribute, target_attribute, battr):
    ...
    old_entropy = entropy_of_list(df[target_attribute], battr) if battr != 'S' else entropy_of_list(df[target_attribute], battr)
...
def id3(df, target_attribute, attribute_names, default_class=None, default_attr='S'):
    ...
    if len(cnt) == 1:
        return next(iter(cnt))  # Pure class

    elif df.empty or (not attribute_names):
        return default_class  # No more attributes
        ...
        tree = {best_attr: {}}  # Create a tree node
