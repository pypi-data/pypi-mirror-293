...
class MysqlTableCompare:
    ...
    def get_query_full_table_statement_params(self, database: str, table: str, ckpt_row: dict = None):
        ...
        statement_with_condition = f"SELECT * FROM {database}.{table} {where_clause} ORDER BY {_key_colns} LIMIT {self.limit_size}"
        statement_without_condition = f"SELECT * FROM {database}.{table} ORDER BY {_key_colns} LIMIT {self.limit_size}"
    ...
    def compare_full_table(self, source_con: MySQLConnection, target_con: MySQLConnection):
        ...
        while True:
            self.logger.info(f"compare keys: {_keyval}")
            ...
            self.logger.info(f"source query statment: '{query_statement_src}', params '{query_params_src}'")
            self.logger.info(f"target query statment: '{query_statement_dst}', params '{query_params_dst}'")
            ...
            if len(diff_rows) >= 1:
                self.logger.info(f"diff rows: {len(diff_rows)}.")

                table_keys = get_table_keys(source_con, self.src_database, self.src_table)

                # source_diff_rows = get_table_rows_by_keys(source_con, self.src_database, self.src_table, table_keys, diff_rows)
                # target_diff_rows = get_table_rows_by_keys(target_con, self.dst_database, self.dst_table, table_keys, diff_rows)

                with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                    future_source_twice = executor.submit(get_table_rows_by_keys, source_con, self.src_database, self.src_table, table_keys, diff_rows)
                    future_target_twice = executor.submit(get_table_rows_by_keys, target_con, self.dst_database, self.dst_table, table_keys, diff_rows)

                source_rows_twice = future_source_twice.result()
                target_rows_twice = future_target_twice.result()

                diff_rows_twice = find_missing_in_b(source_rows_twice, target_rows_twice)
                if len(diff_rows_twice) >= 1:
                    self.write_different(diff_rows_twice)
                    self.logger.info(f"diff rows twice: {len(diff_rows_twice)}.")
                    self.different_rows_number += len(diff_rows_twice)
            ...
            processed_progress = round(self.processed_rows_number / self.source_table_rows_number, 2)
            self.logger.info(f"source rows: {len(source_rows)}, target rows: {len(target_rows)}.")
            self.logger.info(f"processed rows number: {self.processed_rows_number}, progress: {processed_progress}%.")
            self.logger.info(f"compare elapsed time: {get_elapsed_time(_start1, 2)}s.")
    ...
    def run(self) -> None:
        ...
        with connect(**self.source_dsn) as source_con, connect(**self.target_dsn) as target_con:
            ...
            self.logger.info(f"compare completed, processed rows: {self.processed_rows_number}, different: {self.different_rows_number}.")
