...
class MysqlTableCompare:
    ...
    def compare_full_table(self, source_con: MySQLConnection, target_con: MySQLConnection):
        ...
        while True:
            self.logger.info(f"start compare keys: {_keyval}")
            ...
            self.logger.info(f"batch_id[{batch_id}] processed rows number: {self.processed_rows_number}, progress: {processed_progress}%.")
            self.logger.info(f"batch_id[{batch_id}] compare elapsed time: {get_elapsed_time(_start1, 2)}s.")
            batch_id += 1
