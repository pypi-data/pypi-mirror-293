...
class MysqlTableCompare:
    ...
    def compare_full_table(self, source_con: MySQLConnection, target_con: MySQLConnection):
        ...
        while True:
            self.logger.info(f"batch_id[{batch_id}] start compare keys: {_keyval}")
            ...
            if len(diff_rows) >= 1:
                self.logger.info(f"batch_id[{batch_id}] diff rows: {len(diff_rows)}.")
                ...
                if len(diff_rows_twice) >= 1:
                    ...
                    self.logger.info(f"batch_id[{batch_id}] diff rows twice: {len(diff_rows_twice)}.")
            ...
            self.logger.info(
                f"batch_id[{batch_id}] processed rows number: {self.processed_rows_number}/{self.source_table_rows_number}, progress: {processed_progress}%."
            )
