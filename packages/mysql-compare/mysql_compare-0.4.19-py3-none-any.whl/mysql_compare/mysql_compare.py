...
class MysqlTableCompare:
    ...
    def compare_full_table(self, source_con: MySQLConnection, target_con: MySQLConnection):
        ...
        while True:
            ...
            self.logger.info(
                f"query source & target rows, source rows: {len(source_rows)}, target rows: {len(target_rows)}, progress: {processed_progress}/{self.source_table_rows_number}, elapsed time: {get_elapsed_time(_start1, 2)}s."
            )
