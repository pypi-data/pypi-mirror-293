...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        if datadictvalue["C_START_DATE"] != '':
            ...
            page.get_by_placeholder("mm-dd-yyyy").nth(0).fill(datadictvalue["C_START_DATE"])
        if datadictvalue["C_END_DATE"] != '':
            ...
            page.get_by_placeholder("mm-dd-yyyy").nth(1).fill(datadictvalue["C_END_DATE"])
        # page.get_by_role("row", name="Expand Lookup Code Display").get_by_label("Meaning").fill(datadictvalue["C_MNNG"])
        page.locator("//div[contains(@title,'Lookup Codes')]//following::input[contains(@placeholder,'mm-dd-yyyy')][2]//following::input[1]").fill(datadictvalue["C_MNNG"])
        if datadictvalue["C_DSCRPTN"] != '':
            ...
            page.locator("//div[contains(@title,'Lookup Codes')]//following::input[contains(@placeholder,'mm-dd-yyyy')][2]//following::input[2]").fill(datadictvalue["C_DSCRPTN"])
            page.wait_for_timeout(1000)
        if datadictvalue["C_TAG"] != '':
            # page.get_by_role("row", name="Expand Lookup Code Display").get_by_label("Tag").fill(datadictvalue["C_TAG"])
            page.locator("//div[contains(@title,'Lookup Codes')]//following::input[contains(@placeholder,'mm-dd-yyyy')][2]//following::input[3]").fill(datadictvalue["C_TAG"])
            page.wait_for_timeout(1000)
