...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        if datadictvalue["C_LAST_UPDTD_DATE"] != '':
            page.get_by_role("cell", name="m/d/yy Press down arrow to access Calendar Last Updated Date Select Date",
                         exact=True).get_by_placeholder("m/d/yy").fill(datadictvalue["C_LAST_UPDTD_DATE"].strftime("%m/%d/%Y"))
        if datadictvalue["C_PBLSHD_DATE"] != '':
            page.get_by_role("cell", name="m/d/yy Press down arrow to access Calendar Published Date Select Date",
                         exact=True).get_by_placeholder("m/d/yy").fill(datadictvalue["C_PBLSHD_DATE"].strftime("%m/%d/%Y"))
