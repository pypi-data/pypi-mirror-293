...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        # page.get_by_role("row", name="Expand Lookup Code Display").get_by_label("Meaning").fill(datadictvalue["C_CODE_MNNG"])
        page.locator("//div[contains(@title,'Lookup Codes')]//following::input[contains(@placeholder,'m/d/yy')][2]//following::input[2]").fill(datadictvalue["C_CODE_MNNG"])
        if datadictvalue["C_CODE_DSCRPTN"] != '':
            # page.get_by_role("row", name="Expand Lookup Code Display").get_by_label("Description").fill(datadictvalue["C_CODE_DSCRPTN"])
            page.locator("//div[contains(@title,'Lookup Codes')]//following::input[contains(@placeholder,'m/d/yy')][2]//following::input[3]").fill(datadictvalue["C_CODE_DSCRPTN"])
            page.wait_for_timeout(1000)
        if datadictvalue["C_TAG"] != '':
            # page.get_by_role("row", name="Expand Lookup Code Display").get_by_label("Tag").fill(datadictvalue["C_TAG"])
            page.locator("//div[contains(@title,'Lookup Codes')]//following::input[contains(@placeholder,'m/d/yy')][2]//following::input[4]").fill(datadictvalue["C_TAG"])
            page.wait_for_timeout(1000)
