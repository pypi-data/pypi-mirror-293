...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        if datadictvalue["C_NAME"] != PrevQuestionarie:
            ...
            page.wait_for_timeout(3000)
            page.get_by_role("textbox", name="Folder").type(datadictvalue["C_QSTNNR_LBRRY"])
            page.get_by_role("button", name="OK").click()
            page.wait_for_timeout(2000)
            # page.locator("[id=\"__af_Z_tooltip\"]").get_by_text(datadictvalue["C_QSTNNR_LBRRY"], exact=True).click(force=True)
            page.get_by_text(datadictvalue["C_QSTNNR_LBRRY"], exact=True).click(force=True)
            page.wait_for_timeout(5000)

            PrevQuestionarie = datadictvalue["C_NAME"]
        ...
        # Questionnarie Name
        page.get_by_label("Name").clear()
        page.get_by_label("Name").type(datadictvalue["C_NAME_1"])
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + PERF_CONFIG_WRKBK, QUESTIONNARIE):
    ...
    if rows > 0:
        ...
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", PERF_CONFIG_WRKBK)[0] + "_" + QUESTIONNARIE + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
