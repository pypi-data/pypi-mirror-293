...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        page.get_by_label("Line").clear()
        page.get_by_label("Line").fill(str(datadictvalue["C_LINE"]))
        page.get_by_label("Distribution", exact=True).fill(str(datadictvalue["C_DSTRBTN"]))
        page.locator("//span[text()='Description']//following::input[contains(@id,'i6::content')]").fill(datadictvalue["C_LINE_DSCRPTN"])
        page.get_by_label("Distribution Combination").fill(datadictvalue["C_DSTRBTN_CMBNTN"])
        if datadictvalue["C_INCM_TAX_TYPE"] != '':
            page.get_by_label("Income Tax Type").select_option(datadictvalue["C_INCM_TAX_TYPE"])
        ...
        try:
            ...
            print("Distribution set not saved")
            datadictvalue["RowStatus"] = "Distribution set are not added"
