...
from mcli.api.finetuning_runs.api_delete_finetuning_runs import delete_finetuning_runs
...
def confirm_run_update(runs: Union[List[Run], ObjectList[Run], List[Finetune], ObjectList[Finetune]],
                       action: str = 'delete') -> int:
...
@cli_error_handler('mcli delete finetuning-run')
def delete_finetuning_run(
    ...
    **kwargs,
    ...
    if not force and not confirm_run_update(finetuning_runs, 'delete'):
        logger.error(f'{FAIL} Canceling delete finetuning runs')
        return 1

    with console_status('Deleting finetuning runs...'):
        delete_finetuning_runs(finetuning_runs)

    logger.info(f'{OK} Deleted finetuning runs')
    return 0
