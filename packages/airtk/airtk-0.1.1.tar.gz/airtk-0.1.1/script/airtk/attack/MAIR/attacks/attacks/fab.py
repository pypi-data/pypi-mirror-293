        ...
        while counter_restarts < 1:
            ...
            while counter_iter < self.steps:
                with torch.no_grad():
                    ...
                    dg2 = dg[u1, ind]
                    b = -df[u1, ind] + (dg2 * x1).view(x1.shape[0], -1).sum(
                        dim=-1
                    )
                    w = dg2.reshape([bs, -1])
...
def projection_linf(points_to_project, w_hyperplane, b_hyperplane):
    ...
    lmbd_opt = torch.clamp_min(
        (b[c2] - sb[c2, lb]) / (-s[c2, lb]), min=0
    ).unsqueeze(-1)
    d[c2] = torch.min(lmbd_opt, d[c2]) * a[c2] + torch.max(
        -lmbd_opt, d[c2]
    ) * (1 - a[c2])
...
def projection_l2(points_to_project, w_hyperplane, b_hyperplane):
    ...
    s = torch.cat(
        (
            -w5 * rs[:, 0:1],
            torch.cumsum((-rs2 + rs) * ws, dim=1) - w5 * rs[:, 0:1],
        ),
        1,
    )
...
def projection_l1(points_to_project, w_hyperplane, b_hyperplane):
    ...
    if c2.any():
        ...
        u2 = torch.arange(
            0, w.shape[1], device=device, dtype=torch.float
        ).unsqueeze(0)
