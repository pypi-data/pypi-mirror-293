...
class Batches:
    ...
    def __iter__(self):
        ...
        return (
            {"input": x.to(device).float(), "target": y.to(device).long()}
        ...
        for _ in range(attack_iters):
            ...
            if mixup:
                ...
                loss = mixup_criterion(
                    criterion, model(normalize(X + delta)), y_a, y_b, lam
                )
            ...
            if norm == "l_inf":
                d = torch.clamp(
                    d + alpha * torch.sign(g), min=-epsilon, max=epsilon
                )

            elif norm == "l_2":
                g_norm = torch.norm(g.view(g.shape[0], -1), dim=1).view(
                    -1, 1, 1, 1
                )
...
def get_args():
    ...
    parser.add_argument(
        "--norm", default="l_inf", type=str, choices=["l_inf", "l_2"]
    )
    parser.add_argument(
        "--fgsm-init", default="random", choices=["zero", "random", "previous"]
    )
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                os.path.join(
                    args.fname, "eval.log" if args.eval else "output.log"
                )
            ),
    ...
    if args.val:
        ...
        val_set = list(
            zip(
                transpose(dataset["val"]["data"] / 255.0),
                dataset["val"]["labels"],
            )
        )

        val_batches = Batches(
            val_set, args.batch_size, shuffle=False, num_workers=2
        )
    ...
    train_set = list(
        zip(
            transpose(pad(dataset["train"]["data"], 4) / 255.0),
    ...
    test_set = list(
        zip(
            transpose(dataset["test"]["data"] / 255.0),
            dataset["test"]["labels"],
        )
    )

    test_batches = Batches(
        test_set, args.batch_size_test, shuffle=False, num_workers=2
    )
    ...
    if args.model == "ResNet18":
        ...
        proxy = ResNet18(num_classes=10)

    elif args.model == "WideResNet":
        model = WideResNet(depth=34, num_classes=10).to(device)

        proxy = WideResNet(depth=34, num_classes=10).to(device)
    ...
    if args.l2:
        ...
        params = [
            {"params": decay, "weight_decay": args.l2},
    ...
    opt = torch.optim.SGD(
        params, lr=args.lr_max, momentum=0.9, weight_decay=5e-4
    )

    proxy_opt = torch.optim.SGD(proxy.parameters(), lr=0.01)

    awp_adversary = AdvWeightPerturb(
        model=model, proxy=proxy, proxy_optim=proxy_opt, gamma=args.awp_gamma
    )
    ...
    if args.lr_schedule == "superconverge":
        ...
        lr_schedule = lambda t: np.interp(
            [t], [0, 0.4 * args.epochs, args.epochs], [0, args.lr_max, 0]
        )[0]
    ...
    if args.resume:
        ...
        if os.path.exists(os.path.join(args.fname, "model_best.pth")):
        ...
        if args.val:
            best_val_robust_acc = torch.load(
                os.path.join(args.fname, "model_val.pth")
            )["val_robust_acc"]
...
def main_atawp10():
    ...
    torch.save(
        model.state_dict(),
        os.path.join(
            model_dir, "model-at-awp-{}-{}.pt".format("cifar10", "res18")
        ),
    )
