    ...
    def single_attack(self, net, inp, label, eta, target=None):
        ...
        if target is not None:
            ...
            grad_sign = torch.autograd.grad(
                targets, adv_inp, only_inputs=True, retain_graph=False
            )[0].sign()  # was adv_in

        else:
            loss = self.criterion(pred, label)
            grad_sign = torch.autograd.grad(
                loss, adv_inp, only_inputs=True, retain_graph=False
            )[0].sign()
        ...
        tmp_eta = clip_eta(
            tmp_eta, norm=self.norm, eps=self.eps, DEVICE=self.DEVICE
        )
