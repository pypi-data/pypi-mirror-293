...
class FeatureScatterTraining:
    def __init__(
        ...
        n_classes: int = 10,
        image_size: int = 32,
        epsilon: float = 8.0 / 512,
        lr: float = 0.1,
        num_steps: int = 1,
        step_size: int = 8.0 / 512,
        ls_factor: float = 0.5,
        epochs: int = 200,
        batch_size: int = 128,
        momentum: float = 0.9,
    ...
    def __call__(self) -> None:
        main_featurescatter(self._ds_name, self._args, self._config)
...
# "--max_epoch", default=200, type=int, help="max number of epochs"
...
# "--decay_rate", default=0.1, type=float, help="learning rate decay rate"

# )

# parser.add_argument(

# "--batch_size_train", default=128, type=int, help="batch size for training"

# )

# parser.add_argument(

# "--momentum", default=0.9, type=float, help="momentum (1-tf.momentum)"
...
# "--dataset", default="cifar10", type=str, help="dataset"
...
def train_fun(epoch, net, trainloader, optimizer, args):
    print("\nEpoch: %d" % epoch)
    net.train()
...
def main_featurescatter(ds_name, args, config_feature_scatter):
    start_epoch = 0
    ...
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=args.batch_size_train, shuffle=True, num_workers=2
    )
    ...
    basic_net = WideResNet(
        depth=34, num_classes=args.num_classes, widen_factor=10
    ).to(device)
    ...
    if args.resume and args.init_model_pass != "-1":
        ...
        f_path = os.path.join(
            args.model_dir, ("checkpoint-%s" % args.init_model_pass)
        )
    ...
    for epoch in range(1, args.max_epoch):
        train_fun(epoch, net, trainloader, optimizer, args)


if __name__ == "__main__":
    main_featurescatter("cifar10")
