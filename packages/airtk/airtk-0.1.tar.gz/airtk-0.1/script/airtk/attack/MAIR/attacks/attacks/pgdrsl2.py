...
class PGDRSL2(Attack):
    ...
    def forward(
        ...
        if inputs.shape[0] * self.noise_batch_size > self.batch_max:
            ...
            inputs_split = torch.split(
                inputs, split_size_or_sections=split_num
            )
            labels_split = torch.split(
                labels, split_size_or_sections=split_num
            )
    ...
    def _forward(self, images, labels):
        ...
        shape = (
            torch.Size([images.shape[0], self.noise_batch_size])
            + images.shape[1:]
        )  # nopep8
        ...
        for _ in range(self.steps):
            ...
            delta_norms = torch.norm(
                delta.view(data_batch_size, -1), p=2, dim=1
            )
