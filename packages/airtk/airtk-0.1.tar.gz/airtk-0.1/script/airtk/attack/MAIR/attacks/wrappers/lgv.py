...
class LGV(Attack):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not isinstance(nb_models_epoch, int) or nb_models_epoch < 0:
            raise ValueError(
                "nb_models_epoch should be a non-negative integer"
            )
    ...
    def save_models(self, path):
        ...
        if len(self.list_models) == 0:
            raise RuntimeError(
                "Call collect_models() before saving collected models."
            )
    ...
    def forward(self, images, labels):
        ...
        if not self.base_attack:
            ...
            self.list_models = [
                model.to(self.device) for model in self.list_models
            ]
        ...
        if self.targeted:
            if self.attack_mode == "targeted":
                ...
                self.base_attack.set_mode_targeted_least_likely(
                    kth_min=self._kth_min
                )
            elif self.attack_mode == "targeted(random)":
                self.base_attack.set_mode_targeted_random()
            else:
                raise NotImplementedError(
                    "Targeted attack mode not supported by LGV."
                )
...
class LightEnsemble(nn.Module):
    def __init__(self, list_models, order="shuffle", n_grad=1):
        ...
        if not (n_grad > 0 or n_grad == -1):
            raise ValueError(
                "n_grad should be strictly positive or equal to -1"
            )
        if order == "shuffle":
            shuffle(list_models)
