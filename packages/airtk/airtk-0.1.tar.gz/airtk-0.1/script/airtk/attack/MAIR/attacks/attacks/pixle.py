...
class Pixle(Attack):
    ...
    def restart_forward(self, images, labels):
        ...
        for idx in range(bs):
            ...
            loss, callback = self._get_fun(
                image, label, target_attack=self.targeted
            )
            best_solution = None
    ...
    def _get_fun(self, img, label, target_attack=False):
        ...
        @torch.no_grad()
        def func(
            solution, destination=None, solution_as_perturbed=False, **kwargs
        ):
    ...
    def _perturb(self, source, solution, destination=None):
        ...
        source_pixels = np.ix_(
            range(c), np.arange(y, y + yl), np.arange(x, x + xl)
        )
