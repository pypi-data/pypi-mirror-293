...
class CurratTraining:
    def __init__(
        ...
        if dataset_name == "mnist":
            self._args = self._args._replace(net="cnn")

    def __call__(self) -> None:
        main_currat(self._args.data, self._args)
...
def advtrain_CAT(
    epoch, trainloader, validloader, args
):  # basic + batch mixing
    ...
    return validnSaveModel(acc, k, validloader, args)


def validnSaveModel(
    trainAcc, inK, validloader, args, new=True
):  # , k, acc tbd: saveModel contains: state->time->filename(including acc&k)->...
    global starttime, net, epoch, optimizer, k

    savefolder = args.save_name
...
def main_currat(ds_name, args):
    global starttime, epoch, k, prevTime, attack, net, optimizer
    ...
    while epoch < 1 + args.epochs:
        epoch = advtrain_CAT(epoch, trainloader, validloader, args)
