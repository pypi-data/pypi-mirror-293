...
```bash
 cipher-craftt --generate-password --length 16 --no-digits --obfuscate
```
- To generate a passphrase:

```bash
cipher-craftt  --generate-passphrase --wordlist path/to/wordlist.txt --num-words 5 --separator '-'
```

- To check password expiry:

```bash
cipher-craftt  --check-expiry your-password
```

- To record the creation date of a new password:

```bash
cipher-craftt  --record-password your-new-password
```
