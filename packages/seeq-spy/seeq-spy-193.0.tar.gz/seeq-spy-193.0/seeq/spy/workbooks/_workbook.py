...
from seeq.spy.workbooks._data import (Datasource, StoredOrCalculatedItem, CalculatedScalar, CalculatedCondition,
                                      CalculatedSignal, Chart)
...
class Workbook(ItemWithOwnerAndAcl):
    ...
    def referenced_items(self) -> List[Reference]:
        ...
        for worksheet in self.worksheets:
            referenced_items.update(worksheet.referenced_items)

        if self.scoped_items is not None:
            referenced_items.update(self.scoped_items)
    ...
    def _scrape_item_inventory(self, session: Session, item_cache: Optional[_common.LRUCache],
        ...
        # Hold this list in a separate variable so that we can fill in the item_search_preview on the references
        # (The self.referenced_items property gets rebuild every time you access it.)
        references = self.referenced_items

        self._fill_in_item_search_preview_on_references(session, references)
    ...
    def _fill_in_item_search_preview_on_references(self, session: Session, references: Iterable[Reference]):
        if not _login.is_sdk_module_version_at_least(62):
            return

        filtered_references = [r for r in references if r.item_search_preview is None]
        ...
        for i in range(len(filtered_references)):
            reference = filtered_references[i]
    ...
    def _scrape_inventory_from_item_async(self, session: Session, progress: ItemInventoryProgress,
        ...
        def _item_pull(_reference):
            ...
            if not isinstance(_pulled_item, (CalculatedSignal, CalculatedScalar, CalculatedCondition, Chart)):
    ...
    def _scrape_references_from_scope(self, session: Session):
        ...
        while True:
            ...
            self.scoped_items.extend([Reference(item.id, Reference.SCOPED,
