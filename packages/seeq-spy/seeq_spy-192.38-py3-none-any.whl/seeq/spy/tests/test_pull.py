...
def test_pull_condition_as_capsules():
    ...
    if pull_df.spy.status.df.iloc[0]['Data Processed'].bytes_processed >= 2_000_000:
        # TODO CRAB-42830 - This can be removed when the investigation is complete.
        processed = pull_df.spy.status.df.iloc[0]['Data Processed']
        raise AssertionError(f"Data Processed is larger than the expected 2MB: {processed.bytes_processed}B. "
                             f"Please comment on CRAB-42830 if you see this.\n"
                             f"Details: {processed.request_timings}\n")
...
def test_pull_condition_with_timezone_convert():
    ...
    expected_starts = ['2024-01-01 00:00:00-08:00', '2024-01-02 00:00:00-08:00']
    assert pulled_capsules['Capsule Start'].astype(str).to_list() == expected_starts
    expected_ends = ['2024-01-02 00:00:00-08:00', '2024-01-03 00:00:00-08:00']
    assert pulled_capsules['Capsule End'].astype(str).to_list() == expected_ends
