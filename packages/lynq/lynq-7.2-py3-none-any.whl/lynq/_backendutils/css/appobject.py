...
from lynq._backendutils.html.supportswith import SupportsWithKeyword
from lynq._backendutils.lynq.lynqserverorrelated import LynqServerOrRelatedObjects
from lynq._backendutils.html.supportedtags import supported_tags

class StyledAppObject(SupportsWithKeyword):
    ...
    def init_supported_tags(self, supported_tags: list[str]) -> None:
        for tag in supported_tags:
            exec(f"self.{tag} = lambda style = None, args = None: self.tag({repr(tag)}, style or \"\", args or \"\")", {"self": self})

    def tag(self, type_: str, style: Optional[str] = None, args: Optional[str] = None) -> Any:
        from lynq._backendutils.css.tagobject import StyledTagObject

        return StyledTagObject(self.name.removesuffix(".html"), type_, style, args)
