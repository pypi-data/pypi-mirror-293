...
class app(SupportsWithKeyword):
    ...
    def _init_root(self) -> None:
        self.export = new("export", (), # Export methods here:
            standard = self.export_func_standard,
            null = self.export_func_null,
            direct = self.export_func_direct,
            dontpass = self.export_func_nopass
        )

    def export_standard(self, fn: Callable) -> Callable:
        self.fn: Callable = fn

        from lynq._backendutils.pebl.standardappexportobject import StandardAppExportObject
