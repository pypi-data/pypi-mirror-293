...
def cli_get_tasks(ctx, skip: int, limit: int, status: str | None = None):
    ...
    if r.status_code == 200:
        ...
        tasks = pagination_convert_response_list(response, TaskModel)
    ...
    while True:
        ...
        if r.status_code == 200:
            ...
            if len(response.data) == 0:
                break
            for task in pagination_convert_response_list(response, TaskModel):
                idx += 1
                ...
                with open(fname, "w", encoding="utf8") as fd:
                    json.dump(task.model_dump(), fd, indent=2, cls=lsjsonclasses.LSoftJSONEncoder)
                click.echo("done.")
        else:
            raise click.ClickException(r.content)
