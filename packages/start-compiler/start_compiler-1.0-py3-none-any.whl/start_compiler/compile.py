...
def match(line, symbol, scope, member):
    ...
    elif symbol in built_in_functions and (MEMBER_TYPES_RESTRICTED or built_in_functions[symbol] == "string") and member not in built_in_types and member in types:
...
def toPython(t, state, indent, scope):
    ...
    # return statement for a function
        ...
        if t.children[0].data == 'class_variable':
            name = toPython(get(t, "expression"), state, "", scope)
            if get(t.children[0], 'variable') is not None:
                variable = val(get(t.children[0], 'variable'), 'name')
                try:
                    return f"self.{variable}, '{str(eval(name))}'"
                except NameError:
                    return f"self.{variable}, str({name})"
            else:
            ...
            key = toPython(get(t, "expression"), state, "", scope)
        ...
        if function not in user_functions and function not in built_in_functions:
            raise StartError(f"In line {line}: Function or constructor {function} not defined in '{scope_2_str(scope)}' scope!")
        ...
        result = f"\n{indent}if {toPython(get(t, 'expression'), state, '', scope)}:\n"
        ...
        result = f"\n{indent}while {toPython(get(t, 'expression'), state, '', scope)}:\n"
        result += toPython(get(t, 'block'), state, indent + '\t', scope)
        return result
...
def main(fileName, long_names):
    ...
    import_string = "import import_start\n\n"
