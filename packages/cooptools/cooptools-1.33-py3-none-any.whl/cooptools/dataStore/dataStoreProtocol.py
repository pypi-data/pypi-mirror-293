...
from cooptools.dataStore.qualifier import PatternMatchQuery

logger = logging.getLogger(__name__)

StorableData = IdentifiableProtocol | Dict | str
SUPPORTED_DICT_IDENTIFIERS = ['id', '_id', "obj_id", "obj"]
...
class DataStoreProtocol(Protocol):
    ...
    def get(self,
            ...
            limit: int = None,
            query: Dict = None,
            id_query: PatternMatchQuery = None) -> Dict[UniqueIdentifier, StorableData]:
        raise NotImplementedError()
...
def get_identifier(item: StorableData) -> UniqueIdentifier:
    if type(item) == dict and any(x in item.keys() for x in SUPPORTED_DICT_IDENTIFIERS):
        id = item[next(ii for ii in SUPPORTED_DICT_IDENTIFIERS if ii in item.keys())]
        return id
    if type(item) == str:
        try:
            val = json.loads(item)
