...
class CPU(metaclass=Singleton):
    ...
    async def getCpuRate(self, noLog=False):
...
class AppPerformanceMonitor(initPerformanceService):
    ...
    async def collectCpu(self):
        _cpu = CPU(self.pkgName, self.deviceId, self.platform, pid=self.pid)
        result = {}
        while self.is_collect:
            await asyncio.sleep(0.1)
            appCpuRate, systemCpuRate = await _cpu.getCpuRate(noLog=self.noLog)
    ...
    async def collectNetwork(self, wifi=True):
        global net_data
        global f
        _network = Network(self.pkgName, self.deviceId, self.platform, pid=self.pid)
        if self.noLog is False and self.platform == Platform.Android and net_data == None:
    ...
    def setPerfs(self):
        if self.platform == Platform.Android:
            ...
            scene, report_dir = f.make_report(script_name=self.script_name, app=self.pkgName, devices=self.deviceId, corenum=corenum,
    ...
    async def collectAll(self, iscollect):
        ...
        try:
            if self.is_collect:
                ...
                # is_stop = initPerformanceService.stop()
                time.sleep(10)
