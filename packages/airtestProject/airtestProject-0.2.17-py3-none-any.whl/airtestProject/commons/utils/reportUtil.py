...
class ReportUtil(LogToHtml):
    def __init__(self, script_root, log_root="", static_root="", export_dir=None, script_name="", logfile=None, lang="en", plugins=None, profile_report_dir=None):
        ...
        self.run_start = None
        self.run_end = None
        self.profile_report_new_dir = None
    ...
    def _make_export_dir(self):
        ...
        if self.profile_report_dir is not None:
            report_name = os.path.basename(self.profile_report_dir)
            self.profile_report_new_dir = os.path.join(self.export_dir, report_name)
            if os.path.exists(self.profile_report_dir):
                if os.path.isdir(self.profile_report_dir):
                    self.copy_tree(self.profile_report_dir, self.profile_report_new_dir)
    ...
    def report_data(self, output_file=None, record_list=None):
        ...
        data['report'] = self.get_profile_report(output_file)
