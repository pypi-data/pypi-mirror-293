...
class TestPreparation(State):
    def run(self, test_case):
        ...
        def perform_profile_task():
            log.info("性能测试初始化")
            if self.TaskMachine.loop is None:
                self.TaskMachine.loop = asyncio.new_event_loop()
                asyncio.set_event_loop(self.TaskMachine.loop)
            else:
                self.TaskMachine.loop = asyncio.get_event_loop()
            if test_case.is_profile is not None and test_case.is_profile is True:
                if self.TaskMachine.apm is None:
                    self.TaskMachine.apm = AppPerformanceMonitor(script_name=self.TaskMachine.file_path,
                                                                 ...
                                                                 duration=0)
                self.TaskMachine.profile_report = self.TaskMachine.loop.run_until_complete(self.TaskMachine.apm.collectAll(test_case.is_profile))

        if self.TaskMachine.executor is None:
            self.TaskMachine.executor = ThreadPoolExecutor(max_workers=1)
            self.TaskMachine.executor.submit(perform_profile_task)
        elif test_case.is_profile is False:
            log.info("性能采集结束")
            self.TaskMachine.loop = asyncio.get_event_loop()
            self.TaskMachine.profile_report = self.TaskMachine.loop.run_until_complete(self.TaskMachine.apm.collectAll(test_case.is_profile))
