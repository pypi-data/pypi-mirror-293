...
class EnglishTextInput:
    ...
    def draw_text(self,screen):
        if self.NowText=='':
            ...
            screen.blit(self.textbook, textRect)
            if self.IsFocus.is_true():
                pygame.key.set_text_input_rect((textRect.bottomleft[0],textRect.bottomleft[1],0,0))
                if self.ShowFocus.get():
                    pygame.draw.line(screen,self.DefaultTextColor,textRect.topleft,textRect.bottomleft,2)
        else:
            self.textbook = self.font.render(self.NowText, True, self.DefaultTextColor,None)
            textRect = self.textbook.get_rect()
            textRect.center=self.DrawBack_cen.center
            textRect.left=self.DrawBack_cen.left+self.outline
            screen.blit(self.textbook, textRect)
            #主文字
            if self.IsFocus.is_true():
                trybook = self.font.render(self.NowText[0:self.cursor], True, self.DefaultTextColor,None)
                ...
                #光标
                pygame.key.set_text_input_rect((trytextRect.bottomright[0],trytextRect.bottomright[1],0,0))
                if self.ShowFocus.get():
    ...
    def changeevent(self,event):
        ...
        if event.type == pygame.TEXTINPUT:
            ...
            if event.key == pygame.K_DELETE or event.key == pygame.K_BACKSPACE or self.cursor>=1:
                str_list = list(self.NowText)
                ...
                self.cursor+=1
                if self.cursor>=self.NowText.__len__():
                    self.cursor-=1
