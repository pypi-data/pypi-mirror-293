...
def SensorsSignalGeneration_frame(path_d_drate_amp): # input is ssp.Paths in ssp.config.CurrentFrame
    ...
    for radarSpecifications in ssp.RadarSpecifications:
        ...
        for specifications in radarSpecifications:
          ...
          # TotalBandwidth = 1/specifications['Ts']
    ...
    if ssp.config.CUDA_is_available and CUDA_signalGeneration_Enabled:
      for isrx,suiteRX_d_drate_amp in path_d_drate_amp.items():
          for irrx,radarRX_d_drate_amp in suiteRX_d_drate_amp.items():
            ...
            PRI = ssp.RadarSpecifications[isrx][irrx]['PRI']
            Ts = ssp.RadarSpecifications[isrx][irrx]['Ts']
            Frame_PulseTimes = ssp.RadarSpecifications[isrx][irrx]['RadarTiming'].Frame_PulseTimes
    ...
    for isuite in range(len(SuiteRadarRangePulseRXSignals)):
      for iradar in range(len(SuiteRadarRangePulseRXSignals[isuite]['radars'])):
        ...
        # if len(specifications['CPI_Buffer'])==0:
        if len(specifications['CPI_Buffer'])==0:
          specifications['CPI_Buffer'] = SuiteRadarRangePulseRXSignals[isuite]['radars'][iradar]
        else:
          specifications['CPI_Buffer'] = np.concatenate((specifications['CPI_Buffer'], SuiteRadarRangePulseRXSignals[isuite]['radars'][iradar]), axis=1)
        
        
        if ssp.RadarSpecifications[isuite][iradar]['continuousCPIsTrue_oneCPIpeerFrameFalse']:
          SuiteRadarRangePulseRXSignals[isuite]['radars'][iradar]=[]
          if specifications['CPI_Buffer'].shape[1]>=specifications['NPulse']:
            theLastCompleteCPI = int(specifications['CPI_Buffer'].shape[1]/specifications['NPulse'])-1    
            for iCPI in range(theLastCompleteCPI+1):
              SuiteRadarRangePulseRXSignals[isuite]['radars'][iradar].append(specifications['CPI_Buffer'][:,iCPI * specifications['NPulse'] : (iCPI+1) * specifications['NPulse'],:])
            specifications['CPI_Buffer']=specifications['CPI_Buffer'][:, (theLastCompleteCPI+1) * specifications['NPulse']:, :]
          
        else:
          if specifications['CPI_Buffer'].shape[1]>=specifications['NPulse']:
            theLastCompleteCPI = int(specifications['CPI_Buffer'].shape[1]/specifications['NPulse'])-1    
            SuiteRadarRangePulseRXSignals[isuite]['radars'][iradar]=specifications['CPI_Buffer'][:,theLastCompleteCPI * specifications['NPulse'] : (theLastCompleteCPI+1) * specifications['NPulse'],:]
            specifications['CPI_Buffer']=specifications['CPI_Buffer'][:, (theLastCompleteCPI+1) * specifications['NPulse']:, :]
            
          if SuiteRadarRangePulseRXSignals[isuite]['radars'][iradar].shape[1]<specifications['NPulse']:
            SuiteRadarRangePulseRXSignals[isuite]['radars'][iradar]=[]
