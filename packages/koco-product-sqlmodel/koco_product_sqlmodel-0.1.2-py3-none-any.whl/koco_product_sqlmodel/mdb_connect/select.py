...
import koco_product_sqlmodel.viewdef as vdm
...
def select_ccategory_tree_export_targets():
    with Session(mdb_con.mdb_engine) as session:
        statement = (
            ...
            .join(vdm.TViewdefColumn, vdm.TViewdefColumn.id == vdm.TViewDefSelectValue.columnid)
...
def select_ccategory_tree_elements_by_parent_id(parent_id: int|None = None, export_target: str = None):
    with Session(mdb_con.mdb_engine) as session:
        print(parent_id)
        if parent_id == None:
            statement = (
