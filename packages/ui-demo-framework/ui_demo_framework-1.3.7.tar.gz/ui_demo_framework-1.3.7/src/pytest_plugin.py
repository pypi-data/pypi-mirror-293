    ...
    if os.path.exists(ini_path):
        ...
        for section in parser.sections():
            for key, value in parser.items(section):
                ...
                # Ensure value is properly formatted for `addinivalue_line`
                if isinstance(value, str):
                    config.addinivalue_line(formatted_key, value)
                else:
                    print(f"Warning: Value for {formatted_key} is not a string")
    else:
