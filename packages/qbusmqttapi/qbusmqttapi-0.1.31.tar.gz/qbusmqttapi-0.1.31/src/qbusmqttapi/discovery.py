...
class QbusSwitch:
    ...
    def setOn(self) -> json:
        ...
        state.type = "state"
        state.properties = {}
        state.properties.value = True
        jsonState = json.dumps(state)
        return jsonState
    
    @property
    def setOff(self) -> json:
        """Return the setState message."""
        state = {}
        state.id = self.id
        state.type = "state"
        state.properties = {}
        state.properties.value = False
        jsonState = json.dumps(state)
        return jsonState
...
class QbusMqttOutput:
    ...
    @property
    def stateMessage(self) -> json:
        stateMessage = {}
        if self.type == 'onoff':
            set_on_topic = QbusSwitch(self.id).setOn
            set_off_topic = QbusSwitch(self.id).setOff
            stateMessage.setOn = set_on_topic
            stateMessage.setOff = set_off_topic
        return stateMessage
