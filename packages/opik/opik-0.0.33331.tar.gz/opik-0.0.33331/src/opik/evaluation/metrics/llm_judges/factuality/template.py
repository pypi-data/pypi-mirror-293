...
def generate_query(user_input: str, answer: str, contexts: List[str]) -> str:
    return f"""
        ...
        ###EXAMPLE JSON OUTPUT (DO NOT COPY)###

        ****
        [
            {{
              "claim": "'Germany is well known for their cars.",
              "{VERDICT_KEY}": "{VERDICT_TRUTH}",
              "{REASON_KEY}": "'The claim is true. Germany is indeed well known for their cars due to the presence
               of many renowned vehicle manufacturing companies such as BMW, Mercedes-Benz, and Volkswagen."
            }},
            {{
              "claim": "Germany has the fist alien contact",
              "{VERDICT_KEY}": "{VERDICT_UNCLEAR}",
              "{REASON_KEY}": "There is no real evidence of such event. But context says so."
            }},
            {{
              "claim": "Most of Germans live on the Moon",
              "{VERDICT_KEY}": "{VERDICT_LIE}",
              "{REASON_KEY}": "All the humans live on the Earth."
            }},
        ]
        ***
