...
class Dataset:
    def __init__(
        ...
        rest_client: rest_api_client.CometApi,
    ) -> None:
        """
        A Dataset object. This object should not be created directly, instead use :meth:`opik.Comet.create_dataset` or :meth:`opik.Comet.get_dataset`.
        """
        self._name = name
    ...
    def delete(self, items_ids: List[str]) -> None:
        ...
        """
        self._rest_client.datasets.delete_dataset_items(item_ids=items_ids)
    ...
    def get_all_items(self) -> List[dataset_item.DatasetItem]:
        ...
        for chunk in stream:
            lines = chunk.decode(encoding="utf-8").splitlines()
            for line in lines:
                item_content: Dict[str, Any] = json.loads(line)
                item = dataset_item.DatasetItem(
                    ...
                    metadata=item_content.get("expected_output"),  # type: ignore
