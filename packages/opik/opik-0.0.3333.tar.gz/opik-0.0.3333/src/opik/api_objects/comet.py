...
class Comet:
    def __init__(
        ...
        """
        Initialize a Comet object that can be used to log traces and spans manually to Comet Opik.
        ...
        Args:
            ...
            host: The host URL for the Comet Opik server. If not provided, it will default to `http://localhost:8080/api/v1`.
        Returns:
            None
        """
        ...
        self._flush_timeout: int = config_.default_flush_timeout
    ...
    def _initialize_streamer(
        ...
        self._rest_client = rest_api_client.CometApi(
            base_url=base_url,
        ...
        for batch in helpers.list_to_batches(
            ...
            add_span_feedback_scores_batch_message = (
                messages.AddSpanFeedbackScoresBatchMessage(
                    workspace=self._workspace, batch=batch
                )
            )
    ...
    def get_dataset(self, name: str) -> dataset.Dataset:
        ...
        dataset_fern: dataset_public.DatasetPublic = (
            self._rest_client.datasets.get_dataset_by_identifier(
                dataset_name=name, workspace_name=self._workspace
            )
        )
    ...
    def delete_dataset(self, name: str) -> None:
        ...
        self._rest_client.datasets.delete_dataset_by_name(
            workspace_name=self._workspace, dataset_name=name
        )
    ...
    def end(self, timeout: Optional[int] = None) -> None:
        """
        End the Comet session and submit all pending messages.

        Args:
            timeout (Optional[int]): The timeout for closing the streamer. Once the timeout is reached, the streamer will be closed regardless of whether all messages have been sent. If no timeout is set, the default value from the Comet configuration will be used.
