...
from ..api_objects import comet, trace, span
...
class BaseTrackDecorator(abc.ABC):
    ...
    def track(
        ...
        Note:
            You can use this decorator to track nested functions, Comet will automatically create
    ...
    def _before_call(
        ...
        try:
            comet_distributed_trace_headers: Optional[DistributedTraceHeadersDict] = (
                kwargs.pop("comet_distributed_trace_headers", None)
            )
            ...
            if comet_distributed_trace_headers is None:
                self._create_span(start_span_arguments)
            else:
                self._create_distributed_node_root_span(
                    start_span_arguments, comet_distributed_trace_headers
                )
    ...
    def _create_span(
        ...
        if current_span is None and current_trace is None:
            ...
            comet_ = comet.get_client_cached()
            current_trace = comet_.trace(
                name=start_span_arguments.name,
                input=start_span_arguments.input,
                metadata=start_span_arguments.metadata,
                tags=start_span_arguments.tags,
    ...
    def _create_distributed_node_root_span(
        ...
        span_ = comet_.span(
            parent_span_id=distributed_trace_headers["comet_parent_span_id"],
            trace_id=distributed_trace_headers["comet_trace_id"],
            name=start_span_arguments.name,
            input=start_span_arguments.input,
            metadata=start_span_arguments.metadata,
            tags=start_span_arguments.tags,
