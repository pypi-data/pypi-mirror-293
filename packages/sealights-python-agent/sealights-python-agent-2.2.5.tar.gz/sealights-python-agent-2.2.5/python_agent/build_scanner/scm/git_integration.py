...
from python_agent.build_scanner.scm.scm_info import ScmInfo, ContributorData, CommitLogData
...
class GitIntegration(object):
    ...
    def collect_scm_info(self):
        ...
        if not isGitAvailable:
            log.info('Git executable not found. Git-related functionalities will be disabled.')
            return scm_info
    ...
    def try_parse_commit_line(self, raw_line, scm_info):
        ...
        scm_info.commit_log.append(
            CommitLogData(cr.hexsha, cr.author_date, cr.committer_date, cr.message, contrib_index))
        return True
