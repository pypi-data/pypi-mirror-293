...
def binned_to_continuous(Yt: NDArray, burn_in: int = 0):
    ...
    Args:
        Yt: Yt for one node.
        burn_in: Time units of data from the beginning to discard.
    """
    result = []
...
def pvals(cont_Yt: NDArray, lmda: NDArray) -> NDArray:
    ...
    Args:
        ...
        lmda: CIF for each time unit, for one node.
    ...
    for k in range(len(cont_Yt)):
        k_time_unit, k_within_time_unit = int(cont_Yt[k]), cont_Yt[k] % 1  # for k

        interarrival_time.append(
            -lmda[km1_time_unit]*(km1_within_time_unit) +
            np.sum(lmda[km1_time_unit:k_time_unit]) +  # upper-bound exclusive in Python
