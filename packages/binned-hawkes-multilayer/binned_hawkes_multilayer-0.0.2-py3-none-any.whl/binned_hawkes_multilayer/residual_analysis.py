...
def pvals(cont_Yt: NDArray, lmda: NDArray) -> NDArray:
    ...
    km1_hour, km1_within_hour = 0, 0  # for k - 1
    for k in range(len(cont_Yt)):
        k_hour, k_within_hour = int(cont_Yt[k]), cont_Yt[k] % 1  # for k

        interarrival_time.append(
            -lmda[km1_hour]*(km1_within_hour) +
            np.sum(lmda[km1_hour:k_hour]) +  # upper-bound exclusive in Python
            lmda[k_hour]*k_within_hour)

        km1_hour, km1_within_hour = k_hour, k_within_hour
