...
def ls_crossmatch(
  ...
  radius = sanitize_quantity(radius, u.arcsec)
    
  queries = []
  for i, row in df.iterrows():
    ra = sanitize_quantity(row[ra_col], u.deg)
    dec = sanitize_quantity(row[dec_col], u.deg)
...
def sync_query(
  ...
  attempt = 0
  max_attempts = 5
  while table_bytes is None and max_attempts < attempt:
...
def _batch_query(
  ...
  if concat:
    if partial_paths is None:
      ...
      save_paths_aux[0].parent.mkdir(parents=True, exists_ok=True)
