...
class TokenUtils:    
    ...
    def get_tokens_for_user(cls, user):
        token_obtain_url = 'http://localhost/oauth/token/'  # take out to env file
        data = {
            'email': user.email,            
            'user_id': user.id,
            'emp_id': user.emp_id
    ...
    def get_blacklist(cls):
        url = 'http://localhost:80/oauth/jwt/blacklist/all/'        
        try:
            response = requests.get(url)
            response_data = response.json()
            # print(f"\n=========================\nBlacklist: {response_data}\n Length of Refresh List : {len(response_data.get('refresh_tokens'))}\n===================================\n")
