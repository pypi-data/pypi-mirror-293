...
class DirectionCls:
	...
	def set(self, direction: enums.ConnDirection, userIx=repcap.UserIx.Default) -> None:
		...
		Sets the direction of the signal at the connector that can be input or an output. \n
			:param direction: INPut| OUTPut| UNUSed INPut|OUTPut Input signal or output signal UNUSed No signal present at the connector.
	...
	def get(self, userIx=repcap.UserIx.Default) -> enums.ConnDirection:
		...
		Sets the direction of the signal at the connector that can be input or an output. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
			:return: direction: INPut| OUTPut| UNUSed INPut|OUTPut Input signal or output signal UNUSed No signal present at the connector."""
		userIx_cmd_val = self._cmd_group.get_repcap_cmd_value(userIx, repcap.UserIx)
