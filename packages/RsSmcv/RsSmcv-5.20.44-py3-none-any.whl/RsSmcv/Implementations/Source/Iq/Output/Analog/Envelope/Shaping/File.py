...
class FileCls:
	...
	def get_data(self) -> List[float]:
		...
		No command help available \n
			:return: emul_sgt_iq_out_env_shape_data: No help available
		"""
		response = self._core.io.query_bin_or_ascii_float_list('SOURce<HwInstance>:IQ:OUTPut:ANALog:ENVelope:SHAPing:FILE:DATA?')
		return response

	def set_data(self, emul_sgt_iq_out_env_shape_data: List[float]) -> None:
		"""SCPI: [SOURce<HW>]:IQ:OUTPut:[ANALog]:ENVelope:SHAPing:FILE:DATA \n
		Snippet: driver.source.iq.output.analog.envelope.shaping.file.set_data(emul_sgt_iq_out_env_shape_data = [1.1, 2.2, 3.3]) \n
		No command help available \n
			:param emul_sgt_iq_out_env_shape_data: No help available
		"""
		param = Conversions.list_to_csv_str(emul_sgt_iq_out_env_shape_data)
		self._core.io.write(f'SOURce<HwInstance>:IQ:OUTPut:ANALog:ENVelope:SHAPing:FILE:DATA {param}')

	def set_new(self, ipartd_pi_db_emul_sgt_iq_out_env_shape_data_new_file: List[float]) -> None:
		"""SCPI: [SOURce<HW>]:IQ:OUTPut:[ANALog]:ENVelope:SHAPing:FILE:NEW \n
		Snippet: driver.source.iq.output.analog.envelope.shaping.file.set_new(ipartd_pi_db_emul_sgt_iq_out_env_shape_data_new_file = [1.1, 2.2, 3.3]) \n
		No command help available \n
			:param ipartd_pi_db_emul_sgt_iq_out_env_shape_data_new_file: No help available
		"""
		param = Conversions.list_to_csv_str(ipartd_pi_db_emul_sgt_iq_out_env_shape_data_new_file)
	...
	def set_select(self, filename: str) -> None:
		"""SCPI: [SOURce<HW>]:IQ:OUTPut:[ANALog]:ENVelope:SHAPing:FILE:[SELect] \n
		Snippet: driver.source.iq.output.analog.envelope.shaping.file.set_select(filename = 'abc') \n
		No command help available \n
