...
def analyze_temporal_series():
    ...
    if datasetinformation_df.empty:
        raise ValueError("El DataFrame está vacío. Verifica los datos en el archivo JSON.")
    ...
    def filter_dataframe(department, sample_type, ID):
        if ID == 'All':
            ...
            fig.update_xaxes(title_text=r"$\delta^{18}O (VSMOW ‰)$")
            ...
            fig.update_yaxes(title_text=r"$\delta^{18}O (VSMOW ‰)$", title_standoff=16, autorange="reversed")
            fig.show()


    def update_sample_type_dropdown(*args):
        department = department_dropdown.value
        sample_type_dropdown.options = datasetinformation_df[
            datasetinformation_df['Department'] == department
