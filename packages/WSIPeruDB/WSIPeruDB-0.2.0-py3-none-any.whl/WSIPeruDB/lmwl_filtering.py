...
def plot_lmwl():
    ...
    def filter_dataframe(department, sample_type, ID):
        ...
        fig.add_trace(go.Scatter(x=Oxy.flatten(), y=H_gmwl.flatten(), mode='lines', name=r'$\delta^{{2}}H = 8 \delta^{{18}}O + 10 $', line=dict(color='black')))
        ...
        fig.data[1].name = fr'$\delta^{{2}}H = {round(beta, 2)}\delta^{{18}}O + {round(alpha, 2)}$'
