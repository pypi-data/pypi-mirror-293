...
def compare_departments():
    ...
    datasetinformation_df['Sample_Collection_Date'] = pd.to_datetime(datasetinformation_df['Sample_Collection_Date'])
    ...
    def filter_dataframe(department, department_2, sample_type, sample_type_2, season, season_2):
        ...
        fig.update_layout(
            ...
            xaxis_title="Month",
            yaxis_title=r"$\delta^{18}O (VSMOW ‰)$",
            showlegend=True,
            yaxis=dict(
                autorange='reversed'  
            ),
    ...
    def update_sample_type_dropdown_1(*args):
        ...
        sample_type_dropdown_1.options = datasetinformation_df[
            datasetinformation_df['Department'] == department
        ]['Sample_Type'].unique().tolist()

    def update_sample_type_dropdown_2(*args):
        department = department_dropdown_2.value
        sample_type_dropdown_2.options = datasetinformation_df[
            datasetinformation_df['Department'] == department
        ]['Sample_Type'].unique().tolist()
    ...
    interactive_plot = widgets.interactive(filter_dataframe,
