...
class Download():
    def __init__(self, logger_name):
        self.base_path = os.path.join('C:\\Users\\', getpass.getuser(), '\\OneDrive - Landmark Group\\Automations')
        if not os.path.exists(self.base_path):
            print('Automation directory does not exist, creating `Automations` folder in your onedrive landmark')
            os.mkdir(self.base_path)
            print('Creating LOG file')
            open(os.path.join(self.base_path, 'API.log'), 'w+').close()
        if not os.path.exists(os.path.join(self.base_path, 'Keys')):
            print('Keys folder does not exist, creating keys')
            os.mkdir(os.path.join(self.base_path, 'Keys'))
    ...
    def er_v2(self, bookmark_name, save_path, export_format='csv', direct_export=True, filter_name='', start_date='', stop_date='', concept='Home Center', user='Omkar', filters=10):
        ...
        ########################## EDIT MODE ##########################
            ...
            self.edit_filter(filter_name, start_date, stop_date, filters)
