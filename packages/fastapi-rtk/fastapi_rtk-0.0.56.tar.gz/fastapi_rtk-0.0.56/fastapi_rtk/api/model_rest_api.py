...
class ModelRestApi(BaseApi):
    ...
    async def info_headless(
        ...
        Args:
            ...
            session (AsyncSession | Session): An asynchronous database session.
    ...
    async def download_headless(
        ...
        try:
            ...
            await safe_call(query.session.close())
    ...
    async def post_headless(
        ...
        body_json = await smart_run(
            self._process_body, query.session, body, self.add_query_rel_fields
        )
    ...
    async def put_headless(
        ...
        if not item:
            raise HTTPException(status_code=404, detail="Item not found")
        body_json = await smart_run(
            self._process_body, query.session, body, self.edit_query_rel_fields
        )
    ...
    async def _get_column_info(
        ...
        if self.datamodel.is_relation(col):
            ...
            if self.datamodel.is_relation_one_to_many(
                ...
                type = "RelatedList"
            related_interface = self.datamodel.get_related_interface(col)
            query = QueryManager(related_interface, session)
            await smart_run(
                query.add_options,
                page=0,
