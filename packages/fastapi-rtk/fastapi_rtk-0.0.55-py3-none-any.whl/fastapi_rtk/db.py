...
class QueryManager:
    ...
    def __init__(
        self, datamodel: "SQLAInterface", session: AsyncSession | Session | None = None
    ):
...
class DatabaseSessionManager:
    ...
    def get_sync_engine(self, bind: str | None = None):
        """
        Retrieves the sync engine associated with the specified bind. Even when it is an async engine, the sync engine is returned.

        Useful for when using certain features that require a sync engine. Such as `autoload_with`.

        Args:
            bind (str | None, optional): The database URL to bind to. If None, the default database is used. Defaults to None.

        Returns:
            Engine | None: The sync engine associated with the specified bind. If bind is None, returns the default sync engine or None if it does not exist.
        """
        engine = self.get_engine(bind)
        if not engine:
            return None
        return engine.sync_engine if isinstance(engine, AsyncEngine) else engine
    ...
    async def connect(self, bind: str | None = None):
        ...
        ***EVEN IF THE CONNECTION IS SYNC, ASYNC WITH ... AS ... IS STILL NEEDED.***
        ...
        if not engine:
            raise Exception(f"DatabaseSessionManager is not initialized{" for {bind}" if bind else ""}")
    ...
    async def session(self, bind: str | None = None):
        ...
        ***EVEN IF THE SESSION IS SYNC, ASYNC WITH ... AS ... IS STILL NEEDED.***
        ...
        Returns:
            None
        """
        session_maker = self._sessionmaker_binds.get(bind) if bind else self._sessionmaker
