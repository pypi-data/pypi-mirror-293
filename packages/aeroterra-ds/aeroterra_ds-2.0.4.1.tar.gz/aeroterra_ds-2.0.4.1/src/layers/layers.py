...
from .common import ordinal, get_fields_aux, set_display_field_aux, standarize_columns, delete_field_aux, field_present_layer, get_objectid_field_aux
...
def update_layer_aux(layer: FeatureLayer,
    ...
    Parameters:
        ...
        - excl_columns (Optional): List of fields names to exclude, if out_fields set it'll be ignored.
...
def read_layer_gdf_aux(layer: FeatureLayer,
    ...
    if out_fields is None and excl_fields is None:
       out_fields = "*"
    elif out_fields is None and excl_fields is not None:
        out_fields = [field[1] for field in fields]
        ...
        keep_fields = []
        real_fields = [field[1] for field in fields]
        for field in out_fields:
            if field in real_fields:
                keep_fields.append(field)
            else:
                warnings.warn(f"{field} Not Existing In Layer, Not Asking For It")
        out_field = keep_fields
