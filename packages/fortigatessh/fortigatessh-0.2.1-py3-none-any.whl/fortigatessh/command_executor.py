...
class CommandExecutor:
    ...
    def __enter__(self):
        ...
        Returns:
            CommandExecutor: The current instance with an active SSH connection.
        """
    ...
    def run(
        ...
        Args:
            command (Union[str, List[str]]): The command or list of commands to execute.
        ...
        output = ""
        for _cmd in command:
            _cmd = self.__clean_command(_cmd)

            try:
                self.ssh_manager.logger.debug(f"Executing command: {_cmd}")

                # Check if the interaction is initialized
                if not self.ssh_manager._interaction:
                    raise SSHCommandTimeoutError(
                        "SSH connection not initialized. Please check the connection status."
                    )

                self.ssh_manager._interaction.send(_cmd)
                ...
                if follow_up_command:
                    ...
                    self.ssh_manager._interaction.send(follow_up_command)
                ...
                self.ssh_manager.logger.error(
                    f"Exception while running command '{_cmd}': {e}"
                )
                raise SSHCommandTimeoutError(f"Command '{_cmd}' failed due to: {e}")
    ...
    def __collect_and_clear_output(self) -> str:
        ...
        output = str(self.ssh_manager._interaction.current_output_clean or "").strip()
        self.ssh_manager._interaction.current_output_clean = (
    ...
    def _wait_for_prompt(self, prompt_patterns: Optional[List[str]] = None):
        ...
        while True:
            self.ssh_manager.logger.debug(
                f"Current interaction output: {self.ssh_manager._interaction.current_output}"
            )
            found_index = self.ssh_manager._interaction.expect(
            ...
            if found_index == 0:
                ...
                self.ssh_manager._interaction.send(responses[found_index - 1])
            elif found_index == 3:
                self.ssh_manager.logger.debug(
                    f"port {self.ssh_manager.device.port} Detected 'login:' prompt. Sending username."
                )
                self.ssh_manager._interaction.send(self.ssh_manager.device.user)
            elif found_index == 4:
                self.ssh_manager.logger.debug(
                    f"port {self.ssh_manager.device.port} Detected 'Password:' prompt. Sending password."
                )
                self.ssh_manager._interaction.send(self.ssh_manager.device.pwd)
            elif found_index == -1:
                error_count += 1
                self.ssh_manager.logger.debug(
