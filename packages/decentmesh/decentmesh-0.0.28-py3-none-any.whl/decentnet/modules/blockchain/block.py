...
class Block:
    ...
    def compute_hash(self) -> MemoryHash:
        index_bytes = self.index.to_bytes(8, byteorder=ENDIAN_TYPE, signed=False)
    ...
    def to_bytes(self) -> bytes:
        index_bytes = self.index.to_bytes(8, byteorder=ENDIAN_TYPE, signed=False).rjust(
            INDEX_SIZE, b'\x00')
        version_bytes = self.version
        diff_bytes = self.diff.to_bytes()
        previous_hash_bytes = self.previous_hash.rjust(HASH_LEN, b'\x00')
    ...
    def from_bytes(cls, compressed_block_bytes: bytes):
        ...
        # Use a memoryview for more efficient slicing
        ...
        block.previous_hash = block_bytes[cursor:cursor + HASH_LEN].tobytes().lstrip(b'\x00')
        cursor += HASH_LEN
