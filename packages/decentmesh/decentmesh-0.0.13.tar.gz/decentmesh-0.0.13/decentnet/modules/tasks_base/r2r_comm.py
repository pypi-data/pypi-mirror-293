    ...
    def __init__(self, relay):
        ...
        while relay.alive:
            ...
            if process_uid in relay.beam_pipe_comm.keys():
                ...
                logger.debug(f"Received data from PIPE {len(data)} B")
                block = Block.from_bytes(data)

                if block.index == 0:
                    logger.debug(f"Overwriting blockchain with new genesis R2R")
                    relay.beam.comm_bc.clear()
                # some debug
                # else:
                #     is_in_order = len(relay.beam.comm_bc.chain) == block.index
                #     previous_hash_ok = relay.beam.comm_bc.get_last().hash == block.previous_hash

                relay.beam.comm_bc.difficulty = block.diff
                relay.beam.comm_bc.insert(block)

                # In case of race condition happening uncomment
                # relay.beam_pipe_comm[process_uid][1].send(res)
