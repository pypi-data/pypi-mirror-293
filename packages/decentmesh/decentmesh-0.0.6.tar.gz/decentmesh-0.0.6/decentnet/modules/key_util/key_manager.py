...
class KeyManager:
    ...
    def save_to_db(cls, private_key, public_key, description, can_encrypt, alias):
        with session_scope() as session:
            ssh_key_pair = OwnedKeys(private_key=private_key, public_key=public_key,
                                     description=description, can_encrypt=can_encrypt, alias=alias)
            session.add(ssh_key_pair)
            session.commit()
            return ssh_key_pair.id

    @classmethod
    def retrieve_ssh_key_pair_from_db(cls, key_id: int, can_encrypt: bool = False) -> \
            tuple[
                SigningKey,
                VerifyingKey] | \
            tuple[
        ...
        """
        with session_scope() as session:
            key_pair = session.query(OwnedKeys).where(
                OwnedKeys.id == key_id and OwnedKeys.can_encrypt == can_encrypt).first()

            if key_pair:
                return AsymCrypt.key_pair_from_private_key_base64(key_pair.private_key,
                                                                  key_pair.can_encrypt)
            else:
                raise Exception(f"No Key with id {key_id} found")

    @classmethod
    def get_private_key(cls, public_key: str):
        with session_scope() as session:
            key_pair = session.query(OwnedKeys).where(
                OwnedKeys.public_key == public_key).first()

            if key_pair:
                return AsymCrypt.key_pair_from_private_key_base64(key_pair.private_key,
                                                                  key_pair.can_encrypt)[0]
            else:
