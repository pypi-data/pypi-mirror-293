...
from decentnet.consensus.relay_conn_keys import RELAY_CONNECTION_KEY_PAIR
...
class Beam:
    ...
    def __init__(self, pub_key_id: int, pub_key_enc_id: int, target_key: str):
        ...
        _, o_pub_enc_key = KeyManager.retrieve_ssh_key_pair_from_db(pub_key_enc_id,
                                                                    can_encrypt=True)
    ...
    def initialize_incoming_transmission(self, genesis_block: Block) -> bool:
        ...
        :param genesis_block:
        """
        self.conn_bc = Blockchain(owned_pub_key_for_encryption=RELAY_CONNECTION_KEY_PAIR[1],
                                  beam_id=self.beam_id, name="Connection blockchain")
        logger.debug(f"Connection incoming with genesis {genesis_block}")
    ...
    def initialize_outgoing_transmission(self) -> bool:
        ...
        self.conn_bc = Blockchain(BlockchainParams.default_genesis_msg,
                                  owned_pub_key_for_encryption=RELAY_CONNECTION_KEY_PAIR[1],
                                  beam_id=self.beam_id, name="Connection blockchain")
    ...
    def init_comm_blockchain(self, insert: bool):
        ...
        """
        logger.debug("Initiation of communication blockchain")
        self.comm_bc = Blockchain(owned_pub_key_for_encryption=self.pub_enc_key,
    ...
    def __connect_outbound_connection(self, pub_key_id, target_key):
        ...
        logger.debug(f"Response from Relay {response_handshake}")
    ...
    def process_handshake_block(self, block_data, relay: bool, ack=True):
        ...
        if block_data["cmd"] == CMD.HANDSHAKE_ENCRYPTION.value:
            ...
            if not relay:
                private_key_from_db = KeyManager.get_private_key(
                    self.pub_enc_key)
                # TOOD: from relay keys
                data_enc_key = AsymCrypt.decrypt_message(
                    private_key_from_db,
                    encrypted_key)
            else:
                pk_bytes = base64_to_original(RELAY_CONNECTION_KEY_PAIR[0])
            ...
            if encryption_algorithm == "AES":
                if relay:
                    self.encryptor_relay = AESCipher(data_enc_key, encryption_bits)
                else:
                    self.encryptor_beacon = AESCipher(data_enc_key, encryption_bits)
            else:
    ...
    def __connect_incoming(self, genesis_block: Block, target_key: str):
        ...
        logger.debug(f"Received genesis block data: {block_data}")
    ...
    def send_block(self, pub_key_id, target_key, block: Block, request_ack=True) -> dict:
        data = Packager.pack(pub_key_id,
                             block, target_key)
        logger.debug(f"Sending block {block}, will wait for ack...")
    ...
    def send_ack_connect(self):
        pk, pub = KeyManager.retrieve_ssh_key_pair_from_db(self.pub_key_id)
    ...
    def send_new_difficulty(self, diff: Difficulty):
        ...
        logger.debug(f"New difficulty sent {diff}")
    ...
    def send_communication_data(self, data_raw: bytes, ack: bool = False) -> dict:
        ...
        :return:
        """
        logger.debug(f"Trying to send communication data {len(data_raw)} B")
        if len(self.comm_bc.chain_hashes) == 0:
    ...
    def __send_data(self, data_raw: str | bytes, ack: bool, blockchain: Blockchain,
        ...
        logger.info(f"Response from sending message {response}")
        return response
    ...
    def init_secure_b2b_connection(self, target_enc_key: str):
        ...
        """
        block, _ = self.comm_bc.create_handshake_encryption_block_dict(
            target_enc_key)
        ...
        self.process_handshake_block(block, False, False)
