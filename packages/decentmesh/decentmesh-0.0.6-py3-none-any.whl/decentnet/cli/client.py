    ...
    if not wait:
        ...
        with session_scope() as session:
            ...
            if target_enc_key is None:
                ...
                exit(1)
            logger.debug(f"> Communication genesis block is \n{beam.comm_bc.get_last()}")
            # Set lower difficulty for handshake block
            beam.comm_bc.difficulty = BlockchainParams.low_diff
            ...
            logger.debug(f"MSG: {beam.encryptor_beacon.decrypt(data)}")
