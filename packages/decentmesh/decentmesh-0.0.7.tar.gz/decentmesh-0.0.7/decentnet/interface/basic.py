...
from decentnet.consensus.net_constants import SEED_NODES_IPV6, SEED_NODES_IPV4
...
class BasicInterface:
    ...
    def __auto_select_single_seed_node(self, owner_key_id: int, owner_enc_key_id: int):
        ...
        for node_v4 in seed_nodes:
            ...
            if node_v4_concat + str(owner_key_id) not in self.connected_relays:
                try:
                    beacon = Beacon(node_v4[0], node_v4[1], owner_key_id)
                    beam = beacon.create_beam(self._target_key, owner_enc_key_id, loopback=False)

                    if beam is None:
                        continue

                    _, o_pub_key = KeyManager.retrieve_ssh_key_pair_from_db(beam.pub_key_id)
                    pub_key = AsymCrypt.verifying_key_to_string(o_pub_key)

                    self._network.add_edge(pub_key,
                                           beam.target_key, 10)
    ...
    def _init_out_encryption(self, beam: Beam, difficulty: Difficulty = BlockchainParams.low_diff):
        """
        ...
        if beam.encryption_init_complete:
            logger.info(f"Beam {beam.hash} already initialized encryption")
            return True
    ...
    def _fetch_ack(cls, beam: Beam):
        ...
        dec_data = cbor2.loads(beam.encryptor_beacon.decrypt(data))
        return dec_data

    def send(self, data: bytes, difficulty: Difficulty = BlockchainParams.low_diff,
    ...
    def fetch(self) -> Any | None:
        ...
        enc_data = self._beam_receive.fetch_message()[0].data
        data = self._beam_receive.encryptor_beacon.decrypt(enc_data)
        if data[-4:] == b'ACK!':
