...
class Block:
    ...
    def from_bytes(cls, compressed_block_bytes: bytes, compute_hash: bool = False):
        ...
        block.previous_hash = block_bytes[cursor:cursor + HASH_LEN].tobytes()
        cursor += HASH_LEN

        # Unpack nonce
        block.nonce = int.from_bytes(block_bytes[cursor:cursor + NONCE_SIZE], byteorder=ENDIAN_TYPE,
                                     signed=False)
        cursor += NONCE_SIZE

        # Unpack timestamp
        block.timestamp = cls.unpack_timestamp(block_bytes[cursor:cursor + TIMESTAMP_SIZE])
        cursor += TIMESTAMP_SIZE

        # Decompress the remaining data
        block.data = CompressionWrapper.decompress_lz4(block_bytes[cursor:])

        # Compute the hash if requested
        if compute_hash:
