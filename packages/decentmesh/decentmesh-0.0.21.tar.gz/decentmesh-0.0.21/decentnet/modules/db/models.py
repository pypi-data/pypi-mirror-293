...
import pymysql
import sqlalchemy
from sqlalchemy import Integer, String, DateTime, Column, ForeignKey, Float, LargeBinary, \
...
class BlockTable(Base):
    ...
    def __repr__(self):
        return f"<Block(id={self.id}, hash='{self.hash}', previous_hash='{self.previous_hash}', " \
...
class BeamTable(Base):
    ...
    owner_public_key = Column(Integer, index=True)
    target_public_key = Column(String(256), index=True)
    timestamp = Column(DateTime, default=datetime.now, nullable=False)

    @staticmethod
    async def save(beam, pub_key: str, target_key: str, conn_bc_id: int,
                   comm_bc_id: int):
        from decentnet.modules.db.base import session_scope
        with session_scope() as session:
            if beam.beam_id:
                b_found = session.query(BeamTable).where(
                    BeamTable.id == beam.beam_id and (
                            BeamTable.owner_public_key == pub_key or BeamTable.target_public_key == pub_key)).first()
                if target_key == "NOT_KNOWN" and b_found:
                    b_found.target_public_key = target_key
            else:
                bdb = BeamTable(blockchain_id=conn_bc_id, beam_hash=beam.hash,
...
class AliveBeam(Base):
    ...
    async def mark_beam_ready(cls, pub_key: str, ready: bool):
        ...
        with session_scope() as session:
            try:  # TODO: put hash
                csd = session.query(cls).where(cls.pub_key == pub_key).first()
                if not csd:
                    return
                csd.ready = ready
                session.commit()
