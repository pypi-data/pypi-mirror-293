...
class Relay:
    ...
    def __init__(self, client_socket, beam_pipe_comm: dict, callback: AsyncCallbackType | None = None,
        ...
        if block.index == 0:
            ...
            asyncio.run(self.broadcast_connected(block, pub_key, unpacked_request))
        else:
    ...
    async def broadcast_connected(self, genesis_block: Block, connected_pub_key: str,
        ...
        if genesis_block.index == 0:
            async with session_scope() as session:
                ...
                result = await session.execute(select(AliveBeam).filter(AliveBeam.ready))
                ...
                if len(all_alive_beams):
                    ...
                    broadcast_block_signature = Packager.sign_block(
                        self.public_key_id, broadcast_block_bytes
                    )
                ...
                for beam in all_alive_beams:
                    if beam.pub_key != connected_pub_key:
                        ...
                        await self.publisher.publish_message(beam.pub_key, serialized_broadcast)
