...
DUMMY_EGG_DICT_BY_SHA = {
    'abc1234def456': {
        "indexed_tag": "cp27",
        **DUMMY_EGG_INDEX_CP27["MKL-10.3-1.egg"]
    },
    "sha256abcd1234cp27": {
        "indexed_tag": "cp27",
        **DUMMY_EGG_INDEX_CP27["numpy-1.6.0-2.egg"]
    },
    "sha256efgh5678cp27": {
        "indexed_tag": "cp27",
        **DUMMY_EGG_INDEX_CP27["numpy-1.7.0-2.egg"]
    },
    "sha256abcd1234cp34": {
        "indexed_tag": "cp34",
        **DUMMY_EGG_INDEX_CP34["numpy-1.6.0-2.egg"]
    },
    "sha256efgh5678cp34": {
        "indexed_tag": "cp34",
        **DUMMY_EGG_INDEX_CP34["numpy-1.7.0-2.egg"]
    },
}
...
class TestExport(MainTestingMixin, unittest.TestCase):
    ...
    @patch_brood_client
    def test_specify_python_tags(self, BroodClient):
        # Given
        m_brood_client = self._mock_brood_client_class(BroodClient)
        m_brood_client._api_version = 1

        python_tag = 'cp34'
        final_args = ['-t', python_tag, self.tempdir]

        self.patched_find_eggs_for_plat_repo.stop()
        self.m_plat_repo.egg_index = unittest.mock.MagicMock(
            side_effect=lambda t: DUMMY_EGG_INDEX_CP34 if t == 'cp34' else {})

        def egg_metadata(tag, name, full_version):
            for _, entry in DUMMY_EGG_INDEX_CP34.items():
                if ((entry['python_tag'] == tag or entry['python_tag'] is None)
                        and entry['name'] == name
                        and entry['full_version'] == full_version):
                    return entry

            return {}

        self.m_plat_repo.egg_metadata = unittest.mock.MagicMock(
            side_effect=egg_metadata)

        with mock.patch(
            'hatcher.cli.exporter.find_eggs_for_plat_repo',
            side_effect=exporter.find_eggs_for_plat_repo) as m1, mock.patch(
                'hatcher.cli.exporter.unpack_index',
                side_effect=exporter.unpack_index) as m2:
        ...
        for call in m1.call_args_list:
            self.assertEqual(call[0][-1], (python_tag,))

        for call in m2.call_args_list:
            self.assertEqual(call[0][-1], (python_tag,))

        for call in self.m_download_runtimes.call_args_list:
            runtime_dict_by_sha = call[0][1]
            self.assertEqual(len(runtime_dict_by_sha), 1)
            for value in runtime_dict_by_sha.values():
                self.assertTrue(value['version'].startswith('3.4'))

        for call in self.m_download_eggs.call_args_list:
            egg_dict_by_sha = call[0][1]
            self.assertEqual(len(egg_dict_by_sha), 3)
            for value in egg_dict_by_sha.values():
                self.assertIn(value['python_tag'], [python_tag, None])
