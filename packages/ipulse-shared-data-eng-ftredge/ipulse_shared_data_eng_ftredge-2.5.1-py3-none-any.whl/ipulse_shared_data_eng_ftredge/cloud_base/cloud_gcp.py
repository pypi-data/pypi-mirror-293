...
def  read_json_from_gcs_extended(storage_client:GCSClient, bucket_name:str, file_name:str, pipelinemon:Pipelinemon=None,  logger=None, print_out=False):
    ...
    try:
        ...
        if blob.size == 0:
            ...
            logger.warning(msg)
            if pipelinemon:
                pipelinemon.add_log(ContextLog(level=LogLevel.ERROR_READ_FAILED, subject=f"blob {file_name} in {bucket_name}", description="blob.size == 0, meaning empty file"))
            return None
        return data
    except NotFound:
        log_warning(msg=f"Warning: The file {file_name} was not found in the bucket {bucket_name}.", logger=logger, print_out=print_out)
        if pipelinemon:
            pipelinemon.add_log(ContextLog(LogLevel.ERROR_READ_FAILED, subject=file_name, description=f"File not found in GCS: {file_name} in bucket {bucket_name}"))
        return None
    except json.JSONDecodeError:
        msg=f"Error decoding JSON from GCS: {file_name} in bucket {bucket_name}"
        log_error(msg=msg, logger=logger, print_out=print_out)
        if pipelinemon:
            pipelinemon.add_log(ContextLog(LogLevel.ERROR_READ_FAILED, subject=file_name, description=msg))
        return None
    except Exception as e:
...
def query_sql_bigquery_table(project_id: str, query: str, bigquery_client: Optional[bigquery.Client] = None,
                             pipelinemon: Optional[Pipelinemon] = None, logger: Optional[logging.Logger] = None) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
