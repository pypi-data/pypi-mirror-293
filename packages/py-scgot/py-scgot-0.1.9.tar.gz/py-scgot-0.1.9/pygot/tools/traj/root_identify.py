...
'''def compute_spdist(adata, embedding_key='X_pca', n_neighbors=20, start_cells=None):
    
    X = pd.DataFrame(adata.obsm[embedding_key], index=adata.obs.index)
    neighbors = NearestNeighbors(n_neighbors=n_neighbors+1, algorithm='auto').fit(X)
    ...
    # 创建 CSR 矩阵
    knn_graph = csr_matrix((data, (rows, cols)), shape=(num_samples, num_samples))
    if start_cells is not None:
        intersection_index = pd.Index(start_cells).intersection(adata.obs.index)
        for start_cell in intersection_index:
...
def greedy_search_best_source(adata, embedding_key, kernel='dpt', split_k=None, graph_dist=False, n_neighbors=20, connect_anchor=None, n_waypoints=1200):
    ...
    for i in tqdm(range(len(adata))):
        ...
        if len(data) < 100:
            ...
            continue
        #data.obs['scaled_time'] = adjust_time_by_structure(data, embedding_key, cell_type_key, time_key, start_cell=adata.obs.index[i])
...
def generate_time_points(adata, k=4, pseudotime_key = 'dpt_pseudotime', time_key='pseudobin', sigma=.25, ):
...
def determine_source_states(adata, embedding_key, split_k=30, kernel='dpt', n_comps=15, down_sampling=True, n_obs=3000, 
                            cytotrace=True, alpha = 0.1, smooth_k=5,
                            graph_dist=True, n_neighbors=30, connect_anchor=None):
    ...
    #TODO 多线程
    #if len(adata) < 5000  and down_sampling:
    #    print('Warning: the cell number may be too few to do down sampling.')
    #    print('Set enforce = True to do down sampling')
    #    down_sampling = False if enforce == False else True
    ...
    res = greedy_search_best_source(sub_adata, embedding_key, split_k=split_k, kernel=kernel,
