...
def calculate_cluster_centres(data, cluster_labels):
    ...
    参数：
        data: np.ndarray. 数据点坐标数组，形状为 (num_samples, num_features)
        cluster_labels: np.ndarray. 聚类标签数组，形状为 (num_samples,)

    返回：
        cluster_centres: np.ndarray. 每个聚类的中心点，形状为 (num_clusters, num_features)
    """
    # 获取唯一的聚类标签
...
'''
def adjust_time_by_structure(adata, mst_children, time_key):
    #print(adata.obs[time_key], start_cell)
    
    sources, sinks = find_sources_and_sinks(mst_children)
    lineages = search_lineages(mst_children, sources, sinks)
    converted_time = pd.DataFrame(index = adata.obs.index)
    for i in range(len(lineages)):
        converted_time['lineage_time_'+str(i)] = np.nan
        lineage_idx = adata.obs.index[adata.obs['int_cluster'].isin(lineages[i])]
        scaled_time = scale(adata[lineage_idx].obs[time_key])
        converted_time.loc[lineage_idx, 'lineage_time_'+str(i)] = scaled_time
    converted_time = np.nanmean(converted_time.to_numpy(), axis=1)
    converted_time = scale(converted_time)
...
def adjust_time_by_structure(adata, mst_children, time_key):
    ...
    for i in range(len(lineages)):
        converted_time['lineage_time_'+str(i)] = np.nan
        lineage_idx = adata.obs.index[adata.obs['int_cluster'].isin(lineages[i])]
        t = adata[lineage_idx].obs[time_key].to_numpy()
        rank = t.argsort()
        rank_val = np.array(range(len(t))) / len(t)
        converted_time.loc[lineage_idx[rank], 'lineage_time_'+str(i)] = rank_val
    converted_time = np.nanmean(converted_time.to_numpy(), axis=1)
    converted_time = scale(converted_time)
    return converted_time
