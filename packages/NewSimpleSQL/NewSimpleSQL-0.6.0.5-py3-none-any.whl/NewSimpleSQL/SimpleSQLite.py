...
class DBAdapter():
    ...
    def __change_types(self):
        ...
        if isinstance(self, SQLiteAdapter):
            db_types = ("TEXT", "INTEGER", "REAL", "BLOB", "NUMERIC")
            
            for i in self.__dbtypes:
                self.__dbtypes[i] = db_types(db_types.index(i))
...
class Database():
    ...
    def simple_insert_data(self, table: str, data: tuple|list, especific_columns: str|None = None):
        ...
        if bool(especific_columns):
            self.__cursor.execute(f'INSERT INTO {table} VALUES ({especific_columns})', data)
            return
    ...
    def __convert_type(self, value: str|type, inverse: bool = False) -> str|tuple:
        ...
        def sqltype(value_type: str|type) -> str:
            ...
            if value_type == "tuple":
                return f'{sqltype(value[0].__name__)} NOT NULL'
