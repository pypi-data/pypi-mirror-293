...
class AqGeometricClimatologyModel(BaseModel):
    ...
    def fit(self, train_set: Union[AqGeometricInMemoryDataset,
        ...
        for i in train_set.indices[::len(graph_timestamps)]:
            ...
            for i, h3_idx in enumerate(graph_h3_index):
                ...
                for k, tgt in enumerate(self.targets):
                    state_dict[h3_idx][tgt]["mean"] = (
                        state_dict[h3_idx][tgt]["mean"] *
                        state_dict[h3_idx][tgt]["count"] +
                        np.mean(graph_data[i, :, k], axis=None)) / (
                            state_dict[h3_idx][tgt]["count"] + 1)
                    state_dict[h3_idx][tgt]["count"] += 1
        ...
        for i, h3_idx in enumerate(h3_indices):
            if h3_idx in self.state_dict:
                for j, tgt in enumerate(self.targets):
                    pred[i, :,
                         j] = np.tile(self.state_dict[h3_idx][tgt]["mean"],
