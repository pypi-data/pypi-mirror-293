...
class EnhancedAutoencoderForecaster(nn.Module):
    def __init__(
        ...
        self.latent_adjustment = nn.Linear(self.autoencoder.encoder.latent_dim,
                                           self.hidden_dim)

        self.rnn_decoder = nn.GRU(
            input_size=self.hidden_dim *
            2,  # Input will be the concatenation of latent representation and processed input
        ...
        if self.use_gcn_refinement:
            self.gcn_forecaster = GCNConv(self.hidden_dim * 2,
                                          self.hidden_dim * 2)
    ...
    def forward(self, x, edge_index, x_mask):
        ...
        with torch.no_grad():
            _, z = self.autoencoder.forward(
                x, edge_index, x_mask)  # z: (num_nodes, latent_dim)
        if self.verbose: print(f"z shape: {z.shape}")

        z_repeated = z.unsqueeze(1).repeat(
            1, x.size(1), 1)  # Repeat z along the time dimension
        if self.verbose: print(f"z_repeated shape: {z_repeated.shape}")

        # 2. Adjust latent representation and project input
        z_adjusted = self.latent_adjustment(
            z_repeated)  # z_adjusted: (num_nodes, hidden_dim)
        if self.verbose: print(f"z_adjusted shape: {z_adjusted.shape}")

        x_projected = self.input_projection(
            x)  # x_projected: (num_nodes, n_timestamps, hidden_dim)
        ...
        combined_input = torch.cat([
            z_adjusted, x_projected
        ], dim=-1)  # combined_input: (num_nodes, n_timestamps + 1, hidden_dim)
        ...
        node_mask = x_mask_temporal_mean.any(
            dim=-1)  # node_mask: (batch_size, num_nodes)
        if self.verbose:
            print(f"x_mask_temporal_mean shape: {x_mask_temporal_mean.shape}")
        ...
        for t in range(self.forecast_steps):
            ...
            combined_input = torch.cat(
                [combined_input, output.unsqueeze(1)], dim=1)
            if self.verbose:
                print(
                    f"combined_input shape (after concatenation) (timestep {t} of {self.forecast_steps}): {combined_input.shape}"
                )
        ...
        if self.verbose:
            print(f"x_hat shape (after output refinement): {x_hat.shape}")
