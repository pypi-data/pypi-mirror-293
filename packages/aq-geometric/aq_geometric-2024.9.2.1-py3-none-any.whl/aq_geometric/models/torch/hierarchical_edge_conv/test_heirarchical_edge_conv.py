...
class TestAqHierarchicalEdgeConvModel(unittest.TestCase):
    def setUp(self):
        ...
        self.edges_low_to_high_resolution = {
            1: torch.tensor([[0, 1], [1, 2]]),
            2: torch.tensor([[0, 2], [1, 3]])
        }
        self.edges_high_to_low_resolution = {
            1: torch.tensor([[0, 1], [1, 2]]),
            2: torch.tensor([[0, 2], [1, 3]])
        }
    ...
    def test_forward(self):
        ...
        x = torch.randn(10, model.hidden_channels,
                        model.num_features_in_node_feature)
        edge_index = torch.tensor([[0, 1, 2], [1, 2, 3]])
        output = model.forward(x, edge_index)
        self.assertEqual(
            output.shape,
            (10 * model.num_features_in_node_target, model.out_channels))
    ...
    def test_save_and_load(self):
        ...
        self.assertEqual(model.num_samples_in_node_feature,
                         loaded_model.num_samples_in_node_feature)
        self.assertEqual(model.num_samples_in_node_target,
                         loaded_model.num_samples_in_node_target)
        self.assertEqual(model.finest_resolution,
                         loaded_model.finest_resolution)
        self.assertEqual(model.coarsest_resolution,
                         loaded_model.coarsest_resolution)
        self.assertEqual(model.cheb_k, loaded_model.cheb_k)
        self.assertEqual(model.state_dict().keys(),
