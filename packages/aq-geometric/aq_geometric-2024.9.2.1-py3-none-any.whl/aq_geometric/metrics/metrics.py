...
def calculate_metrics(
    ...
    y_true_filtered, y_pred_filtered, null_model_pred_filtered = _filter_missing_values(
        y_true, y_pred, null_model_pred)

    results = {}
    for metric in metrics:
        if metric == "mae":
            results[metric] = mean_absolute_error(y_true_filtered,
                                                  y_pred_filtered)
        elif metric == "mse":
            results[metric] = mean_squared_error(y_true_filtered,
                                                 y_pred_filtered)
        elif metric == "rmse":
            results[metric] = root_mean_squared_error(y_true_filtered,
                                                      y_pred_filtered)
        elif metric == "mape":
            results[metric] = mean_absolute_percentage_error(
                y_true_filtered, y_pred_filtered)
        elif metric == "smape":
            results[metric] = symmetric_mean_absolute_percentage_error(
                y_true_filtered, y_pred_filtered)
        elif metric == "msle":
            results[metric] = mean_squared_log_error(y_true_filtered,
                                                     y_pred_filtered)
            ...
            results[metric] = mean_percentage_error(y_true_filtered,
                                                    y_pred_filtered)
        elif metric == "nmse":
            results[metric] = normalized_mean_squared_error(
                y_true_filtered, y_pred_filtered, null_model_pred_filtered)
        elif metric == "ia":
            results[metric] = index_agreement(y_true_filtered, y_pred_filtered,
                                              null_model_pred_filtered)
        elif metric == "mss":
            results[metric] = mean_skill_score(y_true_filtered,
                                               y_pred_filtered,
                                               null_model_pred_filtered)
        else:
...
def mean_absolute_percentage_error(y_true: np.ndarray,
    ...
    return np.mean(
        np.abs((y_true_filtered - y_pred_filtered) /
               (y_true_filtered + epsilon))) * 100
...
def symmetric_mean_absolute_percentage_error(y_true: np.ndarray,
    ...
    return 100 / len(y_true_filtered) * np.sum(
        2 * np.abs(y_pred_filtered - y_true_filtered) /
        (np.abs(y_true_filtered) + np.abs(y_pred_filtered) + epsilon))
...
def mean_squared_log_error(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    return np.mean(
        np.square(np.log(y_true_filtered + 1) - np.log(y_pred_filtered + 1)))
...
def mean_percentage_error(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    return np.mean((y_true_filtered - y_pred_filtered) /
                   (y_true_filtered + epsilon)) * 100


def normalized_mean_squared_error(y_true: np.ndarray, y_pred: np.ndarray,
                                  null_model_pred: np.ndarray) -> float:
...
def index_agreement(y_true: np.ndarray, y_pred: np.ndarray,
                    null_model_pred: np.ndarray) -> float:
    """Calculates the index of agreement (d) between two model predictions."""
    y_true_filtered, y_pred_filtered, null_model_pred_filtered = _filter_missing_values(
        y_true, y_pred, null_model_pred)

    d1 = 1 - np.sum(np.square(y_true_filtered - y_pred_filtered)) / np.sum(
        np.square(
            np.abs(y_pred_filtered - np.mean(y_true_filtered)) +
            np.abs(y_true_filtered - np.mean(y_true_filtered))))
    d2 = 1 - np.sum(
        np.square(y_true_filtered - null_model_pred_filtered)) / np.sum(
            np.square(
                np.abs(null_model_pred_filtered - np.mean(y_true_filtered)) +
                np.abs(y_true_filtered - np.mean(y_true_filtered))))
    return d1 - d2


def mean_skill_score(y_true: np.ndarray, y_pred: np.ndarray,
                     null_model_pred: np.ndarray,
                     metric_func: Callable = mean_squared_error) -> float:
...
def _filter_missing_values(
    ...
    if null_model_pred is None:
        ...
        mask = ~np.isnan(y_true) & ~np.isnan(y_pred) & ~np.isnan(
            null_model_pred)
