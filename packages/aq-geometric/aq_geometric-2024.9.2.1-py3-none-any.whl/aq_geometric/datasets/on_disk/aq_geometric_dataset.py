...
class AqGeometricDataset:
    def __init__(
            self, root: str, pre_transform: Optional[Callable] = None,
            ...
            filetype: str = "pickle", max_workers: int = 4):
    ...
    def processed_file_names(self):
        ...
        from glob import glob
        return sorted([
            osp.basename(f) for f in glob(
                osp.join(self.processed_dir, f"data_[0-9]*.{self.filetype}"))
        ])
    ...
    def download(self):
        ...
        if self.stations_info_fp is not None:
            stations_info = load_stations_info_from_fp(self.stations_info_fp)
        else:
    ...
    def process(self):
        ...
        file_ids = [
            f.split("_")[-1].split(".")[0] for f in self.raw_file_names
        ...
        if set(existing_files) == set(expected_files):
            ...
            print(
                f"Processed files do not match expected files in {self.processed_dir}, generating..."
            )
            if self.verbose:
                print(
                    f"[{datetime.now()}] raw file indicators: {file_ids} do not match existing files {existing_files}"
                )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for i, (start_time, end_time) in enumerate(
                    zip(self.query_start_times, self.query_end_times)):
                futures.append(
                    executor.submit(process_graph_partial, i, start_time,
                                    end_time))
            ...
            with tqdm(total=len(futures), desc="Processing graphs") as pbar:
                for future in as_completed(futures):
                    try:
                        ...
                        if self.verbose:
                            print(
                                f"[{datetime.now()}] Processed graph {i} ({type(graph)})."
                            )
                        filename = osp.join(self.processed_dir,
                                            f"data_{i}.{self.filetype}")
                        if self.verbose:
                            print(
                                f"[{datetime.now()}] Saving graph {i} to {filename}."
                            )
                        getattr(self,
                                f"save_graph_to_{self.filetype}")(graph,
                                                                  filename)
                        self.graph_index_ranges.append(
                            (graph["feature_timestamps"][0],
                             graph["feature_timestamps"][-1]))
        ...
        if self.verbose:
            print(
                f"[{datetime.now()}] Processed {self.num_graphs_on_disk} graphs."
            )
    ...
    def _process_single_graph(
        ...
        graph = make_graph_data(
            ...
            edge_attr=edge_attr.drop(columns=["from", "to"]).to_numpy()
    ...
    def _load_graph_from_disk(self, i):
        ...
        if i not in self._loaded_graph_indices:
            ...
            graph = getattr(self, f"load_graph_from_{self.filetype}")(osp.join(
                self.processed_dir, f"data_{i}.{self.filetype}"))
            ...
            if i < self.num_graphs_on_disk - 1:
                ...
                next_graph = getattr(self, f"load_graph_from_{self.filetype}")(
                    osp.join(self.processed_dir,
                             f"data_{i+1}.{self.filetype}"))
                ...
                assert all(h1 == h2 for h1, h2 in zip(
                    graph["measurement_names"], next_graph["measurement_names"]
                )), "The measurement_names must be the same for both graphs."

                # concatenate the graphs
                node_data = np.concatenate((graph["data"], next_graph["data"]),
                                           axis=1)
                timestamps = np.concatenate(
                    (graph["timestamps"], next_graph["timestamps"]), axis=0)
                ...
                data_mask = np.concatenate(
                    (graph["data_mask"], next_graph["data_mask"]), axis=1)
    ...
    def _load_data_from_memory(self, idx: int) -> dict:
        ...
        feature_start_time = pd.to_datetime(
            self._graph_data["timestamps"][0]) + pd.Timedelta(
                self.freq) * time_index_start
        feature_timestamps = pd.date_range(
            start=feature_start_time,
            periods=self.num_samples_in_node_features,
            freq=self.freq).to_numpy()

        target_start_time = feature_timestamps[-1] + pd.Timedelta(self.freq)
        target_timestamps = pd.date_range(
            start=target_start_time, periods=self.num_samples_in_node_targets,
            freq=self.freq).to_numpy()
        ...
        feature_indices = [
            i for i, n in enumerate(measurement_names) if n in feature_names
        ]
        target_indices = [
            i for i, n in enumerate(measurement_names) if n in feature_names
        ]
        ...
        data["x"] = self._graph_data[
            "data"][:, time_index_start:time_index_start +
                    self.num_samples_in_node_features, feature_indices]
        data["y"] = self._graph_data[
            "data"][:, time_index_start +
                    self.num_samples_in_node_features:time_index_end,
                    target_indices]
        data["x_mask"] = self._graph_data[
            "data_mask"][:, time_index_start:time_index_start +
                         self.num_samples_in_node_features, feature_indices]
        data["y_mask"] = self._graph_data[
            "data_mask"][:, time_index_start +
                         self.num_samples_in_node_features:time_index_end,
                         target_indices]
        ...
        data["node_features"] = np.nan_to_num(data["x"],
                                              nan=self.node_missing_value)
        data["node_targets"] = np.nan_to_num(data["y"],
                                             nan=self.node_missing_value)
    ...
    def save_graph_to_pickle(self, graph: dict, filename: str) -> None:
        ...
        if self.verbose:
            print(
                f"[{datetime.now()}] Graph saved successfully to: {filename}")
    ...
    def load_graph_from_pickle(self, filename: str) -> dict:
        ...
        if self.verbose:
            print(
                f"[{datetime.now()}] Graph loaded successfully from: {filename}"
            )
            ...
            for key, value in graph.items():
                if isinstance(value, np.ndarray):
                    if value.dtype == np.object_:
                        ...
                        hf.create_dataset(
                            key,
                            data=value.astype('datetime64[s]').view(np.int64),
                            dtype=np.int64)
                    ...
                    if np.issubdtype(np.array(value).dtype, np.number):
                        hf.create_dataset(
                            key, data=np.array(value, dtype=np.float32))
                else:
                    # handle scalars
                    hf.create_dataset(key,
                                      data=np.array(value, dtype=np.object_),
                                      dtype=h5py.special_dtype(vlen=str))
        if self.verbose:
            print(
                f"[{datetime.now()}] Graph saved successfully to: {filename}")
