...
class NumpyDatasetAdapter:
    ...
    def __indexed_getitem__(self, idx):
        ...
        if self.transform:
            if self.verbose:
                print(
                    f"[{datetime.now()}] Transforming graph {idx} of {self.base_dataset.indices[-1]}"
                )
    ...
    def get_dataset_iterator(
        self, ) -> Iterator[Tuple[np.ndarray, np.ndarray]]:
        ...
        for i in range(0, len(self.base_dataset), self.batch_size):
            ...
            if self.verbose:
                print(
                    f"[{datetime.now()}] Yielding batch {i} with X ({np.shape(X)}) and Y ({np.shape(y)})"
                )
