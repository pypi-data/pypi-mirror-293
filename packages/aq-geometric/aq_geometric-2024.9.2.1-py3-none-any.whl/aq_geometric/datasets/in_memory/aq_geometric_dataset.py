...
class AqGeometricInMemoryDataset:
    def __init__(
            self, features: List[str] = ["PM2.5"], targets: List[str] = [
                "OZONE"
    ...
    def _load_data_from_memory(self, idx: int) -> dict:
        ...
        feature_start_time = pd.to_datetime(
            self._graph_data["feature_timestamps"][0]) + pd.Timedelta(
        ...
        data["x"] = data["x"][:, time_index_start:time_index_start +
                              self.num_samples_in_node_features]
        data["y"] = data["y"][:, time_index_start +
                              self.num_samples_in_node_features:time_index_end]
        data["x_mask"] = data["x_mask"][:, time_index_start:time_index_start +
                                        self.num_samples_in_node_features]
        data["y_mask"] = data[
            "y_mask"][:, time_index_start +
                      self.num_samples_in_node_features:time_index_end]
