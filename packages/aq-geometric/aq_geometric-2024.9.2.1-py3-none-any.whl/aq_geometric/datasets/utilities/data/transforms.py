...
def load_from_graph(
    ...
    for h3_index in graph["h3_index"]:
        ...
        if include_target and (np.any(node_data_target_mask == 0) or
                               np.any(graph_data_masks[node_id, -1, :] == 0)):
            if verbose:
                print(
                    f"[{datetime.now()}] skipping {h3_index} (node id {node_id}) due to NaN values in target."
                )
            continue
        ...
        y = np.concatenate(
            ...
            axis=1,
        ) if include_masks_in_targets else node_data_target.reshape(
        ...
        X = np.concatenate(
            ...
            axis=1,
        ) if include_masks_in_features else node_data.reshape(
            1, -1, order=reshape_order)

        if verbose:
            print(
                f"[{datetime.now()}] after obtaining node values, X shape: {np.shape(X)}"
            )
...
def load_from_graph_hierarchical(
    ...
    feature_vector_shape = (
        -1,
        2 * num_edges_for_each_node * num_samples_in_node_feature *
        num_features_in_node_feature
        if include_masks_in_features else num_edges_for_each_node *
        num_samples_in_node_feature * num_features_in_node_feature,
    )
    target_vector_shape = (
        -1,
        2 * num_samples_in_node_target * num_features_in_node_target
        if include_masks_in_targets else num_samples_in_node_target *
        num_features_in_node_target,
    )
    ...
    for h3_index in graph["h3_index"]:
        ...
        if h3_index != "root":
            ...
            for child_h3_index in child_h3_indices:
                ...
                if child_h3_index in h3_index_to_node_id_map:
                    ...
                    child_node_data = np.full((1, num_samples_in_node_feature,
                                               num_features_in_node_feature),
                                              np.nan)
            ...
            # handle neighbors
            neighbor_h3_indices = [
                h3_id for h3_id in h3.k_ring(h3_index, 1) if h3_id != h3_index
            ]
            for neighbor_h3_index in neighbor_h3_indices:
                ...
                if include_masks_in_features:
                    X = np.concatenate([
                        X,
                        np.reshape(neighbor_node_data, (1, -1), reshape_order),
            ...
            if h3.h3_get_resolution(h3_index) > 0:
                ...
                if parent_h3_index in h3_index_to_node_id_map:
                    parent_node_id = h3_index_to_node_id_map[parent_h3_index]
                    # get the data
                    ...
                    # make NaNs of the appropriate shape
                    parent_node_data = np.full((1, num_samples_in_node_feature,
                                                num_features_in_node_feature),
                                               np.nan)
            ...
            if verbose:
                print(f"[{datetime.now()}] root node: X shape: {np.shape(X)}")

        if verbose:
            print(
                f"[{datetime.now()}] after obtaining parent values, X shape: {np.shape(X)}"
            )
        ...
        if X.shape[1] != feature_vector_shape[1]:
            ...
            if include_masks_in_features and x_shape[1] + (
                    ...
                    num_samples_in_node_feature) == feature_vector_shape[1]:
                # pad with NaNs
                # make NaNs of the appropriate shape
...
def transform_graph_hierarchical(graph: dict, ) -> dict:
    ...
    new_x = np.zeros((len(graph["h3_index"]), num_samples_in_node_feature,
                      num_features_in_node_feature, num_edges_for_each_node))
    new_x_mask = np.zeros(
        (len(graph["h3_index"]), num_samples_in_node_feature,
         num_features_in_node_feature, num_edges_for_each_node))
    ...
    for h3_index in graph["h3_index"]:
        ...
        if x.shape[
                1] != num_edges_for_each_node * num_samples_in_node_feature * num_features_in_node_feature:
            # this is the case for the 12 pentagons in the graph
            # we can check that the shape is correct and pad, otherwise skip
            x_shape = x.shape
            # check that X has 2 items in the shape
            ...
            if x_shape[1] + (
                    num_features_in_node_feature * num_samples_in_node_feature
            ) == num_edges_for_each_node * num_samples_in_node_feature * num_features_in_node_feature:
                # pad with NaNs
                # make NaNs of the appropriate shape
                pad_node_data = np.full((1, num_samples_in_node_feature,
                                         num_features_in_node_feature), np.nan)
                pad_node_data_masks = np.zeros((1, num_samples_in_node_feature,
                                                num_features_in_node_feature))
        ...
        new_x[node_id, :, :, :] = np.reshape(
            x, (num_samples_in_node_feature, num_features_in_node_feature,
                num_edges_for_each_node), order=reshape_order)
        new_x_mask[node_id, :, :, :] = np.reshape(
            x_mask, (num_samples_in_node_feature, num_features_in_node_feature,
                     num_edges_for_each_node), order=reshape_order)

    graph["x"] = new_x
    graph["x_mask"] = new_x_mask

    assert len(graph["x"]) == len(
        graph["y"]), "after transform, x and y have different lengths"
    assert len(graph["x"]) == len(
        graph["x_mask"]
    ), "after transform, x and x_mask have different lengths"

    # ensure all shapes are the same for graph["x"] and graph["x_mask"]
    for i in range(1, len(graph["x"])):
        assert np.shape(graph["x"][i, :, :, :]) == np.shape(graph["x_mask"][
            i, :, :, :]), f"x and x_mask shapes are different for node {i}"
        assert np.shape(graph["x"][i, :, :, :]) == np.shape(graph["x"][
            i -
            1, :, :, :]), f"x shapes are different between node {i} and {i-1}"

    return graph


def transform_graph_negative_to_nan(graph: dict, ) -> dict:
...
def transform_graph_remove_nan_targets(graph: dict, ) -> dict:
    """Loads and modifies graph data in-place for model training/inference.
    ...
    if "edges" in graph:
        ...
        graph["edge_attrs"] = np.delete(graph["edge_attrs"], nan_indices,
                                        axis=0)
