...
class OnnxDatasetAdapter(BaseDatasetAdapter):
    ...
    def __indexed_getitem__(self, idx):
        ...
        data["edge_attr"] = data["edge_attr"].astype(
            np.float32
        ) if "edge_attr" in data and data["edge_attr"] is not None else None
        for k in data:
            if "mask" in k:
    ...
    def get_dataset_iterator(
        ...
        for i in range(0, len(self.base_dataset), self.batch_size):
            ...
            if self.verbose:
                print(
                    f"[{datetime.now()}] Yeilding batch {i} with X {np.shape(X)}) and Y ({np.shape(y)})"
                )
