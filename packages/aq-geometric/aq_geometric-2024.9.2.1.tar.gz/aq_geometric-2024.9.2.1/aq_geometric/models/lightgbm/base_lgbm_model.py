...
class BaseAqGeometricLGBMModel(BaseModel):
    def __init__(
        ...
        if self.include_masks_in_features:
            self.feature_vector_shape = (-1,
                                         2 * self.num_samples_in_node_feature *
    ...
    def fit(self, train_set: Union[AqGeometricInMemoryDataset,
                                   AqGeometricDataset],
            val_set: Union[AqGeometricInMemoryDataset,
                           AqGeometricDataset], batch_size: int = 32,
            num_boost_round: int = 100, **kwargs) -> None:
        """
        ...
        dataset_target_to_index_map = {
            v: i
            for i, v in enumerate(self.targets)
        }

        val_adapter = NumpyDatasetAdapter(val_set, batch_size=batch_size,
                                          transform=self.adapter_transform,
                                          verbose=self.verbose)
        val_iterator = val_adapter.get_dataset_iterator()
        train_adapter = NumpyDatasetAdapter(train_set, batch_size=batch_size,
                                            transform=self.adapter_transform,
                                            verbose=self.verbose)
        ...
        for i, (X, y) in enumerate(train_iterator):
            ...
            X_data = X.reshape(
                X.shape[0], -1
            )  # ensure the input Data is 2-D with the correct number of rows
            X_val_data = X_val.reshape(X_val.shape[0], -1)

            for t in self.targets:
                y_data = y[:, :, dataset_target_to_index_map[t]].reshape(-1)
                y_val_data = y_val[:, :,
                                   dataset_target_to_index_map[t]].reshape(-1)

                print(
                    f"for target {t} at iteration {i}, X has shape {X_data.shape}, y has shape {y_data.shape} X_val has shape {X_val_data.shape}, y_val has shape {y_val_data.shape}"
                )
                ...
                self.model[t] = lgb.train(
                    ...
                    init_model=self.model[t]
                    if self.model[t] is not None else None,
    ...
    def predict(self, dataset: Union[AqGeometricInMemoryDataset,
                                     AqGeometricDataset],
        ...
        predictions = {
            t: []
            for t in self.targets
        }  # Initialize empty prediction dict
        data_generator = NumpyDatasetAdapter(
            dataset, batch_size=batch_size, transform=self.adapter_transform,
            verbose=self.verbose).get_dataset_iterator()

        for X, _ in data_generator:
            X = X.reshape(X.shape[0],
                          -1)  # enforce 2D with correct number of rows
            for t in self.targets:
    ...
    def load_model(self, filename_prefix="models/model_"):
        ...
        for feature in self.targets:
            try:
                self.model[feature] = lgb.Booster(
                    model_file=f"{filename_prefix}{feature}.txt")
            except lgb.basic.LightGBMError:
                raise ValueError(
                    f"feature '{feature}' must have a model file with prefix '{filename_prefix}' to load from path."
                )
