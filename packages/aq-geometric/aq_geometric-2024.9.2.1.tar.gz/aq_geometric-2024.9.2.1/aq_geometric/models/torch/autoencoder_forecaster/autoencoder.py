...
class Encoder(nn.Module):
    def __init__(self, num_nodes, num_timestamps, num_features, hidden_dim,
                 latent_dim):
        super().__init__()
    ...
    def forward(self, x, edge_index, x_mask):
        ...
        x_masked = x * x_mask
        x_masked = torch.where(torch.isnan(x_masked), torch.tensor(0.0),
                               x_masked)
        ...
        hidden = hidden.permute(
            1, 0)  # (num_nodes, hidden_dim) -> (hidden_dim, num_nodes)
...
class Decoder(nn.Module):
    ...
    def forward(self, z, edge_index, x_mask):
        ...
        x_mask_temporal_mean = x_mask.to(torch.float32).mean(
            dim=-1)  # Average mask across feature dimension
        node_mask = x_mask_temporal_mean.any(
            dim=-1
        )  # Reduce to (num_nodes,) to see if any value was masked for this node
        ...
        z_repeated = hidden_decoded.unsqueeze(1).repeat(
            1, self.num_timestamps, 1)
        ...
        x_mask_expanded = node_mask.unsqueeze(1).unsqueeze(-1).expand_as(
            output)
        output_masked = output * x_mask_expanded
        output_masked = torch.where(torch.isnan(output_masked),
                                    torch.tensor(0.0), output_masked)

        # Expected shape of output_masked: (num_nodes, num_timestamps, hidden_dim)

        # Apply GCN decoder
        output_masked = output_masked.permute(
            1, 0, 2
        )  # (num_nodes, num_timestamps, hidden_dim) -> (num_timestamps, num_nodes, hidden_dim)
        spatial_embeddings = self.gcn_decoder(output_masked, edge_index)
        spatial_embeddings = spatial_embeddings.permute(1, 0,
                                                        2)  # Permute back
...
class AqGeometricSpatioTemporalAutoencoder(nn.Module):
    def __init__(self, num_nodes=5921, num_timestamps=48, num_features=2,
        ...
        self.encoder = Encoder(num_nodes, num_timestamps, num_features,
                               hidden_dim, latent_dim)
        self.decoder = Decoder(num_nodes, num_timestamps, num_features,
                               hidden_dim, latent_dim)
