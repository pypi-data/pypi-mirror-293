...
class AutoencoderForecaster(nn.Module):
    def __init__(self, autoencoder: AqGeometricSpatioTemporalAutoencoder,
                 forecast_steps: int, num_targets: int,
                 num_gru_layers: int = 2, bidirectional_gru: bool = True):
        super().__init__()
        ...
        self.rnn_decoder = nn.GRU(
            input_size=self.autoencoder.decoder.fc2.
            out_features,  # Input from the latent adjustment layer
        ...
        self.latent_adjustment = nn.Linear(self.autoencoder.encoder.latent_dim,
                                           self.autoencoder.encoder.latent_dim)
        self.output_refinement = nn.Linear(
            self.autoencoder.decoder.fc3.out_features, num_targets)
    ...
    def forward(self, x, edge_index, x_mask):
        ...
        for _ in range(self.forecast_steps):
            ...
            if self.bidirectional_gru:
                ...
                h0 = torch.cat(
                    [h0_forward, h0_backward],
                    dim=0)  # Concatenate along the num_layers dimension
            else:
                h0 = h0.squeeze(0)
            ...
            expanded_node_mask = node_mask.unsqueeze(1).unsqueeze(
                -1).expand_as(output)
            output *= expanded_node_mask
            output_masked = torch.where(torch.isnan(output), torch.tensor(0.0),
                                        output)
            ...
            spatial_embeddings = self.autoencoder.decoder.gcn_decoder(
                output_masked, edge_index)  # (num_nodes, 1, hidden_dim)
            ...
            output_features = self.output_refinement(
                self.autoencoder.decoder.fc3(spatial_embeddings))
