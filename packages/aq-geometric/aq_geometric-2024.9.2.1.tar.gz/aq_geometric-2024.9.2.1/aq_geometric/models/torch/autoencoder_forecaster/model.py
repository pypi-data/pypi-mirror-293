...
class AqGeometricAutoEncoderForecaster(TorchBaseModelAdapter):
    ...
    def __init__(
        ...
        num_nodes:
        int = 5921,  # this is a fixed number for the BatchNorm1d layer
        hidden_dim: int = 256,
        latent_dim: int = 512,
        num_gru_layers: int = 2,
        bidirectional_gru: bool = True,
        device: Optional[str] = None,
        transform_graph_fn: List[Callable] = [
            preprocess_graph_for_autoencoder
        ],
        inverse_transform_graph_fn: List[Callable] = [inverse_scale],
        inverse_transform_predictions_fn: List[Callable] = [
    ...
    def load(self, path: str):
        ...
        self.model = EnhancedAutoencoderForecaster(
            autoencoder, forecast_steps=self.num_samples_in_node_target,
            num_targets=self.num_targets, hidden_dim=self.latent_dim,
            num_gru_layers=self.num_gru_layers,
            bidirectional_gru=self.bidirectional_gru)
    ...
    def fit(self, train_set: TorchGeometricDatasetAdapter,
            val_set: Optional[TorchGeometricDatasetAdapter],
            batch_size: int = 32, n_epoch: int = 20,
            optimizer: Optional[torch.optim.Optimizer] = None,
            scheduler: Optional[
                torch.optim.lr_scheduler.ReduceLROnPlateau] = None,
            loss_fn: Optional[torch.nn.Module] = None, lr: float = 0.005,
            momentum: float = 0.9, verbose: bool = False, **kwargs) -> None:
        """
        ...
        if self.autoencoder_model is None:
            ...
            optimizer = optim.AdamW(autoencoder_model.parameters(), lr=0.001,
                                    weight_decay=1e-5)
            scheduler = optim.lr_scheduler.CosineAnnealingLR(
                optimizer, T_max=n_epoch, eta_min=1e-6)
            loss_fn = F.l1_loss
            ...
            for epoch in range(n_epoch):
                ...
                for batch_indices in self.shuffle_in_batches(
                    ...
                    for i in batch_indices:
                        ...
                        out, _ = autoencoder_model(graph.x, graph.edge_index,
                                                   graph.x_mask)
                        ...
                        if count % 100 == 0:
                            if verbose:
                                print(
                                    f"Epoch {epoch}, Batch {count}, Avg. Loss: {epoch_loss / count:.4f}"
                                )

                epoch_loss_avg = epoch_loss / len(train_set)
                if verbose:
                    print(
                        f"Epoch {epoch} finished. Avg. Loss: {epoch_loss_avg:.4f}"
                    )
                ...
                if avg_val_loss < best_val_loss:
                    ...
                    if verbose:
                        print(
                            f"New best model found with val loss {avg_val_loss:.4f}"
                        )
                    torch.save(autoencoder_model.state_dict(),
                               "autoencoder_best_model.pt")
            ...
            autoencoder_model.load_state_dict(
                torch.load("autoencoder_best_model.pt"))
            autoencoder_model = autoencoder_model.to(self.device)
            self.autoencoder_model = autoencoder_model
            os.remove("autoencoder_best_model.pt")
        if self.model is None:
            model = EnhancedAutoencoderForecaster(
                self.autoencoder_model,
                forecast_steps=self.num_samples_in_node_target,
                num_targets=self.num_targets, hidden_dim=self.latent_dim,
                num_gru_layers=self.num_gru_layers,
                bidirectional_gru=self.bidirectional_gru)
        else:
    ...
    def predict(
        ...
        if not (isinstance(h3_index, np.ndarray)
                or isinstance(h3_index, np.array)):
    ...
    def evaluate(
        ...
        for j in range(len(dataset)):
            ...
            for i, t in enumerate(self.targets):
                ...
                pred_masked = pred[:, 0, i][y_mask[:, 0,
                                                   i]].cpu().detach().numpy()
                for m in metrics:
                    evaluation_results[t][m].append(metric_fns[m](
                        y_masked,
                        pred_masked)  # Calculate metric on masked values
                                                    )
    ...
    def generate_forecasts(
        ...
        if self.is_iterative:
            return self._generate_forecasts_iterative(
                graph=graph, n_forecast_timesteps=n_forecast_timesteps,
    ...
    def _generate_forecasts_iterative(
        ...
        if not (isinstance(h3_indices, np.ndarray)
                or isinstance(h3_indices, np.array)):
            h3_indices = h3_indices.cpu().detach().numpy()
        if not (isinstance(target_timestamps, np.ndarray)
                or isinstance(target_timestamps, np.array)):
        ...
        timestamps = pd.date_range(start=series[0],
                                   periods=n_forecast_timesteps,
                                   freq=inferred_freq)
        ...
        with torch.no_grad():
            ...
            transformed_preds = self.inverse_transform_predictions(
                torch.clone(preds), graph)
        ...
        for i, target in enumerate(targets):
            ...
            forecast_df = pd.DataFrame(all_preds[:, :, i], columns=timestamps,
                                       index=h3_indices)
            ...
            if verbose:
                print(
                    f"[{datetime.now()}] added DataFrame {forecast_df.shape}")
    ...
    def _generate_forecasts_direct(
        ...
        for i, target in enumerate(targets):
            ...
            forecast_df = pd.DataFrame(preds[:, :, i], columns=timestamps,
