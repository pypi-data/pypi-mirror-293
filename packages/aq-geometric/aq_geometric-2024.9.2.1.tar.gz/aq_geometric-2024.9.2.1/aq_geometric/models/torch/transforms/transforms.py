...
def to_float32(
        graph: "torch_geometric.data.Data") -> "torch_geometric.data.Data":
...
def slice_graph(graph: "torch_geometric.data.Data", x_samples: Optional[int],
                y_samples: Optional[int]) -> "torch_geometric.data.Data":
...
def append_predictions_to_graph(
        graph: "torch_geometric.data.Data",
        predictions: "torch.Tensor") -> "torch_geometric.data.Data":
    """Append the predictions to the graph."""
    assert graph.x.size(0) == predictions.size(
        0
    ), "Number of samples in graph and predictions do not match along the node dimension."
    assert graph.x.size(2) == predictions.size(
        2
    ), "Number of samples in graph and predictions do not match along the feature dimension."
    ...
    graph.x_mask = torch.cat(
        [graph.x_mask, graph.x_mask[:, -1:, :].repeat(1, num_timestamps, 1)],
        dim=1)
...
def min_max_scale(
    ...
    for feature_idx in range(num_features):
        ...
        if denominator == 0:
            ...
            x_scaled[:, :,
                     feature_idx] = (feature_slice - min_val) / denominator
...
def quantile_scale(graph: "torch_geometric.data.Data",
                   lower_bound: float = 0.05,
                   upper_bound: float = 0.95) -> "torch_geometric.data.Data":
    ...
    for feature_idx in range(num_features):
        ...
        if denominator == 0:
            ...
            x_scaled[:, :,
                     feature_idx] = (feature_slice_x - min_val) / denominator
        ...
        if feature_idx < graph.y.size(2):
            ...
            if denominator == 0:
                ...
                y_scaled[:, :, feature_idx] = (feature_slice_y -
                                               min_val) / denominator
...
def celcius_to_kelvin(graph: "torch_geometric.data.Data",
                      feature_idx: int = 7) -> "torch_geometric.data.Data":
    """Convert temperature from Celsius to Kelvin."""
    graph.x[:, :, feature_idx] += 273.15
    return graph


def kelvin_to_celcius(graph: "torch_geometric.data.Data",
                      feature_idx: int = 7) -> "torch_geometric.data.Data":
...
def preprocess_graph_for_autoencoder(
        graph: "torch_geometric.Data", scale: str = "quantile",
        handle_temperature: bool = True) -> "torch_geometric.Data":
...
def inverse_scale(graph: "torch_geometric.Data") -> "torch_geometric.Data":
    ...
    for feature_idx in range(num_features):
        ...
        x[:, :, feature_idx] = feature_slice * (
            x_max[feature_idx] - x_min[feature_idx]) + x_min[feature_idx]
...
def inverse_scale_predictions(
        ...
        graph: "torch_geometric.Data") -> "torch_geometric.Data":
    """Inverse transform the scaled graph data back to its original form."""
    ...
    for feature_idx in range(num_targets):
        ...
        predictions[:, :, feature_idx] = feature_slice * (
            x_max[feature_idx] - x_min[feature_idx]) + x_min[feature_idx]
