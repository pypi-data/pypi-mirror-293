...
def masked_layer_norm(x, mask, normalized_shape=(0, 1)):
    ...
    mean = x.sum(dim=normalized_shape, keepdim=True) / mask.sum(
        dim=normalized_shape, keepdim=True)
    variance = (
        (x - mean)**2).sum(dim=normalized_shape, keepdim=True) / mask.sum(
            dim=normalized_shape, keepdim=True)
...
class MPNNLayer(nn.Module):
    def __init__(self, embedding_dim):
        ...
        self.message_passing = GCNConv(in_channels=embedding_dim,
                                       out_channels=embedding_dim, aggr="add",
...
class AqGeometricEncoder(nn.Module):
    def __init__(self, num_samples_per_feature, input_feature_dim,
        ...
        self.embedding_layer = nn.Linear(
            num_samples_per_feature * input_feature_dim, embedding_dim)
        self.attention = nn.MultiheadAttention(embedding_dim, num_heads)
        self.linear = nn.Linear(embedding_dim * num_samples_per_feature,
                                embedding_dim, bias=False)

    def forward(self, x, x_mask):
        x = masked_layer_norm(x, x_mask)  # Apply normalization first
        x = self.embedding_layer(
            x.view(-1, self.num_samples_per_feature *
                   self.input_feature_dim))  # Embed the input
...
class AqGeometricDecoder(nn.Module):
    def __init__(self, embedding_dim, num_samples_per_target,
                 output_feature_dim, output_shape):
        super().__init__()

        self.embedding_layer = nn.Linear(
            embedding_dim, num_samples_per_target * output_feature_dim)
...
class AqGeometricMessagePassingModel(TorchBaseModelAdapter):
    def __init__(
        ...
        super().__init__(
            name=name, guid=guid, stations=stations, features=features,
            targets=targets,
        ...
        self.encoder = AqGeometricEncoder(self.num_samples_in_node_feature,
                                          self.num_features_in_node_feature,
                                          embedding_dim, num_heads)
        self.processor = AqGeometricProcessor(embedding_dim,
                                              num_message_passing_steps)
        if self.is_iterative:
            self.decoder = AqGeometricDecoder(
                embedding_dim, 1, self.num_features_in_node_target,
                (1, self.num_features_in_node_target))
        else:
            self.decoder = AqGeometricDecoder(
                embedding_dim, self.num_samples_in_node_target,
                self.num_features_in_node_target,
                (self.num_samples_in_node_target,
                 self.num_features_in_node_target))
    ...
    def load(self, path: str):
        ...
        self.num_message_passing_steps = model_data.get(
            "num_message_passing_steps")
        ...
        for key, value in model_data.items():
            if key not in [
                    ...
                    "num_samples_in_node_feature",
                    "num_samples_in_node_target", "is_iterative",
                    "num_features_in_node_feature",
                    "num_features_in_node_target", "state_dict"
            ]:
    ...
    def _generate_forecasts_iterative(
        ...
        with torch.no_grad():
            for i in range(self.num_samples_in_node_target):
                ...
                if verbose:
                    print(
                        f"[{datetime.now()}] generated new iterative pred shape: {pred.shape}"
                    )
                preds.append(pred)
                ...
                if verbose:
                    print(
                        f"[{datetime.now()}] next input shape: {inputs.shape}")
                input_x_mask = torch.cat(
                    [input_x_mask[:, 1:, :], input_x_mask[:, -1:, :]], dim=1)
                if verbose:
                    print(
                        f"[{datetime.now()}] next input_x_mask shape: {input_x_mask.shape}"
                    )
        # each prediction in preds is (n_h3_index, 1, n_targets)
        # we want to stack them to get (n_h3_index, n_timestamps, n_targets)
        pred = np.stack(preds, axis=1).reshape(-1,
                                               self.num_samples_in_node_target,
                                               len(targets))
        if verbose:
        ...
        if verbose:
            print(
                f"[{datetime.now()}] after applying mask, forecast shape: {forecasts.shape}"
            )
