...
class AqGeometricTemporalModel(BaseModel):
    ...
    def fit(self, train_set: Union[AqGeometricInMemoryDataset,
                                   AqGeometricDataset],
            val_set: Optional[Union[AqGeometricInMemoryDataset,
                                    AqGeometricDataset]], batch_size: int = 32,
            n_epoch: int = 20, **kwargs) -> None:
        """
        ...
        """
        sample_graph = train_set.get(train_set.indices[0])
        graph_h3_index = sample_graph["h3_index"]
        graph_timestamps = sample_graph["feature_timestamps"]

        if self.state_dict is None:
            # Initialize state_dict if not already set
            state_dict = {
                h3_idx: {
                    tgt: {
                        t: {
                ...
                for h3_idx in graph_h3_index
            }
        else:
            state_dict = self.state_dict

        for i in train_set.indices[::min(len(graph_timestamps), 24)]:
            graph = train_set.get(i)

            graph_data = graph["x"]
            graph_h3_index = graph["h3_index"]
            graph_feature_timestamps = graph["feature_timestamps"]

            if self.verbose:
                print(
                    f"[{datetime.now()}] Training model with {len(graph_h3_index)} h3 indices"
                )
            ...
            for i, h3_idx in enumerate(graph_h3_index):
                ...
                for j, t in enumerate(graph_feature_timestamps):
                    ...
                    for k, tgt in enumerate(self.targets):
                        state_dict[h3_idx][tgt][t]["mean"] = (
                            state_dict[h3_idx][tgt][t]["mean"] *
                            state_dict[h3_idx][tgt][t]["count"] +
                            graph_data[i, j, k]) / (
                                state_dict[h3_idx][tgt][t]["count"] + 1)
                        state_dict[h3_idx][tgt][t]["count"] += 1
    ...
    def predict(
        ...
        for i, h3_idx in enumerate(h3_indices):
            if h3_idx in self.state_dict:
                for k, tgt in enumerate(self.targets):
                    if tgt not in self.state_dict[h3_idx]:
                        pred[i, :, k] = np.nan  # Handle missing targets
                    for j, t in enumerate(target_timestamps):
                        t = pd.Timestamp(t).hour
                        if t in self.state_dict[h3_idx][tgt]:
                            pred[i, j,
                                 k] = self.state_dict[h3_idx][tgt][t]["mean"]

        if self.verbose:
            print(
                f"[{datetime.now()}] After re-indexing to h3_index, predictions have shape {pred.shape}"
            )
    ...
    def evaluate(
        ...
        for i in dataset.indices:
            ...
            if y_mask.shape != pred_aligned.shape:
                raise ValueError(
                    "The shapes of the true mask and predictions are mismatched"
                )
