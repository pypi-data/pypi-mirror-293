...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2056(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2055(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2054(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2053(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2053))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2054))

            return append(singleton("{"), delay(_arrow2055))

        return join("\n", to_list(delay(_arrow2056)))
...
ValidationPackage_reflection = _expr2063
