...
class ProcessParameterValue(Record):
    ...
    def SetCategory(self, c: OntologyAnnotation) -> ProcessParameterValue:
        this: ProcessParameterValue = self
        def _arrow506(__unit: None=None) -> ProtocolParameter | None:
        ...
        return ProcessParameterValue(_arrow506(), this.Value, this.Unit)
    ...
    def PrintCompact(self, __unit: None=None) -> str:
        ...
        return ("" if (value is None) else _arrow507()) if (category is None) else (_arrow508() if (value is None) else _arrow509())
...
ProcessParameterValue_reflection = _expr510
