...
class Data:
    ...
    def Name(self, __unit: None=None) -> str | None:
        this: Data = self
        matchValue: str | None = this._filePath
        matchValue_1: str | None = this._selector
        def _arrow463(__unit: None=None) -> str | None:
            p_1: str = matchValue
            return p_1

        def _arrow464(__unit: None=None) -> str | None:
            p: str = matchValue
            s: str = matchValue_1
            return DataAux_nameFromPathAndSelector(p, s)

        return None if (matchValue is None) else (_arrow463() if (matchValue_1 is None) else _arrow464())
