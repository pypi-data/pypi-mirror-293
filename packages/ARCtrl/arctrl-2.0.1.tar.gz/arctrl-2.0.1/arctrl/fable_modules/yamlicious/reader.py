...
def collect_sequence_elements(eles: FSharpList[PreprocessorElement]) -> FSharpList[FSharpList[PreprocessorElement]]:
    ...
    if pattern_matching_result == 0:
        def _arrow296(__unit: None=None, eles: Any=eles) -> IEnumerable_1[FSharpList[PreprocessorElement]]:
            def _arrow295(__unit: None=None) -> IEnumerable_1[FSharpList[PreprocessorElement]]:
                return collect_sequence_elements(rest)

            return append(singleton(cons(PreprocessorElement(2, value_2(v["Value"])), yaml_ast_list)) if (v["Value"] is not None) else singleton(yaml_ast_list), delay(_arrow295))

        return to_list(delay(_arrow296))
        ...
        if is_empty(eles):
            ...
            if active_pattern_result_1 is not None:
                ...
                def _arrow298(__unit: None=None, eles: Any=eles) -> IEnumerable_1[FSharpList[PreprocessorElement]]:
                    def _arrow297(__unit: None=None) -> IEnumerable_1[FSharpList[PreprocessorElement]]:
                        return collect_sequence_elements(tail(eles))

                    return append(singleton(singleton_1(PreprocessorElement(2, value_2(v_1["Value"])))), delay(_arrow297))

                return to_list(delay(_arrow298))

            elif _007CYamlComment_007C__007C(head(eles)) is not None:
                def _arrow300(__unit: None=None, eles: Any=eles) -> IEnumerable_1[FSharpList[PreprocessorElement]]:
                    def _arrow299(__unit: None=None) -> IEnumerable_1[FSharpList[PreprocessorElement]]:
                        return collect_sequence_elements(tail(eles))

                    return append(singleton(singleton_1(head(eles))), delay(_arrow299))

                return to_list(delay(_arrow300))
...
def tokenize(yaml_list: FSharpList[PreprocessorElement], string_dict: Any, comment_dict: Any) -> YAMLElement:
    def loop_read(restlist_mut: FSharpList[PreprocessorElement], acc_mut: FSharpList[YAMLElement], yaml_list: Any=yaml_list, string_dict: Any=string_dict, comment_dict: Any=comment_dict) -> YAMLElement:
        while True:
            ...
            if pattern_matching_result == 0:
                def _arrow301(e: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                    return is_sequence_element(e)

                sequence_elements: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow301, rest0)))
                def _arrow302(e_1: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                    return is_sequence_element(e_1)

                restlist_mut = to_list(skip_while(_arrow302, rest0))
                def _arrow308(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                    def _arrow307(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                        def _arrow306(i: FSharpList[PreprocessorElement]) -> YAMLElement:
                            return loop_read(i, empty())

                        return map_1(_arrow306, sequence_elements)

                    return append(singleton(loop_read(cons(PreprocessorElement(2, v["Key"]), yaml_ast_list), empty())), delay(_arrow307))

                acc_mut = cons(YAMLElement(2, to_list(delay(_arrow308))), acc)
                continue
                ...
                if pattern_matching_result_1 == 0:
                    def _arrow309(e_2: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                        return is_sequence_element(e_2)

                    sequence_elements_1: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow309, rest0_1)))
                    def _arrow310(e_3: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                        return is_sequence_element(e_3)

                    restlist_mut = to_list(skip_while(_arrow310, rest0_1))
                    def _arrow316(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                        def _arrow315(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                            def _arrow314(i_1: FSharpList[PreprocessorElement]) -> YAMLElement:
                                return loop_read(i_1, empty())

                            return map_1(_arrow314, sequence_elements_1)

                        return append(singleton(loop_read(singleton_1(PreprocessorElement(2, v_1["Key"])), empty())), delay(_arrow315))

                    acc_mut = cons(YAMLElement(2, to_list(delay(_arrow316))), acc)
                    continue
                    ...
                    def _arrow317(e_4: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                        return is_sequence_element(e_4)

                    sequence_elements_2: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow317, rest0_2)))
                    def _arrow318(e_5: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                        return is_sequence_element(e_5)

                    restlist_mut = to_list(skip_while(_arrow318, rest0_2))
                    def _arrow324(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                        def _arrow323(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                            def _arrow322(i_2: FSharpList[PreprocessorElement]) -> YAMLElement:
                                return loop_read(i_2, empty())

                            return map_1(_arrow322, sequence_elements_2)

                        return append(singleton(loop_read(object_list_1, empty())), delay(_arrow323))

                    acc_mut = cons(YAMLElement(2, to_list(delay(_arrow324))), acc)
                    continue
                    ...
                    if pattern_matching_result_2 == 0:
                        def _arrow325(e_6: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                            return is_sequence_element(e_6)

                        sequence_elements_3: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow325, rest0_3)))
                        def _arrow326(e_7: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                            return is_sequence_element(e_7)

                        restlist_mut = to_list(skip_while(_arrow326, rest0_3))
                        def _arrow332(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                            def _arrow331(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                                def _arrow330(i_3: FSharpList[PreprocessorElement]) -> YAMLElement:
                                    return loop_read(i_3, empty())

                                return map_1(_arrow330, sequence_elements_3)

                            return append(singleton(loop_read(singleton_1(PreprocessorElement(2, value_2(v_3["Value"]))), empty())), delay(_arrow331))

                        acc_mut = cons(YAMLElement(2, to_list(delay(_arrow332))), acc)
                        continue
                        ...
                        def _arrow334(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                            def _arrow333(value: str) -> YAMLElement:
                                return loop_read(singleton_1(PreprocessorElement(2, value.strip())), empty())

                            return map_1(_arrow333, split)

                        current_4: YAMLElement = YAMLElement(2, to_list(delay(_arrow334)))
                        restlist_mut = rest_4
                        ...
                        def _arrow336(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                            def _arrow335(i_4: PreprocessorElement) -> IEnumerable_1[YAMLElement]:
                            ...
                            return collect(_arrow335, i_list)

                        current_5: YAMLElement = YAMLElement(2, to_list(delay(_arrow336)))
                        restlist_mut = rest_5
                        def _arrow337(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> FSharpList[YAMLElement]:
                            c2_2: str = c2
                            return of_array_with_tail([YAMLElement(4, c2_2), current_5], acc)

                        def _arrow338(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> FSharpList[YAMLElement]:
                            c1_2: str = c1
                            return of_array_with_tail([current_5, YAMLElement(4, c1_2)], acc)

                        def _arrow339(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> FSharpList[YAMLElement]:
                            c1_1: str = c1
                            c2_1: str = c2
                            return of_array_with_tail([YAMLElement(4, c2_1), current_5, YAMLElement(4, c1_1)], acc)

                        acc_mut = (cons(current_5, acc) if (c2 is None) else _arrow337()) if (c1 is None) else (_arrow338() if (c2 is None) else _arrow339())
                        continue
                        ...
                        if pattern_matching_result_3 == 0:
                            ...
                            def _arrow341(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                                def _arrow340(value_1: str) -> IEnumerable_1[YAMLElement]:
                                ...
                                return collect(_arrow340, split_1)

                            current_6: FSharpList[YAMLElement] = reverse(to_list(delay(_arrow341)))
                            restlist_mut = rest_6
                            ...
                            def _arrow343(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                                def _arrow342(i_5: PreprocessorElement) -> IEnumerable_1[YAMLElement]:
                                ...
                                return collect(_arrow342, i_list_1)

                            current_7: YAMLElement = YAMLElement(0, YAMLContent_create_27AED5E3(opener_1["Key"], c1_3), YAMLElement(3, to_list(delay(_arrow343))))
                            restlist_mut = rest_7
                            ...
                            if pattern_matching_result_4 == 0:
                                ...
                                if pattern_matching_result_5 == 0:
                                    ...
                                    def _arrow344(e_8: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                                        return is_sequence_element(e_8)

                                    sequence_elements_4: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow344, rest0_4)))
                                    def _arrow345(e_9: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                                        return is_sequence_element(e_9)

                                    rest_9: FSharpList[PreprocessorElement] = to_list(skip_while(_arrow345, rest0_4))
                                    def _arrow348(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                                        def _arrow347(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                                            def _arrow346(i_6: FSharpList[PreprocessorElement]) -> YAMLElement:
                                                return loop_read(i_6, empty())

                                            return map_1(_arrow346, sequence_elements_4)

                                        return append(singleton(loop_read(object_list_2, empty())), delay(_arrow347))

                                    seq: YAMLElement = YAMLElement(2, to_list(delay(_arrow348)))
                                    restlist_mut = rest_9
                                    ...
                                    if pattern_matching_result_6 == 0:
                                        ...
                                        def _arrow349(e_10: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                                            return is_sequence_element(e_10)

                                        sequence_elements_5: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow349, rest0_5)))
                                        def _arrow350(e_11: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                                            return is_sequence_element(e_11)

                                        rest_10: FSharpList[PreprocessorElement] = to_list(skip_while(_arrow350, rest0_5))
                                        def _arrow353(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                                            def _arrow352(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                                                def _arrow351(i_7: FSharpList[PreprocessorElement]) -> YAMLElement:
                                                    return loop_read(i_7, empty())

                                                return map_1(_arrow351, sequence_elements_5)

                                            return append(singleton(loop_read(singleton_1(PreprocessorElement(2, value_2(w_1["Value"]))), empty())), delay(_arrow352))

                                        seq_1: YAMLElement = YAMLElement(2, to_list(delay(_arrow353)))
