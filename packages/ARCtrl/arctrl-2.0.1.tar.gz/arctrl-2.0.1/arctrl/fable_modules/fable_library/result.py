...
def Result_Bind(
    binder: Callable[[_A], FSharpResult_2[_B, _C]], result: FSharpResult_2[_A, _C]
) -> FSharpResult_2[_B, _C]:
    if result.tag == 0:
...
def Result_Contains(value: _A, result: FSharpResult_2[_A, Any]) -> bool:
    if result.tag == 0:
...
def Result_DefaultValue(default_value: _A, result: FSharpResult_2[_A, Any]) -> _A:
    if result.tag == 0:
...
def Result_DefaultWith(def_thunk: Callable[[_B], _A], result: FSharpResult_2[_A, _B]) -> _A:
    if result.tag == 0:
        return result.fields[0]
...
def Result_Exists(predicate: Callable[[_A], bool], result: FSharpResult_2[_A, Any]) -> bool:
    if result.tag == 0:
...
def Result_Fold(folder: Callable[[_S, _A], _S], state: _S, result: FSharpResult_2[_A, Any]) -> _S:
    if result.tag == 0:
...
def Result_FoldBack(folder: Callable[[_A, _S], _S], result: FSharpResult_2[_A, Any], state: _S) -> _S:
    if result.tag == 0:
...
def Result_ForAll(predicate: Callable[[_A], bool], result: FSharpResult_2[_A, Any]) -> bool:
    if result.tag == 0:
        return predicate(result.fields[0])
...
def Result_Iterate(action: Callable[[_A], None], result: FSharpResult_2[_A, Any]) -> None:
    if result.tag == 0:
        action(result.fields[0])


def Result_ToArray(result: FSharpResult_2[_A, Any]) -> Array[_A]:
    if result.tag == 0:
        return [result.fields[0]]
...
def Result_ToList(result: FSharpResult_2[_A, Any]) -> FSharpList[_A]:
    if result.tag == 0:
...
def Result_ToOption(result: FSharpResult_2[_A, Any]) -> _A | None:
    if result.tag == 0:
...
def Result_ToValueOption(result: FSharpResult_2[_A, Any]) -> _A | None:
    if result.tag == 0:
        return some(result.fields[0])
