...
from ...fable_modules.fable_library.seq import (try_find, to_array as to_array_1, delay, map as map_2, collect, singleton as singleton_1)
...
def compose_columns(string_cell_columns: Array[Array[str]]) -> Array[CompositeColumn]:
    def _arrow1122(columns: Array[Array[str]], string_cell_columns: Any=string_cell_columns) -> CompositeColumn:
        return from_string_cell_columns(columns)

    return map_1(_arrow1122, group_columns_by_header(string_cell_columns), None)
...
def try_from_fs_worksheet(sheet: FsWorksheet) -> ArcTable | None:
    try: 
        ...
        if match_value is None:
            ...
            def _arrow1126(__unit: None=None) -> IEnumerable_1[Array[str]]:
                def _arrow1125(c: int) -> Array[str]:
                    def _arrow1124(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow1123(r: int) -> IEnumerable_1[str]:
                        ...
                        return collect(_arrow1123, range_big_int(1, 1, FsAddress__get_RowNumber(FsRangeAddress__get_LastAddress(FsRangeBase__get_RangeAddress(t)))))

                    return to_array_1(delay(_arrow1124))

                return map_2(_arrow1125, range_big_int(1, 1, FsAddress__get_ColumnNumber(FsRangeAddress__get_LastAddress(FsRangeBase__get_RangeAddress(t)))))

            composite_columns: Array[CompositeColumn] = compose_columns(map_1(fix_deprecated_ioheader, to_array_1(delay(_arrow1126)), None))
...
def to_fs_worksheet(index: int | None, table: ArcTable) -> FsWorksheet:
    ...
    if len(table.Columns) == 0:
        ...
        def _arrow1127(column_1: CompositeColumn, index: Any=index, table: Any=table) -> FSharpList[FSharpList[str]]:
            return to_string_cell_columns(column_1)

        def _arrow1128(column: CompositeColumn, index: Any=index, table: Any=table) -> enum_type("ARCtrl.Spreadsheet.ArcTable.ColumnOrder", int, [("InputClass", 1.0), ("ProtocolClass", 2.0), ("ParamsClass", 3.0), ("OutputClass", 4.0)]):
        ...
        columns: FSharpList[FSharpList[str]] = collect_1(_arrow1127, sort_by(_arrow1128, of_array(table.Columns), ObjectExpr1129()))
        ...
        fs_table: FsTable = ws.Table(name, FsRangeAddress__ctor_7E77A4A0(FsAddress__ctor_Z37302880(1, 1), FsAddress__ctor_Z37302880(max_row, max_col)))
