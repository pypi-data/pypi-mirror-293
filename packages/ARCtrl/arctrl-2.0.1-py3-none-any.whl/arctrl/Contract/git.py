...
Init_reflection = _expr2229

def _expr2230() -> TypeInfo:
...
def Clone_createCloneContract_5000466F(remote_url: str, merge: bool | None=None, branch: str | None=None, token: tuple[str, str] | None=None, nolfs: bool | None=None) -> Contract:
    ...
    def _arrow2236(__unit: None=None, remote_url: Any=remote_url, merge: Any=merge, branch: Any=branch, token: Any=token, nolfs: Any=nolfs) -> IEnumerable_1[str]:
        def _arrow2235(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2234(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2233(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2232(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow2231(__unit: None=None) -> IEnumerable_1[str]:
                            return singleton(".") if merge_1 else empty()

                        return append(singleton(remote_url_1), delay(_arrow2231))

                    return append(singleton(value(branch)) if (branch is not None) else empty(), delay(_arrow2232))

                return append(singleton(Clone_get_branchFlag()) if (branch is not None) else empty(), delay(_arrow2233))

            return append(singleton(Clone_get_noLFSConfig()) if nolfs_1 else empty(), delay(_arrow2234))

        return append(singleton(Clone_get_clone()), delay(_arrow2235))

    return create_git_contract(to_array(delay(_arrow2236)))
