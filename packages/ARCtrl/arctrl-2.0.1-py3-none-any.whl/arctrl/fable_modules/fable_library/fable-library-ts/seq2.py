...
def distinct(xs: IEnumerable_1[_T], comparer: IEqualityComparer_1[Any]) -> IEnumerable_1[_T]:
    def _arrow115(__unit: None = None, xs: Any = xs, comparer: Any = comparer) -> IEnumerable_1[_T]:
    ...
    def _arrow121(
        ...
        with get_enumerator(xs) as enumerator:
            while enumerator.System_Collections_IEnumerator_MoveNext():
                ...
                def _arrow118(__unit: None = None) -> int:
                    return out_arg

                def _arrow119(v: int) -> None:
                    nonlocal out_arg
                    out_arg = v or 0

                match_value = (try_get_value(dict_1, key, FSharpRef(_arrow118, _arrow119)), out_arg)
                if match_value[0]:
        ...
        def _arrow120(key_1: _KEY | None = None) -> tuple[_KEY, int]:
            return (key_1, get_item_from_dict(dict_1, key_1))

        return map(_arrow120, keys)

    return delay(_arrow121)
    ...
    def _arrow125(
        ...
        with get_enumerator(xs) as enumerator:
            while enumerator.System_Collections_IEnumerator_MoveNext():
                ...
                def _arrow122(__unit: None = None) -> Array[_T]:
                    return out_arg

                def _arrow123(v: Array[_T]) -> None:
                    nonlocal out_arg
                    out_arg = v

                match_value = (try_get_value(dict_1, key, FSharpRef(_arrow122, _arrow123)), out_arg)
                if match_value[0]:
        ...
        def _arrow124(key_1: _KEY | None = None) -> tuple[_KEY, IEnumerable_1[_T]]:
            return (key_1, get_item_from_dict(dict_1, key_1))

        return map(_arrow124, keys)
