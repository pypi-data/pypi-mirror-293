...
def seq(encoder: Callable[[_A], YAMLElement], s: IEnumerable_1[_A]) -> YAMLElement:
    return YAMLElement(2, of_seq(map(encoder, s)))


def array(encoder: Callable[[_A], YAMLElement], arr: Array[_A]) -> YAMLElement:
    return YAMLElement(2, map_1(encoder, of_array(arr)))


def resizearray(encoder: Callable[[_A], YAMLElement], arr: Array[_A]) -> YAMLElement:
    return YAMLElement(2, map_1(encoder, of_seq(arr)))


def list_1(encoder: Callable[[_A], YAMLElement], l: FSharpList[_A]) -> YAMLElement:
    return YAMLElement(2, map_1(encoder, l))


def values(encoder: Callable[[_A], str], values_1: IEnumerable_1[_A]) -> YAMLElement:
...
def try_include(name: str, encoder: Callable[[_A], YAMLElement], value: _A | None=None) -> tuple[str, YAMLElement]:
    return (name, encoder(value_1(value)) if (value is not None) else nil)


def write(whitespaces: int, ele: YAMLElement) -> str:
    def _arrow294(c: Config, whitespaces: Any=whitespaces, ele: Any=ele) -> Config:
        return Config(whitespaces, c.Level)

    return write_1(ele, _arrow294)
