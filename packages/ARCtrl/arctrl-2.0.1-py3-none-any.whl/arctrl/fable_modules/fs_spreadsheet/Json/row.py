...
def encode(row: FsRow) -> Json:
    def _arrow245(__unit: None=None, row: Any=row) -> Json:
    ...
    return Json(5, to_enumerable([("number", _arrow245()), ("cells", seq(map(mapping, row.Cells)))]))
...
def _arrow248(builder: IGetters) -> tuple[int | None, IEnumerable_1[FsCell]]:
    n: int | None
    object_arg: IOptionalGetter = builder.Optional
    n = object_arg.Field("number", int_1)
    def _arrow247(__unit: None=None) -> IEnumerable_1[FsCell] | None:
    ...
    return (n, default_arg(_arrow247(), empty()))


decode: Decoder_1[tuple[int | None, IEnumerable_1[FsCell]]] = object(_arrow248)
