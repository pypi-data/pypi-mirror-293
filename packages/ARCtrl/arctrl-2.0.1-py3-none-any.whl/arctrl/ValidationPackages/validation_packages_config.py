...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow2084(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2083(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2082(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2081(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow2080(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow2079(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow2079))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow2080))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow2081))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow2082))

            return append(singleton("{"), delay(_arrow2083))

        return join("\n", to_list(delay(_arrow2084)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow2087(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow2086(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow2086, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow2087))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr2088
