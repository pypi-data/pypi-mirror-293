...
class EvalsClient:
    ...
    def list_kbs(self) -> list[KnowledgeBase]:
        ...
        response = requests.get(
            f"{self.url}/evals/kb/list", headers={"X-TALC-API": self.api_key}
        )
        response.raise_for_status()
    ...
    def upload_knowledge_base_asbytes(
        ...
        response = requests.post(
            f"{self.url}/evals/kb/create/{name}?verify={1 if verify else 0}&auto_link={1 if auto_link else 0}",
    ...
    #     response = requests.get(
    #         f"{self.url}/evals/kb/{knowledgebase_id}",
    #         headers={"X-TALC-API": self.api_key},
    #     )
    #     response.raise_for_status()
