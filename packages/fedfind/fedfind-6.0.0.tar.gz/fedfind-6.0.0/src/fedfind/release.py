...
def _parse_url(url):
    ...
    if url.rstrip("/").endswith("/alt/live-respins"):
        ...
        return ("Fedora", "eln", "", odcsnum, None, None)
...
def get_release(
    ...
    if str(release).lower() == "eln":
        ...
        if fedfind.helpers.date_check(compose, fail_raise=False):
            return ElnNightly(compose, (respin or None), (cid or None))
...
class ODCSCompose(Pungi4ReleaseFull):
    ...
    def __init__(self, composenum, cid=None, label=None):
        ...
        super(ODCSCompose, self).__init__(location, cid=cid, label=label)


class ElnMixin(object):
    """Some properties that are shared between ELN compose classes."""
...
class ElnCompose(ElnMixin, ODCSCompose):
    """Old ELN composes. These are composes of Fedora Rawhide packages
    ...
    This class handles older ones which were built with ODCS; if we
    wipe https://odcs.fedoraproject.org/composes/ at some point this
    class can probably go away as it will no longer be possible to
    find these.
    """

    @property
    def previous_release(self):
        """These composes aren't in kojipkgs, there's no obvious other
        easy way to do this, and we don't really need it.
        """
        raise NotImplementedError


class ElnNightly(ElnMixin, Pungi4ReleaseFull):
