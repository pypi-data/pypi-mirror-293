...
def largeCsmall(xyz_l,xyz_s):
    '输出大点云-小点云的点云'
    label_l = xyz_l[:,0]*100+xyz_l[:,1]*10+xyz_l[:,2]
    label_s = xyz_s[:,0]*100+xyz_s[:,1]*10+xyz_s[:,2]
...
def AccuracyEvaluation(standard_vehicle,standard_non_vehicle,experiment_vehicle,experiment_non_vehicle):
    ...
    print(f"卡帕系数 (Kappa coefficient): {kappa:.4f}")
    return np.array([type_I_error,type_II_error,total_error,completeness,correctness,accuracy,quality,kappa])
