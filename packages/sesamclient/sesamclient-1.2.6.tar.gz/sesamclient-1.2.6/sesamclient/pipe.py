...
class Pipe(EntityBase):
    ...
    def wait_for_pipe_to_be_deployed(self, timeout=300, **kwargs):
        ...
        while True:
            try:
                self.update_raw_jsondata_from_url(self._connection.get_pipe_url(self.id),
                                                  retries=0,  # we don't want the update_raw_jsondata_from_url() method to do retries, since we want to handle 404's as quickly as possible.
                                                  **kwargs)
            ...
            if self.runtime["state"] not in ["Deploying", "Removing"]:
                try:
                    self.get_pump(
                        retries=0, # we don't want the get_pump() method to do retries, since we want to handle 404's as quickly as possible.
                    )
                    return
                except ExceptionWithResponse as e:
                    if e.response.status_code == 404:
