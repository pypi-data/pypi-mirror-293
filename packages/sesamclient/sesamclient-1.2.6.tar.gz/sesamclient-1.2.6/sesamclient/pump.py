...
class Pump(EntityBase):
    ...
    def wait_for_pump_to_finish_running(self, timeout=60, poll_interval=0.5, verbosity=0,
                                        assert_successful_run=False,
                                        **kwargs):
        ...
        if assert_successful_run and (self.runtime.get("success") is not True):
            raise AssertionError(f"The pipe '{self.pipe_id}' failed! pump.runtime:\n{pprint.pformat(self.runtime)}")

    def has_pending_work(self, **kwargs):
    ...
    def run_pump_until_there_are_no_pending_work(self, timeout=60, sleep_time_between_runs=0.5, verbosity=0, **kwargs):
        ...
        """
        self.wait_for_pump_to_finish_running(timeout=timeout, verbosity=verbosity, **kwargs)
        self.start(**kwargs)
        self.wait_for_pump_to_finish_running(timeout=timeout, verbosity=verbosity, **kwargs)

        starttime = time.monotonic()
        while True:
            if not self.has_pending_work(**kwargs):
                # Ok, we are done.
                return
    ...
    def reset_tracking(self, **kwargs):
        self.run_operation("reset-tracking", allowable_response_status_codes=frozenset([200]), **kwargs)

    def set_tracking_to_start(self, **kwargs):
        self.run_operation("set-tracking-to-start", allowable_response_status_codes=frozenset([200]), **kwargs)

    def unset_last_seen(self, **kwargs):
        self.update_last_seen("", **kwargs)

    def set_last_seen_to_end(self, **kwargs):
        self.run_operation("set-last-seen-to-end", allowable_response_status_codes=frozenset([200]), **kwargs)

    def update_last_seen(self, last_seen, **kwargs):
    ...
    def disable(self, comment=None, **kwargs):
        """
        ...
        """
        self._set_disabled_state(True, comment=comment, **kwargs)

    def enable(self, **kwargs):
        """Enable this pump."""
        self._set_disabled_state(False, **kwargs)

    def _set_disabled_state(self, is_disabled, comment=None, **kwargs):
    ...
    def start(self, dont_wait_for_pump_to_start=False, allow_already_running_pump=True, operation_parameters={},
        ...
        response = self.run_operation("start", operation_parameters=operation_parameters,
                                      allowable_response_status_codes=frozenset([200, 409]), **kwargs)
    ...
    def stop(self, **kwargs):
        ...
        Stops the pump.
        """
        self.run_operation("stop", **kwargs)

    def start_rescan(self, allow_already_running_pump=True, **kwargs):
        """
        Starts a rescan-run.
        :param allow_already_running_pump: If this  flag is set to False, this method will fail with a
                                           "PumpIsAlreadyRunning"-exception if if the pump was already
                                           running.
        """
        response = self.run_operation("start-rescan", allowable_response_status_codes=frozenset([200, 409]), **kwargs)
    ...
    def reset_rescan(self, **kwargs):
        """
        Resets the rescan-run state.
        """
        response = self.run_operation("reset-rescan", allowable_response_status_codes=frozenset([200]), **kwargs)
        validate_response_is_ok(response, 200)
        self.update_raw_jsondata(response.content.decode("utf-8"))
        return True
