...
class Connection:
    ...
    @property
    def health_url(self):
        return self.sesamapi_base_url + "health"
    ...
    def load_config(self, **kwargs):
        """
            Tells the node to reload its config (i.e. from the aggregator-storage-node or a config-file,
             it returns a dict {"message": "OK"} if successful
        """

        url = self.admin_url

        postdata = {"operation": "load_config"}
    ...
    def get_health(self, **kwargs):
        """
        :return: A dict with various health-information (if the last backup was successful, etc.)
        """
        health_response = self.do_get_request(self.health_url, allowable_response_status_codes=[200, 503], **kwargs)
        return utils.parse_json_response(health_response)
    ...
    def get_backup(self, backup_id, allow_non_exisiting_backup=False, **kwargs):
        allowable_response_status_codes = [200]
        if allow_non_exisiting_backup:
            allowable_response_status_codes.append(404)
        url = self.sesamapi_base_url + "backups/" + backup_id
        response = self.do_get_request(url, allowable_response_status_codes=allowable_response_status_codes, **kwargs)
        if response.status_code == 404 and allow_non_exisiting_backup:
            return None
