...
from botrun_ask_folder.litellm_proxy.config.custom_handler import read_botrun_content
    ...
    for event in events:
        if event["type"] == "message" and event["message"]["type"] == "text":
            ...
            if user_setting.model_name == "":
                if mock_check_model_exists(message_text):
                ...
                response_message = query_response(message_text, user_setting.model_name)

            await reply_message(event["replyToken"], response_message)


def query_response(message_text: str, model_name: str) -> str:
    api_base = "http://dev.botrun.ai:4000"
    try:
        # return
        messages = [{"content": message_text, "role": "user"}]
        response = completion(
            model=f"botrun/botrun-{model_name}",
            custom_llm_provider="openai",
            messages=messages,
            base_url=api_base,
            api_key=os.getenv("BOTRUN_API_KEY"),
        )
        # print("==LiteLLM None Stream Content====")
...
def mock_check_model_exists(model_name: str) -> bool:
    # 這裡是一個模擬檢查模型是否存在的函數
    # 在實際應用中，你需要實現真正的檢查邏輯
    try:
