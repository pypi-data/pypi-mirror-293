    ...
    def run(self, documents:List[Document]):
        ...
        def process_interest(interest):
            if self.service == "default":
                ...
                response_json = self.llm.invoke(prompt,json_schema,"MapInterestToQuestionType", self.pipeline_id)
                try:
                    raw_answer = json.loads(response_json["reply"])
                except Exception as e:
                    print(f"MapInterestToQuestionType json decode error:{e}, with response_json:{response_json}")
