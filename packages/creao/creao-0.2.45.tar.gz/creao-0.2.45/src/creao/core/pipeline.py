...
def creao_component(cls):
    ...
    def wrapped_run(self, chained_input:List[Dict[str, str]], *args, **kwargs):
...
class Pipeline:
    ...
    def run_single(self, instance_specific_params=None):
        ...
        for node_id in execution_order[1:]:
            ...
            # Run the class instance with the updated chained_input
            output = class_instance.run(chained_input, self.global_variables, instance_name=node_name, validate_schema=False)
...
class FilterComponent:
    ...
    def run(self, chained_input: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Filters the input data based on the specified condition and returns the filtered output.

        Args:
        - chained_input (List[Dict[str, Any]]): A list of dictionaries where each dictionary represents a row or record.

        Returns:
        - List[Dict[str, Any]]: The filtered list of dictionaries after applying the specified conditions.
        
        Example:
        - Input: [{"price": "100"}, {"price": "30"}]
        - If condition is "numeric_condition" and condition_value is "> 50", the output will be [{"price": "100"}].
        """
        filtered_output = []

        for item in chained_input:
            column_value = item.get(self.filtered_column)  # Retrieve the value from the specified column

            if column_value is None:
                continue  # Skip if the column value is missing
            ...
            if self.condition_type == "exact_match":
                ...
                if column_value == self.condition_value:  # Check for exact match
                    filtered_output.append(item)  # Add the row to the output if it matches
                ...
                try:
                    ...
                    if eval(f"{numeric_value} {self.condition_value}"):  # Evaluate the numeric condition
                        filtered_output.append(item)  # Add the row to the output if it meets the condition
                except ValueError:
                    continue  # Skip if the value can't be converted to a number

        return filtered_output  # Return the filtered data
...
class DedupeComponent:
    ...
    def run(self, chained_input: List[Dict[str, str]]) -> List[Dict[str, str]]:
        ...
        """
        dict_of_lists = self.convert_to_dict_of_lists(chained_input)
        assert self.dedup_column in dict_of_lists, f"Column '{self.dedup_column}' not found in the input data."
        for key in dict_of_lists:
            if key == self.dedup_column:
                texts = dict_of_lists[key]
                drop_res = self.llm.execute(texts)
        
        i = 0
        final_res = []
        for drop in drop_res:
            if not drop:
                final_res.append(chained_input[i])
            i += 1
        return final_res
