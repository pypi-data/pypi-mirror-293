...
class Pipeline:
    ...
    def from_dict(pipeline_dict, dataset_children_map, global_variables):
        ...
        global_dataset_map = pipeline_dict.get("global_dataset_map", {})
        ...
        for node in pipeline_dict["nodes"]:
            ...
            if class_name == "HFDataComponent":
                class_instance.global_dataset_map = global_dataset_map
    ...
    def load_from_rf_json(file_path, rf_dict=None, global_variables=None):
        ...
        final_json = {"nodes":nodes, "connections":connections,"pipeline_id":rf_dict["pipeline_id"],"global_dataset_map":rf_dict["global_dataset_map"]}
...
class HFDataComponent:
    ...
    def run(self, chained_input: List[Dict[str, str]]) -> List[Dict[str, List[str]]]:
        ...
        list_res = []
        global_dataset_map = self.global_dataset_map # which is defined in the pipeline loading step
        print("global_dataset_map:",global_dataset_map)
        for item in dataset:
            # for key in global_dataset_map, replace the key value with the value from the global_dataset_map
            for key in global_dataset_map:
                if key in item:
                    value = item[key]
                    mapped_key = global_dataset_map[key]
                    # pop the key from the item
                    item.pop(key)
                    item[mapped_key] = value
                    print("updated item:",item)
            print("new dataset item:",item)
