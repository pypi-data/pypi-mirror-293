    ...
    def run(self, documents:List[Document]):
        ...
        def filter_question(question):
            if self.service == "default":
                ...
                response_json = self.llm.invoke(prompt,json_schema,"IntelligentFilter",self.pipeline_id)
                try:
                    raw_answer = json.loads(response_json["reply"])
                except Exception as e:
                    print(f"IntelligentFilter json decode error:{e}, with response_json:{response_json}")
