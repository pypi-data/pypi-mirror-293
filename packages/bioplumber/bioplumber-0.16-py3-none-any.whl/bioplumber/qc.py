...
def qc_fastp_(
    ...
    **kwargs
    ...
    if read2 is None and ourdir2 is not None:
        raise ValueError("read2 is None but ourdir2 is not None")
    ...
    if container=="none":
        if paired:
            ...
            qcd1=Path(ourdir1).absolute()
            qcd2=Path(ourdir2).absolute()
            json_file=Path(ourdir1).parent / "fastp.json"
            html_file=Path(ourdir1).parent / "fastp.html"
