...
async def test_创建产品():
    ...
    app_id = "test_app_id"
    price = 9.99
    result = await 创建产品(name, app_id, price)
...
async def test_更新产品():
    ...
    new_price = 19.99
    result = await 更新产品(product_id, name=new_name, price=new_price)
...
async def test_创建订单():
    ...
    total_amount = 29.99
    global product_id
    result = await 创建订单(user_id, app_id, total_amount, product_id)
...
async def test_更新订单状态():
    ...
    result = await 更新订单状态(order_id, new_status)
...
async def test_创建支付():
    ...
    result = await 创建支付(app_id, order_id, payment_method, amount, transaction_id, payment_status, callback_url)
