...
async def 创建产品(name: str, app_id: str, price: float) -> 返回_创建产品:
    async with httpx.AsyncClient() as client:
...
async def 更新产品(product_id: int, name: Optional[str] = None, price: Optional[float] = None) -> 返回_更新产品:
    async with httpx.AsyncClient() as client:
        data = {"name": name, "price": price}
        response = await client.put(f"{BASE_URL}/products/{product_id}", json=data)
        验证请求异常sync(response)
        return 返回_更新产品(**response.json())


async def 获取产品(product_id: int) -> 返回_获取产品:
    async with httpx.AsyncClient() as client:
...
async def 创建订单(user_id: str, app_id: str, total_amount: float, product_id: int) -> 返回_创建订单:
    async with httpx.AsyncClient() as client:
        data = {"user_id": user_id, "app_id": app_id, "total_amount": total_amount, "product_id": product_id}
        response = await client.post(f"{BASE_URL}/orders", json=data)
        验证请求异常sync(response)
        return 返回_创建订单(**response.json())


async def 更新订单状态(order_id: int, status: OrderStatus) -> 返回_更新订单状态:
    async with httpx.AsyncClient() as client:
        data = {"status": status}
        response = await client.put(f"{BASE_URL}/orders/{order_id}/status", json=data)
        验证请求异常sync(response)
        return 返回_更新订单状态(**response.json())


async def 获取订单(order_id: int) -> 返回_获取订单:
    async with httpx.AsyncClient() as client:
...
async def 创建支付(
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(f"{BASE_URL}/payments", json=data)
        验证请求异常sync(response)
        return 返回_创建支付(**response.json())


async def 获取支付(payment_id: int) -> 返回_获取支付:
    async with httpx.AsyncClient() as client:
