...
class ConversationSummaryMemory:
    ...
    def __init__(self, number_of_messages=40, system_prompt=None, user_prompt=None) -> None:
        ...
        if not self.system_prompt:
            self.system_prompt = "You are a world class AI assistant who excels at summarizing conversations. Your task is to provide concise yet comprehensive summaries that capture the key points, context, and flow of the conversation."
...
class ConversationSummaryBufferMemory:
    ...
    def __init__(self, buffer_size: int = 40, system_prompt: str = None, user_prompt: str = None):
        ...
        if not system_prompt:
            self.system_prompt = "You are an AI assistant tasked with maintaining a concise summary of a conversation. Update the existing summary with the new information, keeping the summary brief and relevant. If no existing summary is available, provide a concise summary of the conversation so far. Do not include any explanations — just the summary."
        else:
            self.system_prompt = system_prompt
