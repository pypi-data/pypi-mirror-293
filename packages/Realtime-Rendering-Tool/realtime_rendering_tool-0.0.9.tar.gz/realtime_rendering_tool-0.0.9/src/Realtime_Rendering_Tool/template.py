import Realtime_Rendering_Tool
import subprocess
import os
import importlib.util

def run_node_script(encoded_xml):
# Realtime_Rendering_Tool 패키지의 경로를 찾습니다.
    package_path = os.path.dirname(os.path.abspath(Realtime_Rendering_Tool.__file__))
    
    # 소스 디렉토리로 이동합니다 (setup.py가 있는 위치).
    source_dir = os.path.dirname(os.path.dirname(package_path))
    
    # bundle.mjs 파일의 경로를 구성합니다.
    address = os.path.join(package_path, 'bundle.mjs')
    
    command = ['node', address, encoded_xml]
...
if __name__ == "__main__":
    ...
    output = run_node_script(xmlString)
    if output:
