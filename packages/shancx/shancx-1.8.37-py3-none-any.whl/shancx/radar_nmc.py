...
import datetime
from shancx.wtx.colormap import cmp_hjnwtx
import os
import numpy as np
...
def drawimg_com(array_dt,temp = "temp"):
    now_str = datetime.datetime.now().strftime("%Y%m%d%H%M")
    now_str = datetime.datetime.now().strftime("%Y%m%d%H%M")
    y_coords2, x_coords2 = np.where(array_dt > 0.1)
    if len(array_dt.shape)==3:
        for i , img_ch_nel in enumerate(array_dt): 
            plt.imshow(img_ch_nel,vmin=0,vmax=10,cmap=cmp_hjnwtx["radar_nmc"])
            plt.plot(x_coords2, y_coords2, 'ro', markersize=2)  # Adjust markersize here
            plt.colorbar()
            outpath = f"./radar_nmc/{temp}_{now_str}.png"
            mkDir(outpath)
            plt.savefig(outpath)
            plt.close()
    if len(array_dt.shape)==2: 
            plt.imshow(array_dt, vmax=10, cmap=cmp_hjnwtx["pre_tqw"])
            plt.plot(x_coords2, y_coords2, 'ro', markersize=0.5)  # Adjust markersize here
            plt.colorbar()
            outpath = f"./radar_nmc/{temp}_{now_str}.png"
            mkDir(outpath)
            plt.savefig(outpath)
