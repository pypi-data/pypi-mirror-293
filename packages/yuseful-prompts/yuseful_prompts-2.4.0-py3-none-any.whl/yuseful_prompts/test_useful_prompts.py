...
@pytest.mark.parametrize(
    ...
    [
        (
            {"headline_text": ("Mum and chef die after carnival weekend attacks - Both deaths are now being treated as murder investigations, say police.",)},
            {
                "relevance": False,
            },
        ),
        (
            {"headline_text": ("Russia pushes on key Ukraine city while Kyiv's Kursk incursion slows - Russia is poised to assault Pokrovsk, a crucial supply and reinforcement hub for Ukraine's frontline troops.",)},
            {
                "relevance": True,
            },
        ),
        (
            {"headline_text": ("Oasis gigs selling out as fans scramble for tickets - Some fans face difficulties accessing the websites selling tickets for the band's comeback gigs in the UK and Ireland next year.",)},
            {
                "relevance": False,
            },
        ),
        (
            {"headline_text": ("US rapper Fatman Scoop dies after collapsing on stage - The New Yorker, who has collaborated with artists like Mariah Carey, collapsed during a gig in Connecticut.",)},
            {
                "relevance": False,
            },
        ),
        (
            {"headline_text": ("Thunderstorm warning for England and east Wales - There is a risk of flooding in some areas, with Met Office saying hail and lighting are also likely.",)},
            {
                "relevance": False,
            },
        ),
        (
            {"headline_text": ("China and Philippines trade blame as ships collide - The crash is the latest incident in an escalating row over key zones in the South China Sea.",)},
            {
                "relevance": True,
            },
        ),
...
def test_check_headline_for_stocks_market_relevance(headline_input, expected):
    res = check_headline_for_stocks_market_relevance(**headline_input)
    print(res)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "headline_text": (
                    "Swiss Life AM debuts equity and bond index range for institutional investors",
                )
            },
            {
                "category": "institutional-indices",
            },
        ),
        (
            {
...
def test_classify_headline_category(headline_input, expected):
    assert classify_headline_category(**headline_input) == expected["category"]
