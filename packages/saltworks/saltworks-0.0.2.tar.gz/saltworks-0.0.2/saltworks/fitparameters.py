"""Handles parameters for linear and non-linear fits

This module contains utility classes to handle parameters in linear and
non-linear least square fits implemented in linearmodels and nonlinearmodels.
It provide 2 main features:

- `structarray`: a derivative of numpy class ndarray to manage large vectors
organized into named subgroups.


- `FitParameters` : a class to manage large parameter vectors. It allows to
easily fix/release specific parameters or entire subgroups, and remap the
remaining free parameters into a contiguous vector.
...
class Structure:
    ...
    Slices are specified by a name and a length. If omitted, the length default
    to one and the name to __0__ for the first unamed slice, __1__ for the
    second and so on.
...
class FitParameters:
    """Manages a vector of fit parameters with the possibility to mark a subset
    of them as fixed to a given value.
    ...
    It is possible to modify the parameters in place. Using the indexing of
    slices by name simplifies somewhat the operations, as one does not need to
    care about the position of a slice within the entire parameter vector:
