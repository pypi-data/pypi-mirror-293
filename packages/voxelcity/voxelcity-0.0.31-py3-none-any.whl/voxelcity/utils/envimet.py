    ...
    for i in range(grids_I):
        for j in range(grids_J):
            if canopy_height_grid[j, i] > 0 and np.flipud(building_height_grid)[j, i]==0:
                plantid = f'H{canopy_height_grid[j, i]:02d}W01'
                tree_ij = f"""  <3Dplants>
...
def export_inx(building_height_grid_ori, canopy_height_grid_ori, land_cover_grid_ori, dem_grid_ori, meshsize, output_dir):
    # Prepare grids
    building_height_grid_inx, land_cover_veg_grid_inx, land_cover_mat_grid_inx, canopy_height_grid_inx, dem_grid_inx = prepare_grids(
        building_height_grid_ori.copy(), canopy_height_grid_ori.copy(), land_cover_grid_ori.copy(), dem_grid_ori.copy(), meshsize)
