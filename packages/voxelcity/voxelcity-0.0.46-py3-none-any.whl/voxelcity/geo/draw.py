...
def center_location_map_cityname(cityname, east_west_length, north_south_length, zoom=15):
    ...
    # List to hold coordinates of rectangle
    rectangle_vertices = []
    rectangle_vertices_out = []

    def handle_draw(target, action, geo_json):
        """Handle draw events on the map."""
        # Clear previous vertices and rectangles
        rectangle_vertices.clear()
        rectangle_vertices_out.clear()
    ...
    draw_control.on_draw(handle_draw)
        ...
        # Reorder vertices to match the proper format
    if rectangle_vertices:
        rectangle_vertices_out = [
            rectangle_vertices[3],  # Southwest
            rectangle_vertices[0],  # Northwest
            rectangle_vertices[1],  # Northeast
            rectangle_vertices[2],  # Southeast
        ]

    return m, rectangle_vertices_out