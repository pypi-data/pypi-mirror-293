...
from tecton_gen_ai.checkpoint import _to_uuid, make_deterministic, with_checkpoint


def test_to_uuid():
    assert _to_uuid(None) != ""
    assert _to_uuid(None) == _to_uuid(float("nan"))
    assert _to_uuid(None) != _to_uuid(0)
    assert _to_uuid([1, "a"]) != _to_uuid(1)
    assert _to_uuid([1, "a"]) != _to_uuid([1, "b"])
    assert _to_uuid([1, "a"]) == _to_uuid([1, "a"])
    assert _to_uuid((1, "a")) == _to_uuid([1, "a"])
    # dicts must keep the same order
    assert _to_uuid({"a": 1, "b": "a"}) != _to_uuid({"b": "a", "a": 1})
    assert _to_uuid({"a": 1, "b": "a"}) == _to_uuid({"a": 1, "b": "a"})
    assert _to_uuid(date(2021, 1, 1)) == _to_uuid(date(2021, 1, 1))
    assert _to_uuid(date(2021, 1, 1)) != _to_uuid(date(2021, 1, 2))
    assert _to_uuid(datetime(2021, 1, 1, 1, 1, 1)) == _to_uuid(
    ...
    assert _to_uuid(df) == _to_uuid(df)
