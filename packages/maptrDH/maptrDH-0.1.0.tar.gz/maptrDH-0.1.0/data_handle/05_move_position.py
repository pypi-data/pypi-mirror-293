...
Out_path=r'/root/autodl-fs/MapTR1/data/custom/train_0.3/after_translated'
Lidar_folder_path=r'/root/autodl-fs/MapTR1/data/custom/train_0.3/afterRemoved'
Shp_Out_path=r'/root/autodl-fs/MapTR1/data/custom/train_0.3/afterCutted_translated'
Shp_folder_path=r'/root/autodl-fs/MapTR1/data/custom/train_0.3/afterCutted'     
...
for Lidarfilename in Lidar_files:
    if Lidarfilename.endswith('.bin'):
        ...
        # point_cloud = np.vstack((x, y, z, intensity)).transpose()
        # 计算点云的边界框                              
        ...
        # # 保存平移后的点云到新的 .bin 文件                              
        # Lidarfilename = Lidarfilename.replace(".bin", ".bin")
        # output_file_path = os.path.join(Out_path, Lidarfilename)
        # point_cloud.astype(np.float32).tofile(output_file_path)
        # points = np.fromfile(output_file_path, dtype=np.float32).reshape(-1, 4)
        i+=1
        print("点云",i)
            ...
            if number1 == number2:
                ...
                for feature in data["features"]:
                    ...
                    if geometry["type"] == "Point":
                        ...
                        geometry["coordinates"] = [[x + translation_vector[0], y + translation_vector[1]] for x, y,z in geometry["coordinates"]]
