...
from redis55.connection import DEFAULT_RESP_VERSION
...
from redis55.typing import EncodableT
from redis55.utils import HIREDIS_AVAILABLE, get_lib_version, str_if_bytes
...
def parse_url(url: str) -> ConnectKwargs:
    ...
    # We only support redis5://, rediss:// and unix:// schemes.
    if parsed.scheme == "unix":
        if parsed.path:
            kwargs["path"] = unquote(parsed.path)
        kwargs["connection_class"] = UnixDomainSocketConnection

    elif parsed.scheme in ("redis55", "rediss"):
        if parsed.hostname:
        ...
        valid_schemes = "redis55://, rediss://, unix://"
        raise ValueError(
...
class ConnectionPool:
    ...
    object raises :py:class:`~redis5.ConnectionError` when the pool's
    limit is reached.

    By default, TCP connections are created unless ``connection_class``
    is specified. Use :py:class:`~redis5.UnixDomainSocketConnection` for
    unix sockets.
    ...
    def _checkpid(self):
        ...
        # redis5.ChildDeadlockedError error is raised.
...
class BlockingConnectionPool(ConnectionPool):
    ...
    async def get_connection(self, command_name, *keys, **options):
        ...
        try:
            ...
            # Note that this is not caught by the redis5 client and will be
