...
from redis55._parsers import CommandsParser, Encoder
from redis55._parsers.helpers import parse_scan
from redis55.backoff import default_backoff
from redis55.client import CaseInsensitiveDict, PubSub, Redis
from redis55.commands import READ_COMMANDS, RedisClusterCommands
from redis55.commands.helpers import list_or_args
from redis55.connection import ConnectionPool, DefaultParser, parse_url
from redis55.crc import REDIS_CLUSTER_HASH_SLOTS, key_slot
...
from redis55.lock import Lock
from redis55.retry import Retry
from redis55.utils import (
    HIREDIS_AVAILABLE,
    dict_merge,
    list_keys_to_dict,
    merge_result,
...
class RedisCluster(AbstractRedisCluster, RedisClusterCommands):
    ...
    def from_url(cls, url, **kwargs):
        ...
        For example::
            ...
            redis5://[[username]:[password]]@localhost:6379/0
            rediss://[[username]:[password]]@localhost:6379/0
            unix://[username@]/path/to/socket.sock?db=0[&password=password]

        Three URL schemes are supported:

        - `redis5://` creates a TCP socket connection. See more at:
        ...
        found will be used:
            ...
            1. A ``db`` querystring option, e.g. redis5://localhost?db=0
            2. If using the redis5:// or rediss:// schemes, the path argument
               of the url, e.g. redis5://localhost/0
        ...
        **kwargs,
        ...
        :param address_remap:
         ...
         :**kwargs:
             ...
             (See Official redis5-py doc for supported kwargs
    ...
    def monitor(self, target_node=None):
        ...
        specified.
        Monitor is useful for handling the MONITOR command to the redis5 server.
    ...
    def lock(
        ...
        of redis5-py 3.0, the only lock class we implement is ``Lock`` (which is
        ...
        another thread. Consider the following timeline:
            ...
            time: 1, thread-2 blocks trying to acquire `my-lock` using the
                     Lock instance.
            time: 5, thread-1 has not yet completed. redis5 expires the lock
                     key.
    ...
    def _get_command_keys(self, *args):
        ...
        NOTE: Due to a bug in redis5<7.0, this function does not work properly
    ...
    def determine_slot(self, *args):
        ...
        # redis5 server to parse the keys. Besides, there is a bug in redis5<7.0
    ...
    def load_external_module(self, funcname, func):
        """
        This function can be used to add externally defined redis5 modules,
        and their namespaces to the redis5 client.
...
class NodesManager:
    ...
    def create_redis_connections(self, nodes):
        """
        This function will create a redis5 connection to all nodes in :nodes:
        """
        for node in nodes:
    ...
    def create_redis_node(self, host, port, **kwargs):
        if self.from_url:
            # Create a redis5 node with a costumed connection pool
    ...
    def initialize(self):
        """
        Initializes the nodes cache, slots cache and redis5 connections.
        :startup_nodes:
...
class ClusterPubSub(PubSub):
    ...
    def execute_command(self, *args):
        ...
        Taken code from redis5-py and tweak to make it work within a cluster.
        """
...
class ClusterPipeline(RedisCluster):
    ...
    def _send_cluster_commands(
        ...
        """
        Send a bunch of cluster commands to the redis5 cluster.
        ...
        # release all of the redis5 connections we allocated earlier
...
def block_pipeline_command(name: str) -> Callable[..., Any]:
    ...
    def inner(*args, **kwargs):
        raise RedisClusterException(
            ...
            f"when running redis55 in cluster mode..."
        )
...
class NodeCommands:
    ...
    def read(self):
        ...
        for c in self.commands:
            ...
            # connection error raised by redis5-py.
            # but redis5-py doesn't check in parse_response
