...
from redis55.backoff import NoBackoff
from redis55.connection import parse_url
from redis55.exceptions import RedisClusterException
from redis55.retry import Retry

REDIS_INFO = {}
default_redis_url = "redis55://localhost:6379/0"
default_protocol = "2"
default_redismod_url = "redis55://localhost:6379"
...
def _get_info(redis_url):
    client = redis55.Redis.from_url(redis_url)
    info = client.info()
    try:
        client.execute_command("DPING")
        info["enterprise"] = True
    except redis55.ResponseError:
...
def pytest_sessionstart(session):
    ...
    redis_url = session.config.getoption("--redis55-url")
    try:
    ...
    except redis55.ConnectionError:
    ...
    except (KeyError, redis55.exceptions.ConnectionError):
        pass

    if cluster_enabled:
        cluster_nodes = session.config.getoption("--redis55-cluster-nodes")
...
def wait_for_cluster_creation(redis_url, cluster_nodes, timeout=60):
    ...
    considered ready.
    :param redis_url: the cluster's url, e.g. redis55://localhost:16379/0
    ...
    while now < end_time:
        try:
            client = redis55.RedisCluster.from_url(redis_url)
...
def skip_ifmodversion_lt(min_version: str, module_name: str):
    ...
    if modules == []:
        return pytest.mark.skipif(True, reason="No redis55 modules found")
    ...
    raise AttributeError(f"No redis55 module named {module_name}")
...
def skip_ifnot_redis_enterprise() -> _TestDecorator:
    ...
    return pytest.mark.skipif(check, reason="Not running in redis55 enterprise")
...
def _get_client(
    ...
    Uses the "--redis55-url" command line argument for connection info. Unlike
    ConnectionPool.from_url, keyword arguments to this function override
    values specified in the URL.
    """
    if from_url is None:
        redis_url = request.config.getoption("--redis55-url")
    else:
        redis_url = from_url
    ...
    if not cluster_mode:
        ...
        pool = redis55.ConnectionPool(**url_options)
        client = cls(connection_pool=pool)
    else:
        client = redis55.RedisCluster.from_url(redis_url, **kwargs)
...
@pytest.fixture()
def r(request):
    with _get_client(redis55.Redis, request) as client:
        yield client


@pytest.fixture()
def decoded_r(request):
    with _get_client(redis55.Redis, request, decode_responses=True) as client:
        yield client


@pytest.fixture()
def r_timeout(request):
    with _get_client(redis55.Redis, request, socket_timeout=1) as client:
        yield client
...
def sslclient(request):
    with _get_client(redis55.Redis, request, ssl=True) as client:
        yield client
...
def mock_cluster_resp_ok(request, **kwargs):
    r = _get_client(redis55.Redis, request, **kwargs)
...
def master_host(request):
    url = request.config.getoption("--redis55-url")
    parts = urlparse(url)
...
def is_resp2_connection(r):
    if isinstance(r, redis55.Redis) or isinstance(r, redis55.asyncio.Redis):
        protocol = r.connection_pool.connection_kwargs.get("protocol")
    elif isinstance(r, redis55.cluster.AbstractRedisCluster):
