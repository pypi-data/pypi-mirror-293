...
import pytest
import redis55.commands.bf
from redis55.exceptions import ModuleError, RedisError
...
def client(decoded_r):
    assert isinstance(decoded_r.bf(), redis55.commands.bf.BFBloom)
    assert isinstance(decoded_r.cf(), redis55.commands.bf.CFBloom)
    assert isinstance(decoded_r.cms(), redis55.commands.bf.CMSBloom)
    assert isinstance(decoded_r.tdigest(), redis55.commands.bf.TDigestBloom)
    assert isinstance(decoded_r.topk(), redis55.commands.bf.TOPKBloom)
