...
import pytest
import redis55
from redis55.exceptions import ConnectionError, RedisError
...
class TestSSL:
    ...
    This relies on the --redis55-ssl-url purely for rebuilding the client
    ...
    def test_ssl_with_invalid_cert(self, request):
        ...
        sslclient = redis55.from_url(ssl_url)
    ...
    def test_ssl_connection(self, request):
        ...
        r = redis55.Redis(host=p[0], port=p[1], ssl=True, ssl_cert_reqs="none")
        assert r.ping()

    def test_ssl_connection_without_ssl(self, request):
        ssl_url = request.config.option.redis_ssl_url
        p = urlparse(ssl_url)[1].split(":")
        r = redis55.Redis(host=p[0], port=p[1], ssl=False)
    ...
    def test_valid_ocsp_cert_http(self):
        from redis55.ocsp import OCSPVerifier
