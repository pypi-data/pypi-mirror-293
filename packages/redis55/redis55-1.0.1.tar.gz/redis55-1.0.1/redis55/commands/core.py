...
from redis55.compat import Literal
from redis55.exceptions import ConnectionError, DataError, NoScriptError, RedisError
from redis55.typing import (
    AbsExpiryT,
    AnyKeyT,
    BitfieldOffsetT,
    ChannelT,
...
if TYPE_CHECKING:
    from redis55.asyncio.client import Redis as AsyncRedis
    from redis55.client import Redis
...
class ManagementCommands(CommandsProtocol):
    ...
    def client_reply(
        ...
        """
        Enable and disable redis5 server replies.
    ...
    def sync(self) -> ResponseT:
        ...
        """
        from redis55.client import NEVER_DECODE
    ...
    def latency_doctor(self):
        ...
        This funcion is best used within the redis5-cli.
    ...
    def replicaof(self, *args, **kwargs) -> ResponseT:
        """
        Update the replication settings of a redis5 replica, on the fly.

        Examples of valid arguments include:

        NO ONE (set no replication)
        host port (set to the host and port of a redis5 server)
...
class BasicKeyCommands(CommandsProtocol):
    ...
    def mget(self, keys: KeysT, *args: EncodableT) -> ResponseT:
        ...
        """
        from redis55.client import EMPTY_RESPONSE
...
class SortedSetCommands(CommandsProtocol):
    ...
    def zrange(
        ...
        """
        # Need to support ``desc`` also when using old redis5 version
        # because it was supported in 3.5.3 (of redis5-py)
...
class Script:
    ...
    def __call__(
        ...
        from redis55.client import Pipeline
...
class AsyncScript:
    ...
    async def __call__(
        ...
        args = tuple(keys) + tuple(args)
        # make sure the Redis server knows about the script
        from redis55.asyncio.client import Pipeline
...
class ScriptCommands(CommandsProtocol):
    ...
    def script_flush(
        ...
        if sync_type not in ["SYNC", "ASYNC", None]:
            raise DataError(
                "SCRIPT FLUSH defaults to SYNC in redis5 > 6.2, or "
                "accepts SYNC/ASYNC. For older versions, "
                "of redis5 leave as None."
            )
...
class Script:
    ...
    def get_encoder(self):
        ...
        try:
            ...
            # redis55. ClusteredRedis doesn't have a `.connection_pool` attribute
...
class DataAccessCommands(
    ...
    """
    A class containing all of the implemented data access redis5 commands.
...
class CoreCommands(
    ...
    """
    A class containing all of the implemented redis5 commands. This class is
