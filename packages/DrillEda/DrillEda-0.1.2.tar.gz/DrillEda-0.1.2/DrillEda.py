...
class DrillEda:
    ...
    def process_data(self, geology, assay, geology_columns, assay_columns, combine_lithologies=False, lithology_groups=None):
        ...
        combined_depths = (pd.concat([
            self.combined_geology[[geology_columns['holeid'], geology_columns['from']]].rename(columns={geology_columns['from']: 'FROM'}),
            self.combined_geology[[geology_columns['holeid'], geology_columns['to']]].rename(columns={geology_columns['to']: 'FROM'}),
            assay[[assay_columns['holeid'], assay_columns['from']]].rename(columns={assay_columns['from']: 'FROM'}),
            assay[[assay_columns['holeid'], assay_columns['to']]].rename(columns={assay_columns['to']: 'FROM'})
        ])
        .drop_duplicates()
        .sort_values(['ID', 'FROM'])
        )
    ...
    def scatter(self, x_axis, y_axis, dot_size=5, dot_color='#FF6347', font_size=12, num_x_ticks=10, num_y_ticks=10, x_label_rotation=0, plot_title=None):
        if self.filtered_data is not None:
            fig, ax = plt.subplots()
            ax.scatter(self.filtered_data[x_axis], self.filtered_data[y_axis], s=dot_size, c=dot_color, alpha=0.7)
    ...
    def boxplot(self, numeric_column, categorical_column, categories=None, box_fill=True, circle_size=5, font_size=12, plot_title=None, log_scale_boxplot=False):
        if self.filtered_data is not None:
    ...
    def histogram(self, numeric_column, log_scale=False, bin_size=20, cap_value=None, bar_color="#3498db"):
        if self.filtered_data is not None:
            data = self.filtered_data[numeric_column]
