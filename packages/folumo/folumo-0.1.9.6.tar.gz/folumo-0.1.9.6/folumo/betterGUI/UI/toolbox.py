...
class Slider(Element):
    def __init__(self, xy, wh, min_val, max_val, current_val, color: Color, handle_color: Color, on_change=None):
        super().__init__(xy)
        self.wh = wh
    ...
    def handle_event(self, event):
        if event.type == MOUSEBUTTONDOWN and self.is_hovering(event.pos):
            ...
            valRaw = self.get_value_from_pos(event.pos)

            val = valRaw

            if valRaw > self.max_val:
                val = self.max_val
    ...
    def render(self) -> Canvas:
        ...
        pygame.draw.circle(newCanvas.surf, self.handle_color.getColor(), (handle_x, self.wh[1] // 2), 10)
...
class InputElement(Element):
    def __init__(self, xy, size: tuple[int, int], input_type='normal', centerText=False, fontName=None, fontSize=24):
        super().__init__(xy)
        self.size = size
    ...
    def render(self) -> Canvas:
        ...
        if self.centerText:
            x = (self.size[0] - text_surface.get_width()) // 2
        else:
            x = 5

        canvas.surf.blit(text_surface, (x, (self.size[1] - text_surface.get_height()) // 2))

        # Handle cursor blinking
        self.cursor_counter = (self.cursor_counter + 1) % 60
        if self.active and self.cursor_counter < 30:
            cursor_x = text_surface.get_width() + x
