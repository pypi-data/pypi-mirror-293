...
class SmartData:
    ...
    def create_model(self):
        ...
        The column names in the dataframe may differ from those in the question. Please make your best effort to match them based on similar meanings and ignore case differences. Also you may need to revise and/or complete the question with the chat history if needed.
        ...
        You may need to revise the current question with the chat history before passing to tools. You should use the tools below to answer the question posed of you:
        """
    ...
    def run_model(self, question):
        ...
        if self.memory.is_not_empty():
            question_with_history = (f"My question is: {question}. Below is the chat history in chronological order, from the earliest to the latest.: {self.memory.recall_last_conversation(self.memory_size)}.")
    ...
    def extract_code_from_response(self, response):
        ...
        try:
            ...
            if (1 in last_response and len(last_response[1]) == 0) or (1 not in last_response):
