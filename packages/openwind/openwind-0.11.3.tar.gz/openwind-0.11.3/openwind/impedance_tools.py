...
def read_impedance(filename, df_filt=None, column_sep=None, decimal_sep='.'):
    """
    ...
    * third column: the imaginary part of the impedance
    ...
    decimal_sep : str, optional
        Decimal separator (typically '.' or ','). Default is '.'
    ...
    def parse_line(line):
        ...
        line = line.split('#')[0]
        # replace decimal seprator by '.'
        line = line.replace(decimal_sep, '.')
        # Split the lines according to the column sperator
        return line.split(column_sep)

    if column_sep==decimal_sep:
        raise ValueError(f"The decimal and column separator must be different: here both are '{column_sep}'.")
    ...
    for line in lines:
        ...
        if len(contents) == 3:
            ...
            raise ValueError(f"The file must contain 3 columns, here {len(contents)} are recognize. Please check the 'column_sep' option. {contents}")
...
def plot_impedance(frequencies, impedance, Zc0=1, dbscale=True, figure=None,
    ...
    **kwargs : key word arguments
    ...
    if len(ax)>1 and not modulus_only:
        ax[1].plot(frequencies, np.angle(y)/np.pi, **kwargs)
        ax[1].grid(True)
        ax[1].set_xlabel("Frequency (Hz)")
        ax[1].set_ylabel(ylabel_angle)
        ax[1].yaxis.set_major_locator(ticker.MaxNLocator(5))
        ax[1].yaxis.set_major_formatter(lambda x, pos: "0" if x==0 else f"{x:.8g}$\pi$")
