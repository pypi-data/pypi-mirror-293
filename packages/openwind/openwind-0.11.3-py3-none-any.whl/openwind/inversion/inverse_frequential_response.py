...
# %% Optimization algorithms
    ...
    def optimize_freq_model(self, algorithm='default', max_iter=100,
        ...
        cons_test = self.optim_params.get_active_constraints_for_algo('SLSQP')
        if algorithm == 'default' and all(np.isinf(lb+ub)) and len(cons_test)==0:
            algorithm = 'lm'
            print('Chosen algorithm: "lm" (no bounds, no constrains)')
        elif algorithm == 'default' and len(cons_test)==0:
            algorithm = 'trf'
            print('Chosen algorithm: "trf" (with bounds, no constrains)')
        elif algorithm == 'default':
            algorithm='SLSQP'
            print('Chosen algorithm: "SLSQP" (with constrains)')
    ...
    def _least_squares_scipy(self, initial_params, bounds,
        ...
        cons = self.optim_params.get_active_constraints_for_algo(algorithm)
        (self._residuals,
