...
def get_comments_from_lines(lines):
    ...
    comment_lines = [s[1:].strip() for s in lines if s.startswith('#')
                     and not (all(head in s[1:].split() for head in bore_header)
                              or all(head in s[1:].split() for head in bore_header_xr)
                              or all(head in s[1:].split() for head in bore_header_xd))] # exclude the header line of the main bore
    return '\n'.join(comment_lines)
