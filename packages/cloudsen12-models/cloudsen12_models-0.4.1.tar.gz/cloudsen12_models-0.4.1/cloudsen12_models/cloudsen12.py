...
def load_model_by_name(name:str, weights_folder:str="cloudsen12_models", 
    ...
    if MODELS_CLOUDSEN12[name]["type"] == "weights":
        ...
        if "state_dict" in weights:
            model.load_state_dict(weights["state_dict"], map_location=device)
        else:
...
def plot_cloudSEN12mask(mask:Union[ArrayLike, GeoData], legend: bool = True,
    ...
    return plot_segmentation_mask(mask=mask, color_array=COLORS_CLOUDSEN12,
                                  interpretation_array=INTERPRETATION_CLOUDSEN12, legend=legend, 
                                  ax=ax)