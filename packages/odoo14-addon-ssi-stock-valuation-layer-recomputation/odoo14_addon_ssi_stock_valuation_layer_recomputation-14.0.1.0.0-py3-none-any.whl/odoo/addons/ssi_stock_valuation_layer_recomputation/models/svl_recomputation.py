...
class SVLRecomputation(models.Model):
    _name = "svl_recomputation"
    _description = "SVL Recomputation"
    _inherit = [
    ...
    type_id = fields.Many2one(
        comodel_name="svl_recomputation_type",
        string="Type",
        required=True,
        ondelete="restrict",
        readonly=True,
    ...
    product_ids = fields.Many2many(
        ...
        relation="rel_svl_recomputation_2_product",
        column1="svl_recomputation_id",
        column2="product_id",
        readonly=True,
        states={"draft": [("readonly", False)]},
    )
    ...
    svl_ids = fields.Many2many(
        ...
        relation="rel_svl_recomputation_2_svl",
        column1="svl_recomputation_id",
        column2="svl_id",
        readonly=True,
    )
    ...
    def _01_generate_first_level_queue_done(self):
        ...
        for split_number in range(1, num_split + 1):
            ...
            description = (
                "Generate first level done queue for SVL Recomputation ID %s split number %s"
    ...
    def _generate_second_level_queue_done(self, products, latest):
        ...
        for product in products:
            if latest and product == products[-1]:
                latest_product = True
            description = (
                "Generate second level done queue for SVL Recomputation ID %s Product %s"
    ...
    def _generate_third_level_queue_done(self, product, latest):
        ...
        criteria = [
            ...
            ("quantity", ">", 0.0),
            ("usage_quantity_diff", ">", 0.0),
        ]
        svls = SVL.search(criteria, order="create_date asc, id asc")
        if len(svls) > 0:
            self.write({"svl_ids": [(6, 0, svls.ids)]})

        for svl in svls:
            description = (
                "Generate third level done queue for SVL Recomputation ID %s Product %s SVL %s "
                % (self.id, product.name, svl.id)
            )
            current_sb_delayable = (
                self.with_context(job_batch=self.done_queue_job_batch_id)
                .delayable(description=_(description))
                ._generate_final_level_queue_done(svl)
            )
            sb_delayables.append(current_sb_delayable)

        if len(sb_delayables) > 0:
            chain(*sb_delayables).delay()
        if latest:
            self.done_queue_job_batch_id.enqueue()

    def _generate_final_level_queue_done(self, svl):
        self.ensure_one()
        SVL = self.env["stock.valuation.layer"]
        criteria = [
        ...
        for outgoing_svl in SVL.search(criteria, order="create_date asc, id asc"):
            svl._add_incoming_usage(outgoing_svl)
            if svl.usage_quantity_diff == 0.0:
                break

    @api.model
    def _get_policy_field(self):
        res = super(SVLRecomputation, self)._get_policy_field()
        policy_field = [
            "confirm_ok",
            "approve_ok",
            "cancel_ok",
