...
def get_executable_selectors(proj_path: str, test_selectors: list[str]) -> list[str]:
    ...
    for selector in test_selectors:
        ...
        abs_path = os.path.join(proj_path, url.path if (url.path != "." and url.path != "/") else "")
        if not os.path.exists(abs_path):
            logger.error(f"can't find {abs_path}")
            continue
        if os.path.isdir(abs_path):
            if url.query:
                logger.error(f"selector format: [{selector}] is illegal, please explicitly declare the file where the use case is located ")
                continue
            for root, dirs, files in os.walk(abs_path):
                for f in files:
                    if f.endswith(".py") and f != "__ini__.py":
                        parsed_selectors.append(os.path.relpath(os.path.join(root, f), proj_path))
