...
def test_cli_web_content(mocker):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["https://www.example.com", "--web"])

    print(f"DEBUG: Test output: {result.output}")
    print(f"DEBUG: mock_html_to_markdown.call_count: {mock_html_to_markdown.call_count}")
    print(f"DEBUG: mock_html_to_markdown.call_args_list: {mock_html_to_markdown.call_args_list}")
    print(f"DEBUG: mock_get.call_count: {mock_get.call_count}")
    print(f"DEBUG: mock_get.call_args_list: {mock_get.call_args_list}")
    print(f"DEBUG: mock_display_markdown.call_count: {mock_display_markdown.call_count}")
    print(f"DEBUG: mock_display_markdown.call_args_list: {mock_display_markdown.call_args_list}")
...
def test_cli_web_input(runner, mocker):
    mock_get = mocker.patch('requests.get')
    mock_html_to_markdown = mocker.patch('mrender.main.html_to_markdown_with_depth')
    mock_display_markdown = mocker.patch('mrender.main.display_markdown_hierarchically')
...
def test_cli_invalid_input(runner):
    result = runner.invoke(cli, ["nonexistent_file.py"])
    assert result.exit_code == 1
    assert "Invalid input: nonexistent_file.py" in result.output
...
def test_cli_with_input(runner, tmp_path):
...
def test_cli_web_option(runner, mocker):
    mock_get = mocker.patch('requests.get')
    mock_html_to_markdown = mocker.patch('mrender.main.html_to_markdown_with_depth')
    mock_display_markdown = mocker.patch('mrender.main.display_markdown_hierarchically')
    ...
    result = runner.invoke(cli, ["https://example.com", "--web"])
    assert result.exit_code == 0
    mock_get.assert_called_once_with("https://example.com")
    mock_html_to_markdown.assert_called_once_with("<html><body>Sample content</body></html>", 3)
    mock_display_markdown.assert_called_once_with("# Sample Markdown", 3)
