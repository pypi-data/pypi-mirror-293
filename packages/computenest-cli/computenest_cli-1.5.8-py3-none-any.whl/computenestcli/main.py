...
@click.option('--access_key_id', required=False, help='Access Key ID')
@click.option('--access_key_secret', required=False, help='Access Key Secret')
def export_command(region_id, service_name, file_path, access_key_id, access_key_secret):
    credentials = get_credentials(access_key_id, access_key_secret, None)
    context = Context(region_id, credentials)
    service = ServiceProcessor(context)
