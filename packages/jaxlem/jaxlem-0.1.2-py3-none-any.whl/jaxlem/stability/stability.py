...
from jaxlem.stability.stability_utils import StabilityConfig, FORMResults
...
class SlopeStability(ABC):
    ...
    def safety_factor_vmap(self, rf: Float[Array, "n_rf_mesh"], failure_params: Float[Array, "n_circles 3"])\
        ...
        return fos.min(), jnp.take(failure_params, fos.argmin(), axis=0), fos

    @partial(jax.jit, static_argnums=(0, 2,))
    def safety_factor_gd(self, rf: Float[Array, "n_rf_mesh"], gd_hyperparams: GDHyperparams)\
            -> Tuple[Float[Array, "1"], Float[Array, "3"]]:

        _loss = lambda x, y: self.safety_factor_point(y, x)

        _initilizate = lambda rng: jnp.take(
        ...
        """ Solution using own Gradient Descent code """
        # scheduler = optax.exponential_decay(
        #     gd_hyperparams.lr,
        #     gd_hyperparams.transition_steps,
        #     gd_hyperparams.decay_rate
        # )
        #
        ...
        # gd = GradientDescent(objective_fn=_loss, initilization_fn=_initilizate, optimizer=optimizer,
        #                      obj_threshold=gd_hyperparams.obj_threshold, grad_threshold=gd_hyperparams.grad_threshold,
        #                      max_epochs=gd_hyperparams.max_epochs)
        # res, metrics = gd.fit(rng, n_inits=gd_hyperparams.n_inits)
        # fos = res.objective_value
        # failure_circle = res.theta
        #
        # return fos, failure_circle,

        """ Solution using jaxopt's Gradient Descent code """
        rng = jax.random.PRNGKey(42)
    ...
    def reliability_form(self, rf_mean: Float[Array, "n_rf_mesh"], rf_std: Float[Array, "n_rf_mesh"],
                         ...
                         beta_tol: float = 1e-2) -> FORMResults:

        def _rf_to_u(rf):
            return (rf - rf_mean) / rf_std
        ...
        def _loop(iloop, runner):
            ...
            converged = _convergence(beta, beta_old)
        ...
        rf_chains = jax.vmap(_u_to_rf)(u_chains)
        ...
        results = FORMResults(rf_chains, fos_chains, beta_chains, failure_circle_chains, converged_chains)
