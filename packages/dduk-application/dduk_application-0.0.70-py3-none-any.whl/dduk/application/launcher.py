...
class Launcher:
	...
	def Launch(self) -> int:
		...
		applicationDataPath : str = Path.GetApplicationDataPathWithRelativePaths("dduk-python", "dduk-application")
		...
		# 소스 실행 일 때.
			...
			applicationDataRootPath = os.path.join(applicationDataPath, rootName).replace(BACKSLASH, SLASH)
			metaPath : str = os.path.join(applicationDataRootPath, "meta").replace(BACKSLASH, SLASH)
		...
		applicationDataRootPath = os.path.join(applicationDataPath, rootName)

		# 빌드.
		if manifestData.Type == ExecuteType.BINARY:
			builtins.print("__BINARY__")
			logsPath = os.path.join(applicationDataRootPath, "logs").replace(BACKSLASH, SLASH)
		# 서비스.
		elif manifestData.Type == ExecuteType.SERVICE:
			builtins.print("__SERVICE__")
			logsPath = os.path.join(applicationDataRootPath, "logs").replace(BACKSLASH, SLASH)
			logsPath = os.path.join(cachePath, "logs")
		# 소스.
		...
		if sys.argv:
			...
			sys.argv = sys.argv[1:] # 첫번째 인자 제외. (실행파일 경로)
		...
		if manifestData.Type == ExecuteType.SOURCE:
			sys.argv.clear()
			# sys.argv.append(executeFileName) # 첫번째 인자 제외. (실행파일 경로)
