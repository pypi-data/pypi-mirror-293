...
import json
import os
from dduk.core.platform import PlatformType, GetPlatformType
...
class Preparer:
	...
	def Prepare(self, executeType : ExecuteType) -> None:
		...
		rootPath = Path.FindRootPath(startFilePath)
		...
		cachePath : str = os.path.join(applicationDataPath, "dduk-python", "dduk-application", "projects", rootName).replace(BACKSLASH, SLASH)
