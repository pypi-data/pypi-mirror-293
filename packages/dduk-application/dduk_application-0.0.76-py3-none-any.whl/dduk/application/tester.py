...
class Tester:
	...
	def Test(self, pattern = "test_*.py") -> int:
		...
		testerFilePath = os.path.abspath(sys.modules["__main__"].__file__).replace(BACKSLASH, SLASH)
		testsPath : str = os.path.dirname(testerFilePath).replace(BACKSLASH, SLASH)
		...
		try:
			...
			# 테스트 로거 등록.
			rootNameLower : str = rootName.lower()
			timestamp = GetTimestampString(EMPTY, EMPTY, EMPTY)
			testLoggerName : str = f"tests-{rootName}"
			logFilePath = f"{logsPath}/{testLoggerName}-{timestamp}.log"
