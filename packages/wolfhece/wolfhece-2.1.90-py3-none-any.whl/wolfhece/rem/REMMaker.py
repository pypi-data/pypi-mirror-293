...
class REMMaker(object):
    ...
    :type out_dir: str
    :param bounds: (optional) [x_min,y_min,x_max,y_max] represents the extent wanted that has to be computed et plotted.
    :type bounds : list
    :param count : (optional) geometry number in centerline_shp given that has to be computed and plotted.
    :type count : int
    :param pres : (optional) new resolution of the pixels DEM (length and width are the same).
    :type pres : float
    ...
    """
    def __init__(self, dem, centerline_shp=None, out_dir='./', bounds=None, count=None, pres=None,
                 kd = True, interp_pts=1000, k=None, eps=0.1, workers=4, cache_dir='./.cache'):
        self.dem = dem
        ...
        self.count = count if count is not None else None
        self.pres = pres if pres is not None else None
        self.kd = kd
    ...
    def get_spatial_metadata(self):
        ...
        if self.pres is not None and (self.pres < x_size or self.pres%x_size != 0):
            raise ValueError(f"La résolution doit être supérieure à celle de base et multiple de celle-ci. Résolution initiale : {x_size}m")
        if self.bounds is not None and self.count is not None:
            raise ValueError(f'Soit count ou bounds doit être fourni.')
    ...
    def lines2pts(self):
        ...
        if self.count is None:
            for way, river_segment in self.rivers.iterrows():
                ...
                self.river_curvi.extend(list(distances))
            ...
            line_string = self.rivers.geometry
            point_fraction = line_string.length / self.river_length
            point_num = int(point_fraction * self.interp_pts)
            distances = np.linspace(0, line_string.length, point_num)
            self.river_curvi.extend(list(distances))
    ...
    def make_river_shp(self):
        ...
        layer = ds.CreateLayer('', self.proj, ogr.wkbPoint25D)
        # Add fields
    ...
    def read_centerline_input(self):
        ...
        self.rivers = read_file(self.centerline_shp)
        #.to_crs(epsg=self.epsg_code)
        if self.rivers.geom_type[0] == 'Polygon':
            self.rivers = self.rivers.boundary
        if self.count is None:
            self.rivers = clip(self.rivers, box(*self.extent))
            self.river_length = self.rivers.length.sum()
        else:
            self.rivers = self.rivers.iloc[self.count]
            self.river_length = self.rivers.geometry.length
        self.lines2pts()
        self.make_river_shp()
    ...
    def interp_river_elev(self):
        ...
        for i, chunk in enumerate(np.array_split(c_interpolate, chunk_count)):
            ...
            if self.kd is True:
                ...
                # interpolate (IDW with power = 3)
                weights = 1 / distances**3  # weight river elevations by 1 / distance**3
                ...
                distances, indices = tree.query(chunk, k=[1], eps=self.eps, workers=self.workers)
                if k%2 == 0:
                    test_curvi = self.rivers.curvi
