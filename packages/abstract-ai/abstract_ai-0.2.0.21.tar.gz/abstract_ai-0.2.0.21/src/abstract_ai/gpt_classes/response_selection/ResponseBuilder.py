...
from abstract_utilities import (
    get_date, mkdirs, get_any_value, safe_json_loads
)
...
class ResponseManager:
    ...
    async def send_query(self)->None:
        ...
        if self.prompt.get('blank_prompt')==None:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url=self.endpoint, json=self.prompt, headers=self.header) as response:
                    if response.status == 200:
                        self.api_response = await response.json()
                        print(f'Request successful with status code {response.status}')
                    else:
    ...
    async def initial_query(self) -> list[dict]:
        """
        Manages the initial sequence of sending queries and processing responses asynchronously.
