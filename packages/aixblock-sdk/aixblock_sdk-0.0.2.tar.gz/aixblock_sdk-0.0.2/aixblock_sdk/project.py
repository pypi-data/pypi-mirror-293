...
class AIxBlockException(Exception):
    pass


class AIxBlockAttributeError(AIxBlockException):
    pass
...
class Project(Client):
    ...
    def get_members(self):
        ...
        Returns
        -------
        list of `AIXBLOCK_sdk.users.User`
    ...
    def _get_param(self, param_name):
        if param_name not in self.params:
            self.update_params()
            if param_name not in self.params:
                raise AIxBlockAttributeError(f'Project "{param_name}" field is not set')
    ...
    def get_params(self):
        ...
        show_skip_button: bool
            Whether to show a skip button in the AIxBlockUI and let annotators skip the task
        ...
        organization: int
            Organization ID
        color: str
            Color to decorate the project card in the AIxBlockUI
    ...
    def update_params(self):
        """ Get [all available project parameters](#AIXBLOCK_sdk.project.Project.get_params) and cache them.
        """
    ...
    def start_project(self, **kwargs):
        ...
        Raises AIxBlockException in case of errors.
        ...
        if response.status_code == 201:
            ...
            raise AIxBlockException('Project not created')
    ...
    def import_tasks(self, tasks, preannotated_from_fields: List = None):
        ...
        tasks: list of dicts | dict | path to file
            Tasks in <a href="https://wow-ai.com/guide/tasks.html#Basic-aixblock-JSON-format">
            AIxBlockJSON format</a>

        preannotated_from_fields: list of strings
            Turns flat task JSON formatted like: `{"column1": value, "column2": value}` into AIxBlockprediction
        ...
        if isinstance(tasks, (list, dict)):
            ...
            if not os.path.isfile(tasks):
                raise AIxBlockException(f'Not found import tasks file {tasks}')
    ...
    def export_tasks(self, export_type='JSON'):
        ...
        export_type: string
            ...
            Specify another format type as referenced in <a href="https://github.com/heartexlabs/aixblock-converter/blob/master/AIXBLOCK_converter/converter.py#L32">
            the AIxBlockconverter code</a>.
    ...
    def set_model_version(self, model_version: str):
        ...
        and annotation evaluations in AIxBlockEnterprise, and other operations.
    ...
    def get_tasks(
        ...
        Parameters
        ----------
        filters: AIXBLOCK_sdk.data_manager.Filters.create()
            JSON objects representing Data Manager filters. Use `AIXBLOCK_sdk.data_manager.Filters.create()`
            helper to create it.
            Example:
        ```json
        {
        ...
        ```
        ordering: list of AIXBLOCK_sdk.data_manager.Column
            List with <b>one</b> string representing Data Manager ordering.
            Use `AIXBLOCK_sdk.data_manager.Column` helper class.
            Example:
    ...
    def get_paginated_tasks(
        ...
        try:
            ...
            raise AIxBlockException(f'Error loading tasks: {e}')
    ...
    def get_tasks_ids(self, *args, **kwargs):
        """Same as `AIXBLOCK_sdk.project.Project.get_tasks()` but returns only task IDs.
        """
        kwargs['only_ids'] = True
        return self.get_tasks(*args, **kwargs)

    def get_paginated_tasks_ids(self, *args, **kwargs):
        """Same as `AIXBLOCK_sdk.project.Project.get_paginated_tasks()` but returns
           only task IDs.
        """
        kwargs['only_ids'] = True
    ...
    def create_view(self, filters, ordering=None, title='Tasks'):
        ...
        ----------
        filters: dict
            Specify the filters(`AIXBLOCK_sdk.data_manager.Filters`) of the view
        ordering: list of AIXBLOCK_sdk.data_manager.Column
            List with <b>one</b> string representing Data Manager ordering.
            Use `AIXBLOCK_sdk.data_manager.Column` helper class.
            Example:
    ...
    def get_unlabeled_tasks(self, only_ids=False):
        """ Retrieve all tasks that are <b>not</b> completed.
         If using AIxBlockEnterprise, this can include tasks that have been labeled one or more times, but not the full number of times defined in the
    ...
    def get_unlabeled_tasks_ids(self):
        ...
        AIxBlockEnterprise, this can include tasks that have been labeled one or more times, but not the full
    ...
    def get_task(self, task_id):
        ...
        -------
        dict:
            dict of task data containing all initial data and annotation results in [AIxBlockJSON format](https://wow-ai.com/guide/tasks.html#Basic-aixblock-JSON-format)
    ...
    def update_task(self, task_id, **kwargs):
        ...
        kwargs: kwargs parameters
            List of parameters to update. Check all available parameters [here](https://wow-ai.com/api#operation/api_tasks_partial_update)
    ...
    def create_prediction(
        ...
        result: list or dict or str
            Result in the <a href="https://wow-ai.com/guide/export.html#aixblock-JSON-format-of-annotated-tasks">
            AIxBlockJSON format as for annotations</a>.
    ...
    def create_predictions(self, predictions):
        """ Bulk create predictions for tasks. See <a href="https://wow-ai.com/guide/predictions.html">more
        details about pre-annotated tasks</a>.

        Parameters
        ----------
        predictions: list of dicts
            List of dicts with predictions in the <a href="https://wow-ai.com/guide/export.html#aixblock-JSON-format-of-annotated-tasks">
            AIxBlockJSON format as for annotations</a>.
        """
    ...
    def update_annotation(self, annotation_id, **kwargs):
        ...
        kwargs: kwargs parameters
            List of annotation parameters. Check all available parameters [here](https://wow-ai.com/guide/export.html#aixblock-JSON-format-of-annotated-tasks)
    ...
    def connect_google_import_storage(
        ...
        """Connect a Google Cloud Storage (GCS) bucket to AIxBlockto use as source storage and import tasks.
    ...
    def connect_google_export_storage(
        ...
        """Connect a Google Cloud Storage (GCS) bucket to AIxBlockto use as target storage and export tasks.
    ...
    def connect_s3_import_storage(
        ...
        """Connect an Amazon S3 bucket to AIxBlockto use as source storage and import tasks.
    ...
    def connect_s3_export_storage(
        ...
        """Connect an Amazon S3 bucket to AIxBlockto use as target storage and export tasks.
    ...
    def connect_azure_import_storage(
        ...
        """Connect a Microsoft Azure BLOB storage container to AIxBlockto use as source storage and import tasks.
    ...
    def connect_azure_export_storage(
        ...
        """Connect Microsoft Azure BLOB storage to AIxBlockto use as target storage and export tasks.
    ...
    def export_snapshot_status(self, export_id: int):
        ...
        Returns
        -------
        `AIXBLOCK_sdk.project.ExportSnapshotStatus`
    ...
    def camera_list(self,
        ...
        response = self.make_request('POST',
                                     f'/api/camera/list', json=payload)
    ...
    def get_checkpoint_ids(self):
        """ Get list of checkpoint ID from project ID
        ...
        response = self.make_request(
            method='GET',
            url=f'/api/checkpoint_model_marketplace'
        )
        return [cp['id'] for cp in response.json()]

    def upload_checkpoint(self, checkpoint, checkpoint_id):
        """ Upload checkpoint
        ...
        if isinstance(checkpoint, (str, Path)):
            ...
            if not os.path.isfile(checkpoint):
                raise AIxBlockException(f'Not found checkpoint file {checkpoint}')
            with open(checkpoint, mode='rb') as f:
                response = self.make_request(
                    method='POST',
                    url=f'/api/checkpoint_model_marketplace/upload/{checkpoint_id}',
                    files={'file': f},
                    data={'project_id': self.id},
                )
        else:
            raise TypeError(f'Not supported type provided as "checkpoint" argument: {type(checkpoint)}')
        return response.json()
    ...
    def download_checkpoint(self, checkpoint_id):
        ...
        response = self.make_request(
            method='GET',
            url=f'/api/checkpoint_model_marketplace/download/{checkpoint_id}'
        )
        filename = None
        path = "."
        if response.status_code == 200:
    ...
    def get_dataset_ids(self):
        """ Get list of dataset ID from project ID
        ...
        response = self.make_request(
            method='GET',
            url=f'/api/dataset_model_marketplace'
        )
        return [ds['id'] for ds in response.json()]
    ...
    def upload_dataset(self, dataset, dataset_id):
        ...
        if isinstance(dataset, (str, Path)):
            ...
            if not os.path.isfile(dataset):
                raise AIxBlockException(f'Not found dataset file {dataset}')
            with open(dataset, mode='rb') as f:
                response = self.make_request(
                    method='POST',
                    url=f'/api/dataset_model_marketplace/upload/{dataset_id}',
                    files={'file': f},
                    data={'project_id': self.id},
                )
        else:
            raise TypeError(f'Not supported type provided as "dataset" argument: {type(dataset)}')
        return response.json()
    ...
    def download_dataset(self, dataset_id):
        ...
        response = self.make_request(
            method='GET',
            url=f'/api/dataset_model_marketplace/download/{dataset_id}'
        )
        filename = None
        path = "."
        if response.status_code == 200:
    ...
    def upload_logs(self):
        ...
        response = self.make_request(
                    method='POST',
                    url=f'/api/projects/{self.id}/logs/upload'
                )
    ...
    def download_logs(self):
        ...
        response = self.make_request(
            method='GET',
            url=f'/api/projects/{self.id}/logs/download'
        )
        filename = None
        path = "."
        if response.status_code == 200:
    ...
    def build_model(self, project_id, folder_path):
        """ Build model from source"""
        print("Please wait ...")
        response = self.make_request(
            method='POST',
            url=f'/api/model_marketplace/build-model-source',
            data={'project_id': project_id, 'folder_path': folder_path}
        )
        print("Done !!!")
        return response.text
    
    def get_template_ml(self, model):
        """ Build model from source"""
        script = f"aixblock-ml init {model}"
        result = subprocess.run(script, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
