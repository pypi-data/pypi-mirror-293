...
class ClaudeCompletion(Completion):
    ...
    def _cast_parameter_value(self, key: str, value: any) -> float | int:
        if key == "temperature":
            return float(value)
        ...
        error_message = f"Unsupported parameter: {key}"
        raise ValueError(error_message)
    ...
    def _check_conflicting_parameters(self, request: dict) -> None:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "ClaudeCompletion":
