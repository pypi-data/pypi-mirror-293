...
def add_instrument_standalone_argument(parser: argparse.ArgumentParser=None,load_from_settings=True,required=False,from_jgt_env=False)->argparse.ArgumentParser:
    global default_parser
    if parser is None:
        parser=default_parser
    instrument_value=load_arg_default_from_settings(INSTRUMENT_ARGNAME,None,INSTRUMENT_ARGNAME_ALIAS,from_jgt_env=from_jgt_env) if load_from_settings else None
    if from_jgt_env and instrument_value is not None:
...
def add_timeframe_standalone_argument(parser: argparse.ArgumentParser=None,load_from_settings=True,required=False,load_default_timeframe=False,from_jgt_env=False)->argparse.ArgumentParser:
    global default_parser
    if parser is None:
        parser=default_parser
    
    timeframe_value=load_arg_default_from_settings(TIMEFRAME_ARGNAME,TIMEFRAME_DEFAULT_STANDALONE if load_default_timeframe else None,TIMEFRAME_ARGNAME_ALIAS,from_jgt_env=from_jgt_env) if load_from_settings else None
    #print("Timeframe value: "+str(timeframe_value))
    if from_jgt_env and timeframe_value is not None:
        required=False #We might read it from env
