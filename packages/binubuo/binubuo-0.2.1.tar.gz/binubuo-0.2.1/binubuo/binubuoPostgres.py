...
class binubuoPostgres(binubuo):
    ...
    def columnGeneratorComments(self, table_name, comments, override=False):
        ...
        if self.connected:
            comment_count_sql = self.load_resource("postgres_count_comments.sql")
            ...
            if (cursor_result[0] > 0 or not override) or (cursor_result[0] == 0):
                ...
                if (comments.find('=') > 0):
                    ...
                    ordered_col_sql = self.load_resource("postgres_comments_ordered.sql")
    ...
    def calculateSampleSize(self, table_name):
        ...
        sub_sample_size_stmt = self.load_resource("postgres_table_rows.sql")
        ...
        if (int(sub_sample_size_cal[0]) > 0 and int(sub_sample_size_cal[0]) < self.max_sample_size):
            ...
            # There seem to be issues where table sizes are in the goldielok zone, where the number of rows and percentage will have
            # A high probability of getting zero rows, because of the way postgres calculates the pages to sample.
            # So for now, we do a dirty fix and say that if pct is less than 10, set to 10.
            if sub_sample_size_cal_cust < 10:
                sub_sample_size_cal_cust = 10
        else:
    ...
    def tableIsAnalyzed(self, table_name):
        ...
        table_analyze_check_stmt = self.load_resource("postgres_table_analyzed.sql")
    ...
    def tableSize(self, table_name):
        ...
        if self.tableIsAnalyzed(table_name) is not None:
            table_size_size_stmt = self.load_resource("postgres_table_rows.sql")
        else:
            table_size_size_stmt = self.load_resource("postgres_table_rows_not_analysed.sql")
    ...
    def templateFromTable(self, table_name, use_comments=True, use_infer=False, use_sample_data=False, output_type="json"):
        ...
        if table_exist:
            ...
            if not use_infer:
                ...
                working_cursor = self.connection.cursor()
                generic_column_meta_sql = self.load_resource("postgres_column_meta_simple.sql")
                ...
                working_cursor = self.connection.cursor()
                if use_sample_data:
                    columns_meta_stmt = self.load_resource("postgres_column_meta_plus_sample.sql")
                else:
                    columns_meta_stmt = self.load_resource("postgres_column_meta_plus_standard.sql")
                ...
                for col_name, col_infer_meta in working_cursor:
                    ...
                    if use_sample_data:
                        ...
                        sub_sample_stmt = self.load_resource("postgres_column_sample_data.sql").format(r_col_name=col_name, r_table_name=table_name, r_table_sample_size=sub_sample_size_cal_cust)
                        ...
                        self.logger.info("Fetched %s rows for sample data.", sample_count)
                        if main_meta["column_is_reference"] == 1:
                            ...
                            rel_sample_stmt = self.load_resource("postgres_foreign_key_histogram.sql").format(col_name = main_meta["column_name"], tab_name = table_name, ref_col_name = main_meta["reference_column"], ref_tab_name = main_meta["reference_table"], ref_sample = rel_sample_size_cal_cust)
    ...
    def build_insert_statement(self, source_table, target_table=None):
        ...
        build_insert_stmt = self.load_resource("postgres_build_insert.sql")
    ...
    def table_exist(self, table_name, use_secondary=False):
        ...
        assert_table_sql = self.load_resource("postgres_assert_table.sql")
    ...
    def ddl_from_table(self, table_name, use_secondary=False):
        ...
        ddl_stmt = self.load_resource("postgres_table_ddl.sql")
    ...
    def table_sample(self, table_name):
        ...
        headers_trimmed = []
        headers_deco = []
        header_stmt = self.load_resource("postgres_columns_ordered.sql")
        working_cursor = self.connection.cursor()
        ...
        sample_stmt = self.load_resource("postgres_sample_select.sql").format(r_rows = ','.join(headers), r_table = table_name, r_limit = '3')
    ...
    def build_schema_dataset(self, schema_name, use_comments=True, use_infer=False, use_sample_data=False, alternate_setname=None):
        ...
        working_cursor = self.connection.cursor()
        hierachy_stmt = self.load_resource("postgres_table_hierachy_schema.sql")
        ...
        for tab, dependson, level in working_cursor:
            ...
            depends_stmt = self.load_resource("postgres_foreign_key_depends.sql")
