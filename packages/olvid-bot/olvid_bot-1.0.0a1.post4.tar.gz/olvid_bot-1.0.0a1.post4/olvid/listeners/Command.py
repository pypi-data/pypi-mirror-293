...
#####
# Command: specific listener class that will only be called when a message matching the regexp_filter had been received
#####
class Command(MessageReceivedListener):
	...
	def match(self, message: datatypes.Message) -> bool:
		return re.match(self._regexp_filter, message.body, re.IGNORECASE if self._ignore_case else 0) is not None
