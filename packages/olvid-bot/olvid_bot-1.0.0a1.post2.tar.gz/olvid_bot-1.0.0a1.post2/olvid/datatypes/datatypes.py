...
class Discussion(Discussion):
	...
	async def get_contact(self) -> Contact:
		assert self.is_contact_discussion(), "Cannot get contact, not a contact discussion"
		return await self._client.contact_get(contact_id=self.contact_id)

	async def get_group(self) -> Group:
		assert self.is_group_discussion(), "Cannot get group, not a group discussion"
	...
	#
	# async def set_settings(self, settings: DiscussionSettings) -> None:
	# 	await self._client.discussion_settings_set(settings=settings)
...
class Contact(Contact):
	...
	async def get_associated_groups(self) -> list[Group]:
	...
	async def invite_one_to_one(self) -> None:
		assert not self.has_one_to_one_discussion, "Contact is already one to one"
		await self._client.contact_downgrade_one_to_one_discussion(contact_id=self.id)

	async def downgrade_one_to_one(self) -> None:
		assert self.has_one_to_one_discussion, "Contact is not one to one"
		await self._client.contact_downgrade_one_to_one_discussion(contact_id=self.id)
...
class Group(Group):
	...
	async def add_members(self, contact_ids: list[int], permissions: GroupMemberPermissions) -> Group:
		assert self.has_admin_permissions(), "Cannot add member if you are not admin"
...
class Message(Message):
	def __init__(self, *args, **kwargs):
		...
		self.on = self.OnMessageEventListener(self, self._client)
	...
	async def get_sender_contact(self) -> Contact:
		assert self.is_inbound(), "Cannot get_sender_contact of outbound message"
	...
	async def wait_for_message_to_be_uploaded(self):
		...
		await _wait_for_entity_something(parent_client=self._client, listener_class=MessageUploadedListener, checkers=[get_generic_entity_checker(self.id)])

	async def wait_for_message_to_be_delivered(self):
		if not self.is_outbound():
			raise ValueError("Cannot wait for an inbound message to be uploaded")
		...
		await _wait_for_entity_something(parent_client=self._client, listener_class=MessageDeliveredListener, checkers=[get_generic_entity_checker(self.id)])

	async def wait_for_message_to_be_read(self):
		if not self.is_outbound():
		...
		await _wait_for_entity_something(parent_client=self._client, listener_class=MessageReadListener, checkers=[get_generic_entity_checker(self.id)])

	async def wait_for_message_to_be_edited(self) -> [Message, str]:
		from olvid.listeners import MessageBodyUpdatedListener
		message, body = await _wait_for_entity_something(parent_client=self._client, listener_class=MessageBodyUpdatedListener, checkers=[get_generic_entity_checker(self.id)])
		return message, body
