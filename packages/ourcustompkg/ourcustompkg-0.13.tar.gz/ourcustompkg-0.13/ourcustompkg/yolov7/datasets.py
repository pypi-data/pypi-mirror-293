...
from ourcustompkg.yolov7.utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str
from ourcustompkg.yolov7.utils.torch_utils import torch_distributed_zero_first
...
class LoadWebcam:  # for inference
    def __init__(self, pipe='0', img_size=640, stride=32):
        ...
        self.cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc('M', 'J', 'P', 'G')) ##ADDED THIS TO WORK IN WSL
        self.fps = self.cap.get(cv2.CAP_PROP_FPS) #ADDED this
    ...
    def __next__(self):
        ...
        #img0 = cv2.cvtColor(img0, cv2.COLOR_BGR2GRAY) # bgr to gray
        ...
        #img_gray = img_gray[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        #img_gray = np.ascontiguousarray(img_gray)

        return img_path, img, img0, self.fps
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc('M', 'J', 'P', 'G')) ##ADDED THIS TO WORK IN WSL
    ...
    def update(self, index, cap):
        ...
        while cap.isOpened():
            ...
            cap.grab()
            if n == 1:  # read every 4th frame (CHANGE to 1)
                success, im = cap.retrieve()
                self.imgs[index] = im if success else self.imgs[index] * 0
                n = 0
            time.sleep(0.5 / self.fps)  # wait time
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        #GRAYSCALE IMAGES #######################################################################
        #img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # bgr to gray
        #img = cv2.merge((img,img,img)) #3 channel grayscale
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        img = cv2.imread(path, cv2.IMREAD_GRAYSCALE)  # BGR , cv2.IMREAD_GRAYSCALE
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR) #ADDED THIS
