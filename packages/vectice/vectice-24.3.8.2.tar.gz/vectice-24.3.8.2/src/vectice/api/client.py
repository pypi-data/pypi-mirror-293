...
class Client:
    ...
    def get_version_table(
        self,
        version: ModelVersionRepresentation | DatasetVersionRepresentation,
        table: str,
    ) -> dict[str, Any]:
        """Get a version table.
        ...
        return GqlAttachmentApi(self._gql_client, self.auth).get_version_table(
            version.id,
            ("MODEL_VERSION" if isinstance(version, ModelVersionRepresentation) else "DATASET_VERSION"),
            table,
        )
    ...
    def list_version_representation_attachments(
        ...
        """List the attachments of an asset version.

        Parameters:
            version: The version to list attachments from.

        Returns:
            The attachments of the version.
        """
        attachments = GqlAttachmentApi(self._gql_client, self.auth).list_version_attachments(
            version.id,
            ("MODEL_VERSION" if isinstance(version, ModelVersionRepresentation) else "DATASET_VERSION"),
        )
    ...
    def download_attachment(
        self, version: ModelVersionRepresentation | DatasetVersionRepresentation, attachment: str, file_path: str
    ) -> None:
        """Download the attachment of a version.

        Parameters:
            version: The version to download the attachment from.
        ...
        Returns:
            None.
        """
        return AttachmentApi(self.auth).download_attachment(version, attachment, file_path)
    ...
    def list_phases(self, project: str, size: int = DEFAULT_NUMBER_OF_ITEMS) -> PagedResponse[PhaseOutput]:
        return PhaseApi(self._gql_client, self.auth).list_phases(project, size)
    ...
    def get_dataset_list(self, project_id: str, size: int) -> PagedResponse[DatasetRepresentationOutput]:
    ...
    def get_model_list(self, project_id: str, size: int) -> PagedResponse[ModelRepresentationOutput]:
