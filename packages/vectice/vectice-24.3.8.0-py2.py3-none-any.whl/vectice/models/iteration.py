...
class Iteration:
    ...
    def list_assets(self, full: bool = True) -> list[IterationAsset]:
        """Retrieves a list of assets associated with the iteration.

        Args:
            full: Indicates whether to retrieve the full details of all assets.
                Defaults to True.

        Returns:
            list[IterationAsset]: A list of IterationAsset objects representing the assets
                associated with the iteration.
        """
        self._client.assert_feature_flag_or_raise("iteration_list_assets")
        return []
...
class IterationAsset:
    ...
    Attributes:
        ...
        asset_name: The name of the asset. If the name is not available,
            this can be None.
        asset: The actual asset, which could be a model version, dataset version,
            attachment, or a comment represented as a string.
    """
    ...
    asset_id: int
    asset_name: str | None
    asset: ModelVersionRepresentation | DatasetVersionRepresentation | TAttachment | str
