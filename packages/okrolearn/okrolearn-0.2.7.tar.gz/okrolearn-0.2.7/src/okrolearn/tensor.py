...
class Tensor:
    ...
    def dot(self, other):
        ...
        if self.data.shape[1] != other.data.shape[0]:
            raise ValueError(f"Shape mismatch: ({self.data.shape[0]}, {self.data.shape[1]}) dot ({other.data.shape[0]}, {other.data.shape[1]})")
        ...
        def backward_fn(grad):
            if self.requires_grad:
                ...
                if self.grad is None:
                    self.grad = np.dot(grad, other.data.T)
                else:
                    self.grad += np.dot(grad, other.data.T)
            if other.requires_grad:
                # Compute gradient with respect to other
                if other.grad is None:
                    other.grad = np.dot(self.data.T, grad)
                else:
                    other.grad += np.dot(self.data.T, grad)
