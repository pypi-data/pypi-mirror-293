...
with importlib.resources.open_text('autophylo', 'config') as config_file:
...
def sampling_hmmer_curl(input_sequence, output_file, threads=1):
    ...
    cline = ClustalwCommandline("clustalw2", infile=input_sequence, type="protein",
                                outfile="{output_path}/{output_filename}.aln-clustal".format(
    ...
    os.system(f"hmmbuild --amino --fast -n {output_filename} \
        -o {output_path}/{output_filename}.model \
            -O {output_path}/{output_filename}.annotations \
                --cpu {threads} {output_path}/{output_filename}.hmm \
                    {output_path}/{output_filename}.aln-clustal"
              )
    ...
    curl_command = "curl -L -H 'Expect:' -H 'Accept:application/json' -F taxFilterType=search -F tax_included={taxon} -F seqdb={database} -F seq='<{hmm_file}' {url} > {output_file}.json".format(
        taxon=taxon,
        database=database,
        hmm_file="{output_path}/{output_filename}.hmm".format(
    ...
    try:
        with open(f"{output_file}.json", 'r') as jfile:
            j = json.load(jfile)
    except Exception as e:
        logger.error(e)
        sys.exit(1)
    ...
    for hit in j["results"]["hits"]:
        os.system(
            f"wget https://www.uniprot.org/uniprot/{hit['acc']}.fasta -O ->> {output_file}.fasta")
