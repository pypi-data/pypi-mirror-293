...
use crate::{
    codec::{WasmCodecError, WasmCodecType},
...
pub enum WasmCodecLoaderError {
    ...
    Instantiation { source: WasmCodecError },
}
...
impl WasmCodecClassLoader {
    ...
    #[staticmethod]
    #[allow(clippy::needless_pass_by_value)] // pyo3 requires path to be owned
    pub fn load<'py>(
        py: Python<'py>,
        ...
        Store<(), ValidatedEngine<E>>: Send + Sync,
    {
        ...
        let plugin_instantiater = Arc::new(move |component: &Component| {
            ...
            stdio::add_to_linker(&mut linker, &mut ctx)
                .map_err(LocationError::from2)
                .map_err(WasmCodecError::Runtime)?;
            ...
            CodecPlugin::new(instance, ctx).map_err(crate::codec::WasmCodecError::Runtime)
        });
