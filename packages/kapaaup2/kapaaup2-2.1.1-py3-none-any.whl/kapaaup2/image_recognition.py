...
def load_model():
    ...
    model_eval.fc = nn.Linear(num_ftrs, 8)  # 假设有8个类别
...
def model_inference(model, src_roi):
    ...
    # 将预测的类别索引映射到类别名称
    classes = {0: '方便面', 1: '面包', 2: '矿泉水',3: '饼干',4: '可乐',5: '苹果',6: '香蕉',7: '香肠'}
    pred_class = classes.get(predicted.item(), '未知物料')

    return predicted.numpy()[0], pred_class
    ...
    try:
        ...
        while True:
            ...
            if data == 'plc_tx_wzsb':
                ...
                label_map = {0: 'fangbianmian', 1: 'mianbao',2: 'kuangquanshui',3: 'binggan',
                             4: 'kele',5: 'pingguo',6: 'xiangjiao' ,7: 'xiangchang'}
                msg = 'tx_plc_wzsb,'+ label_map.get(label, 'error')
                # 发送结果回服务器
