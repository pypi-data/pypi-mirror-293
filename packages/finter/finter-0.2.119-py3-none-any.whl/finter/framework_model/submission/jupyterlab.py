...
class JupyterLabSubmission(NotebookExtractor):
    def __init__(
        ...
        self.start_date = start_date if start_date is not None else 20150101
        self.end_date = end_date if end_date is not None else 20231231
    ...
    def on_refresh(self, b):
        ...
        with self.status_output:
            self.colored_print(
                "Refreshing... Make sure you've saved your notebook!", "blue"
            )
            self.colored_print("Clearing cache and reinitializing...", "blue")
    ...
    def on_position_click(self, b):
        ...
        with self.status_output:
            try:
                ...
                print(f"Getting position for period: {start_date} to {end_date}")

                # Load model instance
                self.save_model_and_load_instance()

                # Get position
                self.position = self.model_instance.get(start_date, end_date)
                ...
                validator = ValidationHelper(
                    model_path=self.model_name_input.value,
                    model_info=self.model_info,
                )
                validator.validate()
                self.progress.value = 100
                ...
                self.progress.value = 100
                self.progress.bar_style = "danger"
                self.handle_error("Position", e, raise_error=True)
