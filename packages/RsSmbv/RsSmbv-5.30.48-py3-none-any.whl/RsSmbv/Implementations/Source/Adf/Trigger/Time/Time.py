...
class TimeCls:
	...
	def set(self, hour: int, minute: int, second: int) -> None:
		"""SCPI: [SOURce<HW>]:ADF:TRIGger:TIME:TIME \n
		Snippet: driver.source.adf.trigger.time.time.set(hour = 1, minute = 1, second = 1) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:ADF:TRIGger:TIME:TIME {param}'.rstrip())
	...
	def get(self) -> TimeStruct:
		"""SCPI: [SOURce<HW>]:ADF:TRIGger:TIME:TIME \n
		Snippet: value: TimeStruct = driver.source.adf.trigger.time.time.get() \n
		No command help available \n
			:return: structure: for return value, see the help for TimeStruct structure arguments."""
		return self._core.io.query_struct(f'SOURce<HwInstance>:ADF:TRIGger:TIME:TIME?', self.__class__.TimeStruct())
