...
class OntimeCls:
	...
	def get_value(self) -> str:
		...
		Snippet: value: str = driver.source.pulm.train.ontime.get_value() \n
		Enters the pulse on/off times values in the selected list. \n
			:return: ontime: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:PULM:TRAin:ONTime?')
		return trim_str_response(response)

	def set_value(self, ontime: str) -> None:
		"""SCPI: [SOURce<HW>]:PULM:TRAin:ONTime \n
		Snippet: driver.source.pulm.train.ontime.set_value(ontime = 'abc') \n
		Enters the pulse on/off times values in the selected list. \n
