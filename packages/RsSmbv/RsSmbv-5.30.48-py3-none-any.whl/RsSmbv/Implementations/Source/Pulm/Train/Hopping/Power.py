...
class PowerCls:
	...
	def get_value(self) -> str:
		...
		Snippet: value: str = driver.source.pulm.train.hopping.power.get_value() \n
		No command help available \n
			:return: power: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:PULM:TRAin:HOPPing:POWer?')
		return trim_str_response(response)

	def set_value(self, power: str) -> None:
		"""SCPI: [SOURce<HW>]:PULM:TRAin:HOPPing:POWer \n
		Snippet: driver.source.pulm.train.hopping.power.set_value(power = 'abc') \n
		No command help available \n
			:param power: No help available
		"""
		param = Conversions.value_to_quoted_str(power)
