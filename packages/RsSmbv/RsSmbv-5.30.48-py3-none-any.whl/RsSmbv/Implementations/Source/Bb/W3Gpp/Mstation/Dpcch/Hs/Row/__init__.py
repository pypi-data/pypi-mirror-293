...
class RowCls:
	...
	def poHack(self):
		...
		if not hasattr(self, '_poHack'):
			from .PoHack import PoHackCls
			self._poHack = PoHackCls(self._core, self._cmd_group)
		return self._poHack
	...
	def popCqi(self):
		...
		if not hasattr(self, '_popCqi'):
			from .PopCqi import PopCqiCls
			self._popCqi = PopCqiCls(self._core, self._cmd_group)
		return self._popCqi
