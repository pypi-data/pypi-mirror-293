...
class IntervalCls:
	...
	def set(self, time_interval: int, cellNull=repcap.CellNull.Default, indexNull=repcap.IndexNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:NR5G:NODE:CELL<CC(CH0)>:SSPBch<SSB(ST0)>:SL:INTerval \n
		Snippet: driver.source.bb.nr5G.node.cell.sspbch.sl.interval.set(time_interval = 1, cellNull = repcap.CellNull.Default, indexNull = repcap.IndexNull.Default) \n
		Defines the number of slots between two consecutive S-SS/PSBCH blocks. \n
			:param time_interval: integer Range: 1 to 639
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:NR5G:NODE:CELL{cellNull_cmd_val}:SSPBch{indexNull_cmd_val}:SL:INTerval {param}')
