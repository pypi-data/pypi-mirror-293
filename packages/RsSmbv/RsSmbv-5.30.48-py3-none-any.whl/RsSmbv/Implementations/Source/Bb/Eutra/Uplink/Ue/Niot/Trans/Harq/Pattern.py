...
class PatternCls:
	...
	def set(self, nbiot_an_pat: str, bitcount: int, userEquipment=repcap.UserEquipment.Default, transmission=repcap.Transmission.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:NIOT:TRANs<CH>:HARQ:PATTern \n
		Snippet: driver.source.bb.eutra.uplink.ue.niot.trans.harq.pattern.set(nbiot_an_pat = rawAbc, bitcount = 1, userEquipment = repcap.UserEquipment.Default, transmission = repcap.Transmission.Default) \n
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('nbiot_an_pat', nbiot_an_pat, DataType.RawString), ArgSingle('bitcount', bitcount, DataType.Integer))
	...
	class PatternStruct(StructBase):
		"""Response structure. Fields: \n
			- Nbiot_An_Pat: str: numeric
			- Bitcount: int: integer Range: 1 to 1"""
		__meta_args_list = [
			ArgStruct.scalar_raw_str('Nbiot_An_Pat'),
			ArgStruct.scalar_int('Bitcount')]

		def __init__(self):
			StructBase.__init__(self, self)
			self.Nbiot_An_Pat: str = None
