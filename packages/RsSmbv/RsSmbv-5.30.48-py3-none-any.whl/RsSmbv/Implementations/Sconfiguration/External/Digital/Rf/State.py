...
class StateCls:
	...
	def set(self, rem_conn_state: bool, index=repcap.Index.Default) -> None:
		...
		Snippet: driver.sconfiguration.external.digital.rf.state.set(rem_conn_state = False, index = repcap.Index.Default) \n
		...
		self._core.io.write(f'SCONfiguration:EXTernal:DIGital{index_cmd_val}:RF:STATe {param}')

	def get(self, index=repcap.Index.Default) -> bool:
		"""SCPI: SCONfiguration:EXTernal:DIGital<CH>:RF:STATe \n
		Snippet: value: bool = driver.sconfiguration.external.digital.rf.state.get(index = repcap.Index.Default) \n
		Queries/sets the RF output state of the connected external instrument. \n
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Digital')
			:return: rem_conn_state: 1| ON| 0| OFF"""
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		response = self._core.io.query_str(f'SCONfiguration:EXTernal:DIGital{index_cmd_val}:RF:STATe?')
