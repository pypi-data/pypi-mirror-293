...
class IconnectCls:
	...
	def set(self, instr_name: str, rf_path: str, path=repcap.Path.Default) -> None:
		"""SCPI: SCONfiguration:EXTernal:RF<CH>:REMote:ICONnect \n
		Snippet: driver.sconfiguration.external.rf.remote.iconnect.set(instr_name = 'abc', rf_path = 'abc', path = repcap.Path.Default) \n
		...
		self._core.io.write(f'SCONfiguration:EXTernal:RF{path_cmd_val}:REMote:ICONnect {param}'.rstrip())
