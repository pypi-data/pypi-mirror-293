...
class EnhancedCls:
	"""Enhanced commands group definition. 46 total commands, 3 Subgroups, 0 group commands"""
	...
	def pccpch(self):
		...
		if not hasattr(self, '_pccpch'):
			from .Pccpch import PccpchCls
			self._pccpch = PccpchCls(self._core, self._cmd_group)
		return self._pccpch
	...
	def pcpich(self):
		...
		if not hasattr(self, '_pcpich'):
			from .Pcpich import PcpichCls
			self._pcpich = PcpichCls(self._core, self._cmd_group)
		return self._pcpich
