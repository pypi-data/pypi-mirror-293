...
class LlobeCls:
	...
	def get_frequency(self) -> float:
		"""SCPI: [SOURce<HW>]:[BB]:ILS:[GSLope]:LLOBe:[FREQuency] \n
		Snippet: value: float = driver.source.bb.ils.gslope.llobe.get_frequency() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:ILS:GSLope:LLOBe:FREQuency?')
		return Conversions.str_to_float(response)

	def set_frequency(self, frequency: float) -> None:
		"""SCPI: [SOURce<HW>]:[BB]:ILS:[GSLope]:LLOBe:[FREQuency] \n
		Snippet: driver.source.bb.ils.gslope.llobe.set_frequency(frequency = 1.0) \n
		Sets the modulation frequency of the antenna lobe arranged at the bottom viewed from the air plane for the ILS glide
		slope modulation signal. \n
			:param frequency: float Range: 100 to 200
		"""
		param = Conversions.decimal_value_to_str(frequency)
		self._core.io.write(f'SOURce<HwInstance>:BB:ILS:GSLope:LLOBe:FREQuency {param}')
