...
class SlintCls:
	...
	def set(self, slot_length_int: int, output=repcap.Output.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:NR5G:TRIGger:OUTPut<CH>:SLINt \n
		Snippet: driver.source.bb.nr5G.trigger.output.slint.set(slot_length_int = 1, output = repcap.Output.Default) \n
		Sets the duration of a UL/DL pattern containing a marker. \n
			:param slot_length_int: integer Range: 1 to 20
			:param output: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
		"""
		param = Conversions.decimal_value_to_str(slot_length_int)
		output_cmd_val = self._cmd_group.get_repcap_cmd_value(output, repcap.Output)
		self._core.io.write(f'SOURce<HwInstance>:BB:NR5G:TRIGger:OUTPut{output_cmd_val}:SLINt {param}')
