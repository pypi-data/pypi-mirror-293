...
class TimeCls:
	...
	def set(self, hour: int, minute: int, second: int) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TRIGger:TIME:TIME \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TRIGger:TIME:TIME {param}'.rstrip())
	...
	def get(self) -> TimeStruct:
		...
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:GNSS:TRIGger:TIME:TIME?', self.__class__.TimeStruct())
