...
class DateCls:
	...
	def set(self, year: int, month: int, day: int) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:VOR:TRIGger:TIME:DATE {param}'.rstrip())
	...
	def get(self) -> DateStruct:
		...
		return self._core.io.query_struct(f'SOURce<HwInstance>:VOR:TRIGger:TIME:DATE?', self.__class__.DateStruct())
