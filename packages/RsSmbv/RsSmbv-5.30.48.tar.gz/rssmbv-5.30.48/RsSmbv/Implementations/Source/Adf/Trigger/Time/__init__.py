...
class TimeCls:
	...
	def get_state(self) -> bool:
		"""SCPI: [SOURce<HW>]:ADF:TRIGger:TIME:[STATE] \n
		Snippet: value: bool = driver.source.adf.trigger.time.get_state() \n
		No command help available \n
			:return: state: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:ADF:TRIGger:TIME:STATE?')
		return Conversions.str_to_bool(response)

	def set_state(self, state: bool) -> None:
		"""SCPI: [SOURce<HW>]:ADF:TRIGger:TIME:[STATE] \n
		Snippet: driver.source.adf.trigger.time.set_state(state = False) \n
		No command help available \n
			:param state: No help available
		"""
		param = Conversions.bool_to_str(state)
		self._core.io.write(f'SOURce<HwInstance>:ADF:TRIGger:TIME:STATE {param}')
