...
class DateCls:
	...
	def set(self, year: int, month: int, day: int) -> None:
		"""SCPI: [SOURce<HW>]:ADF:TRIGger:TIME:DATE \n
		Snippet: driver.source.adf.trigger.time.date.set(year = 1, month = 1, day = 1) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:ADF:TRIGger:TIME:DATE {param}'.rstrip())
	...
	def get(self) -> DateStruct:
		"""SCPI: [SOURce<HW>]:ADF:TRIGger:TIME:DATE \n
		Snippet: value: DateStruct = driver.source.adf.trigger.time.date.get() \n
		No command help available \n
			:return: structure: for return value, see the help for DateStruct structure arguments."""
		return self._core.io.query_struct(f'SOURce<HwInstance>:ADF:TRIGger:TIME:DATE?', self.__class__.DateStruct())
