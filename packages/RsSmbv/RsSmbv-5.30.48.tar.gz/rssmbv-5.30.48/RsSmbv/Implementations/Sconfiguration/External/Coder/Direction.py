...
class DirectionCls:
	...
	def get(self, index=repcap.Index.Default) -> enums.SystConfConnDigStat:
		"""SCPI: SCONfiguration:EXTernal:CODer<CH>:DIRection \n
		Snippet: value: enums.SystConfConnDigStat = driver.sconfiguration.external.coder.direction.get(index = repcap.Index.Default) \n
		...
		response = self._core.io.query_str(f'SCONfiguration:EXTernal:CODer{index_cmd_val}:DIRection?')
