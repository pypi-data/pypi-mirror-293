...
class ConnectCls:
	...
	def set(self, iqConnector=repcap.IqConnector.Default) -> None:
		"""SCPI: SCONfiguration:EXTernal:IQOutput<CH>:REMote:CONNect \n
		Snippet: driver.sconfiguration.external.iqOutput.remote.connect.set(iqConnector = repcap.IqConnector.Default) \n
		...
		self._core.io.write(f'SCONfiguration:EXTernal:IQOutput{iqConnector_cmd_val}:REMote:CONNect')

	def set_with_opc(self, iqConnector=repcap.IqConnector.Default, opc_timeout_ms: int = -1) -> None:
		iqConnector_cmd_val = self._cmd_group.get_repcap_cmd_value(iqConnector, repcap.IqConnector)
		"""SCPI: SCONfiguration:EXTernal:IQOutput<CH>:REMote:CONNect \n
		Snippet: driver.sconfiguration.external.iqOutput.remote.connect.set_with_opc(iqConnector = repcap.IqConnector.Default) \n
		Triggers the connection establishment. \n
		Same as set, but waits for the operation to complete before continuing further. Use the RsSmbv.utilities.opc_timeout_set() to set the timeout value. \n
			:param iqConnector: optional repeated capability selector. Default value: Nr1 (settable in the interface 'IqOutput')
			:param opc_timeout_ms: Maximum time to wait in milliseconds, valid only for this call."""
		self._core.io.write_with_opc(f'SCONfiguration:EXTernal:IQOutput{iqConnector_cmd_val}:REMote:CONNect', opc_timeout_ms)
