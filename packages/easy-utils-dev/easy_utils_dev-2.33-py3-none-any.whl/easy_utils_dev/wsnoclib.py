...
class WSNOCLIB : 
    ...
    def execute_task(self , task ):
        ...
        try:
            ...
            self.logger.debug(f"Task {task_id} failed with exception: {e} || args={task}")
            task['completed'] = True
            task['result'] = None
            task['running'] = False
    ...
    def addToQueue(self, func , onComplete=None , auto_run_queue=False, **kwargs ) :
        id = getRandomKey(n=5)
        self.queue.append(
            {
                'function' : func , 
        ...
        if auto_run_queue :
            self.runQueue(await_results=False)
        return id
