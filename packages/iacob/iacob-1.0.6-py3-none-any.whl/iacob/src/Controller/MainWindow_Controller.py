...
from src.globals import version, release_date, colorPalettes, maxRegionDisplayed
...
class MainWindow_Controller:
    ...
    def InitActions(self):
        ...
        self.exportGlobalMeasures_action = QAction("Export Global Measures", self.mainWindow_view)
        ...
        self.exportGlobalMeasures_action.triggered.connect(self.ExportGlobalMeasures)
        ...
        self.exportAllGraph_action = QAction("Export All Graph Local", self.mainWindow_view)
        self.exportAllGraph_action.triggered.connect(self.ExportLocalMeasures)
...
--------------------------------------------------------------
    ...
    def TabWidgetIndexChanged(self, tabIndex: int):
        ...
        match tabIndex:
            ...
            case 1:  # tab Circular
                ...
                self.mainWindow_view.exportGlobalMeasures_button.hide()
            ...
            case 4:  # tab GT
                ...
                self.mainWindow_view.exportCircular_button.hide()
                self.mainWindow_view.exportList_button.hide()
                self.mainWindow_view.exportGlobalMeasures_button.show()
    ...
    def OpenProjectFile(self, fileName):
        ...
        try:
            ...
            if not self.CoherenceVerification_NAMEFLUT():
                ...
                if not self.flutValidation:
                    return

            self.OpenFiltersFile(self.project.currentFiltersFile)
            
            # Warning 
            if len(self.connGraph.areasOrder) > maxRegionDisplayed:
    ...
    def CoherenceVerification_DATAFILTERS(self):
        ...
        maxEdgeFiltered = max(self.connGraph.edgesValues_withoutDuplicata.values())
        ...
        if self.filters.weightBetween_threshold[1] > maxEdgeFiltered:
        ...
        for region1 in allRegionsNames:
            for region2 in allRegionsNames:
                if (region1, region2) not in self.filters.discardInterRegConn:
                    self.filters.discardInterRegConn[(region1, region2)] = True
        ...
        for indexFilter, toIgnore in enumerate(self.filtersToIgnore):
            if toIgnore:
                match indexFilter:
                    ...
                    case 1:
                        self.filters.weightBetween_threshold[0] = min(list(self.connGraph.edgesValues.values()))
                    case 2:
    ...
    def UpdateFilterView(self):
        ...
        self.mainWindow_view.discardRank_spinBox.setDecimals(0)
    ...
    def ExportListToCSV(self):
        """
        Export the table widget data (List) to a CSV file.
        """
            ...
            if filePath:
                ...
                with open(filePath, 'w', newline='') as file:
                    ...
                    for row in range(self.mainWindow_view.infoList_tableWidget.rowCount()):
                        ...
                        if self.mainWindow_view.infoList_tableWidget.isRowHidden(row):
                            continue
    ...
    def ExportGlobalMeasures(self):
        """
        Export the table widget data (Global Measures) to a CSV file.
        """
        
        if self.projectOpened:
            userDirectory = QStandardPaths.writableLocation(QStandardPaths.HomeLocation)
            filePath, _ = QFileDialog.getSaveFileName(self.mainWindow_view, "Save Global Measures in CSV", userDirectory,
                                                    "CSV Files (*.csv)")
            ...
            if filePath:
                ...
                with open(filePath, 'w', newline='') as file:
                    writer = csv.writer(file)
                    field = ["Global Measures", "Values"]

                    writer.writerow(field)

                    for row in range(self.mainWindow_view.gtList_tableWidget.rowCount()):

                        rowData = []
                        for col in range(self.mainWindow_view.gtList_tableWidget.columnCount()):
                            rowData.append(self.mainWindow_view.gtList_tableWidget.item(row, col).text().replace(' ', ''))

                        writer.writerow(rowData)

                self.CreateFileInfo(filePath, "CSV")

    def ExportLocalMeasures(self):

        folderPath = None

        if self.projectOpened and self.displayedLocalMeasures:
            userDirectory = QStandardPaths.writableLocation(QStandardPaths.HomeLocation)
            folderPath = QFileDialog.getExistingDirectory(self.mainWindow_view, "Select Folder to Save Local Measures", userDirectory)
        ...
        if folderPath:
            ...
            for item in range(self.mainWindow_view.region_scroll_layout_GT.count()):
                ...
                if graphicWidget.name in self.displayedLocalMeasures:

                    
                    filePathImage = os.path.join(folderPath, str(graphicWidget.name + ".png"))
                    filePathCSV = os.path.join(folderPath, str(graphicWidget.name + ".csv"))

                    # ===== Image Export =====

                    high_dpi = 300
                    graphicWidget.plot.figure.savefig(filePathImage, bbox_inches='tight', dpi=high_dpi)

                    self.CreateFileInfo(filePathImage, "IMAGE")

                    # ===== CSV Export =====
                    with open(filePathCSV, 'w', newline='') as file:
                        writer = csv.writer(file)
                        field = ["Local Measures", "Values"]

                        writer.writerow(field)

                        for regionName, value in graphicWidget.data.items():
                            elements = [regionName, value]
                            writer.writerow(elements)

                    self.CreateFileInfo(filePathCSV, "CSV")
    ...
    def UpdateFilter_Event(self, name: str = None):
        ...
        if name not in [None, "widthCoefValue_spinBox", "colorPalette_comboBox"] and self.projectOpened:
            ...
            if (name == "discardWeight_checkBox" or
                ...
                # Useless to go futher
                if (name == "discardWeightUp_spinBox" or name == "discardWeightDown_spinBox") and not discardWeight:
                    return

                # Update Binary threshold based on which threshold is actif
                if not discardWeight:
                    binaryThresholdValue = self.filters.absWeightBetween_threshold[0]
                else:
                    binaryThresholdValue = max((self.filters.WeightMax(), self.filters.WeightMin()), key=abs)
            ...
            if (name == "discardAbsWeight_checkBox" or
                ...
                # Useless to go futher
                if name == "discardAbsWeight_spinBox" and not discardAbsWeight:
                    return

                # Update Binary threshold based on which threshold is actif
                if not discardAbsWeight:
                    binaryThresholdValue = self.filters.absWeightBetween_threshold[0]
                else:
                    binaryThresholdValue = self.mainWindow_view.discardAbsWeight_spinBox.value()
            ...
            if (name == "discardRank_checkBox" or
                ...
                # Useless to go futher
                if name == "discardRank_spinBox" and not discardRank:
                    return
                ...
                if not discardRank:
                    ...
                    binaryThresholdValue = self.mainWindow_view.discardRank_spinBox.value()

            if binaryThresholdValue:
                self.UpdateBinaryThreshold(binaryThresholdValue)
    ...
    def ResetGraphicFilter(self):
        ...
        if self.projectOpened:
            ...
            self.filters.colorEdges = list(colorPalettes.values())[0][0]
            self.mainWindow_view.displayColorPalette_Label.setStyleSheet(list(colorPalettes.values())[0][1])
            self.BlockFilterSignals(False)
