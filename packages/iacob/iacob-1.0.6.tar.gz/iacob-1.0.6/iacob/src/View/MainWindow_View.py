...
class MainWindowView(QMainWindow, Ui_IACOB):
    ...
    def InitActions(self):
        ...
        self.exportGlobalMeasures_button.setDefaultAction(self.mainWindow_controller.exportGlobalMeasures_action)

        self.exportAllGraphLocal.setDefaultAction(self.mainWindow_controller.exportAllGraph_action)
    ...
    def InitLoadingImages(self):
        ...
        imagePath = os.path.join(resourcedir, "images", "all.png")

        # Load image using the variable path
        pixmap_all = QPixmap(imagePath)
        pixmap_all = pixmap_all.scaled(self.exportAllGraphLocal.width(), self.exportAllGraphLocal.height(),
                               Qt.KeepAspectRatioByExpanding)

        # Check if pixmap loaded successfully
        if not pixmap_all.isNull():

            # Set pixmap as icon for the tool button
            icon_Qicon = QIcon(pixmap_all)

            self.exportAllGraphLocal.setIcon(icon_Qicon)
            self.exportAllGraphLocal.setIconSize(pixmap_all.size())
    ...
    def InitColorPaletteChoice(self):
        ...
        self.displayColorPalette_Label.setStyleSheet(list(colorPalettes.values())[0][1])
