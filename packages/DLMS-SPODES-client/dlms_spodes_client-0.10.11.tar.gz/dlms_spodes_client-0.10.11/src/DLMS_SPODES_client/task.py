...
class FindServerId(ExTask):
    """try find COSEM server identifier, return: ServerId"""

    async def exchange(self, c: Client) -> collection.ServerId:
        for desc in (ut.CosemAttributeDescriptor((1, "0.0.0.2.1.255", 2)), ut.CosemAttributeDescriptor((1, "0.0.96.1.2.255", 2))):
            try:
...
class InitType(ExTask):
    ...
    async def exchange(self, c: Client) -> bool:
        ...
        if man in (b"101", b"102", b"103", b"104", b"KPZ"):  # find device_id(type for Russia, KPZ) todo: refactoring to read obj_list before
            c.log(logL.WARN, F"find KPZ manufacturer")
            id_par = bytes((0, 0, 96, 1, 1, 255, 2))
            type_value, _ = cdt.get_instance_and_pdu_from_value(await c.read_attr(ut.CosemAttributeDescriptor((1, bytearray(id_par[:6]), id_par[6]))))
            try:
                c.objects = collection.get_collection(
                    manufacturer=ldn.get_manufacturer(),
                    server_type=collection.ServerId(
                        par=id_par,
                        value=type_value),
                    server_ver=await FindServerVersion().exchange(c))
                c.objects.LDN.set_attr(2, ldn)
            except exc.NoConfig as e:
                c.log(logL.WARN, F"false KPZ init type procedure: {e}")
        if not c.objects:
            c.log(logL.INFO, "start get Collection from object list")
            new_collection = collection.Collection(ldn=ldn)
            # new_collection.set_server_id(type_value)  # todo: add type
            # read association
