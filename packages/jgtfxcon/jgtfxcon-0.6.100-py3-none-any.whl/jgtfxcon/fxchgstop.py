...
def parse_args():
    parser = jgtcommon.new_parser("JGT FX MV Trade Stop CLI", "Change stop order of trade by id on FXConnect", "fxmvstop",add_exiting_quietly_flag=True)
    
    parser=jgtcommon.add_demo_flag_argument(parser,from_jgt_env=True)
    parser=jgtcommon.add_instrument_standalone_argument(parser,required=False,from_jgt_env=True)
    parser=jgtcommon.add_orderid_arguments(parser,required=False,from_jgt_env=True)
    parser=jgtcommon.add_tradeid_arguments(parser,required=False,from_jgt_env=True)
    parser=jgtcommon.add_stop_arguments(parser,pips_flag=True,from_jgt_env=True)
