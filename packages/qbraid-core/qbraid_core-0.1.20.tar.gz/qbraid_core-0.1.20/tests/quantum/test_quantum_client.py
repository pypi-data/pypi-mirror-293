...
class TestQuantumClient(unittest.TestCase):
    ...
    def test_update_device(self, mock_session):
        """Test the update_device method."""
        mock_session.put.return_value.json.return_value = {"updated": True}
        result = self.client.update_device({"name": "Quantum One"})
        self.assertEqual(result, {"updated": True})
        mock_session.put.assert_called_once_with("/quantum-devices", json={"name": "Quantum One"})

    @patch("qbraid_core.services.quantum.client.QuantumClient.session")
    def test_update_job(self, mock_session):
        """Test the update_job method."""
        mock_session.put.return_value.json.return_value = {"status": "updated"}
        result = self.client.update_job({"status": "running"})
        self.assertEqual(result, {"status": "updated"})
        mock_session.put.assert_called_once_with("/quantum-jobs", json={"status": "running"})
