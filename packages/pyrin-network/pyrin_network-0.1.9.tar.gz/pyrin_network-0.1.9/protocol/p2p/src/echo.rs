...
use crate::{
    ...
    IncomingRoute,
    KaspadMessagePayloadType, pb::{self, VersionMessage}, Router,
};

pub struct EchoFlow {
    pub receiver: IncomingRoute,
...
impl EchoFlow {
    pub async fn register(router: Arc<Router>) {
        ...
        let receiver = router.subscribe(msg_types!());
    ...
    pub async fn call(&self, msg: pb::KaspadMessage) -> bool {
        // echo
...
pub struct ListenerFlow {
    pub receiver: IncomingRoute,
    pub router: Arc<Router>,
    listener: Option<mpsc::Sender<pb::KaspadMessage>>,
}

impl ListenerFlow {
    pub fn new(router: Arc<Router>) -> (Self, mpsc::Receiver<pb::KaspadMessage>) {
        let (tx, rx) = mpsc::channel(1024);
        (
            Self {
                receiver: router.subscribe(msg_types!()),
                router,
                listener: Some(tx),
            },
            rx,
    ...
    pub async fn register(mut self) {
        ...
        tokio::spawn(async move {
            while let Some(msg) = self.receiver.recv().await {
                ...
                // Send message to listener if it exists
                // if let Some(listener) = &self.listener {
                //     if let Err(e) = listener.send(msg.clone()).await {
                //         println!("Failed to send message to listener: {:?}", e);
                //     }
                // }

                if !self.call(msg).await {
...
impl ConnectionInitializer for EchoFlowInitializer {
    async fn initialize_connection(&self, router: Arc<Router>) -> Result<(), ProtocolError> {
        ...
        let (echo_flow, mut listener) = ListenerFlow::new(router.clone());

        echo_flow.register().await;
