...
impl RPC {
    ...
    pub fn connect<'a>(&mut self, py: Python<'a>, url: Option<String>, testnet: Option<bool>) -> PyResult<&'a PyAny> {
        // let url = self.url.clone().unwrap_or("grpc://127.0.0.1:13110".to_string());
        let url = self.url.clone().unwrap_or(format!("ws://{}", url.unwrap_or("127.0.0.1:17110".to_string())));
        let network_id = if testnet.unwrap_or(false) {
            NetworkId::with_suffix(NetworkType::Testnet, 10)
        } else {
            NetworkId::new(NetworkType::Mainnet)
        };

        self.client = Some(KaspaRpcClient::new(WrpcEncoding::Borsh, Some(url.as_str().as_ref()), None, Some(network_id), None).unwrap());
                            ...
                            Ok(notification) => {
                                ...
                                match notification {
                                    ...
                                    Notification::VirtualChainChanged(ref payload) => {
                                        let added_chain_block_hashes: Vec<String> = payload.added_chain_block_hashes.iter().map(|h| h.to_hex()).collect();
                                        let removed_chain_block_hashes: Vec<String> = payload.removed_chain_block_hashes.iter().map(|h| h.to_hex()).collect();
                                        let accepted_transaction_ids: Vec<PyObject> = Python::with_gil(|py| {
                                        payload.accepted_transaction_ids
                                            .iter()
                                            .map(|t| {
                                                let mut dict = PyDict::new(py);
                                                let ids = PyList::new(py, t.accepted_transaction_ids.iter().map(|id| id.to_hex()));
                                                dict.set_item("accepting_block_hash", t.accepting_block_hash.to_hex()).unwrap();
                                                dict.set_item("accepted_transaction_ids", ids).unwrap();
                                                dict.into()
                                            })
    ...
    pub fn submit_block<'a>(&mut self, py: Python<'a>, block_dict: &PyDict, allow_non_daa_blocks: bool, grpc_url: Option<String>) -> PyResult<&'a PyAny> {
        let client = Arc::new(self.client.clone().unwrap());
        ...
        pyo3_asyncio::tokio::future_into_py(py, async move {
            ...
            let result = match grpc_url {
                Some(url) => {
                    let subscription_context = SubscriptionContext::new();

                    // TODO: Research the hash is added into wRPC causing faulty nodes
                    ...
                    let result = client.submit_block(block, allow_non_daa_blocks).await.unwrap().report;
                ...
                None => {
                    panic!("Currently disabled")
                    // client.rpc_api().submit_block(block, allow_non_daa_blocks).await.unwrap().report
                }
            };
            ...
            let result = match result {
                ...
                kaspa_rpc_core::SubmitBlockReport::Reject(kaspa_rpc_core::SubmitBlockRejectReason::RouteIsFull) => 3,
            };
