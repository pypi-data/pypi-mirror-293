...
def reset_num_outputs_on_model(
    ...
    ----------
    datamodule
        The datamodule, from which targets will be extracted so the function
        correctly ponders the number of outputs required by the model.
    model
        The model that will have its number of outputs potentially modified.
    """

    first_batch = next(iter(datamodule.train_dataloader()))
    num_classes = first_batch["target"].shape[1]
