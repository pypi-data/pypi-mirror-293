...
def form_submission_list(req, form_id):
    ...
    filter_by_slug = req.headers.get('FILTER_BY_SLUG', '{}')  # {"state":{"type":"dropdown","value":"128"}}
    ...
    for slug_field_key, slug_field_value in filter_by_slug.items():
        form_field_mapped_to_slug = FormField.objects.filter(slug=slug_field_key).first()
