        ...
        for form_submission in page_obj.object_list:
            ...
            for dynamic_field_value in dynamic_field_values:
                ...
                #     formatted_dynamic_field_values[str(dynamic_field_value.form_field.id)] = dynamic_field_value.checkbox_field
                    ...
                    formatted_dynamic_field_values[
                        str(dynamic_field_value.form_field.id)] = dynamic_field_value.date_field
...
def validate_gsheet_access(gsheet_url, credentials):
    ...
    try:
        ...
        # Use a far-off cell to test edit access (e.g., Z1000)
        test_cell = 'Z1000'
        worksheet.update(test_cell, "Test Edit")

        # Now delete the value from the test cell
...
def get_sheet_columns(request):
    ...
    try:
        ...
        # Extract spreadsheet_id and sheet name from the Google Sheet URL
        ...
        # Get the sheet metadata to retrieve the first sheet's name
        spreadsheet_metadata = sheets_service.spreadsheets().get(spreadsheetId=spreadsheet_id).execute()
        sheets = spreadsheet_metadata.get('sheets', [])
        if not sheets:
            return Response({'error': 'No sheets found in the spreadsheet'}, status=404)

        # Get the sheet name (you can choose the first sheet by default or other logic)
        sheet_name = sheets[0]['properties']['title']  # Default to the first sheet
...
def get_dropdown_field_option_values(option_ids = []):
  ...
  Get Field Option Values - If option is non deleted
   ...
   if field_options:
       for field_option in field_options:
           option_values.append(field_option.option_value)
