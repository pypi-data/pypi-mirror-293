...
def generate_Xstrips(stdata):
    """
    Generate strips along the X-axis for correlation matrix calculation.
    ...
    for i in range(length):
        idx = np.isclose(stdata.obs['array_col'] - x_min, i)
        if idx.sum() == 0:
            continue
        strips.append(stdata.X[idx, :].sum(axis=0).A1)
        stdata.obs.loc[idx, 'x'] = i

    return np.array(strips)

def generate_Ystrips(stdata):
    """
    Generate strips along the Y-axis for correlation matrix calculation.
    ...
    for i in range(length):
        idx = np.isclose(stdata.obs['array_row'] - y_min, i)
        if idx.sum() == 0:
            continue
        strips.append(stdata.X[idx, :].sum(axis=0).A1)
        stdata.obs.loc[idx, 'y'] = i

    return np.array(strips)

def generate_Zstrips(stdata):
    """
    Generate Z strips by rotating the coordinates of spatial data and aggregating values along the new Z-axis.

    Args:
        stdata (anndata.AnnData): Spatial transcriptomics data.

    Returns:
        np.ndarray: Array of aggregated values for each Z strip.
    """
    slides_z = []
    
    # Rotate the coordinates by 45 degrees (pi/4 radians)
    Mx_p, My_p = rotate_coordinates(stdata.obs['array_col'], stdata.obs['array_row'], angle=pi/4)
    
    # Calculate the number of strips needed along the Z-axis
    length = int(np.round((Mx_p.max() - Mx_p.min()) / sqrt(2)) + 1)
    start_value = Mx_p.min()
    j = 0 
    
    # Aggregate the values for each strip
    for i in range(length):
        idx = np.abs(Mx_p - start_value - i * sqrt(2)) < 0.1
        if idx.sum() == 0:
            continue
...
def preprocess(scdata, stdata, genes=None):
    ...
    scdata.uns["overlap_genes"] = overlap_genes
...
def SpaGCN_cluster(selected_pcs, stdata, label, shape="square", target_num=10):
    ...
    stdata.obs[label] = y_pred.astype('category')
    
    refined_pred = refine(
        sample_id=stdata.obs.index.tolist(),
        pred=stdata.obs[label].tolist(),
        dis=adj_no_img, shape=shape
    )
    stdata.obs[label] = refined_pred.astype('category')
