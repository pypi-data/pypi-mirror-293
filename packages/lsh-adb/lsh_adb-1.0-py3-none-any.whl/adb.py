...
class Adb:
    ...
    adb 操作
    """
    start_activity = {'hpt': 'com.htjy.xiaofeiji.hpt/com.htjy.xiaofeiji.ui.activity.SplashActivity'}
    ...
    def start_app(self):
        hpt = self.start_activity['hpt']
        os.system(f'adb -s {self.device_ip} shell am start -n {hpt}')

    def exit_app(self):
        hpt = self.start_activity['hpt']
        os.system(f'adb -s {self.device_ip} shell am force-stop {hpt}')

    def get_app_active(self):
        out_put = os.popen(f'adb -s {self.device_ip} shell dumpsys activity | findstr /i "mResumedActivity"').read()
        pattern = r'com..*?Activity'
        print(out_put)
        activity = re.findall(pattern, out_put)[0]
        return activity
    ...
    def run(self):
        while 1:
            func = {1:'screenshot', 2:'video', 3:'get_logs', 4:'get_package_3', 5:'install', 6:'uninstall',
                    7:'exit', 8:'restart', 9:'export', 10:'clear_log', 11:'start_app'}
...
if __name__ == '__main__':
    package = r"D:\资料\飞书下载记录\app-HongtuHaiPuTianXiaoFeiJi-debug(6).apk"  # 安装包路径
    adb = Adb('192.168.83.116', package)
    adb.connect()
    adb.run()
    # adb.start_app()
