...
def get_token(env_config: EnvConfig):
    ...
    If EnvConfig is LOCAL returns an unsigned JWT token that has { roles: [ADMIN, local_test] }
    ...
    if env_config.environment is Environment.LOCAL:
        return ("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9"
                ".eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyLCJyZWFsbV9hY2Nlc3MiOnsicm9sZXMiOlsiQURNSU4iLCJsb2NhbF90ZXN0Il19fQ"
                ".STxNrPhnQ_E8MfyGHi6l3FlddBgdYRgPNRaxo1RBF-Y")
    else:
...
def login_and_return_token(env_config: EnvConfig) -> str:
    """Start login and wait for a token to be extracted.
...
def start_server_and_login(env_config: EnvConfig, queue: Queue) -> tuple[Thread, BaseServer]:
    ...
    webbrowser.get(using='chromium').open(env_config.keycloak_url + "/auth?" + urlencode(query))
