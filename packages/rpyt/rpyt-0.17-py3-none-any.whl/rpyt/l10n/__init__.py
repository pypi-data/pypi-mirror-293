...
L10N_SRC_DIR = f"{L10N_DIR}/src"

DEFAULT_PROCESSOR = "rpyt.l10n.processors.DynamicJsonProcessor"
...
def make_tl_files(
    ...
    **kwargs: Any,
    ...
    tl_dir += "/" + language
    src_dir = f"{L10N_DIR}/{language}" if src_dir is None else src_dir
    files = dict(
        map(
            lambda e: (f"{tl_dir}/{e[0]}", e[1]),
            get_tl_files(
                language, src_dir, tlstrings, indent, processor, **kwargs
            ).items(),
        )
    )
