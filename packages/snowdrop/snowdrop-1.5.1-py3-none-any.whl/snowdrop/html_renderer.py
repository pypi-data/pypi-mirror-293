...
from snowdrop import block_token
from snowdrop import span_token
from snowdrop.block_token import HtmlBlock
from snowdrop.span_token import HtmlSpan
from snowdrop.base_renderer import BaseRenderer
...
class HtmlRenderer(BaseRenderer):
    ...
    See snowdrop.base_renderer module for more info.
    """
    def __init__(
        self,
        *extras,
    ...
    def render_paragraph(self, token: block_token.Paragraph) -> str:
        ...
        output_text = re.sub('<.*?>', '', output)
        output_text = re.sub('[^A-Za-z ]', '', output_text)
        output_text = output_text.lower()
        first_three_words = output_text.split()[:3]
        className = '-'.join(first_three_words)
        return f'<p class="{className}">{output}</p>'
