...
class Connection:
    ...
    def __init__(self, server, user=None, password=None):
        ...
        response = self.session.get(server+"/msgpack/FLAMINGO", verify=verify_cert)
        raise_for_status(response)

    @staticmethod
    def get(server, user, password=None):
    ...
    def request_directory(self, path):
        """
        Request the msgpack representation of a directory from the server
        """
        url = f"{self.server}/msgpack/{path}"
        with self.session.get(url, stream=True, verify=verify_cert) as response:        
            raise_for_status(response)
