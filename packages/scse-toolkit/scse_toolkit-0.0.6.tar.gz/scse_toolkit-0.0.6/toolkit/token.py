...
class PreencodedToken(Token):
    ...
    def to_token(self) -> Token:
        """Returns the pre-encoded token as the `Token` base class"""
        return Token(**self.model_dump(mode="json", exclude={"encoded_str"}))


@lru_cache
def cached_encode(token: Token, secret: str) -> str:
    return jwt.encode(
        token.model_dump(mode="json", exclude_none=True),
        secret,
