...
class Credential(Resource):
    ...
    def __init__(
        ...
        super().__init__(name, g, ignore_changes)
    ...
    def _api_credential_secret_change_input(self) -> Any:
        # We need to import the validio_sdk module due to the `eval`
        # ruff: noqa: F401
        import validio_sdk

        cls = eval(
            f"validio_sdk.graphql_client.input_types.{self.__class__.__name__}SecretChangedInput"
        )
        return cls(
            **{
                "id": self._must_id(),
                **{f: getattr(self, f) for f in self._secret_fields() or []},
            }
        )
...
class SnowflakeCredentialKeyPair(Diffable):
    """
    ...
    def __init__(
        self, user: str, private_key: str, private_key_passphrase: str | None = None
    ):
        """
        Constructor.
    ...
    def _api_input(self) -> SnowflakeCredentialAuthInput:
        return SnowflakeCredentialAuthInput(
            key_pair=SnowflakeCredentialKeyPairInput(
                user=self.user,
...
class SnowflakeCredentialUserPassword(Diffable):
    ...
    def _api_input(self) -> SnowflakeCredentialAuthInput:
        return SnowflakeCredentialAuthInput(
            user_password=SnowflakeCredentialUserPasswordInput(
                user=self.user,
...
class SnowflakeCredential(Credential):
    ...
    def __init__(
        ...
        if user:
            self.add_field_deprecation("user", "auth")
        if password:
            self.add_field_deprecation("password", "auth")
    ...
    def _secret_fields(self) -> set[str] | None:
        # When the new auth field is used we pretend that the password
        # field is not a secret. Otherwise we would get a diff on that
        # when the auth secrets change because we don't know which
        # secret changed so all secret fields are marked as changed.
    ...
    def _api_create_input(self, namespace: str, _ctx: "DiffContext") -> Any:
        auth = self.auth._api_input() if self.auth else None
        return _api_create_input_params(
            self,
            namespace=namespace,
            overrides={"auth": auth},
    ...
    def _api_credential_secret_change_input(
        self,
    ) -> SnowflakeCredentialSecretChangedInput:
        return SnowflakeCredentialSecretChangedInput(
            id=self._must_id(),
            auth=self.auth._api_input() if self.auth else None,
            password=self.password,
        )
...
class KafkaSaslSslPlainCredential(Credential):
    ...
    def _mutable_fields(self) -> set[str]:
        return {"bootstrap_servers", "enable_catalog", "ca_certificate"}
