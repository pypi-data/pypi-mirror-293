...
class FFSimmer:
    ...
    def compute_grid(self, grid, gmpes, myimts):
        ...
        Args:
            ...
            gmpes (dictionary):
                A dictionary of MultiGMPEs to be used for the simulations keyed to
                myimts.
            myimts (list):
    ...
    def compute_points(self, sctx, gmpes, myimts, rock_vs30=760.0, soil_vs30=180.0):
        """
        ...
        Args:
            ...
            gmpes: (dictionary)
                A dictionary (keyed to myimts) of gmpes to perform the ground-motion
                  realizations.
            myimts (list)
        ...
        amps = np.empty((nctx, nimt, nsim, nporig))
        ...
        for ix in range(nimt):
            ...
            for ictx, ctx_name in enumerate(ctxt_names):
                ...
                # Find the variance over the realizations, this is the variance we
                # have to account for
                ...
                mean_sigs = np.sqrt(np.mean(sigs[ictx, ix] ** 2, axis=0) + extra_vars)
                if len(sd_types) > 1:
                    mean_taus = np.sqrt(
                ...
                if len(sd_types) > 1:
                    outdict[myimts[ix]][ctx_name][oqconst.StdDev.INTER_EVENT] = (
                        mean_taus.reshape(sctx.lons.shape)
                    )
                    outdict[myimts[ix]][ctx_name][oqconst.StdDev.INTRA_EVENT] = (
    ...
    def true_grid(self, grid, gmpes, myimts):
        ...
        for iimt in range(nimt):
            ...
            if len(sd_types) > 1:
                griddict[myimts[iimt]][oqconst.StdDev.INTER_EVENT] = mean_taus.reshape(
                    (ny, nx)
                )
                griddict[myimts[iimt]][oqconst.StdDev.INTRA_EVENT] = mean_phis.reshape(
                    (ny, nx)
                )
    ...
    def __init__(
        ...
        Args:
            ...
            delta_depth (float): The depth range of the hypocenter is uniformly drawn
                ...
                (Default: 20.0)
            ztor (float): Fix the top of rupture to a specific depth (kilometers,
                positive down). Negative values allow the TOR to float. (Default -1.)
